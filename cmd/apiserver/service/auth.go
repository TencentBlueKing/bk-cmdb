...
func (s *service) AuthVerify(req *restful.Request, resp *restful.Response) {
	...
	attrs := make([]meta2.ResourceAttribute, 0)
	needExactAuthAttrs := make([]meta2.ResourceAttribute, 0)
	needExactAuthMap := make(map[int]bool)

	for i, res := range body.Resources {
		resources[i].AuthResource = res
		attr := meta2.ResourceAttribute{
			Basic: meta2.Basic{
				Type:         meta2.ResourceType(res.ResourceType),
				Action:       meta2.Action(res.Action),
		...
		for _, item := range res.ParentLayers {
			attr.Layers = append(attr.Layers, meta2.Item{Type: meta2.ResourceType(item.ResourceType), InstanceID: item.ResourceID, InstanceIDEx: item.ResourceIDEx})
		}
...
func (s *service) GetAnyAuthorizedAppList(req *restful.Request, resp *restful.Response) {
	...
	userInfo := meta2.UserInfo{
		UserName:        util.GetUser(pheader),
		SupplierAccount: util.GetOwnerID(pheader),
	}

	authInput := meta2.ListAuthorizedResourcesParam{
		UserName:     util.GetUser(pheader),
		ResourceType: meta2.Business,
		Action:       meta2.ViewBusinessResource,
	}
