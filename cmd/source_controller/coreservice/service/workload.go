...
func (s *coreService) CreateWorkload(ctx *rest.Contexts) {
	...
	kind := types2.WorkloadType(ctx.Request.PathParameter(types2.KindField))
	tableName, err := kind.Table()
	if err != nil {
		ctx.RespAutoError(ctx.Kit.CCError.CCErrorf(common.CCErrCommParamsInvalid, types2.KindField))
		return
	}

	req := types2.WlCreateReq{Kind: kind}
	...
	respData := types2.WlCreateRespData{
...
func (s *coreService) GetNamespaceSpec(kit *rest.Kit, spec *types2.NamespaceSpec) (*types2.NamespaceSpec, error) {
	if spec.BizID == nil {
	...
	if spec.ClusterUID != nil && spec.Namespace != nil {
		filter[types2.ClusterUIDField] = *spec.ClusterUID
		filter[common.BKFieldName] = *spec.Namespace
	}

	ns := types2.Namespace{}
	if err := mongodb.Client().Table(types2.BKTableNameBaseNamespace).Find(filter).One(kit.Ctx, &ns); err != nil {
...
func (s *coreService) UpdateWorkload(ctx *rest.Contexts) {
	...
	req := types2.WlUpdateReq{Kind: kind}
	if err := ctx.DecodeInto(&req); nil != err {
		ctx.RespAutoError(err)
		return
	}
