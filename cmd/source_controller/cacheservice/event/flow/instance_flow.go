...
import (
	...
	types2 "configcenter/pkg/types"
	"configcenter/pkg/watch"
	"configcenter/thirdparty/monitor"
	"configcenter/thirdparty/monitor/meta"
	...
	watchOpts := &types.WatchOptions{
		Options: types.Options{
			...
			CollectionFilter: map[string]interface{}{
				common.BKDBLIKE: event2.ObjInstTablePrefixRegex,
			},
			StartAfterToken:         nil,
...
func (f *InstanceFlow) classifyEvents(es []*types.Event, oidDetailMap map[oidCollKey][]byte, rid string) (
	...
	mainlineColl := event2.MainlineInstanceKey.Collection()
...
func (f *InstanceFlow) getKeyByCollection(collection string) event2.Key {
	switch collection {
	case event2.MainlineInstanceKey.Collection():
		return event2.MainlineInstanceKey
	default:
		return f.key
	}
}
