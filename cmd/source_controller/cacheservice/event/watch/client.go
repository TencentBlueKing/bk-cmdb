...
import (
	event2 "configcenter/cmd/source_controller/cacheservice/event"
	"configcenter/pkg/storage/dal"
	"configcenter/pkg/storage/dal/redis"
	daltypes "configcenter/pkg/storage/dal/types"
	"configcenter/pkg/storage/driver/mongodb/instancemapping"
	"configcenter/pkg/storage/stream/types"
	"fmt"
	"time"
...
func (c *Client) getLatestEvent(kit *rest.Kit, key event2.Key) (*watch.ChainNode, bool, error) {
...
func (c *Client) getEarliestEvent(kit *rest.Kit, key event2.Key) (*watch.ChainNode, bool, error) {
	filter := map[string]interface{}{
		common.BKClusterTimeField: map[string]interface{}{
			common.BKDBGTE: metadata.Time{Time: time.Now().Add(-time.Duration(key.TTLSeconds()) * time.Second).UTC()},
		},
...
func (c *Client) getEventDetail(kit *rest.Kit, node *watch.ChainNode, fields []string, key event2.Key) (*string,
	bool, error) {

	coll := key.Collection()
	switch coll {
	case event2.HostIdentityKey.Collection():
	...
	case event2.BizSetRelationKey.Collection():
...
func (c *Client) getEventDetailFromRedis(kit *rest.Kit, cursor string, fields []string, key event2.Key) (
	*string, error) {
...
func (c *Client) getEventDetailFromMongo(kit *rest.Kit, node *watch.ChainNode, fields []string, key event2.Key) (
	*string, bool, error) {
...
func (c *Client) getLastEventID(kit *rest.Kit, key event2.Key) (uint64, error) {
	filter := map[string]interface{}{
		"_id": key.Collection(),
	}
...
*/
func (c *Client) searchEventDetailsFromRedis(kit *rest.Kit, cursors []string, key event2.Key) (
...
func (c *Client) searchEventDetailsFromMongo(kit *rest.Kit, nodes []*watch.ChainNode, fields []string,
	errCursorIndexMap map[string]int, key event2.Key) (map[int]string, error) {
...
func (c *Client) searchDeletedEventDetailsFromMongo(kit *rest.Kit, coll string, deletedOids []string, fields []string,
	...
	if coll == common.BKTableNameBaseHost {
		docs := make([]event2.HostArchive, 0)
