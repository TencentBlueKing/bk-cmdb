...
import (
	...
	"configcenter/cmd/source_controller/cacheservice/event/mix-event"
...
func newBizSetRelation(ctx context.Context, opts mixevent.MixEventFlowOptions) error {
	relation := bizSetRelation{
		...
		metrics:           event2.InitialMetrics(opts.Key.Collection(), "biz_set_relation"),
	}
...
func (b *bizSetRelation) rearrangeEvents(rid string, es []*types.Event) ([]*types.Event, error) {
	...
	case event2.BizSetKey.Collection():
		return b.rearrangeBizSetEvents(es, rid)
	case event2.BizKey.Collection():
...
func (b *bizSetRelation) getBizSetRelationDetail(e *types.Event, rid string) (string, error) {
	...
	if e.OperationType == types.Delete {
		return event2.GenBizSetRelationDetail(bizSet.BizSetID, ""), nil
	}
	...
	if bizSet.Scope.MatchAll {
		...
		return event2.GenBizSetRelationDetail(bizSet.BizSetID, allBizID), nil
	}
	...
	return event2.GenBizSetRelationDetail(bizSet.BizSetID, bizIDStr), nil
}
