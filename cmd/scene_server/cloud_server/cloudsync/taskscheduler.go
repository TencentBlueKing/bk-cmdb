...
import (
	...
	reflector2 "configcenter/pkg/storage/reflector"
	stypes "configcenter/pkg/storage/stream/types"
	"context"
	"fmt"
	"sync"
	"time"
...
type taskScheduler struct {
	...
	addrport   string
	reflector  reflector2.Interface
...
func NewTaskScheduler(conf *SchedulerConf) (*taskScheduler, error) {
	reflector, err := reflector2.NewReflector(conf.MongoConf)
	if err != nil {
...
func (t *taskScheduler) watchTaskTable(ctx context.Context) error {
	...
	capable := &reflector2.Capable{
		OnChange: reflector2.OnChangeEvent{
			OnAdd:        t.changeOnAdd,
