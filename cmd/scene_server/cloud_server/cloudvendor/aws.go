...
import (
	"configcenter/cmd/scene_server/cloud_server/common"
...
func (c *awsClient) GetVpcs(region string, opt *common.VpcOpt) (*metadata.VpcsInfo, error) {
...
func (c *awsClient) GetInstances(region string, opt *common.InstanceOpt) (*metadata.InstancesInfo, error) {
	...
	// 如果查到的不是全量，则去获取实例总数
	if !isAll {
		instOpt := &common.InstanceOpt{common.BaseOpt{
...
func (c *awsClient) getInstances(region string, opt *common.InstanceOpt) ([]*metadata.Instance, bool, error) {
	sess, err := c.newSession(region)
	if err != nil {
	...
	if opt == nil {
		opt = common.GetDefaultInstanceOpt()
	}
	...
	for {
		...
		for _, reservation := range output.Reservations {
			for _, inst := range reservation.Instances {
				...
				instances = append(instances, &metadata.Instance{
					...
					PrivateIp:     privateIP,
					PublicIp:      publicIP,
					InstanceState: common.CovertInstState(state),
					VpcId:         vpcID,
				})
			}
		}
		...
		if loopCnt > common.MaxLoopCnt {
			blog.Errorf("DescribeVpcs loopCnt:%d, bigger than MaxLoopCnt, len(instances):%d",
				loopCnt, instances)
			return nil, false, common.ErrorLoopCnt
		}
	}
	isAll := false
...
func (c *awsClient) GetInstancesTotalCnt(region string, opt *common.InstanceOpt) (int64, error) {
...
func (c *awsClient) newDescribeVpcsInput(opt *common.VpcOpt) *ec2.DescribeVpcsInput {
...
func (c *awsClient) newDescribeInstancesInput(opt *common.InstanceOpt) *ec2.DescribeInstancesInput {
...
// setFilters 设置过滤条件
func (c *awsClient) setFilters(dst *[]*ec2.Filter, src []*common.Filter) {
