...
func (s *Service) SearchReducedBusinessSetList(ctx *rest.Contexts) {
	...
	if s.AuthManager.Enabled() {
		isAny, authBizSetList, err := s.getAuthBizSetIDList(ctx.Kit, meta2.AccessBizSet)
		if err != nil {
...
func (s *Service) getAuthBizSetIDList(kit *rest.Kit, action meta2.Action) (bool, []int64, error) {
...
func (s *Service) SearchBusinessSet(ctx *rest.Contexts) {
	...
	if s.AuthManager.Enabled() {
		isAny, authBizSetIDs, err := s.getAuthBizSetIDList(ctx.Kit, meta2.Find)
		if err != nil {
			blog.Errorf("get authorized biz set id failed, err: %v, rid: %s", err, ctx.Kit.Rid)
			ctx.RespEntity(bizSetResult)
			return
