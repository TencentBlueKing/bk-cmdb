...
import (
	...
	"configcenter/pkg/mapstruct"
	"configcenter/pkg/metadata"
	gparams "configcenter/pkg/paraparse"
	"configcenter/pkg/querybuilder"
	"configcenter/pkg/util"
	"configcenter/thirdparty/hooks"
)
...
func (s *Service) UpdateBizPropertyBatch(ctx *rest.Contexts) {
	...
	if s.AuthManager.Enabled() {
		if err := s.AuthManager.AuthorizeByBusinessID(ctx.Kit.Ctx, ctx.Kit.Header, meta2.Update, bizIDs...); err ==
			ac.NoAuthorizeError {
			perm, err := s.AuthManager.GenBizBatchNoPermissionResp(ctx.Kit.Ctx, ctx.Kit.Header, meta2.Update, bizIDs)
