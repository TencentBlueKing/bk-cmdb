...
func (s *Service) FindNodePathForHost(ctx *rest.Contexts) {
	...
	hostsPath := make([]types2.HostNodePath, len(req.HostIDs))
	for outerIdx, hostID := range req.HostIDs {
		nodes := relation.HostWithNode[hostID]
		paths := make([]types2.NodePath, 0)
		...
		hostsPath[outerIdx] = types2.HostNodePath{
			HostID: hostID,
			Paths:  paths,
		}
	}

	ctx.RespEntity(types2.HostPathData{
		Info: hostsPath,
	})
}

func (s *Service) getHostNodeRelation(kit *rest.Kit, hostIDs []int64) (*types2.HostNodeRelation, error) {
	cond := mapstr.MapStr{common.BKHostIDField: mapstr.MapStr{common.BKDBIN: hostIDs}}
	fields := []string{
		common.BKFieldID, common.BKAppIDField, types2.BKClusterIDFiled, common.BKHostIDField,
	}
	...
	bizIDs := make([]int64, 0)
	hostWithNode := make(map[int64][]types2.Node)
	...
	return &types2.HostNodeRelation{
		BizIDs:              bizIDs,
...
func (s *Service) BatchCreateNode(ctx *rest.Contexts) {
	data := new(types2.CreateNodesOption)
...
func (s *Service) SearchNodes(ctx *rest.Contexts) {
	...
	searchCond := new(types2.QueryNodeOption)
	...
	filter[types2.BKBizIDField] = bizID
	...
	if searchCond.ClusterID != 0 {
		filter[types2.BKClusterIDFiled] = searchCond.ClusterID
	}
	...
	if searchCond.Page.EnableCount {
		...
		counts, err := s.Engine.CoreAPI.CoreService().Count().GetCountByFilter(ctx.Kit.Ctx, ctx.Kit.Header,
			types2.BKTableNameBaseNode, filter)
		if err != nil {
...
func (s *Service) UpdateNodeFields(ctx *rest.Contexts) {
	...
	data := new(types2.UpdateNodeOption)
	if err := ctx.DecodeInto(data); err != nil {
		ctx.RespAutoError(err)
		return
	}
