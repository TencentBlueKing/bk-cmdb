...
type ClusterOperationInterface interface {
	CreateCluster(kit *rest.Kit, data *types2.ClusterBaseFields, bizID int64, bkSupplierAccount string) (int64, error)
	DeleteCluster(kit *rest.Kit, bizID int64, option *types2.DeleteClusterOption, bkSupplierAccount string) error
	BatchDeleteNode(kit *rest.Kit, bizID int64, option *types2.BatchDeleteNodeOption, bkSupplierAccount string) error
	BatchCreateNode(kit *rest.Kit, data *types2.CreateNodesOption, bizID int64, supplierAccount string) ([]int64, error)
	BatchCreatePod(kit *rest.Kit, data *types2.CreatePodsOption, bizID int64) ([]int64, error)
	SetProxy(inst ClusterOperationInterface)
}

// NewClusterOperation create a business instance
func NewClusterOperation(client api.ClientSetInterface,
...
type kube struct {
	clientSet   api.ClientSetInterface
...
func (b *kube) BatchDeleteNode(kit *rest.Kit, bizID int64, option *types2.BatchDeleteNodeOption,
	...
	for _, node := range option.Nodes {
		cond := map[string]interface{}{
			...
			types2.BKIDField:        map[string]interface{}{common.BKDBIN: node.IDs},
			common.BKOwnerIDField:   supplierAccount,
		}
		podCond = append(podCond, map[string]interface{}{
			types2.BKBizIDField:     bizID,
			types2.BKClusterIDFiled: node.ClusterID,
			types2.BKNodeIDField:    map[string]interface{}{common.BKDBIN: node.IDs},
			common.BKOwnerIDField:   supplierAccount,
		})
		num += len(node.IDs)
	...
	counts, err := b.clientSet.CoreService().Count().GetCountByFilter(kit.Ctx, kit.Header,
		types2.BKTableNameBasePod, podCond)
	if err != nil {
...
func (b *kube) DeleteCluster(kit *rest.Kit, bizID int64, option *types2.DeleteClusterOption,
	supplierAccount string) error {
...
func (b *kube) isExistKubeResource(kit *rest.Kit, option *types2.DeleteClusterOption, bizID int64,
	supplierAccount string) (bool, error) {

	var wg sync.WaitGroup
	var firstErr ccErr.CCErrorCoder

	workLoads := types2.GetWorkLoadTables()
	tables := []string{
		types2.BKTableNameBaseNamespace,
		types2.BKTableNameBaseNode,
		types2.BKTableNameBasePod,
	}
	count := make([]int64, 0)
		...
		go func(table string, bizID int64, supplierAccount string) {
			...
			if len(option.IDs) > 0 {
				filter = []map[string]interface{}{
					{
						types2.BKClusterIDFiled: map[string]interface{}{
							common.BKDBIN: option.IDs,
						},
						common.BKAppIDField:   bizID,
						common.BKOwnerIDField: supplierAccount,
...
func (b *kube) BatchCreatePod(kit *rest.Kit, data *types2.CreatePodsOption, bizID int64) (
	[]int64, error) {
	...
	for _, pod := range data.Pods {
		...
		filter[types2.BKClusterIDFiled] = *pod.Spec.ClusterID
		filter[types2.BKNamespaceIDField] = *pod.Spec.NamespaceID
		filter[types2.BKNodeIDField] = *pod.Spec.NodeID
		filter[types2.KubeNameField] = *pod.Name
		filter[types2.RefKindField] = *pod.Spec.WorkloadKind
		filter[types2.RefIDField] = *pod.Spec.WorkloadID
		filters = append(filters, filter)
	}

	counts, err := b.clientSet.CoreService().Count().GetCountByFilter(kit.Ctx, kit.Header,
		types2.BKTableNameBasePod, filters)
	if err != nil {
...
func (b *kube) BatchCreateNode(kit *rest.Kit, data *types2.CreateNodesOption, bizID int64, supplierAccount string) (
	[]int64, error) {
	...
	counts, err := b.clientSet.CoreService().Count().GetCountByFilter(kit.Ctx, kit.Header,
		types2.BKTableNameBaseNode, []map[string]interface{}{cond})
	if err != nil {
...
func (b *kube) CreateCluster(kit *rest.Kit, data *types2.ClusterBaseFields, bizID int64, supplierAccount string) (
	int64, error) {
	...
	counts, err := b.clientSet.CoreService().Count().GetCountByFilter(kit.Ctx, kit.Header,
		types2.BKTableNameBaseCluster, []map[string]interface{}{cond})
	if err != nil {
		blog.Errorf("count cluster failed, cond: %#v, err: %v, rid: %s", cond, err, kit.Rid)
	...
	generateAuditParameter := auditlog.NewGenerateAuditCommonParameter(kit, metadata.AuditCreate)
	audit := auditlog.NewKubeAudit(b.clientSet.CoreService())
	auditLog, err := audit.GenerateClusterAuditLog(generateAuditParameter, []types2.Cluster{*result.Info})
	if err != nil {
		blog.Errorf("create cluster, generate audit log failed, err: %v, rid: %s", err, kit.Rid)
		return 0, err
	}
