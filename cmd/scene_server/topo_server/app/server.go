...
import (
	"configcenter/cmd/scene_server/topo_server/app/options"
	"configcenter/cmd/scene_server/topo_server/logics"
	"configcenter/cmd/scene_server/topo_server/service"
	...
	"configcenter/thirdparty/elasticsearch"
)
...
func (t *TopoServer) onTopoConfigUpdate(previous, current cc.ProcessConfig) {
	...
	t.Config.Auth, err = iamtype.ParseConfigFromKV("authServer", nil)
	if err != nil {
...
func Run(ctx context.Context, cancel context.CancelFunc, op *options.ServerOption) error {
	...
	if auth.EnableAuthorize() {
		...
		iamCli, err = iamtype.NewIAM(server.Config.Auth, engine.Metric().Registry())
		if err != nil {
