...
func (s *Service) SyncHostIdentifier(ctx *rest.Contexts) {
	...
	for retry && time.Now().Unix() < endTime {
		...
		for _, hostInfo := range task.HostInfos {
			key := hostidentifier2.HostKey(strconv.FormatInt(hostInfo.CloudID, 10), hostInfo.HostInnerIP)
			...
			if code == hostidentifier2.Handling {
				retry = true
			}
...
func (s *Service) PushHostIdentifier(ctx *rest.Contexts) {
	...
	for _, host := range task.HostInfos {
		key := hostidentifier2.HostKey(strconv.FormatInt(host.CloudID, 10), host.HostInnerIP)
