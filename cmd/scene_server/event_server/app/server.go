...
import (
	"configcenter/cmd/scene_server/event_server/app/options"
	svc "configcenter/cmd/scene_server/event_server/service"
	hostidentifier2 "configcenter/cmd/scene_server/event_server/sync/hostidentifier"
	...
	"configcenter/thirdparty/gse/client"
)
...
func (es *EventServer) RedisCli() redis2.Client {
	return es.redisCli
}
...
func (es *EventServer) initConfigs() error {
	...
	es.config.Auth, err = iamtype.ParseConfigFromKV("authServer", nil)
	if err != nil {
		blog.Errorf("parse auth center config failed: %v", err)
		return err
	}

	identifierConf, err := hostidentifier2.ParseIdentifierConf()
	if err != nil {
...
func (es *EventServer) initModules() error {
	...
	// connect to cc redis.
	redisCli, err := redis2.NewFromConfig(es.config.Redis)
	if err != nil {
	...
	es.service.SetAuthorizer(iamtype.NewAuthorizer(es.engine.CoreAPI))

	iamCli := new(iamtype.IAM)
	if auth.EnableAuthorize() {
		blog.Info("enable auth center access")
		iamCli, err = iamtype.NewIAM(es.config.Auth, es.engine.Metric().Registry())
		if err != nil {
...
func (es *EventServer) runSyncData() error {
	...
	syncData, err := hostidentifier2.NewHostIdentifier(es.ctx, es.redisCli, es.engine, es.config.IdentifierConf,
