...
import (
	"configcenter/cmd/scene_server/host_server/app/options"
	"configcenter/cmd/scene_server/host_server/logics"
	hostsvc "configcenter/cmd/scene_server/host_server/service"
...
func Run(ctx context.Context, cancel context.CancelFunc, op *options.ServerOption) error {
	...
	hostSrv.Config.Auth, err = iamtype.ParseConfigFromKV("authServer", nil)
	if err != nil {
	...
	if auth.EnableAuthorize() {
		...
		iamCli, err = iamtype.NewIAM(hostSrv.Config.Auth, engine.Metric().Registry())
		if err != nil {
