...
func addswitchAssociation(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	falseVar := false
	...
	_, _, err := upgrader2.Upsert(ctx, db, common.BKTableNameObjAsst, switchAsst, "id", []string{"bk_obj_id", "bk_asst_obj_id"}, []string{"id"})
	if err != nil {
		return err
	}
...
func changeNetDeviceTableName(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
