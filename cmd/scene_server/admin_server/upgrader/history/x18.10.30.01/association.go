...
func createAssociationTable(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
...
func createInstanceAssociationIndex(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
...
func addPresetAssociationType(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	...
	for _, asstType := range asstTypes {
		_, _, err := upgrader2.Upsert(ctx, db, tablename, asstType, "id", []string{"bk_asst_id"}, []string{"id"})
		if err != nil {
			return err
		}
	}
...
func reconcilAsstData(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
...
func dropFlagColumn(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	flag := "updateflag"
