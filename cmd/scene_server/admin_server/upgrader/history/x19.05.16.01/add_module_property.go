...
func addModuleProperty(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	...
	_, _, err := upgrader2.Upsert(ctx, db, common.BKTableNameObjAttDes, serviceCategoryIDProperty, "id", uniqueFields, []string{})
	if nil != err {
	...
	if _, _, err := upgrader2.Upsert(ctx, db, common.BKTableNameObjAttDes, serviceTemplateIDProperty, "id", uniqueFields, []string{}); err != nil {
