...
func updateConfigAdmin(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	config := `
{
    "backend": {
        "snapshotBizName": "蓝鲸",
...
`
	config = upgrader2.SetConfigSiteTitle(config)
	upgrader2.AddConfigAdminChangeItem("y3.10.202105261459", "调整拓扑实例和模板名称的校验规则的配置", config)
	return upgrader2.UpgradeConfigAdmin(ctx, db, "y3.10.202105261459")
}
