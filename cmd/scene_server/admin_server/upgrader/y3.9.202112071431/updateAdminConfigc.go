...
func updatePlatformConfigAdmin(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	...
	// 将上一份配置文件写入到history中
	upgrader2.AddConfigAdminChangeItem("y3.9.202105261459", "调整拓扑实例和模板名称的校验规则的配置", config)

	// 将本次平台管理配置文件写入history中
	upgrader2.AddConfigAdminChangeItem("y3.9.202112071431", "调整平台管理配置", metadata.InitAdminConfig)

	return upgrader2.UpgradePlatConfigAdmin(ctx, db, "y3.9.202112071431")
}
