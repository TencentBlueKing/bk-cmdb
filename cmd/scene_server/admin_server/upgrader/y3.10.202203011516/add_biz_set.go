...
func addBizSetObjectRow(ctx context.Context, db dal.RDB, ownerID string) error {
	...
	_, _, err = upgrader2.Upsert(ctx, db, common.BKTableNameObjDes, dataRows, "id", uniqueKeys, []string{"id"})
	if err != nil {
...
func addObjectUnique(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
...
func addBizSetPropertyGroup(ctx context.Context, db dal.RDB, ownerID string) error {
	...
	for _, row := range rows {
		if _, _, err := upgrader2.Upsert(ctx, db, common.BKTableNamePropertyGroup, row, "id",
...
func addBizSetObjectAttrRow(ctx context.Context, db dal.RDB, ownerID string) error {
	...
	for _, row := range dataRows {
		...
		row.Description = ""
		_, _, err := upgrader2.Upsert(ctx, db, common.BKTableNameObjAttDes, row, "id", uniqueFields, []string{})
		if err != nil {
...
func addBizSetPropertyOption(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
