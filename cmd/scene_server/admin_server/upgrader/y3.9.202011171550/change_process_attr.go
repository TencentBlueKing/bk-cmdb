...
func changeProcessAttrs(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
...
func addProcessAttr(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	//  增加【启动等待时长】字段
	now := time.Now()
	row := attribute{
		BizID:             0,
	...
	if _, _, err := upgrader2.Upsert(ctx, db, common.BKTableNameObjAttDes, row, "id", uniqueFields, []string{}); err != nil {
...
func deleteProcessInstsFields(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	filter := map[string]interface{}{
...
func deleteProcessTemplateInstsFields(ctx context.Context, db dal.RDB, conf *upgrader2.Config) error {
	filter := map[string]interface{}{
		"$or": []map[string]interface{}{
			{
