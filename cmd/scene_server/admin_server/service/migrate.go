...
import (
	...
	streamtypes "configcenter/pkg/storage/stream/types"
	"encoding/json"
	"fmt"
	"net/http"
	"path/filepath"
...
func (s *Service) createWatchDBChainCollections(rid string) error {
	...
	for _, cursorType := range cursorTypes {
		key, err := event2.GetResourceKeyWithCursorType(cursorType)
		if err != nil {
		...
		if key.Collection() == event2.HostIdentityKey.Collection() {
		...
		if key.Collection() == event2.BizSetRelationKey.Collection() {
