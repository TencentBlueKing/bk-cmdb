...
func (s *AuthService) genResourcePullMethod(kit *rest.Kit, resourceType iamtype.TypeID) (types.ResourcePullMethod, error) {
	switch resourceType {
	case iamtype.Host:
	...
	case iamtype.Business, iamtype.BusinessForHostTrans:
		...
		return types.ResourcePullMethod{
			ListAttr:      s.lgc.ListAttr,
			ListAttrValue: s.lgc.ListAttrValue,
			ListInstance: func(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.ListInstanceFilter,
				page types.Page) (*types.ListInstanceResult, error) {
				return s.lgc.ListSystemInstance(kit, resourceType, filter, page, extraCond)
			},
			FetchInstanceInfo: func(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.FetchInstanceInfoFilter) (
				[]map[string]interface{}, error) {
				return s.lgc.FetchInstanceInfo(kit, resourceType, filter, extraCond)
			},
			ListInstanceByPolicy: func(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.ListInstanceByPolicyFilter,
	...
	case iamtype.BizCustomQuery, iamtype.BizProcessServiceTemplate, iamtype.BizSetTemplate:
		return types.ResourcePullMethod{
	...
	case iamtype.SysModelGroup, iamtype.SysCloudAccount, iamtype.SysCloudResourceTask, iamtype.InstAsstEvent, iamtype.BizSet:
		return types.ResourcePullMethod{
			ListInstance: func(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.ListInstanceFilter,
				page types.Page) (*types.ListInstanceResult, error) {
	...
	case iamtype.SysModel, iamtype.SysInstanceModel, iamtype.SysModelEvent, iamtype.MainlineModelEvent:
		...
		switch resourceType {
		case iamtype.SysModelEvent, iamtype.SysInstanceModel:
	...
	case iamtype.SysOperationStatistic, iamtype.SysAuditLog, iamtype.BizCustomField, iamtype.BizHostApply,
		iamtype.BizTopology, iamtype.SysEventWatch, iamtype.BizProcessServiceCategory, iamtype.BizProcessServiceInstance:
		return types.ResourcePullMethod{}, nil
	case iamtype.KubeWorkloadEvent:
	...
	default:
		if iamtype.IsIAMSysInstance(resourceType) {
			return types.ResourcePullMethod{
				...
				ListInstanceByPolicy: func(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.ListInstanceByPolicyFilter, page types.Page) (result *types.ListInstanceResult, e error) {
					return s.lgc.ListInstanceByPolicy(kit, resourceType, filter, page, nil)
				},
			}, nil
		}
...
func (s *AuthService) genKubeWorkloadEventMethod(kit *rest.Kit) (types.ResourcePullMethod, error) {
	return types.ResourcePullMethod{
		...
		ListInstanceByPolicy: func(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.ListInstanceByPolicyFilter,
			page types.Page) (*types.ListInstanceResult, error) {
			return nil, fmt.Errorf("%s do not support %s", iamtype.KubeWorkloadEvent, types.ListInstanceByPolicyMethod)
		},
	}, nil
}
