...
func getResourceTableName(resourceType iamtype.TypeID) string {
	switch resourceType {
	case iamtype.Host:
	...
	case iamtype.BizCustomQuery:
		return common.BKTableNameDynamicGroup
	case iamtype.BizProcessServiceTemplate:
		return common.BKTableNameServiceTemplate
	case iamtype.BizProcessServiceCategory:
		return common.BKTableNameServiceCategory
	case iamtype.BizProcessServiceInstance:
		return common.BKTableNameServiceInstance
	case iamtype.BizSetTemplate:
...
func isResourceIDStringType(resourceType iamtype.TypeID) bool {
	switch resourceType {
	case iamtype.BizCustomQuery:
		return true
	}
...
func getInstanceResourceObjID(resourceType iamtype.TypeID) string {
	switch resourceType {
	case iamtype.Host:
...
func (lgc *Logics) GetObjIDFromResourceType(ctx context.Context, header http.Header, resourceType iamtype.TypeID) (string,
	error) {
	rid := util.ExtractRequestIDFromContext(ctx)

	modelID, err := iamtype.GetModelIDFromIamSysInstance(resourceType)
	if err != nil {
		return "", err
	}
