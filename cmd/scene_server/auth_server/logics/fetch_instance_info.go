...
var resourceParentMap = iamtype.GetResourceParentMap()

// FetchInstanceInfo TODO
// fetch resource instances' specified attributes info using instance ids
func (lgc *Logics) FetchInstanceInfo(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.FetchInstanceInfoFilter,
...
func (lgc *Logics) FetchHostInfo(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.FetchInstanceInfoFilter) (
...
func (lgc *Logics) FetchObjInstInfo(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.FetchInstanceInfoFilter) (
	[]map[string]interface{}, error) {
...
func (lgc *Logics) getResourceIamPath(kit *rest.Kit, resourceType iamtype.TypeID, instance map[string]interface{}) ([]string, error) {
	if resourceType == iamtype.Host {
		return nil, kit.CCError.CCErrorf(common.CCErrCommParamsInvalid, common.BKResourceTypeField)
	}
...
func (lgc *Logics) getHostIamPath(kit *rest.Kit, resourceType iamtype.TypeID, hostList []int64) (map[int64][]string,
	error) {
	...
	for _, relation := range res.Info {
		...
		if relation.AppID == defaultBizID {
			path = "/" + string(iamtype.SysResourcePoolDirectory) + "," + strconv.FormatInt(relation.ModuleID, 10) + "/"
		} else {
			path = "/" + string(iamtype.Business) + "," + strconv.FormatInt(relation.AppID, 10) + "/"
		}
