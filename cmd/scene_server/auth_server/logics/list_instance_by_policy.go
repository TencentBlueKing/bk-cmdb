...
func (lgc *Logics) ListInstanceByPolicy(kit *rest.Kit, resourceType iamtype.TypeID,
...
func (lgc *Logics) ListHostByPolicy(kit *rest.Kit, resourceType iamtype.TypeID, filter *types.ListInstanceByPolicyFilter,
...
func (lgc *Logics) ListInstancesWithAttributes(ctx context.Context, opts *sdktypes.ListWithAttributes) (
	[]string, error) {
	resourceType := iamtype.TypeID(opts.Type)
...
func (lgc *Logics) getAggrPolicy(opType operator2.OperType, attrPolicies []*operator2.Policy) (*operator2.Policy, error) {
	if len(attrPolicies) == 0 {
		return nil, fmt.Errorf("attribute policies can't be empty")
	}

	var policy *operator2.Policy
	...
	if opType == operator2.Or || opType == operator2.And {
		policy = &operator2.Policy{
			Operator: opType,
			Element: &operator2.Content{
