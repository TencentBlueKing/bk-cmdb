...
type Interface interface {
	GetUserPolicy(ctx context.Context, opt *types2.GetPolicyOption) (*operator.Policy, error)
	ListUserPolicies(ctx context.Context, opts *types2.ListPolicyOptions) ([]*types2.ActionPolicy, error)
	GetSystemToken(ctx context.Context) (string, error)
}

// NewClient TODO
func NewClient(conf types2.IamConfig, opt types2.Options) (Interface, error) {
...
type authClient struct {
	...
	// iam config
	config types2.IamConfig
}

func (ac *authClient) cloneHeader(ctx context.Context) http.Header {
	h := http.Header{}
	rid, ok := ctx.Value(types2.RequestIDKey).(string)
	if ok {
		h.Set(types2.RequestIDHeaderKey, rid)
	}
