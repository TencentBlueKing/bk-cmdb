...
import (
	...
	"configcenter/pkg/json"
)

func (a *Authorize) calculatePolicy(
	ctx context.Context,
	resources []types2.Resource,
	p *operator2.Policy) (bool, error) {
	...
	rscMap := make(map[string]*types2.Resource)
...
func (a *Authorize) authFieldValue(ctx context.Context, p *operator2.Policy, rscMap map[string]*types2.Resource) (bool, error) {
	...
	case operator2.IamPathKey:
		...
		if len(authPath) == 0 {
			...
			return a.authResourceAttribute(ctx, p.Operator, []*operator2.Policy{p}, authRsc)
		}
...
func (a *Authorize) authContent(ctx context.Context, p *operator2.Policy, rscMap map[string]*types2.Resource) (
	bool, error) {

	rid := ctx.Value(common.ContextRequestIDField)

	content, canContent := p.Element.(*operator2.Content)
	...
	if (p.Operator != operator2.And) && (p.Operator != operator2.Or) {
	...
	for _, policy := range content.Content {
		...
		default:
			...
			case operator2.IamPathKey:
				...
				// compatible for cases when resources to be authorized hasn't put its paths in attributes
				if len(authPath) == 0 {
					authorized, err = a.authResourceAttribute(ctx, policy.Operator, []*operator2.Policy{policy}, authRsc)
				} else {
			...
			default:
				...
				if policy.Operator != operator2.Equal && policy.Operator != operator2.In {
...
func (a *Authorize) authWithPath(p *operator2.Policy, fv *operator2.FieldValue, authPath []string) (bool, error) {
...
func (a *Authorize) authResourceAttribute(ctx context.Context, op operator2.OperType, attrPolicies []*operator2.Policy,
	rsc *types2.Resource) (bool, error) {

	listOpts := &types2.ListWithAttributes{
		Operator:     op,
...
// if a user has a path based auth policy, then we need to check if the user's path is matched with policy's path or
// not, if one of use's path is matched, then user is authorized.
func (a *Authorize) calculateAuthPath(p *operator2.Policy, fv *operator2.FieldValue, authPath []string) (bool, error) {
	if !reflect.ValueOf(fv.Value).IsValid() && len(authPath) == 0 {
		// if policy have the path, then user's auth path must can not be empty.
		// we consider this to be unauthorized.
		return false, nil
...
func getIamPath(attr types2.ResourceAttributes) ([]string, error) {
	path, exist := attr[types2.IamPathKey]
	if exist {
		if path == nil {
