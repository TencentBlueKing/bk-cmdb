...
func (a *Authorize) countPolicy(ctx context.Context, p *operator2.Policy, resourceType types2.ResourceType) (
	*types2.AuthorizeList, error) {
	...
	switch p.Operator {
	case operator2.And, operator2.Or:
		content, can := p.Element.(*operator2.Content)
		if !can {
	...
	case operator2.Any:
		...
		return &types2.AuthorizeList{IsAny: true}, nil

	default:
		fv, can := p.Element.(*operator2.FieldValue)
		if !can {
			return nil, errors.New("policy with invalid FieldValue field")
		}

		if fv.Field.Attribute == operator2.IamIDKey {
...
func (a *Authorize) countIamIDKey(op operator2.OperType, fv *operator2.FieldValue) ([]string, error) {
	if op == operator2.Equal {
...
func preAnalyzeContent(op operator2.OperType, content *operator2.Content) error {

	fieldMap := make(map[string]struct{})

	for _, conPolicy := range content.Content {
		fv, can := conPolicy.Element.(*operator2.FieldValue)
		// 不支持多层嵌套场景，此处直接返回
		if !can {
...
func (a *Authorize) countContent(ctx context.Context, op operator2.OperType, content *operator2.Content,
	resourceType types2.ResourceType) (idList *types2.AuthorizeList, err error) {

	err = preAnalyzeContent(op, content)
	if err != nil {
		return nil, err
	}
	allAttrPolicies := make([]*operator2.Policy, 0)
	allList := make([]types2.AuthorizeList, 0)
	idList = new(types2.AuthorizeList)

	for _, policy := range content.Content {
		switch policy.Operator {
		case operator2.And, operator2.Or:
			content, can := policy.Element.(*operator2.Content)
			if !can {
				return nil, errors.New("policy with invalid content field")
			}
		...
		default:
			fv, can := policy.Element.(*operator2.FieldValue)
			if !can {
				return nil, errors.New("policy with invalid FieldValue field")
			}
			...
			if fv.Field.Attribute == operator2.IamIDKey {
				...
				allList = append(allList, types2.AuthorizeList{Ids: list})
	...
	if len(allAttrPolicies) != 0 {
		...
		allList = append(allList, types2.AuthorizeList{Ids: ids})
	}

	return calculateSet(op, allList)
}

func calculateSetForAnd(sets []types2.AuthorizeList, cnt int) (*types2.AuthorizeList, error) {
	...
	return &types2.AuthorizeList{Ids: set}, nil
}

func calculateSetForOr(sets []types2.AuthorizeList, cnt int) (*types2.AuthorizeList, error) {
	if cnt == 1 {
		return &sets[0], nil
	}
	// now, at least we have two set.
...
func calculateSet(op operator2.OperType, sets []types2.AuthorizeList) (*types2.AuthorizeList, error) {
	if sets == nil {
	...
	cnt := len(sets)
	if cnt == 0 {
		return &types2.AuthorizeList{}, nil
	}
...
func hasIamPath(p *operator2.Policy) bool {
	switch p.Operator {
	case operator2.And, operator2.Or:
		content, can := p.Element.(*operator2.Content)
		if !can {
	...
	default:
		...
		if fv.Field.Attribute == types2.IamPathKey {
			return true
		}
