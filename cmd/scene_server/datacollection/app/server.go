...
import (
	"configcenter/cmd/scene_server/datacollection/app/options"
	collections2 "configcenter/cmd/scene_server/datacollection/collections"
	"configcenter/cmd/scene_server/datacollection/collections/hostsnap"
	"configcenter/cmd/scene_server/datacollection/collections/middleware"
	"configcenter/cmd/scene_server/datacollection/collections/netcollect"
	svc "configcenter/cmd/scene_server/datacollection/service"
	"configcenter/pkg/ac/extensions"
	iamtype "configcenter/pkg/ac/iam"
	"configcenter/pkg/storage/dal"
	kafka2 "configcenter/pkg/storage/dal/kafka"
	"configcenter/pkg/storage/dal/mongo"
	"configcenter/pkg/storage/dal/mongo/local"
	...
	apiutil "configcenter/api/util"
	"configcenter/pkg/auth"
	"configcenter/pkg/backbone"
	cc "configcenter/pkg/backbone/configcenter"
	...
	"configcenter/thirdparty/esbserver"
	"configcenter/thirdparty/esbserver/esbutil"
...
type DataCollection struct {
	...
	snapRedisCli redis2.Client

	// disRedisCli is discover redis client.
	disRedisCli redis2.Client

	// netRedisCli is net collect redis client.
	netRedisCli redis2.Client
	...
	porterManager *collections2.PorterManager
...
func NewDataCollection(ctx context.Context, op *options.ServerOption) (*DataCollection, error) {
	...
	// set hash.
	newDataCollection.hash = collections2.NewHash(svrInfo.RegisterIP, svrInfo.Port, engine.Discovery())
...
func (c *DataCollection) initConfigs() error {
	...
	c.config.Auth, err = iamtype.ParseConfigFromKV("authServer", nil)
	if err != nil {
...
func (c *DataCollection) initModules() error {
	...
	redisCli, err := redis2.NewFromConfig(c.config.CCRedis)
	if err != nil {
	...
	if c.config.SnapRedis.Enable != "false" {
		snapCli, err := redis2.NewFromConfig(c.config.SnapRedis)
		if err != nil {
	...
	if c.config.DiscoverRedis.Enable != "false" {
		disCli, err := redis2.NewFromConfig(c.config.DiscoverRedis)
		if nil != err {
	...
	if c.config.NetCollectRedis.Enable != "false" {
		netCli, err := redis2.NewFromConfig(c.config.NetCollectRedis)
		if nil != err {
	...
	if c.config.SnapReportMode == "kafka" {
		...
		if c.config.SnapKafka.User != "" && c.config.SnapKafka.Password != "" {
			...
			config.Net.SASL.SCRAMClientGeneratorFunc = func() sarama.SCRAMClient {
				return &kafka2.XDGSCRAMClient{HashGeneratorFcn: kafka2.SHA512}
			}
	...
	if auth.EnableAuthorize() {
		...
		iamCli, err = iamtype.NewIAM(c.config.Auth, c.engine.Metric().Registry())
		if err != nil {
...
func (c *DataCollection) runCollectPorters() {
	...
	c.porterManager = collections2.NewPorterManager()
	...
	if metadata.GseConfigReportMode(c.config.SnapReportMode) == metadata.GseConfigReportModeKafka {
		...
		kafkaPorter := collections2.NewKafkaPorter(snapPorterName, c.engine, c.ctx, analyzer, c.snapConsumerGroup,
			topic, c.registry)
	...
	if c.snapRedisCli != nil {
		...
		porter := collections2.NewSimplePorter(snapPorterName, c.engine, c.hash, analyzer, c.snapRedisCli, topic,
			c.registry)
	...
	if c.disRedisCli != nil {
		...
		porter := collections2.NewSimplePorter(middlewarePorterName, c.engine, c.hash, analyzer, c.disRedisCli, topic,
			c.registry)
		c.porterManager.AddPorter(porter)
	...
	if c.netRedisCli != nil {
		...
		porter := collections2.NewSimplePorter(netCollectPorterName, c.engine, c.hash, analyzer, c.netRedisCli, topic,
			c.registry)
		c.porterManager.AddPorter(porter)
