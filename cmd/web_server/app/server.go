...
import (
	...
	websvc "configcenter/cmd/web_server/service"
...
func Run(ctx context.Context, cancel context.CancelFunc, op *options.ServerOption) error {
	...
	if webSvr.Config.Redis.MasterName == "" {
		...
		service.Session, redisErr = redis2.NewRedisStore(10, "tcp", webSvr.Config.Redis.Address,
		...
		service.Session, redisErr = redis2.NewRedisStoreWithSentinel(address, 10, webSvr.Config.Redis.MasterName, "tcp",
			webSvr.Config.Redis.Password, webSvr.Config.Redis.SentinelPassword, []byte("secret"))
		if redisErr != nil {
			return fmt.Errorf("failed to create new redis store, error info is %v", redisErr)
		}
	}
	cacheCli, err := redis2.NewFromConfig(webSvr.Config.Redis)
