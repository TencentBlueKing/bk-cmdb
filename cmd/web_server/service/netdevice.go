...
func (s *Service) ExportNetDevice(c *gin.Context) {
	...
	fields := logics2.GetNetDevicefield(defLang)
	logics2.AddNetDeviceExtFields(fields, defLang)

	if err = logics2.BuildNetDeviceExcelFromData(ctx, defLang, fields, deviceInfo, sheet, file); err != nil {
	...
	logics2.AddDownExcelHttpHeader(c, "netdevice.xlsx")
	c.File(dirFileName)
...
func (s *Service) BuildDownLoadNetDeviceExcelTemplate(c *gin.Context) {
	...
	if err := logics2.BuildNetDeviceExcelTemplate(c.Request.Header, defLang, file); nil != err {
	...
	logics2.AddDownExcelHttpHeader(c, fmt.Sprintf("template_%s.xlsx", common.BKNetDevice))
...
func getNetDevicesFromFile(
	...
	netDevice, errMsgs, err := logics2.GetImportNetDevices(header, defLang, file)
	if nil != err {
		blog.Errorf("[Import Net Device] failed, error:%s, rid: %s", err.Error(), rid)
