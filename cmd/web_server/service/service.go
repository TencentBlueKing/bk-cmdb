...
import (
	"configcenter/cmd/web_server/app/options"
	"configcenter/cmd/web_server/logics"
	middleware2 "configcenter/cmd/web_server/middleware"
	...
	"configcenter/pkg/webservice/ginservice"
...
type Service struct {
	*options.ServerOption
	...
	CacheCli redis2.Client
	*logics.Logics
	Config  *options.Config
	Session redis2.RedisStore
}
...
func (s *Service) WebService() *gin.Engine {
	...
	ws.Use(middleware2.RequestIDMiddleware)
	ws.Use(sessions.Sessions(s.Config.Session.Name, s.Session))
	ws.Use(middleware2.ValidLogin(*s.Config, s.Discovery()))
