...
import (
	webcom "configcenter/cmd/web_server/common"
	user2 "configcenter/cmd/web_server/middleware/user"
	"encoding/json"
	"fmt"
	"net/http"
	"strconv"
...
func (s *Service) GetUserList(c *gin.Context) {
	...
	userManger := user2.NewUser(*s.Config, s.Engine, s.CacheCli)
...
func (s *Service) UpdateSupplier(c *gin.Context) {
	...
	ownerM := user2.NewOwnerManager(uin, supplier.OwnerID, language)
