...
import (
	common2 "configcenter/cmd/web_server/common"
	logics2 "configcenter/cmd/web_server/logics"
	"context"
	"encoding/json"
	"fmt"
	"math/rand"
	...
	lang "configcenter/pkg/language"
...
func (s *Service) ImportNetProperty(c *gin.Context) {
	...
	language := common2.GetLanguageByHTTPRequest(c)
	defLang := s.Language.CreateDefaultCCLanguageIf(language)
	defErr := s.CCErr.CreateDefaultCCErrorIf(language)
	common2.SetProxyHeader(c)
...
func (s *Service) ExportNetProperty(c *gin.Context) {
	...
	fields := logics2.GetNetPropertyField(defLang)
	logics2.AddNetPropertyExtFields(fields, defLang)

	if err = logics2.BuildNetPropertyExcelFromData(ctx, defLang, fields, netPropertyInfo, sheet, file); err != nil {
	...
	dirFileName := fmt.Sprintf("%s/export", common2.ResourcePath)
	...
	logics2.ProductExcelCommentSheet(ctx, file, defLang)
	...
	logics2.AddDownExcelHttpHeader(c, "netproperty.xlsx")
	c.File(dirFileName)
...
func (s *Service) BuildDownLoadNetPropertyExcelTemplate(c *gin.Context) {
	...
	dir := common2.ResourcePath + "/template/"
	...
	if err := logics2.BuildNetPropertyExcelTemplate(c.Request.Header, defLang, file); nil != err {
	...
	logics2.AddDownExcelHttpHeader(c, fmt.Sprintf("template_%s.xlsx", common.BKNetProperty))
...
func openNetPropertyUploadedFile(c *gin.Context, defErr errors.DefaultCCErrorIf) (file *xlsx.File, err error, errMsg string) {
	...
	dir := common2.ResourcePath + "/import/"
	if _, err = os.Stat(dir); nil != err {
		if err = os.MkdirAll(dir, os.ModeDir|os.ModePerm); nil != err {
...
func getNetPropertiesFromFile(header http.Header, defLang lang.DefaultCCLanguageIf, defErr errors.DefaultCCErrorIf, file *xlsx.File) (
	...
	netProperty, errMsgs, err := logics2.GetImportNetProperty(header, defLang, file)
	if nil != err {
		blog.Errorf("[Import Net Property] failed, error:%s, rid: %s", err.Error(), rid)
