...
func (s *Service) ExportHost(c *gin.Context) {
	...
	filterFields := logics2.GetFilterFields(objID)
	customFields := logics2.GetCustomFields(filterFields, input.CustomFields)
	...
	logics2.AddDownExcelHttpHeader(c, "bk_cmdb_export_host.xlsx")
	c.File(dirFileName)
...
func (s *Service) BuildDownLoadExcelTemplate(c *gin.Context) {
	...
	if objID == common.BKInnerObjIDHost {
		logics2.AddDownExcelHttpHeader(c, "bk_cmdb_import_host.xlsx")
	} else {
		logics2.AddDownExcelHttpHeader(c, fmt.Sprintf("bk_cmdb_inst_%s.xlsx", objID))
	}
...
func (s *Service) handleHostInfo(c *gin.Context, fields map[string]logics2.Property, appID int64,
