## [Version: v3.10.19-beta4] - 2022-09-26
**功能优化**
- 优化更新服务实例后同步模板处显示
...
## [Version: v3.10.19-beta3] - 2022-09-26
**功能优化**
- 优化权限申请资源实例显示问题
- 优化点击icon后菜单显示
...
## [Version: v3.10.19-beta2] - 2022-09-23
**功能优化**
- 创建模块还没有填写就报错
- 优化模版列表的操作提示
- 状态栏缺少名称，优化变更的底色
...
- 优化下拉框的间距问题
- 优化主机详情页字体大小
- 对于自动应用字段的场景，优化未一致的主机提示文案
- 主机自动应用中，优化添加字段时的忽略操作的文案
- 服务模板编辑界面，大编辑套小编辑，两处都有保存按钮，但实际上只有外面的保存按钮有效
- 优化加锁和开锁的icon
- 资源池移动空闲机至某目录时，目录下拉框没有把主界面上已置顶的目录置顶
- 优化防御性限制的提示
- 导入主机更改成事务保证一次性导入全部成功或完全失败
- 优化筛选项和标签字段命名
- 优化切换主机搜索和全文检索后，里面的输入框长度
- 优化横向导航
- 优化主机列表下的新增按钮被禁用处理方式
- 优化查看服务模板图标
- 优化业务下拉选项的图标
...
**缺陷修复**
- 解决主机属性自动应用里面，如果配置了按服务模板的自动应用。但是在拓扑里面，这个模板上，看到的“主机属性自动应用”却是未启动的问题。
- 解决主机自动应用，按服务模板配置如果这个业务没有模板会有报错的问题。
- 解决云资源发现-新建/编辑一个任务-跳转账户管理按钮，点击后，左边面板会跳转到云账户页面，但是右边tab没有关闭，页面的问题。
- 修复云账号里编辑一个已经存在的账号，改了id连通测试可以通过的问题。
- 解决同一个“云区域”对象的数据，节点管理和CMDB两边源数据保持同步的问题。
- 解决模型关系的“隐藏&显示”icon没有正常变化的问题
- 修复集群模板中无集群名字
- 修复主线模型相关功能点的判断逻辑及创建主线模型的分组设置为未分类
- 修复删除不存在的模型属性分组panic的问题
- 解决添加主机时显示的进程信息和添加后查看主机的进程信息不一致的问题
- 解决全文检索高级搜索切换条件点确定后未立刻生效的问题

## [Version: v3.10.19-beta] - 2022-09-15
**新增功能**
