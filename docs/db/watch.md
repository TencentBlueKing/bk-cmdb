...
|---------------------|------------|----------------------------------------------|
| _id                 | ObjectId   | 数据唯一ID                                       |
| id                  | NumberLong | 自增ID，用于从某个token开始批量查询之后的token，相比链式操作减少读DB的次数 |
| cluster_time        | ISODate    | 事件时间，用于设置mongodb TTL索引，2天过期（不用手动gc）          |
| oid                 | String     | 事件ID，redis查详情事件失败时根据这个查                      |
| type                | String     | 操作类型                                         |
| token               | String     | 数据库恢复令牌，用于重新监听                               |
| cursor              | String     | cc的事件游标                                      |
| inst_id             | NumberLong | 实例ID，用于后续事件去重                                |
