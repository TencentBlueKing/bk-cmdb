...
func (lgc *Logics) fetchAssociationData(ctx context.Context, header http.Header, objID string, instIDArr []int64,
	...
	if err != nil {
		blog.Errorf("fetchAssociationData fetch %s association  err: %v, input: %+v, rid: %s", objID, err, input, rid)
		return nil, ccErr.Error(common.CCErrCommHTTPDoRequestFailed)
	}
	if ccErr := bkObjRst.CCError(); ccErr != nil {
		blog.Errorf("fetchAssociationData fetch %s association  err: %v, input: %+v, rid:%s", objID, ccErr, input, rid)
		return nil, ccErr
	}
