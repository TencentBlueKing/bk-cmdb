...
func getAllOrganization(kit *rest.Kit, orgIDs []int64) ([]user.DepartmentItem, errors.CCErrorCoder) {
	...
	for _, ids := range orgIDList {
		...
		go func(ids []int64) {
			...
			result, err := apigw.Client().User().ListDeptByIDs(kit.Ctx, kit.Header, ids)
			if err != nil {
				firstErr = err
				blog.Errorf("get department by apigw failed, params: %+v, err: %v, rid: %s", ids, err, kit.Rid)
				return
			}

			lock.Lock()
			departments = append(departments, result.Results...)
			lock.Unlock()
		}(ids)
	}
	wg.Wait()
...
func (d *Client) getUsernameFromApigw(kit *rest.Kit, userList []string) (map[string]string, error) {
	...
	for _, users := range userListArr {
		userListEsbSub, errNew := user.GetUserList(d.GinCtx, &metadata.GetUserListOptions{Usernames: users})
		if errNew != nil {
			blog.Errorf("get users(%+v) list from apigw failed, err: %v, rid: %s", users, errNew, kit.Rid)
			return nil, errNew.ToCCError(kit.CCError)
		}

		for _, userInfo := range userListEsbSub {
