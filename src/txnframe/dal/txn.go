...
func (t *txn) Commit(commit func() (interface{}, error)) (interface{}, error) {
    ...
    cResult, err := commit()
    if err != nil {
        blog.Errorf("commit txn[%s] sub txn[%s] operation failed, err: %v", t.txnID, t.subTxnID, err)
        return nil, err
    }

    // step 7: record delete success operation
    if err := t.txnClient.SubTxnSuccess(t.txnID, t.subTxnID); err != nil {
        blog.Errorf("commit operation success, but report sub txn success status failed, txnID[%s], subTxnID[%s], err: %v", t.txnID, t.subTxnID, err)
