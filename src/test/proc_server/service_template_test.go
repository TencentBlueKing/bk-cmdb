...
var _ = Describe("service template test", func() {
	...
	Describe("process template test", func() {
		...
		It("search service instance by set template id", func() {
			...
			rsp, err := serviceClient.SearchServiceInstanceBySetTemplate(context.Background(),
				strconv.FormatInt(bizId, 10), header, input)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.Result).To(Equal(true), rsp.ToString())
		})
	...
	Describe("update template test", func() {
		...
		It("update service template with same name as another module in the same set with a module using this template",
			func() {
				input := map[string]interface{}{
					common.BKAppIDField:   bizId,
					"id":                  serviceTemplateId,
...
var _ = Describe("service template host apply rule test", func() {
	...
	var servTmplTestBizID, idleModuleID, servTmplTestSetID, servTmplTestModuleID, categoryID, servTmplID int64
	...
	It("service template host apply rule test", func() {
		...
		func() {
			...
			Expect(err).NotTo(HaveOccurred())
			idleModuleID = test.GetBizIdleModule(servTmplTestBizID)
		}()
		...
		func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(len(result.Success)).To(Equal(3))
		...
		func() {
			input := &metadata.DefaultModuleHostConfigParams{
				...
				HostIDs:       []int64{hostID1, hostID2, hostID3},
		...
		func() {
			for {
				...
				rsp, err := serviceClient.ListHostApplyRuleTaskStatus(context.Background(), header, &input)
				util.RegisterResponseWithRid(rsp, header)
		...
		func() {
			urlTemplate := "/module/host_apply_enable_status/bk_biz_id/%d"
