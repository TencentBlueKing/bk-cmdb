...
func generateObject(objectIDs []string) {
	for _, objID := range objectIDs {
		count, err := test.GetDB().Table(common.BKTableNameObjDes).Find(map[string]interface{}{"bk_obj_id": objID}).Count(context.Background())
		if err != nil {
			blog.Errorf("get count failed %v", err)
		}
		if count == 0 {
