...
var _ = Describe("Transaction Test", func() {
	Describe("test AutoRunTxn function", func() {
		...
		err = clientSet.CoreService().Txn().AutoRunTxn(ctx, true, test.GetHeader(), func() error {
		...
		attResult, err := clientSet.CoreService().Model().ReadModel(ctx, h, &query)
		Expect(err).Should(BeNil())
