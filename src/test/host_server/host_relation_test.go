...
var _ = Describe("find many host test", func() {
	...
	It("find many host test", func() {
		...
		func() {
			...
			Expect(rsp.Data[0].ModuleID).To(Equal(rsp.Data[1].ModuleID))
		...
		func() {
			...
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.BizID).To(Equal(hostTestBizID))
			Expect(rsp.CpuCount).To(Equal(int64(6)))
			Expect(rsp.HostCount).To(Equal(int64(2)))
			Expect(rsp.NoCpuHostCount).To(Equal(int64(0)))
		}()
		...
		func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.BizID).To(Equal(hostTestBizID))
			Expect(rsp.CpuCount).To(Equal(int64(20)))
			Expect(rsp.HostCount).To(Equal(int64(2)))
			Expect(rsp.NoCpuHostCount).To(Equal(int64(0)))
		}()
		...
		func() {
			...
			cloudID, err1 := commonutil.GetInt64ByInterface(rsp.Info[0][common.BKCloudIDField])
			Expect(err1).NotTo(HaveOccurred())
		...
		func() {
			...
			Expect(err1).NotTo(HaveOccurred())
			Expect(hostID).Should(BeElementOf(hostID4, hostID5))
			hostName := commonutil.GetStrByInterface(rsp.Relation[0].Host[common.BKHostNameField])
			Expect(hostName).Should(BeElementOf("host4", "host5"))
		...
		By("transfer host to recycle module test1")
		func() {
		...
		By("transfer host to recycle module test2")
		func() {
			opt := metadata.DefaultModuleHostConfigParams{
				ApplicationID: hostTestBizID,
		...
		By("find host topo relation text")
		func() {
			...
			Expect(rsp.Info[0].HostID).Should(BeElementOf(idleHostID1, idleHostID2))
		}()
		...
		func() {
			...
			Expect(rsp[common.BKSetNameField].(string)).To(Equal("hostRelationTest"))
		...
		By("find host relation with topo test3")
		func() {
			opt := metadata.FindHostRelationWtihTopoOpt{
				Business: hostTestBizID,
