...
var _ = Describe("find many host test", func() {
	...
	It("dynamic group object is the host test", func() {
		...
		func() {
			...
			rsp, err := hostServerClient.ExecuteDynamicGroup(ctx, dynamicGroupTestBizID, groupID1, header, &input)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		...
		func() {
			rsp, err := hostServerClient.GetDynamicGroup(ctx, dynamicGroupTestBizID, groupID1, header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		...
		func() {
			...
			rsp, err := hostServerClient.UpdateDynamicGroup(ctx, dynamicGroupTestBizID, groupID1, header, &input)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		}()
		...
		func() {
			...
			rsp, err := hostServerClient.SearchDynamicGroup(ctx, dynamicGroupTestBizID, header, &input)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
	...
	It("dynamic group object is the set test", func() {
		...
		func() {
			...
			rsp, err := hostServerClient.ExecuteDynamicGroup(ctx, dynamicGroupTestBizID, groupID2, header, &input)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
			Expect(rsp.Data.Count).To(Equal(2))
			Expect(len(rsp.Data.Info)).To(Equal(2))
		...
		By("get dynamic group test1")
		func() {
			rsp, err := hostServerClient.GetDynamicGroup(ctx, dynamicGroupTestBizID, groupID2, header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		...
		func() {
			...
			rsp, err := hostServerClient.UpdateDynamicGroup(ctx, dynamicGroupTestBizID, groupID2, header, &input)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		}()
		...
		func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(len(rsp.Data.Info)).To(Equal(2))
			Expect(rsp.Data.Info[0].Name).Should(BeElementOf("cc_group1", "cc_group2"))
		}()

		By("delete dynamic group test1")
		func() {
			rsp, err := hostServerClient.DeleteDynamicGroup(ctx, dynamicGroupTestBizID, groupID1, header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		}()
		...
		func() {
			rsp, err := hostServerClient.DeleteDynamicGroup(ctx, dynamicGroupTestBizID, groupID2, header)
			util.RegisterResponseWithRid(rsp, header)
			Expect(err).NotTo(HaveOccurred())
		}()
