...
var _ = Describe("host abnormal test", func() {
	...
	Describe("transfer host test", func() {
		...
		It("transfer multiple hosts with a nonexist host to module", func() {
			...
			rsp1, err := hostServerClient.SearchHostWithBiz(context.Background(), header, input1)
...
func prepareData() {
	...
	rsp3, err := hostServerClient.SearchHostWithBiz(context.Background(), header, input3)
	...
	rsp5, err := hostServerClient.SearchHostWithBiz(context.Background(), header, input5)
