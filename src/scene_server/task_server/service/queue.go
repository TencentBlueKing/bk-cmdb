...
func (tq *TaskQueue) compensateDBExecute(ctx context.Context) {
	...
	for {
		...
		taskIDs, err := tq.service.Logics.CompensateStatus(ctx, interruptedTaskIDs, rid)
		if err != nil {
			blog.Errorf("compensate duplicate tasks failed, tasks: %#v, err: %v, rid: %s", err, rows, rid)
			return
		}
		...
		blog.Infof("compensate tasks(%+v) status success, remaining: %+v, rid: %s", interruptedTaskIDs, taskIDs, rid)
		...
		blog.Infof("compensate tasks(%+v) to execute success, rid: %s", taskIDs, rid)
	}
}
