...
func NewHostSnap(chanName string, topicName string, maxSize int, redisCli, snapCli *redis.Client, disCli *redis.Client) *HostSnap {
	if 0 == maxSize {
		maxSize = 100
	}
...
func (h *HostSnap) Run() {
	...
	if h.saveRunning() {
		...
		go h.subChan(h.chanName)
		go h.subChan(h.topicName)
	} else {
...
func (h *HostSnap) subChan(chanName string) {
	...
	defer func() {
		...
		h.subscribing = false
		blog.Infof("subChan Close: %s", chanName)
	}()

	var ts = time.Now()
	var cnt int64
	blog.Infof("subcribing channel %s", chanName)
	for {
		if false == h.isMaster {
