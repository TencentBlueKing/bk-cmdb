...
type SyncI interface {
	SyncData(kit *rest.Kit, opt *types.SyncDataOption) error
	Migrate(ctx context.Context, rid string) (*types.MigrateResult, error)
	GetWatchTasks() []*task.Task
}

// New full-text search sync interface instance
func New(conf *Config, cacheCli cacheservice.Cache) (SyncI, error) {
	if !conf.EnableSync {
	...
	// new cache instance
	cache, err := cache.New(cacheCli)
	if err != nil {
		blog.Errorf("new sync server cache failed, err: %v", err)
		return nil, err
	}
	f.cache = cache
	...
	if err = parser.InitParser(parserClientSet, cache); err != nil {
	...
	upgrader.InitUpgrader(f.esCli.Client, f, indexSetting)

	if err = f.addIncrementalSyncTask(); err != nil {
...
func (f *fullTextSearch) GetWatchTasks() []*task.Task {
	return f.tasks
}
