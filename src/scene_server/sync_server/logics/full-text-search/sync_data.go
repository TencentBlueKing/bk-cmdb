...
func (f *fullTextSearch) syncDataByIndex(kit *rest.Kit, index string) error {
	...
	case metadata.IndexNameObjectInstance:
		...
		ferrors.FatalErrHandler(200, 100, func() error {
			err := mongodb.Shard(kit.ShardOpts()).Table(common.BKTableNameObjDes).Find(nil).Fields(common.BKObjIDField,
				metadata.ModelFieldObjUUID).All(kit.Ctx, &objs)
			if err != nil {
				blog.Errorf("get objects failed, err: %v, rid: %s", err, kit.Rid)
				return err
			}
			return nil
		})
		...
		for _, obj := range objs {
			if common.IsInnerModel(obj.ObjectID) {
				continue
			}
			isQuoted, _, _ := metadata.ParseModelQuoteDestObjID(obj.ObjectID)
			if isQuoted {
				continue
			}
			collections = append(collections, common.GetInstTableName(obj.ObjectID, obj.UUID))
		}
	default:
	...
	for _, collection := range collections {
		existEsIDs, err := f.syncCollection(kit, index, collection, nil)
		if err != nil {
			return err
		}
	...
	return f.cleanEsData(kit, index, existEsIDMap)

}

func (f *fullTextSearch) cleanEsData(kit *rest.Kit, index string, existEsIDMap map[string]struct{}) error {
	...
	for {
		...
		err = ferrors.EsRespErrHandler(func() (bool, error) {
			scrollRes, err = f.esCli.Client.Scroll(index).Query(elastic.NewTermQuery(metadata.IndexPropertyTenantID,
				kit.TenantID)).ScrollId(scrollID).Do(kit.Ctx)
			if err != nil && err != io.EOF {
				blog.Errorf("scroll get %s es data failed, err: %v, rid: %s", index, err, kit.Rid)
				return false, err
			}
			return false, nil
		})
		...
		if err != nil || scrollRes.Hits == nil || scrollRes.Hits.TotalHits == nil {
			blog.Errorf("scroll get %s es data failed, err: %v, res: %v, rid: %s", index, err, scrollRes, kit.Rid)
			return err
		}
		...
		if len(delEsIDs) > 0 {
			syncer.addEsDeleteReq(delEsIDs, kit.Rid)
			if err = syncer.doBulk(kit.Ctx, kit.Rid); err != nil {
				blog.Infof("do %s es bulk delete failed, err: %v, del ids: %+v, rid: %s", index, err, delEsIDs, kit.Rid)
				continue
			}
		}
	}
...
func (f *fullTextSearch) syncCollection(kit *rest.Kit, index, coll string, oids []string) ([]string, error) {
	syncer, err := newDataSyncer(f.esCli.Client, index)
	if err != nil {
		return nil, err
	}
	...
	if len(oids) > 0 {
		...
		for i, oid := range oids {
			...
			if err != nil {
				blog.Errorf("parse mongodb oid from %s failed, err: %v, rid: %s", oid, err, kit.Rid)
				return nil, err
			}
			mongoOids[i] = mongoOid
		}
		cond := mapstr.MapStr{common.MongoMetaID: mapstr.MapStr{common.BKDBIN: mongoOids}}
		f.upsertDataByCond(kit, syncer, coll, cond)
		return nil, err
	}

	// lock full-text search compensate sync operation
	locker := lock.NewLocker(redis.Client())
	lockKey := genSyncLockKey(kit.TenantID, coll)
	locked, err := locker.Lock(lockKey, 10*time.Minute)
	if err != nil {
		blog.Errorf("lock full-text search sync failed, key: %s, err: %v, rid: %s", lockKey, err, kit.Rid)
		return nil, err
	}
	...
	defer func() {
		if err = locker.Unlock(); err != nil {
			blog.Errorf("unlock full-text search sync failed, key: %s, err: %v, rid: %s", lockKey, err, kit.Rid)
		}
	}()
	...
	cond := mapstr.MapStr{}
	for {
		oids := f.upsertDataByCond(kit, syncer, coll, cond)
		if len(oids) == 0 {
			return existEsIDs, nil
		}

		for _, oid := range oids {
			existEsIDs = append(existEsIDs, syncer.parser.GenEsID(kit.TenantID, oid.Hex()))
		}
...
func genSyncLockKey(tenantID, collection string) lock.StrFormat {
	return lock.GetLockKey("full:text:search:sync:%s:%s", tenantID, collection)
}
...
func (f *fullTextSearch) upsertDataByCond(kit *rest.Kit, syncer *dataSyncer, coll string,
	cond mapstr.MapStr) []primitive.ObjectID {

	findOpt := dbtypes.NewFindOpts().SetWithObjectID(true)
	allData := make([]mapStrWithOid, 0)
	ferrors.FatalErrHandler(200, 100, func() error {
		err := mongodb.Shard(kit.ShardOpts()).Table(coll).Find(cond, findOpt).Sort(common.MongoMetaID).
			Limit(ftypes.SyncDataPageSize).All(kit.Ctx, &allData)
		if err != nil {
			blog.Errorf("get data failed, cond: %+v, err: %v, rid: %s", cond, err, kit.Rid)
			return err
		}
		return nil
	})
	...
	dataMap := dataGetterMap[syncer.index](kit, f.cache, coll, allData)

	oids := make([]primitive.ObjectID, len(allData))
	for i, data := range allData {
		oids[i] = data.Oid
		syncer.addUpsertReq(kit, coll, data.Oid.Hex(), dataMap[data.Oid])
	}

	if err := syncer.doBulk(kit.Ctx, kit.Rid); err != nil {
		blog.Errorf("do es bulk request failed, err: %v, coll: %s, cond: %+v, rid: %s", err, coll, cond, kit.Rid)
		return oids
	}
...
type dataGetter func(*rest.Kit, *cache.Cache, string, []mapStrWithOid) map[primitive.ObjectID][]mapstr.MapStr
...
func modelDataGetter(kit *rest.Kit, _ *cache.Cache, _ string,
	allData []mapStrWithOid) map[primitive.ObjectID][]mapstr.MapStr {
	...
	attributes := pagedGetMongoData(kit, common.BKTableNameObjAttDes, cond, fields)
...
func objInstDataGetter(kit *rest.Kit, cache *cache.Cache, coll string,
	allData []mapStrWithOid) map[primitive.ObjectID][]mapstr.MapStr {

	objID := parser.GetObjIDByData(kit, cache, coll, allData[0].MapStr)

	instIDs := make([]int64, len(allData))
	for i, data := range allData {
		id, err := util.GetInt64ByInterface(data.MapStr[common.GetInstIDField(objID)])
		if err != nil {
			blog.Errorf("get %s instance id failed, err: %v, data: %+v, rid: %s", objID, err, data.MapStr, kit.Rid)
			continue
		}
		instIDs[i] = id
	}
	...
	ferrors.FatalErrHandler(200, 100, func() error {
		err := mongodb.Shard(kit.ShardOpts()).Table(common.BKTableNameModelQuoteRelation).Find(relCond).All(kit.Ctx,
			&relations)
		if err != nil {
	...
	for _, relation := range relations {
		table := common.GetObjInstTableName(relation.DestModelUUID)
		cond := mapstr.MapStr{common.BKInstIDField: mapstr.MapStr{common.BKDBIN: instIDs}}

		instances := pagedGetMongoData(kit, table, cond, make([]string, 0))
		for _, instance := range instances {
			instID, err := util.GetInt64ByInterface(instance[common.BKInstIDField])
			if err != nil {
				blog.Errorf("get quote instance id failed, err: %v, instance: %+v, rid: %s", err, instance, kit.Rid)
				continue
			}
...
func pagedGetMongoData(kit *rest.Kit, table string, cond mapstr.MapStr, fields []string) []mapstr.MapStr {
	...
	for {
		...
		ferrors.FatalErrHandler(200, 100, func() error {
			err := mongodb.Shard(kit.ShardOpts()).Table(table).Find(cond, findOpt).Fields(fields...).
				Sort(common.MongoMetaID).Limit(ftypes.SyncDataPageSize).All(kit.Ctx, &data)
			if err != nil {
				blog.Errorf("get paged data failed, table: %s, cond: %+v, err: %v, rid: %s", table, cond, err, kit.Rid)
				return err
			}
			return nil
		})
