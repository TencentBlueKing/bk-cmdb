...
func newObjInstParser(index string, cs *ClientSet, cache *cache.Cache) *objInstParser {
	return &objInstParser{commonParser: newCommonParser(index, cs, cache)}
}

// ParseData parse mongo data to es data
func (p *objInstParser) ParseData(kit *rest.Kit, info []mapstr.MapStr, coll string) (bool, mapstr.MapStr, error) {
	if len(info) == 0 {
	...
	data = p.cache.EnumIDToName(kit, data, objID)

	// get es id
	id, err := convMetaIDToStr(data, metadata.GetInstIDFieldByObjID(objID))
	if err != nil {
		blog.Errorf("get meta id failed, err: %v, data: %+v, obj: %s, rid: %s", err, data, objID, kit.Rid)
		return false, nil, err
	}

	_, esDoc, err := p.commonParser.ParseData(kit, info, coll)
	if err != nil {
		return false, nil, err
	}
	...
	if len(info) > 1 {
		...
		for i := 1; i < len(info); i++ {
			...
			if err != nil {
				blog.Errorf("parse quoted inst %+v keywords failed, err: %v, rid: %s", quotedInst, err, kit.Rid)
				return false, nil, err
			}
