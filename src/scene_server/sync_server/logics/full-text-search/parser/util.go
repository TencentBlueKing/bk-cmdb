...
func GetObjIDByData(kit *rest.Kit, cache *cache.Cache, coll string, data mapstr.MapStr) string {
	switch coll {
	...
	default:
		...
		// get obj id by collection
		uuid, err := common.GetObjectUUIDByTableName(coll)
		if err != nil {
			blog.Errorf("get uuid by table name %s failed, err: %v, rid: %s", coll, err, kit.Rid)
			return ""
		}

		return cache.GetObjIDByUUID(kit, uuid)
	}
}
