...
// deleteIndex delete old index
func (u *upgrader) deleteIndex(ctx context.Context, delIndexMap map[string]struct{}, rid string) error {
	if len(delIndexMap) == 0 {
		return nil
	}

	// get all exist es index names, ignores the not exist indexes
	existIndexes, err := u.esCli.IndexNames()
	if err != nil {
		blog.Errorf("get all exist es index names failed, err: %v, rid: %s", err, rid)
		return err
	}

	delIndexes := make([]string, 0)
	for _, index := range existIndexes {
		_, exists := delIndexMap[index]
		if exists {
	...
	// delete index with the target index name
	_, err = u.esCli.DeleteIndex(delIndexes...).Do(ctx)
	if err != nil {
		blog.Errorf("delete elastic index[%+v] failed, err: %v, rid: %s", delIndexes, err, rid)
		return err
	}
