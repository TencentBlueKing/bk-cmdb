...
func (u *upgrader) initCurrentEsIndex() {
	...
	u.indexSetting.Analysis = metadata.ESIndexAnalysisSettings{
		Normalizer: map[string]metadata.ESIndexNormalizerSettings{
	...
	u.currentIndexNameMap = make(map[string]struct{})
	for _, name := range types.AllIndexNames {
		meta := &metadata.ESIndexMetadata{
			...
			Mappings: metadata.ESIndexMetaMappings{
				Properties: map[string]metadata.ESIndexMetaMappingsProperty{
					...
					metadata.IndexPropertyTenantID: {PropertyType: metadata.IndexPropertyTypeKeyword},
					metadata.IndexPropertyBKBizID:  {PropertyType: metadata.IndexPropertyTypeKeyword},
					metadata.IndexPropertyKeywords: {
						PropertyType: metadata.IndexPropertyTypeKeyword,
						Fields: map[string]metadata.ESIndexMetaMappingsField{
							"lowercase": {
								Type:       metadata.IndexPropertyTypeKeyword,
		...
		esIndex := metadata.NewESIndex(name, types.IndexVersionMap[name], meta)
		types.IndexMap[name] = []*metadata.ESIndex{esIndex}
		u.currentIndexNameMap[esIndex.Name()] = struct{}{}
	}
}
