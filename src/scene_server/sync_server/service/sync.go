...
func (s *Service) SyncFullTextSearchData(cts *rest.Contexts) {
	...
	go func() {
		cts.Kit.Ctx = util.SetDBReadPreference(context.Background(), common.SecondaryPreferredMode)
		err := s.lgc.FullTextSearch.SyncData(cts.Kit, opt)
		if err != nil {
...
func (s *Service) MigrateFullTextSearch(cts *rest.Contexts) {
	...
	go func() {
		res, err := s.lgc.FullTextSearch.Migrate(cts.Kit.Ctx, cts.Kit.Rid)
		if err != nil {
