...
func (s *Service) AddHostFromAgent(req *restful.Request, resp *restful.Response) {
	...
	// auth: check authorization
	if err := s.AuthManager.AuthorizeBusinessesByID(srvData.ctx, srvData.header, authmeta.Update, appID); err != nil {
...
func (s *Service) NewHostSyncAppTopo(req *restful.Request, resp *restful.Response) {
	...
	// auth: check authorization
	if err := s.AuthManager.AuthorizeByBusinessID(srvData.ctx, srvData.header, authmeta.Update, hostList.ApplicationID); err != nil {
...
func (s *Service) MoveSetHost2IdleModule(req *restful.Request, resp *restful.Response) {
	...
	if err := s.AuthManager.AuthorizeBusinessesByID(srvData.ctx, srvData.header, authmeta.Update, hostIDArr...); err != nil {
		blog.Errorf("check business authorization failed, err: %v", err)
		resp.WriteError(http.StatusForbidden, &meta.RespError{Msg: srvData.ccErr.Error(common.CCErrCommAuthorizeFailed)})
		return
	}
