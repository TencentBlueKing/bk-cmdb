...
func (s *Service) MoveSetHost2IdleModule(req *restful.Request, resp *restful.Response) {
	...
	//check exist of app
	appQuery := mapstr.MapStr{common.BKAppIDField: data.ApplicationID}
	appResult, err := srvData.lgc.GetSingleApp(srvData.ctx, appQuery)
	if nil != err {
		blog.Errorf("MoveSetHost2IdleModule app error:%v,input:%v,rid:%s", err, data, srvData.rid)
		_ = resp.WriteError(http.StatusInternalServerError, &meta.RespError{Msg: err})
		return
	}
	if 0 == len(appResult) {
		blog.Errorf("MoveSetHost2IdleModule appid isn't exist,input:%v,rid:%s", data, srvData.rid)
		_ = resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: srvData.ccErr.Errorf(common.CCErrHostAPPNotFoundFail, data.ApplicationID)})
		return
	}
	...
	if data.SetID != 0 {
		//check exist of set
		setQuery := mapstr.MapStr{common.BKSetIDField: data.SetID}
		setResult, err := srvData.lgc.GetSetMapByCond(srvData.ctx, []string{common.BKSetIDField}, setQuery)
		if nil != err {
			blog.Errorf("MoveSetHost2IdleModule set error:%v,input:%+v,rid:%s", err, data, srvData.rid)
			_ = resp.WriteError(http.StatusInternalServerError, &meta.RespError{Msg: err})
			return
		}
		if 0 == len(setResult) {
			blog.Errorf("MoveSetHost2IdleModule, set not exist,input:%+v,rid:%s", data, srvData.rid)
			_ = resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: srvData.ccErr.Errorf(common.CCErrHostSetNotExist, data.SetID)})
			return
		}

		// check whether the relationship of app and set match
		setQuery1 := mapstr.MapStr{common.BKSetIDField: data.SetID, common.BKAppIDField: data.ApplicationID}
		setResult1, err := srvData.lgc.GetSetMapByCond(srvData.ctx, []string{common.BKSetIDField}, setQuery1)
		if nil != err {
			blog.Errorf("MoveSetHost2IdleModule set error:%v,input:%+v,rid:%s", err, data, srvData.rid)
			_ = resp.WriteError(http.StatusInternalServerError, &meta.RespError{Msg: err})
			return
		...
		if 0 == len(setResult1) {
			blog.Errorf("MoveSetHost2IdleModule, set didn't belong to the app,input:%+v,rid:%s", data, srvData.rid)
			_ = resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: srvData.ccErr.Errorf(common.CCErrHostSetNotBelongBusinessErr, data.SetID, data.ApplicationID)})
			return
		}
	}
	...
	if data.ModuleID != 0 {
		//check exist of set
		moduleQuery := mapstr.MapStr{common.BKModuleIDField: data.ModuleID}
		moduleResult, err := srvData.lgc.GetModuleMapByCond(srvData.ctx, []string{common.BKModuleIDField}, moduleQuery)
		if nil != err {
			blog.Errorf("MoveSetHost2IdleModule module error:%v,input:%+v,rid:%s", err, data, srvData.rid)
			_ = resp.WriteError(http.StatusInternalServerError, &meta.RespError{Msg: err})
			return
		}
		if 0 == len(moduleResult) {
			blog.Errorf("MoveSetHost2IdleModule module not exist,input:%+v,rid:%s", data, srvData.rid)
			_ = resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: srvData.ccErr.Errorf(common.CCErrHostModuleNotExist, fmt.Sprintf("%d", data.ModuleID))})
			return
		}

		// check whether the relationship of module and set match
		if data.SetID != 0 {
			moduleQuery1 := mapstr.MapStr{common.BKModuleIDField: data.ModuleID, common.BKSetIDField: data.SetID}
			moduleResult1, err := srvData.lgc.GetModuleMapByCond(srvData.ctx, []string{common.BKModuleIDField}, moduleQuery1)
			if nil != err {
				blog.Errorf("MoveSetHost2IdleModule GetModuleMapByCond error:%v,input:%+v,rid:%s", err, data, srvData.rid)
				_ = resp.WriteError(http.StatusInternalServerError, &meta.RespError{Msg: err})
				return
			}
			if 0 == len(moduleResult1) {
				blog.Errorf("MoveSetHost2IdleModule, module didn't belong to the set,input:%+v,rid:%s", data, srvData.rid)
				_ = resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: srvData.ccErr.Errorf(common.CCErrHostModuleNotBelongSetErr, data.ModuleID, data.SetID)})
				return
			}
		}

		// check whether the relationship of module and app match
		moduleQuery2 := mapstr.MapStr{common.BKModuleIDField: data.ModuleID, common.BKAppIDField: data.ApplicationID}
		moduleResult2, err := srvData.lgc.GetModuleMapByCond(srvData.ctx, []string{common.BKModuleIDField}, moduleQuery2)
		if nil != err {
			blog.Errorf("MoveSetHost2IdleModule module error:%v,input:%+v,rid:%s", err, data, srvData.rid)
			_ = resp.WriteError(http.StatusInternalServerError, &meta.RespError{Msg: err})
			return
		}
		if 0 == len(moduleResult2) {
			blog.Errorf("MoveSetHost2IdleModule module didn't belong to the app,input:%+v,rid:%s", data, srvData.rid)
			_ = resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: srvData.ccErr.Errorf(common.CCErrHostModuleNotBelongBusinessErr, data.ModuleID, data.ApplicationID)})
			return
		}
	}
