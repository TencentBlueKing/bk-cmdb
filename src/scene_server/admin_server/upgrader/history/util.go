...
func Upsert(ctx context.Context, db dal.RDB, tableName string, row interface{}, idField string, uniqueKeys []string,
	...
	if _, _, err := getIDFiled(ctx, db, tableName, idField, existOne, data, ignoreSet); err != nil {
		return 0, nil, err
	}
...
func getIDFiled(ctx context.Context, db dal.RDB, tableName string, idField string, existOne map[string]interface{},
	data map[string]interface{}, ignoreSet map[string]bool) (instID uint64, preData map[string]interface{}, err error) {
