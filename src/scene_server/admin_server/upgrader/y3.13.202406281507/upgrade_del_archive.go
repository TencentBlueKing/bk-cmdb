...
func upgradeDelArchive(ctx context.Context, db dal.RDB, conf *upgrader.Config) error {
	...
	if err := addContainerDelArchiveTable(ctx, db); err != nil {
		blog.Errorf("add container del archive table failed, err: %v", err)
		return err
	}

	if err := moveContainerData(ctx, db); err != nil {
		blog.Errorf("move container data to container del archive table failed, err: %v", err)
		return err
	}
...
func delExpiredData(ctx context.Context, db dal.RDB) error {
	...
	for {
		...
		if err := db.Table(common.BKTableNameDelArchive).Delete(ctx, delCond); err != nil {
			blog.Errorf("delete container del archive data failed, err: %v, cond: %+v", err, delCond)
			return err
		}
...
func addContainerDelArchiveTable(ctx context.Context, db dal.RDB) error {
	// add container del archive table
	table := common.BKTableNameContainerDelArchive
	...
	// add container del archive index
	indexes := []types.Index{
		{
			Name:               common.CCLogicIndexNamePrefix + "time",
			Keys:               bson.D{{"time", -1}},
			...
			Name:       common.CCLogicIndexNamePrefix + "detail_bk_pod_id",
			Keys:       bson.D{{"detail.bk_pod_id", 1}},
			Background: true,
		},
	}
...
func moveContainerData(ctx context.Context, db dal.RDB) error {
	filter := mapstr.MapStr{
	...
	for {
		...
		if len(needMovedData) > 0 {
			if err := db.Table(common.BKTableNameContainerDelArchive).Insert(ctx, needMovedData); err != nil {
				blog.Errorf("insert container del archive data failed, err: %v, data: %+v", err, needMovedData)
				return err
			}
		}
