...
func initTableIndex(kit *rest.Kit, db dal.RDB, tableIndexMap map[string][]daltypes.Index) error {
	...
	for _, table := range platTableIndexesArr {
		if err := tools.CreateTable(kit, mongodb.Dal().Shard(kit.SysShardOpts()), table); err != nil {
			fmt.Errorf("create plat table failed, err: %v", err)
			return err
		}
	}

	err := tools.CreateIndexes(kit, mongodb.Dal().Shard(kit.SysShardOpts()), common.BKTableNameTenantTemplate,
		templateIndexes)
	if err != nil {
...
func createInstAsstTable(kit *rest.Kit, db dal.RDB) error {
	for _, obj := range tableInstAsstArr {
		tableName := common.GetObjectInstAsstTableName(obj, kit.TenantID)
		if err := tools.CreateTable(kit, db, tableName); err != nil {
			blog.Errorf("create table %s failed, err: %v", tableName, err)
			return err
		}

		if err := tools.CreateIndexes(kit, db, tableName, instAsstCommonIndexes); err != nil {
			return err
		}
	}
