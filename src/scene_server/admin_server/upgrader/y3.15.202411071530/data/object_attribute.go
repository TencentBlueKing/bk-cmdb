...
func addObjAttrData(kit *rest.Kit, db dal.Dal) error {
	...
	var attributeData []interface{}
	var attributeAudit []tools.AuditField
	for _, attr := range objAttrData {
		...
		attributeAudit = append(attributeAudit, tools.AuditField{
	...
	cmpField := &tools.CmpFiled{
		Unique:     []string{common.BKObjIDField, common.BKPropertyIDField, common.BKAppIDField},
		IgnoreKeys: make([]string, 0),
		ID:         common.BKFieldID,
	}
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameObjAttDes, attributeData,
		cmpField, attributeAudit)
	if err != nil {
