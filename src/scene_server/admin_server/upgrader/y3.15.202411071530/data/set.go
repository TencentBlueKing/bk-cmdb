...
var setData = map[string]interface{}{
...
var auditSet = tools.AuditField{
	AuditType:    common.BKInnerObjIDSet,
	ResourceType: metadata.SetRes,
	ResourceName: common.DefaultResSetName,
}

func addSetBaseData(kit *rest.Kit, db dal.Dal, bizID int64) ([]uint64, error) {
	...
	cmpField := &tools.CmpFiled{
		Unique:     []string{common.BKAppIDField, common.BKSetNameField, common.BKParentIDField},
		IgnoreKeys: []string{common.BKSetIDField},
		ID:         common.BKSetIDField,
	}
	ids, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameBaseSet, []interface{}{setData},
		cmpField, []tools.AuditField{auditSet})
	if err != nil {
		blog.Errorf("insert bizSetData for table %s failed, err: %v", common.BKTableNameBaseApp, err)
		return nil, err
	}
