...
func addObjAssociationData(kit *rest.Kit, db dal.Dal) error {
	...
	var asstData []interface{}
	var objAssociationAudit []tools.AuditType
	for obj, asstObj := range associationMap {
		...
		objAssociationAudit = append(objAssociationAudit, tools.AuditType{
	...
	auditDataField := &tools.AuditDataField{
		BusinessID:   "",
		ResourceID:   "id",
		ResourceName: "bk_obj_asst_id",
	}
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameObjAsst,
		asstData, cmpField, objAssociationAudit, auditDataField)
	if err != nil {
