...
func addObjAssociationData(kit *rest.Kit, db dal.Dal) error {
	...
	var objAssociationAudit []tools.AuditField
	for obj, asstObj := range associationMap {
		...
		objAssociationAudit = append(objAssociationAudit, tools.AuditField{
			AuditType:    metadata.AssociationKindType,
			ResourceType: metadata.MainlineInstanceRes,
			ResourceName: asst.AssociationName,
		})
	}
	...
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameObjAsst,
		asstData, cmpField, objAssociationAudit)
	if err != nil {
