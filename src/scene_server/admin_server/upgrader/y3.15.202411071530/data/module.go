...
func addModuleData(kit *rest.Kit, db dal.Dal, bizID int64, moduleNames []string, setID uint64) error {
	...
	categoryID, err := getDefaultServiceCategoryID(kit, db)
	if err != nil {
		return err
	}
	var moduleAdd []interface{}
	var moduleAudit []tools.AuditField
	for _, moduleName := range moduleNames {
		...
		moduleAudit = append(moduleAudit, tools.AuditField{
	...
	cmpField := &tools.CmpFiled{
		Unique:     []string{common.BKAppIDField, common.BKSetIDField, common.BKModuleNameField},
		IgnoreKeys: []string{common.BKModuleIDField},
		ID:         common.BKModuleIDField,
	}
	_, err = tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameBaseModule, moduleAdd, cmpField,
		moduleAudit)
	if err != nil {
		blog.Errorf("insert bizSetData for table %s failed,common.BKTableNameAsstDes, err: %v", err)
		return err
	}
	return nil
}
