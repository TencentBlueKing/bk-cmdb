...
func addAssociationData(kit *rest.Kit, db dal.Dal) error {
	...
	auditLogArray := make([]tools.AuditType, 0)
	for _, asstType := range asstTypes {
		...
		auditLogArray = append(auditLogArray, tools.AuditType{
	...
	auditDataField := &tools.AuditDataField{
		BusinessID:   "",
		ResourceID:   "id",
		ResourceName: "bk_asst_name",
	}
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameAsstDes, dataInterface, cmpField,
		auditLogArray, auditDataField)
	if err != nil {
