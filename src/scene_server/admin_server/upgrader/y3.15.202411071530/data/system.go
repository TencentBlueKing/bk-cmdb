...
func addSystemData(kit *rest.Kit, db dal.Dal) error {
	blog.Infof("start add init bizSetData for table: %s", common.BKTableNameSystem)

	data := map[string]interface{}{common.HostCrossBizField: common.HostCrossBizValue}
	systemAudit := tools.AuditField{
		AuditType:    metadata.PlatformSetting,
		ResourceType: metadata.PlatFormSettingRes,
		ResourceName: common.HostCrossBizField,
	}

	cmpField := &tools.CmpFiled{
		Unique:     []string{common.HostCrossBizField},
		IgnoreKeys: make([]string, 0),
	}
	_, err := tools.InsertData(kit, db.Shard(kit.SysShardOpts()), common.BKTableNameSystem, []interface{}{data},
		cmpField, []tools.AuditField{systemAudit})
	if err != nil {
		blog.Errorf("insert bizSetData for table %s failed, err: %v", common.BKTableNameSystem, err)
		return err
	}
	...
	blog.Infof("end add init bizSetData for table: %s", common.BKTableNameSystem)
	return nil
}
...
func initConfigAdmin(kit *rest.Kit, db dal.Dal) error {
	...
	systemAudit := tools.AuditField{
		AuditType:    metadata.PlatformSetting,
		ResourceType: metadata.PlatFormSettingRes,
		ResourceName: common.ConfigAdminID,
	}
