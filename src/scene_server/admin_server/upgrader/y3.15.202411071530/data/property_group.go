...
func addPropertyGroupData(kit *rest.Kit, db dal.Dal) error {
	...
	var propertyGroupAudit []tools.AuditField
	for _, group := range propertyGroupData {
		group.IsDefault = true
		propertyGroupArr = append(propertyGroupArr, group)
		propertyGroupAudit = append(propertyGroupAudit, tools.AuditField{
	...
	cmpField := &tools.CmpFiled{
		Unique:     []string{common.BKObjIDField, common.BKAppIDField, common.BKPropertyGroupIndexField},
		IgnoreKeys: make([]string, 0),
		ID:         common.BKFieldID,
	}
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNamePropertyGroup,
		propertyGroupArr, cmpField, propertyGroupAudit)
	if err != nil {
