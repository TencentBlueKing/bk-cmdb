...
func addObjectData(kit *rest.Kit, db dal.Dal) error {
	var objectDataArr []interface{}
	var objectAudit []tools.AuditType
	for _, obj := range objectData {
		obj.Time = tools.NewTime()
		objectDataArr = append(objectDataArr, obj)
		objectAudit = append(objectAudit, tools.AuditType{
	...
	auditDataField := &tools.AuditDataField{
		BusinessID:   "",
		ResourceID:   "id",
		ResourceName: "bk_obj_name",
	}
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameObjDes, objectDataArr, cmpField,
		objectAudit, auditDataField)
	if err != nil {
