...
func addCloudAreaData(kit *rest.Kit, db dal.Dal) error {
	...
	var cloudAudit []tools.AuditField
	for _, data := range cloudAreaData {
		data.Time = tools.NewTime()
		cloudData = append(cloudData, data)
		cloudAudit = append(cloudAudit, tools.AuditField{
			AuditType:    metadata.ModelType,
			ResourceType: metadata.ModuleRes,
			ResourceName: data.CloudName,
		})
	}

	cmpField := &tools.CmpFiled{
		Unique:     []string{common.BKCloudNameField},
		IgnoreKeys: []string{common.BKCloudIDField},
		ID:         common.BKCloudIDField,
	}
	_, err := tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameBasePlat, cloudData, cmpField,
		cloudAudit)
	if err != nil {
