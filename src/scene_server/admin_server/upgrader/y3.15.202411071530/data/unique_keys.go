...
func getUniqueKeys(kit *rest.Kit, db dal.Dal) ([]objectUnique, error) {
	...
	agentIDUnique := make([]objectUnique, 0)
	for objID, value := range objUniqueKeys {
		for _, property := range value {
			...
			agentIDUnique = append(agentIDUnique, objectUnique{
				Keys:     Keys,
				ObjID:    objID,
				IsPre:    true,
				LastTime: time.Now(),
...
func addObjectUniqueData(kit *rest.Kit, db dal.Dal) error {
	...
	var objUniqueData []interface{}
	var objUniqueAudit []tools.AuditField
	for _, key := range uniqueKeysArr {
		objUniqueData = append(objUniqueData, key)
		objUniqueAudit = append(objUniqueAudit, tools.AuditField{
			AuditType:    metadata.ModelType,
			ResourceType: metadata.FieldTemplateUniqueRes,
			ResourceName: key.ObjID,
		})
	}
	...
	_, err = tools.InsertData(kit, db.Shard(kit.ShardOpts()), common.BKTableNameObjUnique, objUniqueData,
		cmpField, objUniqueAudit)
	if err != nil {
