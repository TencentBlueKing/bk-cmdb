...
func addPodTableBizIDIndex(ctx context.Context, db dal.RDB) error {
	bizIDIndex := dbtypes.Index{
	...
	for _, index := range existIndexes {
		if index.Name == bizIDIndex.Name {
			return nil
		}
	}

	err = db.Table(types.BKTableNameBasePod).CreateIndex(ctx, bizIDIndex)
	if err != nil && !db.IsDuplicatedError(err) {
		blog.Errorf("create pod biz_id index %+v failed, err: %v", bizIDIndex, err)
		return err
	}
