...
type migratePlatBase struct {
	tableName string
}

func (m *migratePlatBase) createTable(ownerID string, metaData dbStorage.DI, instData dbStorage.DI) error {
...
func (m *migratePlatBase) addData(ownerID string, metaData dbStorage.DI, instData dbStorage.DI) error {
	err := models.AddPlatData(m.tableName, instData, metaData)
	return err
}

func init() {
	m := &migratePlatBase{tableName: "cc_PlatBase"}
