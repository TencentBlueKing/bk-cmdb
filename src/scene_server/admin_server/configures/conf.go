...
func (cc *ConfCenter) writeConfs2Center(confRootPath string) error {
	...
	modules = append(modules, types.CCModuleDataCollection.Name)
	modules = append(modules, types.CCModuleHost.Name)
	// modules = append(modules, types.CCModuleMigrate)
	modules = append(modules, types.CCModuleProc.Name)
	modules = append(modules, types.CCModuleTop.Name)
	modules = append(modules, types.CCModuleAPIServer.Name)
