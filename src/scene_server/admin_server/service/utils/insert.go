...
func InsertData(kit *rest.Kit, db local.DB, table string, data []mapstr.MapStr, insertOps *InsertOptions) (
	...
	var dbCli = mongodb.Shard(kit.SysShardOpts())
	if !common.IsPlatformTable(table) {
		dbCli = db
	}
	result := make([]mapstr.MapStr, 0)
	err := dbCli.Table(table).Find(mapstr.MapStr{}).All(kit.Ctx, &result)
	if err != nil {
	...
	if err = dbCli.Table(table).Insert(kit.Ctx, insertData); err != nil {
