...
func (ih *IAMHandler) HandleServiceCategorySync(task *meta.WorkRequest) error {
	...
	serviceCategories, err := ih.authManager.CollectServiceCategoryByBusinessIDs(context.Background(), *header, bizID)
	if err != nil {
		blog.Errorf("get service categories by business %d failed, err: %+v", businessSimplify.BKAppIDField, err)
		return err
	}
	resources := ih.authManager.MakeResourcesByServiceCategory(*header, authmeta.EmptyAction, bizID, serviceCategories...)
	...
	taskName := fmt.Sprintf("sync service category for business: %d", businessSimplify.BKAppIDField)
	iamIDPrefix := ""
