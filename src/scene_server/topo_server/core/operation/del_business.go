...
func (b *business) cleanProcess(kit *rest.Kit, bizID int64) error {
	...
	for i := 0; i < idsLen; i += batchSize {
		...
		if !rstDel.Result {
			blog.Errorf("failed to delete process instance, ids: %v, err: %v, rid: %s,", idsBatch, rstDel.ErrMsg,
				kit.Rid)
...
func (b *business) cleanPropertyGroup(kit *rest.Kit, bizID int64) error {
	...
	rsp, err := b.clientSet.CoreService().Model().DeleteAttributeGroupByCondition(kit.Ctx, kit.Header, delOpt)
	if err != nil {
		blog.Errorf("failed to request object controller, err: %v, rid: %s", err, kit.Rid)
		return err
	}
	...
	if !rsp.Result {
		blog.Errorf("failed to delete property group, opt: %+v, err: %s, rid: %s", delOpt, rsp.ErrMsg, kit.Rid)
