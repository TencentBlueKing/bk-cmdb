...
func (c *commonInst) BatchCreateInstance(kit *rest.Kit, objID string, data []mapstr.MapStr) (
	*metadata.BatchCreateInstRespData, error) {
	...
	if len(data) == 0 || len(data) > 100 {
		blog.Errorf(" instances created failed, err: the number of batch create ranges from 0 to 100, data: %v,"+
			" rid: %s", data, kit.Rid)
		return nil, kit.CCError.CCErrorf(common.CCErrCommInstDataNil, "data")
	}
