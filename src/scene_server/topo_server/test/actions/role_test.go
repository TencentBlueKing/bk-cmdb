...
func TestCreatePrivilege(t *testing.T) {
	...
	r, _ := http.NewRequest("POST", server.URL+"/topo/v1/privilege/1/1/1", bytes.NewBuffer(reqbody))
...
func TestGetPrivilege(t *testing.T) {
	...
	originData := map[string]interface{}{}
	reqbody, _ := json.Marshal(originData)
	r, _ := http.NewRequest("GET", server.URL+"/topo/v1/privilege/1/1/1", bytes.NewBuffer(reqbody))
	resp, err := http.DefaultClient.Do(r)
	require.NoError(t, err)
	respbody, err := ioutil.ReadAll(resp.Body)
	require.NoError(t, err)
