...
func TestUpdateMultiSet(t *testing.T) {
	...
	r, _ := http.NewRequest("PUT", server.URL+"/host/v1/set/multi/1", bytes.NewBuffer(reqbody))
...
func TestDeleteMultiSet(t *testing.T) {
	...
	originData := map[string]interface{}{}
	reqbody, _ := json.Marshal(originData)
	r, _ := http.NewRequest("DELETE", server.URL+"/host/v1/set/multi/1", bytes.NewBuffer(reqbody))
	resp, err := http.DefaultClient.Do(r)
	require.NoError(t, err)
	respbody, err := ioutil.ReadAll(resp.Body)
	require.NoError(t, err)
