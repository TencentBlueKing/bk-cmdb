...
func (s *Service) getModelAttrByID(kit *rest.Kit, id, bizID int64) (metadata.Attribute, error) {
	...
	attrs, err := s.Engine.CoreAPI.CoreService().Model().ReadModelAttrByCondition(kit.Ctx, kit.Header, queryCond)
	if err != nil {
...
func (s *Service) deleteTableObject(kit *rest.Kit, objID string) error {
	...
	txnErr := s.Engine.CoreAPI.CoreService().Txn().AutoRunTxn(kit.Ctx, kit.Header, func() error {
		_, err := s.Logics.ObjectOperation().DeleteTableObject(kit, cond)
		if err != nil {
...
func (s *Service) DeleteObjectAttribute(ctx *rest.Contexts) {
	...
	if attr.PropertyType == common.FieldTypeInnerTable {
		if err := s.deleteTableObject(ctx.Kit, attr.ObjectID); err != nil {
			ctx.RespAutoError(err)
			return
		}
		ctx.RespEntity(nil)
