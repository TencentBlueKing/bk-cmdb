...
func (s *Service) fullTextMetadata(ctx *rest.Contexts, hits []*elastic.SearchHit, request FullTextSearchReq) (
	...
	blog.V(5).Infof("fulltext metadata query models: %s, instances: %+v, rid: %s",
...
func (s *Service) FullTextSearch(ctx *rest.Contexts) {
	...
	// main search.
	mainSearchResult, err := s.Es.Search(ctx.Kit, esQuery, indexes, request.Page.Start, request.Page.Limit)
	if err != nil {
