		...
		for _, item := range condition.Condition {
			if item.Field == "user" {
				...
				default:
					...
					ctx.RespAutoError(ctx.Kit.CCError.CCErrorf(common.CCErrCommParamsInvalid, fmt.Errorf("'user' operator currently allows only '$in' and '$nin'")))
					return
				}
				...
				default:
					blog.Errorf("'resource_name' operator currently allows '$in', '$nin' and '$regex'")
					ctx.RespAutoError(ctx.Kit.CCError.CCErrorf(common.CCErrCommParamsInvalid, fmt.Errorf("'resource_name' operator currently allows '$in', '$nin' and '$regex'")))
					return
				}
			} else {
