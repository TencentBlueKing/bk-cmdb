...
func (s *Service) DeleteBusiness(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("DeleteBusiness failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
...
func (s *Service) UpdateBusiness(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("UpdateBusiness failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
...
func (s *Service) UpdateBusinessStatus(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("UpdateBusinessStatus failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
...
func (s *Service) CreateDefaultBusiness(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("CreateDefaultBusiness failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
