...
func (s *Service) CreateObjectUnique(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("CreateObjectUnique failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
...
func (s *Service) UpdateObjectUnique(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("UpdateObjectUnique failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
...
func (s *Service) DeleteObjectUnique(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("DeleteObjectUnique failed, err: %v, rid: %s", err, ctx.Kit.Rid)
		return
	}
}
