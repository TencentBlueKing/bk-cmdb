...
func (s *Service) ListKubeContainerInBizSet(ctx *rest.Contexts) {
	opt := new(kubetypes.ListKubeContainerInBizSetOption)
	if err := ctx.DecodeInto(opt); err != nil {
		ctx.RespAutoError(err)
		return
	}
	...
	containerFields := append(opt.ContainerFields, common.BKFieldID, kubetypes.BKPodIDField, common.BKAppIDField)
	...
	if len(containers) == 0 {
		ctx.RespEntity(make([]kubetypes.ListKubeContainerInBizSetRes, 0))
		return
	}
	...
	podMap := make(map[int64]kubetypes.Pod)
	...
	result := make([]kubetypes.ListKubeContainerInBizSetRes, 0, len(containers))
	for _, container := range containers {
		pod := podMap[container.PodID]
		result = append(result, kubetypes.ListKubeContainerInBizSetRes{
			Container: &container,
			Pod:       &pod,
...
func (s *Service) getContainerByIDs(kit *rest.Kit, ids []int64, fields []string) ([]kubetypes.Container, error) {
...
func (s *Service) getPodByIDs(kit *rest.Kit, podIDs []int64, fields []string) ([]kubetypes.Pod, error) {
	if len(podIDs) == 0 {
		return make([]kubetypes.Pod, 0), nil
	}
