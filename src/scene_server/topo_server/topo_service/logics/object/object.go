...
func (cli *objLogic) DeleteObject(id int, params []byte, errProxy errors.DefaultCCErrorIf) error {
	...
	if nil != err {
		...
		// 检查被删除的�������������据是否包含内建模型
		for _, tmpitem := range items {
			if tmpitem.ObjectID == common.BKInnerObjIDApp {
				return fmt.Errorf("the built-in model[%s:%s] is forbidden to delete", tmpitem.ObjectID, tmpitem.ObjectName)
			}
			if tmpitem.ObjectID == common.BKInnerObjIDHost {
				return fmt.Errorf("the built-in model[%s:%s] is forbidden to delete", tmpitem.ObjectID, tmpitem.ObjectName)
			}
			if tmpitem.ObjectID == common.BKInnerObjIDModule {
				return fmt.Errorf("the built-in model[%s:%s] is forbidden to delete", tmpitem.ObjectID, tmpitem.ObjectName)
			}
			if tmpitem.ObjectID == common.BKInnerObjIDSet {
				return fmt.Errorf("the built-in model[%s:%s] is forbidden to delete", tmpitem.ObjectID, tmpitem.ObjectName)
			}
			/*
