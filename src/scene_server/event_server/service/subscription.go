...
func (s *Service) Subscribe(req *restful.Request, resp *restful.Response) {
	...
	if err != nil {
		blog.Errorf("subscribe %s success, but update to auditLog failed, err: %v", sub.SubscriptionName, err)
...
func (s *Service) UnSubscribe(req *restful.Request, resp *restful.Response) {
	...
	if err != nil {
		blog.Errorf("unsubscribe %s success, but update to auditLog failed, err: %v", sub.SubscriptionName, err)
		result := &metadata.RespError{Msg: defErr.Errorf(common.CCErrAuditSaveLogFailed, err)}
		resp.WriteError(http.StatusOK, result)
	}
...
func (s *Service) updateSubscription(header http.Header, id int64, ownerID string, sub *metadata.Subscription) error {
	...
	if err != nil {
		blog.Errorf("update subscribe %s success, but update to auditLog failed, err: %v", sub.SubscriptionName, err)
	}
