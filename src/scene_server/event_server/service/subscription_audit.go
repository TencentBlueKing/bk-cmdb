...
func (log *EventAudit) SaveAuditLog(auditAction metadata.ActionType) errors.CCError {
	...
	if err != nil {
		blog.ErrorJSON("%s %s audit log failed, err: %s, result: %+v", auditAction, log.resourceType, err, auditResult)
		return err
	}
	if auditResult.Result != true {
		blog.ErrorJSON("%s %s audit log failed, err: %s, result: %+v", auditAction, log.resourceType, auditResult.ErrMsg, auditResult)
