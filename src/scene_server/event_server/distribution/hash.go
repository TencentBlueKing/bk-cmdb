...
func (h *Hash) IsMatch(hash string) bool {
	...
	if err != nil {
		blog.Errorf("can't get target node hash, %+v", err)
		return false
	}
...
// LocalHashValue returns local hash value.
func (h *Hash) LocalHashValue() string {
	return h.localHashValue
}
...
func (h *Hash) updateLoop() {
	...
	for {
		...
		// discovery.
		servers, err := h.discovery.EventServer().GetServers()
		if err != nil {
			blog.Errorf("update services hash values, %+v", err)
			continue
		}
		blog.Infof("discovery newest servers now, %+v", servers)
		...
		for _, svr := range servers {
			...
			u, err := url.Parse(svr)
			if err != nil {
				blog.Errorf("update newest servers, node: %s, %+v", svr, err)
				continue
			}
		...
		for hashValue, svr := range newest {
			if _, isExist := h.nodes[hashValue]; !isExist {
				...
				blog.Infof("add new consistent hash node, %s", hashValue)
		...
		for hashValue, _ := range h.nodes {
			if _, isExist := newest[hashValue]; !isExist {
				blog.Infof("remove old consistent hash node, %s", hashValue)
				h.consistent.Remove(hashValue)
				delete(h.nodes, hashValue)
			}
		}
		blog.Infof("sync consistent hash done, members %+v", h.consistent.Members())
	}
}
