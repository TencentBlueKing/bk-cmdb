...
function createIdProperty(objId, isPrependName) {
  const keyMap = {
    [CONTAINER_OBJECTS.CLUSTER]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.CLUSTER].ID,
    [CONTAINER_OBJECTS.NAMESPACE]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.NAMESPACE].ID,
    [CONTAINER_OBJECTS.WORKLOAD]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.WORKLOAD].ID,
    [CONTAINER_OBJECTS.FOLDER]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.FOLDER].ID,
    [CONTAINER_OBJECTS.POD]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.POD].ID,
    [CONTAINER_OBJECTS.NODE]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.NODE].ID,
    [CONTAINER_OBJECTS.CONTAINER]:
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.CONTAINER].ID,
  }
  ...
  return {
    ...
    bk_property_name: isPrependName
      ? getPropertyName(propertyIdKey, objId, i18n.locale) || 'ID'
      : 'ID',
    bk_property_index: -1,
...
export const find = ({ objId, params }, config) =>
  http.get(`find/kube/${objId}/attributes`, params, config)
    ...
    fields.forEach(field => {
      ...
      if (mergeResult[field]) {
        ...
        const newItems = data.filter(
          item =>
            !mergeResult[field].some(
              x => x.key === item.key && x.val === item.val
            )
        )
