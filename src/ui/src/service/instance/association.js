...
const getIdKey = modelId =>
  ({
    [BUILTIN_MODELS.HOST]: [
      BUILTIN_MODEL_PROPERTY_KEYS[BUILTIN_MODELS.HOST].ID,
    ],
...
const findTopology = async ({
  ...
  try {
    ...
    const all = [
      ...(result.data.association.dst || []),
      ...(result.data.association.src || []),
    ]
    const data = all.map(association => {
      ...
      const instanceName = getInstanceName(
        instance,
        isSource ? asstObjId : objId
      )
      return {
        id: association.id,
        bk_obj_id: isSource ? asstObjId : objId,
        bk_inst_id: isSource ? asstInstId : instId,
        bk_inst_name: instance
          ? instanceName
          : `${i18n.t('已删除的实例')}(ID: ${isSource ? asstInstId : instId})`,
    ...
    const rootInstance = (result.data.instance[currentModelId] || []).find(
      root => root[rootIdKey] === currentInstId
    )
    return {
      ...
      root: {
        bk_obj_id: currentModelId,
        bk_inst_id: currentInstId,
        bk_inst_name: rootInstance
          ? getInstanceName(rootInstance, currentModelId)
          : currentInstName,
      },
      data,
    }
    ...
    return {
      count: 0,
      data: [],
      root: { bk_obj_id: currentModelId, bk_inst_id: currentInstId },
    }
  }
}
...
const find = async (params, config) => {
  try {
    const [{ info }, [{ count }]] = await Promise.all([
      http.post(
        `search/instance_associations/object/${params.bk_obj_id}`,
        params,
        config
      ),
      http.post(
        `count/instance_associations/object/${params.bk_obj_id}`,
        params
      ),
    ])
