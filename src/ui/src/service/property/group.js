...
export const find = async (options, config = {}) => {
  try {
    ...
    const groups = await http.post(
      `find/objectattgroup/object/${options.bk_obj_id}`,
      params,
      config
    )
    const bizGroups = groups
      .filter(group => !!group.bk_biz_id)
...
export const findBizSet = config =>
  find({ bk_obj_id: BUILTIN_MODELS.BUSINESS_SET }, config)
