...
const getters = {
  ...
  defaultDirectory: state =>
    state.directoryList.find(directory => directory.default === 1),
...
const mutations = {
  ...
  updateDirectory(state, directory) {
    const index = state.directoryList.findIndex(
      data => data.bk_module_id === directory.bk_module_id
    )
    if (index > -1) {
  ...
  refreshDirectoryCount(state, newList = []) {
    state.directoryList.forEach(directory => {
      const newDirectory = newList.find(
        newDirectory => newDirectory.bk_module_id === directory.bk_module_id
      )
