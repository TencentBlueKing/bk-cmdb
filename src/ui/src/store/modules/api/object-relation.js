  ...
  if (multiple) {
    if (updateType === 'remove') {
      newRelation = newRelation.filter(
        associatedValue => associatedValue !== value
      )
    } else {
...
const actions = {
  ...
  getInstRelation(
    ...
    { objId, instId, params, config }
  ) {
    return $http.post(
      `find/instassttopo/object/${objId}/inst/${instId}`,
      params,
      config
    )
  },
  ...
  getInstRelationTopo(
    ...
    return $http.post(
      `/findmany/inst/association/object/${objId}/inst_id/${instId}/offset/0/limit/200/web`,
      params,
      config
    )
  },
  ...
  updateInstRelation(
    ...
    switch (params.objId) {
      ...
      case 'biz':
        promise = $http.put(
          `biz/${rootGetters.supplierAccount}/${params.bk_biz_id}`,
          updateParams
        )
        break
      default:
        promise = $http.put(
          `inst/${rootGetters.supplierAccount}/${params.objId}/${params.bk_inst_id}`,
          updateParams
        )
    }
    return promise
