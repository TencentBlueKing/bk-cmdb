...
<template>
  <div class="import-state">
    ...
    <template v-else-if="state === 'success'">
      ...
      <bk-button class="mt20" theme="default" @click="closeImport">{{
        $t('关闭')
      }}</bk-button>
    </template>
    <template v-else-if="state === 'error'">
      ...
      <bk-button class="mt20" theme="default" @click="reset">{{
        $t('重新导入')
      }}</bk-button>
    </template>
    <ul v-if="instError" class="error-list">
      <li v-for="(error, index) in instError" :key="index" class="error-item">
        {{ error }}
      </li>
    </ul>
    <ul v-if="asstError" class="error-list">
      <li v-for="(error, index) in asstError" :key="index" class="error-item">
