...
<template>
  <div class="import-relation">
    <bk-checkbox v-model="allowImport" class="allow-import-checkbox">{{
      $t('是否导入关联的模型实例')
    }}</bk-checkbox>
    <div v-show="allowImport">
      ...
      <bk-table
        ...
        <bk-table-column
          type="selection"
          :selectable="() => allowImport"></bk-table-column>
        <bk-table-column
          :label="$t('关联的模型')"
          prop="model"
          width="250"
          ...
          <template slot-scope="{ row }">
            ...
            <span v-if="row.bk_obj_id === currentModelId" class="model-desc">{{
              `(${$t('自关联')})`
            }}</span>
          </template>
        </bk-table-column>
    ...
    <div class="options">
      <bk-button theme="default" @click="previousStep">{{
        $t('上一步')
      }}</bk-button>
      <bk-button
        theme="primary"
        ...
        >{{ $t('开始导入') }}</bk-button
      >
      <bk-button theme="default" class="ml10" @click="closeImport">{{
        $t('取消')
      }}</bk-button>
    </div>
  </div>
...
export default {
  ...
  setup(props, setupContext) {
    ...
    const computedRelationModels = computed(() =>
      Object.keys(computedRelationMap.value)
    )
    ...
    const [{ properties, pending: propertyPending }] = useProperty({
      bk_obj_id: {
        $in: [...computedRelationModels.value, currentModelId.value],
      },
    })
    ...
    const setSelectionUniqueCheck = row => {
      ...
      const preset =
        row.relation_unique_checks.find(uniqueCheck => uniqueCheck.ispre) ||
        firstUniqueCheck
    ...
    const pending = usePending(
      [modelUniquePending, propertyPending, uniqueCheckPending],
      true
    )
    ...
    async startImport() {
      try {
        ...
        if (!response.result || response.data.error) {
          ...
          const importError = response.data?.error
            ? response.data
            : { error: [response.bk_error_msg] }
