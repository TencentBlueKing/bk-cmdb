...
<template>
  <cmdb-sticky-layout class="export">
    ...
    <div
      slot="footer"
      slot-scope="{ sticky }"
      :class="['options', { 'is-sticky': sticky }]">
      ...
      <template v-if="currentStep === 2">
        <bk-button class="mr10" theme="default" @click="previousStep">{{
          $t('上一步')
        }}</bk-button>
        <bk-button
          class="mr10"
...
export default {
  ...
  setup() {
    ...
    const nextStepDisabled = computed(
      () => fields.value.length <= presetFields.value.length
    )
  ...
  data() {
    return {
      steps: [
        { title: this.$t('选择字段'), icon: 1 },
        { title: this.$t('选择关联模型'), icon: 2 },
      ],
    }
  },
}
