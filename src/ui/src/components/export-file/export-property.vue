...
<template>
  <div v-bkloading="{ isLoading: pending }" class="property-selector">
    <div class="filter">
      <bk-input
        v-model.trim="keyword"
        :placeholder="$t('请输入字段名称')"></bk-input>
    </div>
    <div
  ...
  setup() {
    ...
    watch(properties, value => {
      ...
      exportState.defaultSelectedFields.value.forEach(field => {
        ...
        property &&
          selection.value.indexOf(property) === -1 &&
          selection.value.push(property)
      })
    })
    ...
    const setAllSelection = (properties, selected) => {
      if (selected) {
        ...
        selection.value = selection.value.filter(
          property => !properties.includes(property) || isPreset(property)
        )
      }
    }
    const isSelected = property => selection.value.includes(property)
    const isAllSelected = properties =>
      properties.every(property => selection.value.includes(property))
