  ...
  <div class="export-status">
    <div class="status">
      ...
      <template v-else-if="isFinished">
        ...
        <div>
          <bk-button class="mt20 mr10" theme="default" @click="resetStep">{{
            $t('重新导出')
          }}</bk-button>
          <bk-button class="mt20" theme="default" @click="close">{{
            $t('关闭')
          }}</bk-button>
        </div>
      </template>
      <template v-else-if="hasError">
        ...
        <div>
          <bk-button class="mt20 mr10" theme="default" @click="restart">{{
            $t('重试失败')
          }}</bk-button>
          <bk-button class="mt20" theme="default" @click="close">{{
            $t('关闭')
...
export default {
  ...
  setup() {
    ...
    const hasError = computed(() =>
      all.value.some(task => task.state === 'error')
    )
    const isFinished = computed(() =>
      all.value.every(task => task.state === 'finished')
    )
    const pending = computed(
      () => current.value && current.value.state === 'pending'
    )
