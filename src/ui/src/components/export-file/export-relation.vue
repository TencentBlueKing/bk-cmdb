...
<template>
  <div v-bkloading="{ isLoading: pending }" class="export-relation">
    <bk-checkbox v-model="allowExport" class="allow-export-checkbox">{{
      $t('是否导出关联的模型实例')
    }}</bk-checkbox>
    <div v-show="allowExport">
      <div class="allow-export-model">
        ...
        <div v-if="modelUniqueChecks.length" class="unique-flag">
          <label class="unique-flag-label">{{ $t('唯一校验标识') }}：</label>
          <bk-select
            v-model="objectUniqueId"
            class="unique-selector"
            :clearable="false">
            <bk-option
              v-for="uniqueCheck in modelUniqueChecks"
              :id="uniqueCheck.id"
              :key="uniqueCheck.id"
      ...
      <bk-table
        ...
        <bk-table-column
          type="selection"
          :selectable="() => allowExport"></bk-table-column>
        <bk-table-column
          :label="$t('关联的模型')"
          prop="model"
          width="250"
          ...
          <template slot-scope="{ row }">
            ...
            <span
              v-if="row.bk_obj_id === row.bk_asst_obj_id"
              class="model-desc"
              >{{ `(${$t('自关联')})` }}</span
            >
          </template>
        ...
        <bk-table-column
          ...
          <template slot-scope="{ row }">
            <bk-select
              ...
              <bk-option
                v-for="uniqueCheck in row.relation_unique_checks"
                :id="uniqueCheck.id"
                :key="uniqueCheck.id"
                :name="getUniqueCheckName(uniqueCheck)">
              </bk-option>
...
export default {
  ...
  setup(props, setupContext) {
    ...
    const [{ uniqueChecks: modelUniqueChecks, pending: modelUniquePending }] =
      useUniqueCheck(currentModelId)

    // 获取当前模型的关联关系
    const [{ relations, pending: relationPending }] =
      useModelAssociation(currentModelId)
    ...
    const [{ properties, pending: propertyPending }] =
      useProperty(propertyOptions)

    // 加载关联模型的唯一校验
    const [
      { uniqueChecks: relationUniqueChecks, pending: uniqueCheckPending },
    ] = useBatchUniqueCheck(relationModels)
    ...
    const uniqueRelations = computed(() => {
      ...
      relations.value.forEach(relation => {
        const modelId =
          relation.bk_obj_id === currentModelId.value
            ? relation.bk_asst_obj_id
            : relation.bk_obj_id
    ...
    const computedRelations = computed(() =>
      uniqueRelations.value.map(relation => {
        const modelId =
          relation.bk_obj_id === currentModelId.value
            ? relation.bk_asst_obj_id
            : relation.bk_obj_id
        const uniqueChecks = relationUniqueChecks.value.find(uniqueChecks => {
          const isMatched = uniqueChecks.some(
            uniqueCheck => uniqueCheck.bk_obj_id === modelId
          )
          return isMatched
        })
        return {
          relation_obj_id: modelId,
          relation_unique_checks: uniqueChecks || [],
          ...relation,
        }
      })
    )
    ...
    const setSelectionUniqueCheck = relation => {
      ...
      const preset =
        relation.relation_unique_checks.find(
          uniqueCheck => uniqueCheck.ispre
        ) || firstUniqueCheck
  ...
  methods: {
    getRelationModel(relation) {
      ...
      if (typeof relation === 'string') {
        ...
        modelId =
          relation.bk_obj_id === this.currentModelId
            ? relation.bk_asst_obj_id
            : relation.bk_obj_id
      }
