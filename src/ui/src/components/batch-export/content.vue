    ...
    <div class="header">
      ...
      <div class="subtitle-wrapper">
        <i18n class="subtitle" tag="h2" path="分批下载副标题">
          <template #count
            ><strong class="count">{{ count }}</strong></template
          >
          <template #limit
            ><span>{{ limit }}</span></template
          >
        </i18n>
      ...
      <i
        v-if="isFinished"
        class="close-trigger bk-icon icon-close"
        @click="$emit('close')">
      </i>
      <bk-popconfirm
        v-else
    ...
    <ul ref="list" class="list">
      <li
        ...
        <span :class="['state', task.state]">
          <i
            v-if="task.state !== 'waiting'"
            :class="['state-icon', iconMapping[task.state]]"></i>
          {{ textMapdding[task.state] }}
        </span>
        <span class="info">
          <span class="info-name">{{
            `${name}_download_${index + 1}.${ext}`
          }}</span>
          <span
...
export default {
  ...
  methods: {
    setupSchedule() {
      const queue = new Array(Math.ceil(this.count / this.limit))
        .fill(null)
        .map((_, index) => ({
          state: 'waiting',
          page: {
