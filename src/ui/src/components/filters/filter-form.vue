...
<template>
  <bk-sideslider
    ...
    :quick-close="false"
    @hidden="handleClosed">
    <div slot="header" class="filter-form-header">
      {{ $t('高级筛选') }}
    ...
    <cmdb-sticky-layout slot="content" class="filter-layout">
      <bk-form class="filter-form" form-type="vertical">
        <bk-form-item class="filter-ip" label="IP">
          ...
          <p v-if="errors.has('ip')" class="filter-ip-error">
            {{ errors.first('ip') }}
          </p>
          <div>
            <bk-checkbox
              v-model="IPCondition.inner"
              class="mr20"
              @change="handleIPOptionChange('outer', ...arguments)">
              {{ $t('内网IP') }}
            </bk-checkbox>
            <bk-checkbox
              v-model="IPCondition.outer"
              class="mr20"
              @change="handleIPOptionChange('inner', ...arguments)">
              {{ $t('外网IP') }}
            </bk-checkbox>
            <bk-checkbox
              v-model="IPCondition.exact"
              v-bk-tooltips.top="$t('ipv6暂不支持模糊搜索')"
              >{{ $t('精确') }}</bk-checkbox
            >
          </div>
        </bk-form-item>
        <bk-form-item
          ...
          <label class="item-label">
            ...
            <span class="item-label-suffix"
              >({{ getLabelSuffix(property) }})</span
            >
          </label>
          <div class="item-content-wrapper">
            ...
            <component
              ...
              @active-change="
                handleComponentActiveChange(property, ...arguments)
              ">
            </component>
          </div>
          <i
            class="item-remove bk-icon icon-close"
            @click="handleRemove(property)"></i>
        </bk-form-item>
        <bk-form-item>
          <bk-button
            class="filter-add-button ml10"
            type="primary"
            text
      ...
      <div
        ...
        <template v-if="collectable">
          ...
          <bk-popover
            ...
            <bk-button
              theme="default"
              :disabled="!allowCollect || isMixCondition"
              @click="handleCreateCollection">
              {{ $t('收藏此条件') }}
            </bk-button>
            <section slot="content" class="collection-form">
              ...
              <p
                v-if="collectionForm.error || errors.has('collectionName')"
        ...
        <bk-button class="option-reset" theme="default" @click="handleReset">{{
          $t('清空')
        }}</bk-button>
      </div>
    </cmdb-sticky-layout>
...
export default {
  ...
  computed: {
    ...
    isMixCondition() {
      ...
      const hasNormalTopoField = Utils.hasNormalTopoField(
        this.selected,
        this.condition
      )
  ...
  watch: {
    storageSelected: {
      ...
      handler() {
        ...
        this.selected = [...this.storageSelected].filter(
          item => !filterCondition.includes(item.bk_property_id)
        )
      },
    },
    storageIPCondition: {
  ...
  methods: {
    ...
    getComponentType(property) {
      ...
      if (
        ...
        this.condition[property.id].operator !== '$regex'
      ) {
      ...
      const isModuleName =
        modelId === 'module' && propertyId === 'bk_module_name'

      // 在业务视图并且非模糊查询的情况，模块与集群名称使用专属的输入联想组件，否则使用与propertyType匹配的相应组件
      if (
        (isSetName || isModuleName) &&
        this.condition[property.id].operator !== '$regex'
      ) {
        return `cmdb-search-${modelId}`
      }
    ...
    getBindProps(property) {
      ...
      if (isContainerObject(modelId) && propertyType === 'map') {
        return Object.assign(props, {
          options:
            FilterStore.containerPropertyMapValue?.[modelId]?.[propertyId],
        })
      }
    ...
    handleReset() {
      ...
      Object.keys(this.condition).forEach(id => {
        const property = this.selected.find(
          property => property.id.toString() === id.toString()
        )
        const propertyCondititon = this.condition[id]
        const defaultValue = Utils.getOperatorSideEffect(
          property,
          propertyCondititon.operator,
          null
        )
