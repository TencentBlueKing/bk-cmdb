        ...
        <ul class="property-list clearfix">
          <li
            ...
            <bk-checkbox
              ...
              :disabled="
                disabledProperties[group.id].includes(property.bk_property_id)
              "
              :checked="isChecked(property)"
              @change="handleToggleProperty(property, ...arguments)">
              <span
                v-bk-tooltips.top-start="{
                  disabled: !disabledProperties[group.id].includes(
...
export default {
  ...
  computed: {
    ...
    propertyMap() {
      ...
      // 暂时不支持node对象map类型的字段
      modelPropertyMap.node = modelPropertyMap.node?.filter(
        property => !['map'].includes(property.bk_property_type)
      )
    ...
    groups() {
      ...
      return Object.keys(this.propertyMap)
        ...
        .sort(
          (groupA, groupB) =>
            sequence.indexOf(groupA.id) - sequence.indexOf(groupB.id)
        )
    },
    disabledProperties() {
