  ...
  <bk-form form-type="vertical">
    <bk-form-item>
      ...
      <div class="form-options">
        <bk-button class="mr10" text @click="handleConfirm">{{
          $t('确定')
        }}</bk-button>
        <bk-button class="mr10" text @click="handleCancel">{{
          $t('取消')
        }}</bk-button>
      </div>
    </bk-form-item>
...
export default {
  ...
  computed: {
    ...
    operator: {
      get() {
        return (
          this.localOperator || FilterStore.condition[this.property.id].operator
        )
      },
      set(operator) {
  ...
  methods: {
    ...
    handleOperatorChange(operator) {
      this.value = Utils.getOperatorSideEffect(
        this.property,
        operator,
        this.value
      )
