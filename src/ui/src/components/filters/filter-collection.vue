  ...
  <bk-select
    ...
    <bk-option
      ...
      <cmdb-loading
        :class="[
          'collection-item',
          { 'is-editing': editState.id === collection.id },
        ]"
        ...
        <template v-else>
          <i
            v-if="selected.includes(collection.id)"
            class="collection-state bk-icon icon-check-1"></i>
          <span class="collection-name">{{ collection.name }}</span>
          <span class="collection-options">
            <i
              class="option-icon option-edit icon-cc-edit"
              @click.stop="handleEdit(collection)"></i>
            <bk-popconfirm
              trigger="click"
              placement="top"
              :tippy-options="{
  ...
  methods: {
    ...
    handleApply(value) {
      ...
      const collection = selected
        ? this.collections.find(collection => collection.id === selected)
        : null
