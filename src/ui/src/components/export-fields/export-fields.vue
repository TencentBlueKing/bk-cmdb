...
<template>
  <bk-dialog
    ...
    <div slot="tools" class="title">
      <span>{{ title }}</span>
      <bk-input
        v-model.trim="filter"
        class="filter-input"
        clearable
        :placeholder="$t('请输入关键字搜索')"></bk-input>
    </div>
    <div v-bkloading="{ isLoading: !ready }">
      <div v-show="!isEmpty" class="property-tips">
      ...
      <section v-show="!isEmpty" class="property-selector">
        <ul
          v-if="independentProperties.length"
          class="property-list property-list-independent clearfix">
          <li
        ...
        <div v-for="group in renderGroups" :key="group.id" class="group">
    ...
    <footer slot="footer" class="footer">
      <i18n v-show="!!selected.length" path="已选个数">
        <template #count
          ><span class="count">{{ selected.length }}</span></template
        >
      </i18n>
      <div class="selected-options">
        <bk-button
          ...
          >{{ $t('开始导出') }}</bk-button
        >
        <bk-button class="ml10" theme="default" @click="close">{{
          $t('取消')
        }}</bk-button>
      </div>
    </footer>
  ...
  computed: {
    availableProperties() {
      return this.properties.filter(
        property => !this.invisibleProperties.includes(property.bk_property_id)
      )
    },
    independentProperties() {
      ...
      return this.availableProperties.filter(
        property =>
          !this.propertyGroups.some(
            group =>
              group.bk_group_id === property.bk_property_group &&
    ...
    groups() {
      // eslint-disable-next-line max-len
      const sortedGroups = [...this.propertyGroups].sort(
      ...
      const groups = sortedGroups.map(group => ({
        ...
        properties: this.availableProperties.filter(
    ...
    visibleProperties() {
      return this.renderGroups.reduce(
        (accumulator, group) => accumulator.concat(group.properties),
        []
      )
    },
    isAllChecked() {
      if (this.filter) {
        return this.visibleProperties.every(property =>
          this.selected.includes(property)
        )
      }
      return (
        !!this.selected.length &&
        this.selected.length === this.visibleProperties.length
      )
    },
    isEmpty() {
    ...
    isGroupAllChecked() {
      ...
      this.renderGroups.forEach(group => {
        isGroupAllChecked[group.id] = group.properties.every(property =>
          this.selected.includes(property)
        )
      })
  ...
  methods: {
      ...
      if (this.filter) {
        if (this.isAllChecked) {
          this.selected = this.selected.filter(
            property => !this.visibleProperties.includes(property)
          )
        } else {
          const newSelected = this.visibleProperties.filter(
            property => !this.selected.includes(property)
          )
    ...
    handleGroupToggleAll(groupId) {
      ...
      if (isGroupAllChecked) {
        this.selected = this.selected.filter(
          property => !group.properties.includes(property)
        )
      } else {
        const newSelected = group.properties.filter(
          property => !this.selected.includes(property)
        )
        this.selected = this.selected.concat(newSelected)
      }
