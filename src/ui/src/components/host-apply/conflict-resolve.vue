...
<template>
  <cmdb-sticky-layout class="conflict-layout">
    <cmdb-tips class="resolve-tips" tips-key="hostApplyConfirmTips">{{
      $t('策略失效提示语')
    }}</cmdb-tips>
    <div ref="conflictTable" class="conflict-table-wrapper">
      <bk-table
        ...
        <bk-table-column
          ...
          width="160"
          :resizable="false"
          prop="bk_property_name"></bk-table-column>
        <bk-table-column
          :label="$t('所属模块')"
          ...
          <template slot-scope="{ row, $index }">
            <div class="conflict-modules">
              <div
                ...
                @click="
                  handlePickValue(row, index, item.bk_property_value, $index)
                ">
                <span v-if="item.is_current">{{ $t('主机当前值') }}</span>
                <span v-else :title="getModulePath(item.bk_module_id)">{{
                  getModulePath(item.bk_module_id)
                }}</span>
                <span
                  :title="
                    item.bk_property_value
                      | formatter(row.bk_property_type, row.option)
                  ">
                  {{
                    item.bk_property_value
                      | formatter(row.bk_property_type, row.option)
        ...
        <bk-table-column
          ...
          <template slot-scope="{ row, $index }">
            <property-form-element
              :property="row"
              @value-change="
                value => handlePropertyValueChange(value, row, $index)
              ">
            </property-form-element>
          </template>
        </bk-table-column>
    ...
    <div
      slot="footer"
      slot-scope="{ sticky }"
      :class="['footer-btns', { sticky: sticky }]">
      <bk-button
        theme="primary"
        class="mr10"
...
export default {
  ...
  methods: {
    ...
    setConflictPropertyList() {
      ...
      if (this.dataCache.length) {
        ...
        conflicts.forEach(item => {
          ...
          const findProperty = this.configPropertyList.find(
            property => property.bk_property_id === item.bk_property_id
          )
          ...
          property.__extra__.conflictList = [
            hostCurrent,
            ...item.host_apply_rules,
          ]
      ...
      this.conflictPropertyListSnapshot =
        this.$tools.clone(conflictPropertyList)
    },
    getTopopath() {
      return this.$store.dispatch('hostApply/getTopopath', {
        bizId: this.bizId,
        params: {
          topo_nodes: this.moduleIds.map(id => ({
            bk_obj_id: 'module',
            bk_inst_id: id,
          })),
        },
    ...
    renderColumnHeader(h) {
      ...
      return h('div', { class: 'conflict-custom-label' }, [
        h('span', { style }, this.$t('所属模块')),
        h('span', { style }, this.$t('冲突值')),
      ])
    },
    restoreConflictPropertyList() {
      this.conflictPropertyList = this.$tools.clone(
        this.conflictPropertyListSnapshot
      )
    },
    toggleConfirmButtonDisabled() {
      const everyValidTruthy = this.conflictPropertyList.every(
        property => property.__extra__.valid !== false
      )
    ...
    handlePickValue(row, index, value, $index) {
      row.__extra__.conflictList.forEach((item, i) => {
        this.$set(
          this.conflictPropertyList[$index].__extra__.conflictList[i],
          'selected',
          index === i
        )
      })
    ...
    handleConfirm() {
      this.conflictPropertyListSnapshot = this.$tools.clone(
        this.conflictPropertyList
      )
