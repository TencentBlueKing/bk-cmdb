  ...
  <nav
    ...
    <div class="nav-wrapper" :class="{ unfold: unfold, flexible: !navStick }">
      <div v-if="isBusinessView" class="business-wrapper">
        ...
        <transition name="fade">
          <i v-show="!unfold" class="business-flag bk-icon icon-angle-down"></i>
        </transition>
      </div>
      <div class="menu-list">
      ...
      <div class="nav-option">
        <i
          class="nav-stick icon icon-cc-nav-toggle"
          :class="{
            sticked: navStick,
          }"
...
export default {
  ...
  methods: {
      ...
      if (expandedId) {
        ...
        const parent = this.currentMenus.find(menu => {
          ...
          return menu.submenu.some(
            submenu => submenu.route.name === this.relativeActiveName
          )
        })
        if (parent) {
    ...
    checkExactActive() {
      ...
      this.$nextTick(() => {
        this.hasExactActive = this.$refs.menuLink.some(link =>
          link.$el.classList.contains('active')
        )
      })
    },
    ...
    async refreshAuthorizedList() {
      try {
        const { info } = await this.$store.dispatch(
          'objectBiz/getAuthorizedBusiness'
        )
        this.$store.commit(
          'objectBiz/setAuthorizedBusiness',
          Object.freeze(info)
        )
        const { bizId } = this.$route.params
        const exist = info.some(
          biz => biz.bk_biz_id.toString() === bizId.toString()
        )
        if (!exist) {
