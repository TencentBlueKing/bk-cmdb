...
<template>
  <div class="breadcrumbs-layout clearfix">
    <template v-if="customize.enable">
      <i
        v-if="customize.backward"
        class="icon icon-cc-arrow fl"
        @click="customize.backward"></i>
    ...
    <template v-else>
      <i
        ...
        class="icon icon-cc-arrow fl"
        @click="handleClick"></i>
      <h1 v-bk-overflow-tips class="current fl">{{ current }}</h1>
    </template>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    defaultFrom() {
      ...
      if (menu.relative) {
        return {
          name: Array.isArray(menu.relative) ? menu.relative[0] : menu.relative,
        }
      }
      return null
    },
      ...
      if (has(this.$route.query, '_f')) {
        try {
          const historyList = JSON.parse(
            window.sessionStorage.getItem('history')
          )
