...
<template>
  <header v-test-id.global="'header'" class="header-layout">
    <div class="logo">
      <router-link class="logo-link" to="/index">
        {{ $t('蓝鲸配置平台') }}
      </router-link>
    </div>
    <nav v-test-id.global="'headerNav'" class="header-nav">
      <router-link
    ...
    <section class="header-info">
      ...
      <bk-popover
        ...
        <template slot="content">
          <a class="link-item" target="_blank" :href="helpDocUrl">{{
            $t('产品文档')
          }}</a>
          <a
            class="link-item"
          ...
          <a
            class="link-item"
            target="_blank"
            href="https://github.com/TencentBlueKing/bk-cmdb"
            >{{ $t('开源社区') }}</a
          >
        </template>
      </bk-popover>
...
export default {
  ...
  computed: {
    ...
    isBusinessView() {
      ...
      return (
        topRoute?.name === MENU_BUSINESS_SET || topRoute?.name === MENU_BUSINESS
      )
    },
    currentSysLang() {
  ...
  async mounted() {
    ...
    this.currentVersion =
      versionList.find(item => item.is_current === true)?.version || ''
  ...
  methods: {
    ...mapActions('versionLog', ['getLogList']),
    isLinkActive(nav) {
      const {
        matched: [topRoute],
      } = this.$route
      ...
      return (
        topRoute.name === nav.id ||
        (topRoute.name === MENU_BUSINESS_SET && nav.id === MENU_BUSINESS)
      ) // 业务集时高亮业务菜单
    },
    getHeaderLink(nav) {
...
.header-nav {
  ...
  .header-link {
    ...
    &:hover {
      background-color: rgb(49 64 94 / 50%);
      color: #fff;
    }

    &.router-link-active,
    &.active {
      background-color: rgb(49 64 94 / 100%);
      color: #fff;
    }
  }
}
