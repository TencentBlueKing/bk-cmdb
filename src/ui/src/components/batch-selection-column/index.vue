  ...
  methods: {
    ...
    emitRows() {
      const {
        onCrossPageMode,
        reserveSelection,
        reservedSelectedRows,
        reservedUnselectedRows,
      } = this
      let { innerSelectedRows } = this

      if (onCrossPageMode) {
        if (this.fullData) {
          const unselectedRowKeys = reservedUnselectedRows.map(r =>
            safeGet(r, this.rowKey)
          )
          innerSelectedRows = this.fullData.filter((i, index) => {
            const isSelected = !unselectedRowKeys.includes(
              safeGet(i, this.rowKey)
            )
            if (this.selectable) {
    ...
    columnHeader() {
      const pageSelection = (
        <bk-checkbox
          ...
          class={[
            { 'is-total-selected': this.onCrossPageMode },
            'page-select-checkbox',
          ]}
          disabled={
            this.pageSelectionDisabled || this.selectableRows.length === 0
          }
          vModel={this.isPageSelected}
          onChange={this.handlePageSelectionChange}></bk-checkbox>
      )
    ...
    generatePageSelection() {
      ...
      this.isPageSelected =
        selectedRowsLen === selectabeRowslLen && selectabeRowslLen > 0

      if (this.indeterminate) {
        this.isAllSelected =
          this.reservedUnselectedRows.length === 0 && this.onCrossPageMode
        this.pageSelectionIndeterminate =
          selectedRowsLen > 0 && !this.isPageSelected
        this.allSelectionIndeterminate =
          this.reservedUnselectedRows.length > 0 && !this.isAllSelected
      } else {
    ...
    generateRowSelection(currentChecked) {
      this.rows = this.rows.map((i, index) => {
        ...
        if (this.reserveSelection) {
          if (this.onCrossPageMode) {
            // 跨页全选时，记住没有选择的项目
            checked = !this.reservedUnselectedRows.some(
              unselectedRow =>
                safeGet(unselectedRow, this.rowKey) === safeGet(i, this.rowKey)
            )
          } else {
            // 非跨页全选时，记住已选择的项目
            checked = this.reservedSelectedRows.some(
              selectedRow =>
                safeGet(selectedRow, this.rowKey) === safeGet(i, this.rowKey)
            )
          }
        }
    ...
    toggleRowSelection(row) {
      const currentRow = this.rows.find(
        r => safeGet(row, this.rowKey) === safeGet(r, this.rowKey)
      )
