...
<template>
  <div class="import-wrapper">
    ...
    <div v-bkloading="{ isLoading: isLoading }" class="up-file upload-file">
    ...
    <div
      ...
      <div class="upload-file-name" :title="fileInfo.name">
        {{ fileInfo.name }}
      </div>
      ...
      <div class="upload-file-status-icon" hidden>
        <i
          :class="[
            'bk-icon ',
            {
              'icon-check-circle-shape': uploaded,
              'icon-close-circle-shape': failed,
            },
          ]"></i>
      </div>
    </div>
      ...
      <div v-else-if="hasUploadError()">
        <div
          ...
          <slot name="successTips" v-bind="uploadResult">
            <span>{{
              $t(successTips, { N: uploadResult.success.length })
            }}</span>
          </slot>
        </div>
        <!-- 上传失败列表  -->
        <div
          v-if="uploadResult.error && uploadResult.error.length"
          ...
          <ul ref="failList" class="upload-details-fail-list">
            <li
              v-for="(errorMsg, index) in uploadResult.error"
              :key="index"
              :title="errorMsg">
              {{ errorMsg }}
            </li>
        ...
        <div
          v-if="uploadResult.update_error && uploadResult.update_error.length"
          class="upload-details-fail">
          <div class="upload-details-fail-title">
            <i class="bk-icon icon-close-circle-shape"></i>
            <slot name="updateErrorTips" v-bind="uploadResult">
              <span
                >{{ $t(updateErrorTips) }}({{
                  uploadResult.update_error.length
                }})</span
              >
            </slot>
          </div>
          <ul ref="failList" class="upload-details-fail-list">
            <li
              v-for="(errorMsg, index) in uploadResult.update_error"
              :key="index"
              :title="errorMsg">
              {{ errorMsg }}
            </li>
        ...
        <div
          v-if="uploadResult.asst_error && uploadResult.asst_error.length"
          class="upload-details-fail">
          <div class="upload-details-fail-title">
            <i class="bk-icon icon-close-circle-shape"></i>
            <span
              >关联关系导入失败列表({{ uploadResult.asst_error.length }})</span
            >
          </div>
          <ul ref="failList" class="upload-details-fail-list">
            <li
              v-for="(errorMsg, index) in uploadResult.asst_error"
              :key="index"
              :title="errorMsg">
              {{ errorMsg }}
            </li>
    ...
    <div v-if="templdateAvailable" class="clearfix down-model-content">
  ...
  methods: {
    handleFile(e) {
      ...
      if (!this.allowTypeRegExp.test(fileInfo.name)) {
        this.$refs.fileInput.value = ''
        this.$error(
          this.$t('文件格式非法', { allowType: this.allowType.join(',') })
        )
        return false
      }
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.uploadResult = Object.assign(
            this.uploadResult,
            res.data || defaultResult
          )
          if (res.result) {
            this.uploaded = true
    ...
    hasUploadError() {
      ...
      return (
        (uploadResult.success && uploadResult.success.length) ||
