...
const modelOptions = computed(() => ({
  bk_obj_id: state.bk_obj_id,
  bk_biz_id: state.bk_biz_id,
}))
const instanceOptions = computed(() => ({
  bk_inst_id: state.bk_inst_id,
  ...modelOptions.value,
}))
const [{ properties, pending: propertyPending, invisibleProperties }] =
  useProperty(modelOptions)
...
const pending = usePending(
  [propertyPending, groupPneding, instancePending],
  true
)
...
const createDetails = () => {
  const Component = Vue.extend({
    ...
    render() {
      const directives = [
        { name: 'bkloading', value: { isLoading: pending.value } },
      ]
      const close = () => {
        visible.value = false
      ...
      return (
        <bk-sideslider
          ...
          <cmdb-details
            ...
            invisibleProperties={invisibleProperties.value}></cmdb-details>
        </bk-sideslider>
      )
    },
  })
