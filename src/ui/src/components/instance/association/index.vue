...
export default {
  ...
  setup() {
    const [
      { map: instanceMap, pending: instancePending },
      findInstanceAssociation,
    ] = useInstanceAssociation()
    const [{ associations, pending: associationPending }, findAssociation] =
      useAllAssocation()
    const pending = computed(
      () => instancePending.value || associationPending.value
    )
    return {
      pending,
      instanceMap,
  ...
  methods: {
    ...
    getDirection(instance) {
      const define = this.associations.find(
        association => association.bk_asst_id === instance.bk_asst_id
      )
      if (!define) {
        return {
      ...
      return {
        ...
        arrow:
          !instance.target && define.direction === 'src_to_dest'
            ? 'dest_to_src'
            : define.direction,
      }
    },
