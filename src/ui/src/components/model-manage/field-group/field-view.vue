...
<template>
  <cmdb-sticky-layout class="field-view-layout">
    <div ref="fieldList" class="field-view-list">
      ...
      <div class="property-item">
        ...
        <span class="property-value">{{
          fieldTypeMap[field.bk_property_type]
        }}</span>
      </div>
      <div class="property-item">
        <div class="property-name">
      ...
      <div class="property-item">
        ...
        <span class="property-value">{{
          field.isrequired ? $t('必填') : $t('非必填')
        }}</span>
      </div>
      <div v-if="hasMultipleType" class="property-item">
        <div class="property-name">
          <span>{{ $t('是否可多选') }}</span>
        </div>
        <span class="property-value">{{
          field.ismultiple ? $t('可多选') : $t('不可多选')
        }}</span>
      </div>
      <div
        v-if="stringLikes.includes(field.bk_property_type)"
        class="property-item">
        <div class="property-name">
          <span>{{ $t('正则校验') }}</span>
        </div>
      ...
      <template v-else-if="numberLikes.includes(field.bk_property_type)">
        ...
        <div class="property-item">
          ...
          <span class="property-value">{{
            option.max || (option.max === 0 ? 0 : '--')
          }}</span>
        </div>
      </template>
      <div
        v-if="numberLikes.includes(field.bk_property_type)"
        class="property-item">
        <div class="property-name">
          <span>{{ $t('单位') }}</span>
        </div>
      ...
      <div v-if="isTableType" class="property-item full-width">
        <div class="property-name">
          <span>{{ $t('表格列设置') }}</span>
        </div>
      ...
      <div
        ...
        class="property-item enum-list">
        <div class="property-name">
          <span>{{
            ['list'].includes(this.type) ? $t('列表值') : $t('枚举值')
          }}</span>
        </div>
      ...
      <div v-else-if="!isTableType" class="property-item enum-list">
        <div class="property-name">
          <span>{{ $t('默认值') }}</span>
        </div>
        <cmdb-property-value
        ...
        <span v-else class="property-value">{{ defaultValue || '--' }}</span>
      </div>
    </div>
    <template v-if="canEdit" slot="footer" slot-scope="{ sticky }">
      <div class="btns" :class="{ 'is-sticky': sticky }">
        <bk-button class="mr10" theme="primary" @click="handleEdit">{{
          $t('编辑')
        }}</bk-button>
        <bk-button
