<template>
  ...
  >
    <div class="field-options">
      <cmdb-auth :auth="authResources">
        <template #default="{ disabled }">
          <bk-button theme="primary" :disabled="disabled" @click="handleAddField">新建字段</bk-button>
        </template>
      </cmdb-auth>
      <cmdb-auth :auth="authResources">
        <template #default="{ disabled }">
          <bk-button :disabled="disabled" @click="handleAddGroup">新建分组</bk-button>
        </template>
      </cmdb-auth>
          ...
          <bk-transition name="collapse" duration-type="ease">
            ...
            >
              <li
                ...
                :key="fieldIndex"
                @click="
                  handleFieldDetailsView({ group, groupIndex, fieldIndex, property })
                "
              >
                ...
                <template v-if="isGlobalView || isBizCustomData(property)">
                  ...
                  >
                    ...
                    >
                      <i class="field-button-icon icon-cc-edit"></i>
                    </bk-button>
                  </cmdb-auth>
                  <cmdb-auth
                    class="mr10"
                  ...
                  >
                    <bk-button
                      ...
                      @click.stop="
                        handleDeleteField({ property, groupIndex, fieldIndex })
                      "
                    >
                      <i class="field-button-icon bk-icon icon-cc-delete"></i>
                    </bk-button>
                  </cmdb-auth>
                </template>
              </li>
    ...
    >
      <div class="group-dialog-header" slot="tools">
        {{ groupDialog.title }}
      </div>
  ...
  export default {
    ...
    computed: {
      ...
      canEditSort() {
        return (
          !this.customObjId
          && this.curModel.bk_classification_id !== 'bk_biz_topo'
        )
      },
    },
    watch: {
    ...
    methods: {
      ...
      handleConfirmAddProperty() {
        const { selectedProperties, addedProperties, deletedProperties } = this.dialog
      ...
      async handleUpdateGroup() {
        ...
        // eslint-disable-next-line max-len
        const isExist = this.groupedProperties.some(originalGroup => originalGroup !== curGroup
