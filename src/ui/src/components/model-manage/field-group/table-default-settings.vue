...
const scrollAddButton = () => {
  if (tableEmptyAddButtonRef.value) {
    tableEmptyAddButtonRef.value.$el
      ?.closest('.bk-table-empty-text')
      ?.scrollIntoView?.()
  }
}
  ...
  nextTick(() => {
    const component =
      instacne.$refs[`property-form-el-${index}`]?.[0].$refs[
        `component-${props.headers[0].bk_property_id}`
      ]
    component?.focus?.()
  })
}
...
<template>
  <div :class="['table-default-settings', { preview: props.preview }]">
    <bk-table
      ...
      <template v-if="!props.readonly" #empty>
        <icon-text-button
    ...
    <div
      v-if="list.length > 0 && !props.readonly && addIndex === -1"
      class="table-append">
      <icon-text-button
        :text="$t('新增')"
