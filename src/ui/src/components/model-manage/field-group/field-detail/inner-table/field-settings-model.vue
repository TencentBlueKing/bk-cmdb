...
Validator.extend('maxLongcahr', {
  validate: value => {
    ...
    return (
      existingTypes.value.filter(type => type === PROPERTY_TYPES.LONGCHAR)
        .length < 2
    )
  },
...
const isRequiredShow = computed(
  () =>
    ![PROPERTY_TYPES.BOOL, PROPERTY_TYPES.ENUMMULTI].includes(
      settings.bk_property_type
    )
)

const isDefaultShow = computed(
  () =>
    ![PROPERTY_TYPES.ENUMMULTI, PROPERTY_TYPES.BOOL].includes(
      settings.bk_property_type
    )
)
const isUnitShow = computed(() =>
  [PROPERTY_TYPES.INT, PROPERTY_TYPES.FLOAT].includes(settings.bk_property_type)
)
const isMultipleShow = computed(() =>
  [PROPERTY_TYPES.ENUMMULTI].includes(settings.bk_property_type)
)
...
const validateValue = async () => {
  const validate = [instance.$validator.validateAll()]
...
const handleConfirm = async () => {
  ...
  if (!results.every(result => result)) {
    const comps = [instance.$refs.componentBase, instance.$refs.componentOption]
  ...
  <bk-dialog
    ...
    <div class="content-layout">
      <grid-layout
        ...
        <grid-item
          ...
          :class="[
            'cmdb-form-item',
            'form-item',
            { 'is-error': errors.has('propertyId') },
          ]"
          :label="$t('列ID')">
          <bk-input
            v-model="settings.bk_property_id"
            v-validate="
              `required|fieldId|reservedWord|length:128|excluded:${existingIds}`
            "
            name="propertyId"
            :data-vv-as="$t('列ID')"
          ...
          <template #append>
            <div v-if="errors.has('propertyId')" class="form-error">
              {{ errors.first('propertyId') }}
            </div>
          </template>
        </grid-item>
        <grid-item
          ...
          :class="[
            'cmdb-form-item',
            'form-item',
            { 'is-error': errors.has('propertyName') },
          ]"
          :label="$t('列名称')">
          <bk-input
          ...
          <template #append>
            <div v-if="errors.has('propertyName')" class="form-error">
        ...
        <grid-item
          ...
          :class="[
            'cmdb-form-item',
            'form-item',
            { 'is-error': errors.has('propertyType') },
          ]"
          :label="$t('列类型')">
          <bk-select
          ...
          <template #append>
            <div v-if="errors.has('propertyType')" class="form-error">
              {{ errors.first('propertyType') }}
            </div>
          </template>
        </grid-item>
        <grid-item
          ...
          :label="$t('列属性')">
          <bk-checkbox v-model="settings.editable" class="checkbox">
            <span
      ...
      <grid-layout
        ...
        <div v-if="isDefaultShow" class="form-label">
          <div class="label-text">
            <span
              v-bk-tooltips="$t('表格字段列默认值提示语')"
              class="g-has-dashed-tooltips"
              >{{ $t('列默认值') }}</span
            >
          </div>
          <div
            ...
            <div v-if="errors.has('defalut')" class="form-error">
              {{ errors.first('defalut') }}
            </div>
          </div>
        </div>
      </grid-layout>
