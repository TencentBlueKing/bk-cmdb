...
<template>
  <div class="form-list-layout">
    ...
    <vue-draggable
      ...
      :options="dragOptions"
      @end="handleDragEnd">
      <li v-for="(item, index) in list" :key="index" class="form-item clearfix">
        <div class="list-name">
          <div
        ...
        <bk-button
          text
          class="list-btn"
          :disabled="list.length === 1 || isReadOnly"
    ...
    list: {
      ...
      handler(value) {
        ...
        if (this.defaultValue) {
          this.defaultListValue = this.settingList.find(
  ...
  methods: {
    ...
    handleSettingDefault(id) {
      const defaultValue = id
        ? this.settingList.find(item => item.id === id).name
        : ''
