...
<template>
  <ul class="icon-set">
    <li
      ...
      <i
        v-bk-tooltips="{
          content: language === 'zh_CN' ? icon.nameZh : icon.nameEn,
        }"
        :class="icon.value"></i>
    ...
    curIconList() {
      if (this.filterIcon) {
        ...
        return this.iconList.filter(
          icon =>
            icon.nameZh.toLowerCase().indexOf(this.filterIcon.toLowerCase()) >
              -1 ||
            icon.nameEn.toLowerCase().indexOf(this.filterIcon.toLowerCase()) >
              -1
        )
      }
