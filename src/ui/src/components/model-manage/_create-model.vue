    ...
    <div class="dialog-content">
      ...
      <div class="content clearfix">
        ...
        <div class="content-right">
          <div v-if="!isMainLine" class="label-item">
            <span class="label-title">{{ $t('所属分组') }}</span>
            <span class="color-danger">*</span>
            <div
              class="cmdb-form-item"
              :class="{ 'is-error': errors.has('modelGroup') }">
              <bk-select
                ref="groupSelector"
                v-validate="'required'"
                style="width: 100%"
              ...
              <p class="form-error" :title="errors.first('modelGroup')">
                {{ errors.first('modelGroup') }}
              </p>
            </div>
          </div>
          <label>
            ...
            <div
              class="cmdb-form-item"
              :class="{ 'is-error': errors.has('modelId') }">
              <bk-input
              ...
              <p class="form-error" :title="errors.first('modelId')">
                {{ errors.first('modelId') }}
              </p>
            </div>
            <i
              v-bk-tooltips="$t('模型唯一标识提示语')"
          ...
          <label>
            ...
            <div
              class="cmdb-form-item"
              :class="{ 'is-error': errors.has('modelName') }">
              <bk-input
              ...
              <p class="form-error" :title="errors.first('modelName')">
      ...
      <div v-if="modelDialog.isIconListShow" class="model-icon-wrapper">
        <the-choose-icon
          v-model="modelDialog.data['bk_obj_icon']"
          @chooseIcon="modelDialog.isIconListShow = false"
          @close="modelDialog.isIconListShow = false"></the-choose-icon>
      </div>
    </div>
    <div slot="footer" class="footer">
      <bk-button theme="primary" :loading="operating" @click="confirm">{{
        $t('提交')
      }}</bk-button>
      <bk-button theme="default" :disabled="operating" @click="cancel">{{
        $t('取消')
      }}</bk-button>
    </div>
  </bk-dialog>
