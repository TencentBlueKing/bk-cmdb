...
export default ({ properties, propertyGroups, exclude }) => {
  ...
  const sortedProperties = computed(() => {
    ...
    const propertyList = properties.value?.filter(
      property =>
        !exclude.value.includes(property.bk_property_id) && !property.bk_isapi
    )
  ...
  const groupedProperties = computed(() =>
    sortedGroups.value.map(group =>
      sortedProperties.value?.filter(property => {
  ...
  const displayGroups = computed(() =>
    sortedGroups.value.filter(
      (item, index) => groupedProperties.value[index]?.length > 0
    )
  )

  // 去除空项后的分组属性列表
  const displayProperties = computed(() =>
    groupedProperties.value.filter(properties => properties.length > 0)
  )
