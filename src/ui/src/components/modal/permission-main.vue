  ...
  <div class="permission-main">
    ...
    <div class="permission-footer">
      <template v-if="applied">
        ...
        <bk-button class="ml10" @click="handleClose">{{
          i18n.close
        }}</bk-button>
      </template>
      <template v-else>
    ...
    setList() {
      ...
      this.list = this.permission.actions.map(action => {
            ...
            fullPaths.forEach(pathData => {
              ...
              if (isComobj) {
                ...
                const modelPath = [
                  modelView,
                  modelId,
                  IAM_VIEWS_NAME[modelView][languageIndex],
                ]
                const instPath = instId
                  ? [
                      instView,
                      pathData.id,
                      IAM_VIEWS_NAME[instView][languageIndex],
                    ]
                  : null
                ...
                topoPath.push([
                  pathData.type,
                  pathData.id,
                  IAM_VIEWS_NAME[pathData.type][languageIndex],
                ])
              }
            })
        ...
        if (!allRelationPath.length && actionId.indexOf('comobj') > -1) {
          ...
          const modelView = IAM_VIEWS.INSTANCE_MODEL
          allRelationPath.push([
            [modelView, modelId, IAM_VIEWS_NAME[modelView][languageIndex]],
          ])
        }
        return {
          id: actionId,
