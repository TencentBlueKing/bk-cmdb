      ...
      permission.actions.forEach(action => {
        ...
        relatedResourceTypes.forEach(({ instances = [] }) => {
          const insts = instances.filter(fullPaths => {
            const matched = fullPaths.find(item =>
              batchInstTypes.includes(item.type)
            )
            if (matched) {
              const authed = authResults.find(
                item => String(item.resource_id) === matched.id
              )
        ...
        // 替换更新整个instances
        relatedResourceTypes.forEach(
          (item, i) => (item.instances = newInstances[i])
        )
      })
