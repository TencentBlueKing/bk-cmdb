  ...
  <div
    ...
    <div class="tag-input-layout">
      <div
        ...
        <template v-if="multiple || !isFocus">
          <span
            ...
            <i
              v-if="tagClearable"
              class="tag-input-selected-clear bk-icon icon-close"
              @mouseup.left.stop
      ...
      <i
        ...
        class="tag-input-clear bk-icon icon-close-circle-shape"
...
export default {
  ...
  computed: {
    containerStyle() {
      const style = {}
      if (this.isFocus) {
        style.maxHeight = this.fixedHeight
          ? `${this.focusRowLimit * this.singleRowHeight}px`
          : 'auto'
  ...
  methods: {
    ...
    async search(value, next) {
      ...
      try {
        ...
        const { results: data, next: nextPage } = await new Promise(
          async (resolve, reject) => {
            if (value) {
              try {
                const promise = [
                  (this.fuzzySearchMethod || this.defaultFuzzySearchMethod)(
                    value,
                    next
                  ),
                ]
                ...
                const [fuzzySearchData, defaultAlternateData] =
                  await Promise.all(promise)
                if (defaultAlternateData) {
                  fuzzySearchData.results.unshift(
                    ...defaultAlternateData.results
                  )
                }
    ...
    async getDefaultAlternateData(keyword) {
      let data = []
      const isMatch = (tag, keyword) =>
        tag.value.toLowerCase().indexOf(keyword.toString().toLowerCase()) > -1
      if (this.defaultAlternate === 'history') {
        data = [
          { display_name: this.historyLabel, children: this.getHistoryData() },
        ]
      ...
      data.forEach(tag => {
        if (has(tag, 'children')) {
          ...
          const children = tag.children.filter(
            child =>
              !flattened.some(
                flattenedTag => flattenedTag.value === child.value
              )
          )
          if (this.multiple) {
            const unexistTag = children.filter(
              child => !this.localValue.includes(child.value)
            )
        ...
        const repeat = flattened.some(
          flattenedTag => flattenedTag.value === tag.value
        )
    ...
    async getTagTips(instance, value) {
      try {
        ...
        if (typeof this.tagTipsContent === 'function') {
          ...
          const tag = await (
            this.exactSearchMethod || this.defaultExactSearchMethod
          )(value)
    ...
    getHistoryData() {
      try {
        if (this.historyKey) {
          const data =
            JSON.parse(window.localStorage.getItem(this.historyKey)) || []
    ...
    updateHistoryData(tag) {
      if (this.historyKey) {
        try {
          ...
          const exist = histories.findIndex(
            history => history.value === tag.value
          )
          if (exist > -1) {
            histories.splice(exist, 1)
          }
          Array.isArray(tag)
            ? histories.unshift(...tag)
            : histories.unshift(tag)
          // eslint-disable-next-line max-len
          const newHistories = histories
            .filter(history => !this.disabledData.includes(history.value))
            .slice(0, this.historyRecord)
    ...
    updatePopover() {
      this.popoverInstance &&
        this.popoverInstance.popperInstance &&
    ...
    handleSelectedMouseleave(event, { value }) {
      if (this.displayTagTips) {
        this.selectedTipsTimer[value] &&
          clearTimeout(this.selectedTipsTimer[value])
      }
    },
    ...
    getMatchedTag(nameToMatch) {
      const tag = this.flattenedData.find(tag => {
        const { value } = tag
        const { text } = tag
        const isMatch = [value, text].some(
          name => name.toLowerCase() === nameToMatch.toLowerCase()
        )
    ...
    async handlePaste(event) {
      ...
      try {
        ...
        const newValues = validValues.filter(
          value => !this.localValue.includes(value)
        )
    ...
    getSelectedDOM() {
      return Array.from(
        this.$refs.container.querySelectorAll('.tag-input-selected')
      )
    },
    // 移动光标位置
    ...
    updateScroller() {
      ...
      this.$nextTick(() => {
        ...
        if (highlightIndex !== -1) {
          const $alternateItem =
            this.alternateContent.$refs.alternateItem[highlightIndex].$el
          ...
          if (itemOffsetTop <= listScrollTop) {
            ...
            listScrollTop + listClientHeight
          ) {
            $alternateList.scrollTop =
              itemOffsetTop + itemOffsetHeight - listClientHeight
          }
        } else {
    ...
    calcOverflow() {
      ...
      this.overflowTimer = setTimeout(() => {
        ...
        const userIndexInSecondRow = selectedData.findIndex(
          (currentTag, index) => {
    ...
    insertOverflowTag() {
      ...
      if (referenceTag) {
        overflowTagNode.textContent = `+${
          this.localValue.length - this.overflowTagIndex
        }`
      ...
      setTimeout(() => {
        ...
        if (overflowTagNode.offsetTop !== previousTag.offsetTop) {
          ...
          this.$refs.container.insertBefore(
            overflowTagNode,
            overflowTagNode.previousSibling
          )
          overflowTagNode.textContent = `+${
            this.localValue.length - this.overflowTagIndex
          }`
    ...
    removeOverflowTagNode() {
      if (
        this.overflowTagNode &&
        this.overflowTagNode.parentNode === this.$refs.container
      ) {
