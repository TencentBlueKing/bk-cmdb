...
export default defineComponent({
  ...
  setup(props) {
    ...
    const colMinWidth = computed(() =>
      isNaN(Number(props.minWidth)) ? props.minWidth : `${props.minWidth}px`
    )
    const colMaxWidth = computed(() =>
      isNaN(Number(props.maxWidth)) ? props.maxWidth : `${props.maxWidth}px`
    )

    const rowMinHeight = computed(() =>
      isNaN(Number(props.minHeight)) ? props.minHeight : `${props.minHeight}px`
    )
...
.cmdb-grid-layout {
  ...
  grid-template-columns: repeat(
    var(--max-columns, auto-fill),
    minmax(var(--min-width, 200px), var(--max-width, 1fr))
  );
