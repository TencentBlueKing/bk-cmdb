...
<template>
  <div class="resize-layout" :class="localDirections">
    ...
    <template v-if="!disabled">
      <i
        v-for="(_direction, index) in localDirections"
        :key="index"
    ...
    <i ref="resizeProxy" :class="['resize-proxy', state.direction]"></i>
    <div ref="resizeMask" class="resize-mask"></div>
  </div>
</template>
      ...
      const handleMouseMove = event => {
        if (direction === 'right') {
          ...
          $resizeProxy.style.left = `${
            Math.min(maxLeft, Math.max(minLeft, proxyLeft)) + this.handlerOffset
          }px`
        } else {
          ...
          $resizeProxy.style.top = `${
            Math.min(maxTop, Math.max(minTop, proxyTop)) + this.handlerOffset
          }px`
        }
      }
      const handleMouseUp = () => {
    ...
    setStoreState() {
      if (this.storeId) {
        localStorage.setItem(
          `${this.storeId}-resizeHeight`,
          this.storeState.resizeHeight
        )
        localStorage.setItem(
          `${this.storeId}-resizeWidth`,
          this.storeState.resizeWidth
        )
      }
    },
    initStoreState() {
      if (this.storeId) {
        const storedResizeHeight = localStorage.getItem(
          `${this.storeId}-resizeHeight`
        )
        const storedResizeWidth = localStorage.getItem(
          `${this.storeId}-resizeWidth`
        )
