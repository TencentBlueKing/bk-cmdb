<template>
  <bk-table
    ...
    :class="[
      'data-row',
      { 'is-on-empty-add': !readonly && tableData.length === 0 && adding },
    ]"
    :data="tableData"
    :max-height="420"
    ...
    <bk-table-column
      ...
      <template #default="{ $index }">
        <template v-if="editState.index.includes($index)">
          <bk-button text @click="handleSaveRow($index)">{{
            $t(immediate ? '保存' : '确定')
          }}</bk-button>
          <bk-button text class="ml10" @click="handleCancelEdit($index)">{{
            $t('取消')
          }}</bk-button>
        </template>
        <template v-else>
          <template v-if="disabled">
            <bk-button text :disabled="disabled" @click="handleEditRow($index)">
              <span
                v-bk-tooltips="{
                  disabled: !disabled,
                  allowHtml: true,
...
import { t } from '@/i18n'
import { $success, $bkInfo } from '@/magicbox/index.js'
...
const scrollAddButton = () => {
  if (tableEmptyAddButtonRef.value) {
    const emptyTextEl = tableEmptyAddButtonRef.value.$el?.closest(
      '.bk-table-empty-text'
    )
    const emptyTextWidth = emptyTextEl.offsetWidth
    if (emptyTextEl?.offsetParent) {
      emptyTextEl.offsetParent.scrollLeft =
        (emptyTextEl.parentElement.offsetWidth - emptyTextWidth) / 2
    }
  }
}
...
const focus = (index = 0) => {
  nextTick(() => {
    const component =
      instacne.$refs[`property-form-el-${index}`]?.[0]?.$refs?.[
        `component-${header.value?.[0].bk_property_id}`
      ]
    component?.focus?.()
  })
}
...
const validateAll = async index => {
  // 获得每一个表单元素的校验方法
  const validates = (instacne.$refs[`property-form-el-${index}`] || []).map(
    formElement => formElement.$validator.validateAll()
  )
