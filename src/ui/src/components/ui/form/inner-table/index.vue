<template>
  <div class="cmdb-form-innertable">
    <div v-bkloading="{ isLoading }" class="innertable-container">
      <data-row
        v-model="tableData"
        type="list"
        :mode="mode"
        ...
        :adding="isShowAddRow"
        @add="handleClickAdd" />
      <div v-show="isShowAddRow" class="row-add">
        <data-row
          type="add"
          :mode="mode"
          :property="property"
      ...
      <div
        v-if="!isShowAddRow && tableData.length > 0 && !readonly"
        class="row-append">
        <cmdb-auth :auth="auth">
    ...
    <i
      v-show="showCopyBtn"
      class="title-copy icon-cc-details-copy"
      @click="handleCopyTable"></i>
  </div>
</template>
<script setup>
