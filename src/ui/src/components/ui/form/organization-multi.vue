...
<script setup>
  ...
  async function getLazyData(parentId) {
    try {
      const params = {
        lookup_field: 'level',
        exact_lookups: 0
  ...
  async function getSearchData(params) {
    return store.dispatch('organization/getDepartment', {
      params,
  ...
  function lazyMethod(node) {
    return getLazyData(node.id)
  }

  function lazyDisabled(node) {
    return !node.data.has_children
  }

  async function getCheckedFullTreeData(chekcedTreeData) {
    // 获取所有节点id
    const ids = []
    const getId = (nodes) => {
      nodes.forEach((node) => {
  ...
  function setTreeSearchData(data) {
    const finalTreeData = getTreeSearchData(data)
    tree.value.setData(finalTreeData)
  ...
  const handleValueChange = (options) => {
    if (props.multiple) {
      checked.value = [...options.id]
    } else {
      tree.value.removeChecked({ emitEvent: false })
      tree.value.setChecked(options.id, { emitEvent: false })
      checked.value = [options.id]
    }
  }
  const handleCheckChange = (ids, node) => {
    if (props.multiple) {
      checked.value = [...ids]
    } else {
      tree.value.removeChecked({ emitEvent: false })
      tree.value.setChecked(node.id, { emitEvent: false })
...
<script>
  export default {
    name: 'cmdb-form-organization-multi'
  }
</script>
