...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const isEditable = computed(
      () => props.property.editable && !props.property.bk_isapi
    )

    const isEditing = computed(
      () => props.property === props.editState.property
    )

    const formElementFontSize = computed(() =>
      props.formElementSize === 'small' ? 'normal' : 'medium'
    )
    ...
    const setEditState = (property: IProperty) => {
      ...
      nextTick(() => {
        const component =
          propertyFormEl.value.$refs[`component-${property.bk_property_id}`]
        component?.focus?.()
      })
    }
    ...
    const clickOutSideMiddleware = event => {
      ...
      return !path?.some?.(
        node => node.className === 'bk-picker-panel-body-wrapper'
      )
    }
...
<template>
  <div :class="['editable-property']">
    ...
    <template v-if="!loading">
      <!-- 显示编辑入口 -->
      <div v-show="!isEditing" class="property-actions">
        <cmdb-auth
          v-bk-tooltips="{
            disabled: isEditable,
            content: $t('系统限定不可修改'),
      ...
      <!-- 编辑态，显示表单项 -->
      <div v-if="isEditing" class="property-form">
