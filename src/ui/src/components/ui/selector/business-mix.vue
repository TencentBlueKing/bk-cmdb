...
<template>
  <bk-select
      ...
      <auth-mask tag="div" :auth="option.auth" :authorized="option.authorized">
        <div
          ...
          <i
            ...
            class="icon icon-cc-business-set"
            :title="$t('业务集')"></i>
          <i
            :class="[
              'icon',
              'bk-icon',
              'collection',
              isCollected(option) ? 'icon-star-shape' : 'icon-star',
            ]"
    ...
    <div slot="extension" class="business-extension">
      <cmdb-auth
...
export default {
  ...
  methods: {
    async getData() {
      const [{ info: bizList = [] }, { info: bizsetList = [] }] =
        await Promise.all([
      ...
      allList.forEach(item => {
        ...
        if (this.useIAM) {
          authList.push({
            type: isBizSet
              ? this.$OPERATION.R_BIZ_SET_RESOURCE
              : this.$OPERATION.R_BIZ_RESOURCE,
            relation: [rawId],
          })
        }
      })
      ...
      if (this.useIAM) {
        const authResult =
          (await verifyAuth(TRANSFORM_TO_INTERNAL(authList))) || []
        authResult.forEach(
          (
            { resource_id: id, resource_type: type, is_pass: isPass },
            index
          ) => {
            const matched = normalizationList.find(
              item => item.authType === type && item.rawId === id
            )
