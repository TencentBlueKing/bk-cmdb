...
<template>
  <div class="service-table-layout">
    <div class="title" @click="localExpanded = !localExpanded">
      <div class="fl">
        <template v-if="!editing.name">
          <i v-if="localExpanded" class="bk-icon icon-down-shape"></i>
          <i v-else class="bk-icon icon-right-shape"></i>
          {{ name }}
          <span v-if="addible && !processList.length" class="empty-process-tips"
            >（{{ $t('未添加进程') }}）</span
          >
          <i
            v-if="editable"
            class="name-edit icon-cc-edit-shape"
            @click.stop="handleEditName" />
        </template>
        <service-instance-name-edit-form
          v-else
          ref="nameEditForm"
          :value="instance.name"
          :width="350"
      ...
      <div class="fr right-content">
        <span v-if="topology" class="service-topology" :title="topology">{{
          topology
        }}</span>
        <i
          v-if="deletable"
    ...
    <bk-table v-if="localExpanded" class="service-table" :data="processList">
      <bk-table-column
      ...
      <bk-table-column v-if="showOperation" :label="$t('操作')" fixed="right">
        <template slot-scope="{ $index }">
          <a
            href="javascript:void(0)"
            class="text-primary mr10"
            @click="handleEditProcess($index)">
            {{ $t('编辑') }}
          </a>
          <a
      ...
      <template v-if="addible" slot="empty">
        <button
          class="add-process-button text-primary"
  ...
  computed: {
    header() {
      ...
      processTableHeader.forEach(id => {
        const property = this.processProperties.find(
  ...
  methods: {
    ...
    handleEditProcess(rowIndex) {
      Form.show({
        ...
        serviceTemplateId: this.templates[rowIndex]
          ? this.templates[rowIndex].service_template_id
          : 0,
        processTemplateId: this.templates[rowIndex]
          ? this.templates[rowIndex].id
          : 0,
        hostId: this.id,
        bizId: this.bizId,
