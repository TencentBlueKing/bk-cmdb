...
export default {
  ...
  async created() {
    try {
      const request = [this.getProperties(), this.getPropertyGroups()]
  ...
  methods: {
    ...
    async getPropertyGroups() {
      try {
        this.propertyGroups = await this.$store.dispatch(
    ...
    async getProcessTemplate() {
      try {
        ...
        if (this.isBizSet) {
          ...
          processTemplate = await this.$store.dispatch(
