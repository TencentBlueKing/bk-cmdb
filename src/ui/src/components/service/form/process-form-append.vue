...
<template>
  ...
  <bk-popover v-else class="process-form-tips" ext-cls="process-lock-popover">
    <i class="icon-cc-lock-fill"></i>
    <template slot="content">
      <i18n path="进程表单锁定提示">
        <template #link>
          <bk-link theme="primary" class="goto-link" @click="handleRedirect">{{
            $t('跳转服务模板')
          }}</bk-link>
        </template>
      </i18n>
...
export default {
  ...
  computed: {
    isLimited() {
      return ['bk_func_name', 'bk_process_name'].includes(
        this.property.bk_property_id
      )
    },
  },
  mounted() {
  ...
  methods: {
    ...
    hackRadius() {
      const hackDOM = this.$el.parentElement.querySelectorAll(
        '.bk-form-input,.bk-form-textarea,.bk-textarea-wrapper,.bk-select'
      )
