...
<template>
  <div class="cmdb-host-topo-path">
    ...
    <template v-else>
      <span v-bk-overflow-tips class="path">
        {{ topologyList[0] && topologyList[0].path }}
      </span>
      <template v-if="!isResourcePool">
        <i
          v-if="isSingle"
          class="path-single-link icon-cc-share"
    ...
    <div v-if="!isSingle" ref="tooltipContent" style="display: none">
      <div class="path-tooltip-content">
        <div
          v-for="(item, index) in topologyList"
          :key="`${item.id}_${index}`"
          class="path-tooltip-item">
          <span class="path-tooltip-text" :title="item.path">{{
            item.path
          }}</span>
          <i
            class="path-tooltip-link icon-cc-share"
