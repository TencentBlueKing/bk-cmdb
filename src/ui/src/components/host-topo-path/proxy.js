...
export default new Vue({
  ...
  methods: {
    ...
    dispatchRequest() {
      ...
      this.requestTimer = setTimeout(async () => {
        ...
        if (this.isResourceAssigned) {
          const { info: paths } = await containerHostService.getNodePath({
            ids: hostIds,
          })
          ...
          if (this.isContainerSearchMode) {
            // 通过主机获取获取传统拓扑路径
            const normalTopoPaths =
              (await hostSearchService.getTopoPath({ ids: hostIds })) || []

            queue.forEach(meta => {
              const containerPaths = this.containerTopoPaths.find(
                item => item.bk_host_id === meta.data.hostId
              )?.paths

              // 将数据格式统一为`find/topopath/biz/${bizId}`接口格式，便于后续使用
              const hostNormalTopoList = normalTopoPaths.find(
                item => item.id === meta.data.hostId
              )?.topo_path
    ...
    seperateNode(bizId, requestSet) {
      ...
      const newModules = uniqueModules.filter(
        moduleId =>
          !(this.results.has(bizId) && this.results.get(bizId).has(moduleId))
      )
    ...
    resolvePromise(bizId, requestSet) {
      ...
      requestSet.forEach(meta => {
        const result = meta.data.modules?.map(
          moduleId =>
            resultMap.get(moduleId) || {
              bk_biz_id: bizId,
              topo_node: {
                bk_inst_id: moduleId,
                bk_obj_id: 'module',
