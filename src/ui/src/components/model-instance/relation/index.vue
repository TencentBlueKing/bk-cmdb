...
<template>
  <div class="relation">
    <div class="options clearfix">
      <div v-show="activeView === viewName.list" class="fl">
        <cmdb-auth
          v-if="hasRelation"
        ...
        <bk-button v-show="false" theme="default" class="options-button">{{
          $t('批量取消')
        }}</bk-button>
      </div>
      <div class="fr">
  ...
  computed: {
    ...
    componentProps() {
      if (this.activeView === cmdbInstanceAssociation.name) {
        return {
          objId: this.objId,
          instId: this.formatedInst.bk_inst_id,
          instName: this.formatedInst.bk_inst_name,
        }
      }
      return {
