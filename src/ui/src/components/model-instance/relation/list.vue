...
<template>
  <div v-bkloading="{ isLoading: loading }" class="association-list">
    <div v-if="!hasRelation" class="association-empty">
    ...
    <div v-if="hasRelation && !hasRelationInstance" class="association-empty">
      <div class="empty-content">
        <cmdb-data-empty slot="empty" :stuff="dataEmpty"> </cmdb-data-empty>
      </div>
    </div>
...
export default {
  ...
  computed: {
      ...
      this.associationObject.forEach(association => {
        ...
        list.push({
          ...
          instances: this.instances.filter(item => {
            const sameType =
              item.bk_asst_id === association.bk_asst_id &&
    ...
    loading() {
      return this.$loading(['getSourceAssociation', 'getTargetAssociation'])
    },
    resourceType() {
  ...
  methods: {
    ...mapActions('objectAssociation', ['searchObjectAssociation']),
