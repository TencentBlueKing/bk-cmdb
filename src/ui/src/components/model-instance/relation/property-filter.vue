...
export default {
  ...
  computed: {
    ...
    selectedProperty() {
      ...
      return (
        this.filteredProperties.find(
          ({ bk_property_id: bkPropertyId }) =>
            bkPropertyId === this.localSelected.id
        ) || {}
      )
    },
    operatorOptions() {
      if (this.selectedProperty) {
        if (
          ['bk_host_innerip', 'bk_host_outerip'].includes(
            this.selectedProperty.bk_property_id
          ) ||
          this.objId === 'biz'
        ) {
        ...
        // eslint-disable-next-line max-len
        const propertyOperator = has(this.propertyOperator, propertyType)
          ? this.propertyOperator[propertyType]
          : this.propertyOperator.default
  ...
  watch: {
    ...
    operatorOptions(operatorOptions) {
      this.localSelected.operator = operatorOptions.length
    ...
    async objId(objId) {
      ...
      this.filteredProperties = properties.filter(property => {
        ...
        return (
          !bkIsapi &&
          !this.excludeType.includes(bkPropertyType) &&
          !this.excludeId.includes(bkPropertyId)
        )
      })
    },
  },
