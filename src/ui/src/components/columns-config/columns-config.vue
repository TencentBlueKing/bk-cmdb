...
<template>
  <div class="config-layout clearfix">
    <div class="config-wrapper config-unselected fl">
      ...
      <ul
        v-if="unselectedProperties.length"
        class="property-list property-list-unselected">
        <li
          v-for="(property, index) in unselectedProperties"
          ref="unselectedPropertyItem"
          :key="index"
          class="property-item"
      ...
      <div class="property-list-layout">
        <ul class="property-list property-list-selected">
          <li
            ...
            <span class="property-name" :title="property['bk_property_name']">{{
        ...
        <vue-draggable
          ...
          <li
            ...
            <i
              ...
              @click="unselectProperty(property)"></i>
          </li>
        </vue-draggable>
      </div>
    </div>
  ...
  computed: {
    sortedProperties() {
      return [...this.properties].sort((propertyA, propertyB) =>
        propertyA.bk_property_name.localeCompare(
          propertyB.bk_property_name,
          'zh-Hans-CN',
          { sensitivity: 'accent' }
        )
      )
    ...
    unselectProperty(property) {
      if (this.localSelected.length > this.min) {
        this.localSelected = this.localSelected.filter(
          propertyId => propertyId !== property.bk_property_id
        )
      } else {
