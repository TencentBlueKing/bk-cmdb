...
<template>
  <div class="business-scope-settings-details">
    <div v-if="isMatchAll" class="condition-list">
      <div class="condition-item">{{ $t('全业务') }}</div>
    </div>
    <template v-else>
      <div v-if="selectedBusiness.length" class="condition-list">
        <dl class="condition-item">
          <dt class="item-title">{{ $t('业务') }}：</dt>
          <dd class="item-content">
            <span
              v-for="(business, index) in selectedBusiness"
              :key="index"
              class="item-value">
      ...
      <div v-if="condition.length" class="condition-list">
        <dl
          v-for="item in condition"
          :key="item.property.bk_property_id"
      ...
      scopeData.value?.filter?.rules?.forEach(rule => {
        if (rule.field === 'bk_biz_id') {
          rule.value.forEach(bizId => {
            const business = allBusiness.value.find(
              business => business.bk_biz_id === bizId
            )
            if (business) {
          ...
          condition.push({
            ...
            value:
              Array.isArray(rule.value) && rule.value.length > 1
                ? rule.value
                : rule.value[0],
          })
        }
