...
<template>
  <bk-sideslider
    ...
    <div
      ...
      <component :is="detailsType" v-if="details" :details="details">
      </component>
    </div>
  </bk-sideslider>
</template>
...
export default {
  ...
  computed: {
    detailsType() {
      ...
      let isTableViewTarget = tableViewTargets.includes(
        this.details.resource_type
      )
  ...
  methods: {
    isModelExisted() {
      ...
      return Boolean(
        this.$store.getters['objectModelClassify/getModelById'](modelId)
      )
    },
    show() {
      this.isShow = true
