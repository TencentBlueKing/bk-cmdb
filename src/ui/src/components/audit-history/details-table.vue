...
<template>
  <div class="details-layout">
    <div class="details-info">
      <div v-if="details.bk_biz_id" class="info-group">
        <label class="info-label">{{ $t('所属业务') }}</label>
        <span v-bk-overflow-tips class="info-content">
          <audit-business-selector
            type="info"
            :value="details.bk_biz_id"></audit-business-selector>
        </span>
      </div>
      ...
      <div class="info-group">
        ...
        <span v-bk-overflow-tips class="info-content">{{
          action ? action.name : details.action
        }}</span>
      </div>
      <div v-if="modelId" class="info-group">
        <label class="info-label">{{ $t('模型类型') }}</label>
        <span v-bk-overflow-tips class="info-content">{{ modelName }}</span>
      </div>
      <div class="info-group">
      ...
      <div class="info-group">
        ...
        <span v-bk-overflow-tips class="info-content">{{ description }}</span>
      </div>
      <div class="info-group">
        <label class="info-label">{{ $t('操作时间') }}</label>
        <span v-bk-overflow-tips class="info-content">{{
          $tools.formatTime(details.operation_time)
        }}</span>
      </div>
      <div class="info-group">
        ...
        <span v-bk-overflow-tips class="info-content">
          <cmdb-form-objuser
            type="info"
            :value="details.user"></cmdb-form-objuser>
        </span>
      </div>
    </div>
      ...
      <bk-table
        ...
        <bk-table-column
          ...
          <template slot-scope="{ row }">
            ...
            <div
              v-else
              class="details-custom-content"
              v-html="row.before"></div>
          </template>
        </bk-table-column>
        <bk-table-column
          v-if="showAfter"
          ...
          <template slot-scope="{ row }">
            ...
            <div v-else class="details-custom-content" v-html="row.after"></div>
          </template>
        </bk-table-column>
      </bk-table>
      <div v-if="showToggle" class="details-toggle">
        <bk-link theme="primary" @click="toggleDetails">
...
export default {
  ...
  computed: {
    ...
    resourceType() {
      return this.dictionary.find(
        type => type.id === this.details.resource_type
      )
    },
    action() {
      ...
      return this.resourceType.operations.find(
        action => action.id === this.details.action
      )
    },
    description() {
  ...
  async created() {
    await Promise.all([this.getAuditDictionary(), this.getModelProperty()])
    this.setList()
  },
  methods: {
    ...
    setList() {
      const isTopologyPathChange = [
        'unassign_host',
        'assign_host',
        'transfer_host_module',
      ].includes(this.details.action)
