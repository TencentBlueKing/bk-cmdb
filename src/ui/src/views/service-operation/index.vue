    ...
    <template v-if="ready">
      <div class="info clearfix mb20">
        ...
        <i18n tag="div" path="N个" class="info-content">
          <template #count
            ><b class="info-count">{{ serviceInstanceIds.length }}</b></template
          >
        </i18n>
      </div>
            ...
            <ul class="tab clearfix">
              <template v-for="(item, index) in availableTabList">
                ...
                <li
                  ...
                  <span
                    :class="['tab-count', { 'has-badge': !item.confirmed }]">
      ...
      <div class="options" :class="{ 'is-sticky': hasScrollbar }">
        <bk-button
          theme="primary"
          :loading="$loading(request.confirm)"
          @click="handleConfirm"
          >{{ $t('确认') }}</bk-button
        >
...
export default {
  ...
  computed: {
    ...
    moduleId() {
      return (
        this.$route.params.moduleId && parseInt(this.$route.params.moduleId, 10)
      )
    },
    serviceInstanceIds() {
      return String(this.$route.params.ids)
        .split('/')
  ...
  methods: {
    async getPreviewData() {
      try {
        const data = await this.$store.dispatch(
          'serviceInstance/previewDeleteServiceInstances',
          {
            params: {
    ...
    setHostAttrsAutoApply(data = {}) {
      ...
      tab.props.info = applyList.filter(
