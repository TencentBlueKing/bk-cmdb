...
<template>
  <div :class="['topo-wrapper', { hover: isTopoHover }]">
    <div class="toolbar">
      <cmdb-auth
        ref="addBusinessLevel"
        style="display: none"
        :auth="{ type: $OPERATION.SYSTEM_TOPOLOGY }"
        @update-auth="handleReceiveAuth">
      </cmdb-auth>
      <cmdb-auth
        v-show="!topoEdit.isEdit"
        :auth="{ type: $OPERATION.SYSTEM_MODEL_GRAPHICS }">
        <bk-button
          slot-scope="{ disabled }"
          class="edit-button"
          theme="primary"
      ...
      <div class="vis-button-group">
        <i
          ...
          :class="[
            'bk-cc-icon',
            mainFullScreen
              ? 'icon-cc-fullscreen-outlined-reset'
              : 'icon-cc-fullscreen-outlined',
          ]"
          @click="resizeFull">
        </i>
        <i
          v-bk-tooltips="$t('还原')"
          class="bk-cc-icon icon-cc-fit"
          @click="resizeFit"></i>
        <i
          v-bk-tooltips="$t('缩小')"
          class="bk-cc-icon icon-cc-zoom-out"
          @click="zoomOut"></i>
        <i
          v-bk-tooltips="$t('放大')"
          class="bk-cc-icon icon-cc-zoom-in"
          @click="zoomIn"></i>
      ...
      <li class="group-item">
        <div
          :class="[
            'group-info',
            'group-total',
            { selected: topoNav.selectedGroupId === -1 },
          ]"
          @click="handleSelectGroup()">
          <span class="group-name">{{ $t('全部模型') }}</span>
          <span class="model-count">{{
            localTopoModelList.length > 1000
              ? '999+'
              : localTopoModelList.length
          }}</span>
        </div>
      </li>
      <li
        v-for="(group, groupIndex) in localClassifications"
        :key="groupIndex"
        class="group-item">
        <div
          class="group-info"
          :class="{
            ...
            invisible: topoNav.hideGroupIds.includes(
          ...
          <span class="group-name" :title="group.bk_classification_name">{{
          ...
          <i
            :class="[
              'bk-cc-icon',
              'icon-eye',
              topoNav.hideGroupIds.includes(group['bk_classification_id'])
                ? 'icon-cc-close-eye'
                : 'icon-cc-open-eye',
            ]"
        ...
        <cmdb-collapse-transition name="model-box">
          <ul
            v-show="topoNav.activeGroupId === group['bk_classification_id']"
            class="model-box">
            <li
              ...
              <div class="info">
                <p class="name" :title="model['bk_obj_name']">
                  {{ model['bk_obj_name'] }}
                </p>
              </div>
              <i
                :class="[
                  'bk-cc-icon',
                  'icon-eye',
                  topoNav.hideNodeIds.includes(model['bk_obj_id'])
                    ? 'icon-cc-close-eye'
                    : 'icon-cc-open-eye',
                ]"
    ...
    <bk-sideslider
      ...
      <component
        ...
        @cancel="handleSliderCancel"></component>
    </bk-sideslider>

    <div
      ref="topo"
      v-bkloading="{ isLoading: loading }"
      class="global-model"></div>

    <div
      v-show="topoTooltip.hoverNode"
      ref="nodeTooltips"
      class="topology-node-tooltips">
      <div ref="addEdgeIcon" class="icon-box" @click="handleAddEdge">
        <i class="icon-cc-line"></i>
      </div>
      <div ref="hideNodeIcon" class="icon-box" @click="handleHideNode">
    ...
    <the-create-model
      ...
      :title="$t('新建层级')"
      :operating="creating"
      @confirm="handleCreateBusinessLevel"></the-create-model>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('objectModelClassify', ['classifications', 'getModelById']),
    noPositionModels() {
    ...
    localClassifications() {
      return this.$tools
        .clone(this.classifications)
        .filter(classify => !classify?.bk_ishidden)
        .map(classify => {
          classify.bk_objects = classify.bk_objects.filter(
            model => !model.bk_ishidden && !model.bk_ispaused
          )
          return classify
        })
    },
  ...
  methods: {
    ...mapActions('objectAssociation', ['searchAssociationType']),
    ...mapActions('objectMainLineModule', [
      'searchMainlineObject',
      'createMainlineObject',
    ]),
    ...mapActions('objectModelClassify', ['searchClassificationsObjects']),
    getMainLineModel() {
    ...
    async getTopoElements() {
      ...
      // 包含分类属性的节点数据
      const nodeObjects = this.localClassifications.reduce(
        (acc, cur) => acc.concat(cur.bk_objects),
        []
      )
      ...
      this.localTopoModelList.forEach(nodeItem => {
        ...
        elements.push({
          data: {
            ...
            groupId: (
              nodeObjects.find(item => item.bk_obj_id === nodeObjId) || {}
        ...
        if (Array.isArray(nodeItem.assts) && nodeItem.assts.length) {
          nodeItem.assts.forEach(asstItem => {
            // 关联关系源数据
            const { direction, asstName, asstId } = this.getAsstDetail(
              asstItem.bk_asst_inst_id
            )

            // 所关联的节点必须存在
            if (
              this.localTopoModelList.findIndex(
                ({ bk_obj_id: objId }) => objId === asstItem.bk_obj_id
              ) !== -1
            ) {
              elements.push({
                data: {
    ...
    updateElementPostion() {
      ...
      try {
        ...
        if (this.noPositionModels?.length) {
          ...
          const boundingBoxW =
            (collectionBoundingBox.w + nodeGutter) * maxCountInOneRow
    ...
    handleToggleNode(model, group) {
      ...
      const hideNodeCount = this.topoNav.hideNodeIds.filter(id =>
        nodeIds.includes(id)
      ).length
    ...
    toggleNodeByGroup(group, display) {
      ...
      if (display) {
        // 显示则从隐藏记录中过滤掉
        this.topoNav.hideNodeIds = this.topoNav.hideNodeIds.filter(
    ...
    makeSvg(nodeData) {
      return new Promise(resolve => {
        ...
        image.onload = () => {
          const model = this.getModelById(nodeData.id)
          const svg = {
            unselected: `data:image/svg+xml;charset=utf-8,${encodeURIComponent(
            ...
            selected: `data:image/svg+xml;charset=utf-8,${encodeURIComponent(
              generateObjIcon(image, {
                name: nodeData.name,
                iconColor: '#fff',
        ...
        }/static/svg/${nodeData.icon.substr(5)}.svg`
      })
    },
    handleRelationSave(params) {
      const fromNode = this.localTopoModelList.find(
        model => model.bk_obj_id === params.bk_obj_id
      )
      if (!has(fromNode, 'assts')) {
        Object.assign(fromNode, { assts: [] })
      }
      fromNode.assts.push({
        bk_asst_inst_id: this.associationList.find(
          asst => asst.bk_asst_id === params.bk_asst_id
        ).id,
        ...
        this.localTopoModelList.forEach(model => {
          if (has(model, 'assts')) {
            const index = model.assts.findIndex(asst => {
              ...
              return (
                asst.asstInfo.bk_obj_id === data.params.bk_obj_id &&
                asst.asstInfo.bk_asst_id === data.params.bk_asst_id &&
                asst.asstInfo.bk_asst_obj_id === data.params.bk_asst_obj_id
              )
            })
            if (index > -1) {
    ...
    handleHideNode() {
      ...
      const group =
        this.localClassifications.find(
          item => item.bk_classification_id === groupId
        ) || {}
      const model =
        this.localTopoModelList.find(item => item.bk_obj_id === id) || {}
    ...
    completeEditingEdge(params) {
      const asstInstId = this.associationList.find(
        asst => asst.bk_asst_id === params.bk_asst_id
      ).id
    ...
    getAsstDetail(asstId) {
      ...
      return {
        ...
        asstName: asst.bk_asst_name.length
          ? asst.bk_asst_name
          : asst.bk_asst_id,
    ...
    saveHideModelConfig(hideNodeIds) {
      this.$store.dispatch('userCustom/saveUsercustom', {
        [this.hideModelConfigKey]: {
          hideNodeIds,
          hideGroupIds: this.topoNav.hideGroupIds,
        },
      })
    },
...
.topology-edge-tooltips {
  ...
  color: #868b97;
  background: #fff;
  box-shadow: 0 2px 4px 0 rgb(147 147 147 / 50%);
  border-radius: 2px;
  cursor: pointer;
