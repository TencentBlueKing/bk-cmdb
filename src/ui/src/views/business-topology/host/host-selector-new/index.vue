              ...
              <ul slot="content" class="host-list">
                <li
                  ...
                  <div v-bk-overflow-tips="{ placement: 'left' }" class="ip">
                    ...
                    <span
                      v-if="repeatSelected.includes(row)"
                      v-bk-tooltips="{
                        content: `${$t('管控区域')}：${foreignkey(
                          row.host.bk_cloud_id
                        )}`,
                      }"
                      class="repeat-tag">
                      {{ $t('IP重复') }}
...
export default {
  ...
  methods: {
    ...
    handleSelect(hosts) {
      ...
      const uniqueData = ids.map(id =>
        selectData.find(data => data.host.bk_host_id === id)
      )
      const newSelectData = []
      uniqueData.forEach(data => {
        const isExist = this.selected.some(
