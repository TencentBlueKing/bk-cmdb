  ...
  <div class="host-selector-topology">
    <cmdb-resize-layout
      ...
      <div class="topo-wrapper">
        ...
        <div
          v-show="filter.show"
          ref="topoSearchResult"
          class="topo-search-result">
          <div class="search-result-head">
            <span class="title">{{ $t('搜索结果') }}</span>
          </div>
          <div v-if="filter.list.length" class="search-result-body">
            <div
              v-for="(module, index) in filter.list"
              :key="index"
              class="search-result-item"
              @click="handleClickFilterItem(module)">
              <div class="path-name">
                <p class="name">{{ module.bk_inst_name }}</p>
                <p class="path" :title="module.path.join(' / ')">
                  {{ module.path.join(' / ') }}
                </p>
              </div>
              <div class="checkbox" @click.stop="handleCheckFilterItem(module)">
                <bk-checkbox
                  :value="filter.checked[module.bk_inst_id]"></bk-checkbox>
              </div>
            </div>
          </div>
          <cmdb-data-empty
        ...
        <div class="tree-wrapper">
          <bk-big-tree
            ...
            <div slot-scope="{ node, data }" class="node-info clearfix">
...
export default {
  ...
  methods: {
    ...
    searchHost(node) {
      ...
      const targetCondition = params.condition.find(
        target => target.bk_obj_id === modelId
      )
    ...
    searchTopology() {
      ...
      const result = this.topoModuleList.filter(
        mod =>
          mod.path.findIndex(
            path => path.toLowerCase().indexOf(keyword) !== -1
          ) !== -1
      )
    ...
    async handleCheckFilterItem(module) {
      ...
      const result = await this.searchHost(
        this.$refs.tree.getNodeById(this.getNodeId(module))
      )
      if (checked) {
