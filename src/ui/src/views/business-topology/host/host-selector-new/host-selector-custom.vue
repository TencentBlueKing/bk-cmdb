...
<template>
  <div class="custom-layout">
    <div class="input-wrapper">
      ...
      <p v-show="invalidList.length" class="ip-error">
        {{ $t('IP不正确或主机不存在') }}
      </p>
      <bk-button
        class="ip-confirm"
...
export default {
  ...
  methods: {
        ...
        if (this.validList.length) {
          ...
          const unexistList = this.validList.filter(ip => {
            const exist = list.some(
              target =>
                target.host.bk_host_innerip === ip ||
                target.host.bk_host_innerip_v6 === ip
            )
            return !exist
          })
          const newHostList = list.filter(
            ({ host }) =>
              !this.hostList.some(
                target => target.host.bk_host_id === host.bk_host_id
              )
          )
    ...
    getReqs() {
      ...
      if (IPv6List.length) {
        const params = defaultParams()
        const hostCond = params.condition.find(
          item => item.bk_obj_id === 'host'
        )
        hostCond.condition = [
          {
