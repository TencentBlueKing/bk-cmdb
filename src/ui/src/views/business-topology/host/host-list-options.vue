  ...
  <div class="options-layout clearfix">
    <div class="options options-left fl">
      ...
      <bk-dropdown-menu
        ...
        <bk-button
          ...
          <i
            :class="[
              'dropdown-icon bk-icon icon-angle-down',
        ...
        <ul
          ...
          <li
            v-test-id="'transferResource'"
            v-bk-tooltips.top-start="
              $t('主机需在“空闲机池”下才允许转移至主机池', {
                idleSet: $store.state.globalConfig.config.set,
              })
            "
            :class="['bk-dropdown-item', { disabled: !isIdleSetModules }]"
            :disabled="isIdleSetModules"
            @click="handleTransfer($event, 'resource', !isIdleSetModules)">
            {{ $t('主机池') }}
          </li>
          <li
            v-test-id="'transferAcrossBusiness'"
            v-bk-tooltips.top-start="
              $t('主机需在“空闲机池”下才允许转移至其他业务', {
                idleSet: $store.state.globalConfig.config.set,
              })
            "
            :class="['bk-dropdown-item', { disabled: !isIdleSetModules }]"
            :disabled="isIdleSetModules"
            @click="
              handleTransfer($event, 'acrossBusiness', !isIdleSetModules)
            ">
            {{ $t('其他业务') }}
          </li>
        </ul>
      ...
      <bk-dropdown-menu
        ...
        <ul slot="dropdown-content" v-test-id="'more'" class="bk-dropdown-list">
          <li
            v-test-id="'export'"
            :class="['bk-dropdown-item', { disabled: !hasSelection }]"
          ...
          <li
            ...
            :class="['bk-dropdown-item', { disabled: !count }]"
            @click="handleBatchExport($event)">
            {{ $t('导出全部') }}
          </li>
          <cmdb-auth
            v-show="!isContainerHost"
    ...
    <div class="options options-right">
      ...
      <icon-button
        ...
        :class="['option-filter', 'ml10', { active: hasCondition }]"
        icon="icon-cc-funnel"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('businessHost', ['getProperties', 'selectedNode']),
    hostProperties() {
    ...
    isIdleSetModules() {
      ...
      return this.selection.every(data =>
        data.module.every(module => module.default >= 1)
      )
    },
    removeAvailable() {
      return (
