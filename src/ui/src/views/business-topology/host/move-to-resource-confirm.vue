...
<template>
  <div :class="['confirm-wrapper', { 'has-invalid': hasInvalid }]">
    ...
    <i18n
      ...
      class="content">
      <template #count
        ><span class="count">{{ count }}</span></template
      >
      <template #invalid
        ><span class="invalid">{{ invalidList.length }}</span></template
      >
      <template #idleSet
        ><span>{{ idleSet }}</span></template
      >
    </i18n>
    <i18n v-else tag="p" path="确认归还主机池主机数量" class="content">
      <template #count
        ><span class="count">{{ count }}</span></template
      >
      <template #idleSet
    ...
    <div class="directory">
      ...
      <bk-select
        ...
        <cmdb-auth-option
          ...
          :auth="{
            type: $OPERATION.HOST_TO_RESOURCE,
            relation: [[[bizId], [directory.bk_module_id]]],
          }">
        </cmdb-auth-option>
      </bk-select>
    </div>
