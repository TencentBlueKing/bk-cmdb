...
<template>
  <div class="list-layout">
    <host-list-options v-test-id @transfer="handleTransfer"></host-list-options>
    <host-filter-tag ref="filterTag" class="filter-tag"></host-filter-tag>
    <bk-table
      ref="tableRef"
      v-test-id.businessHostAndService="'hostList'"
      ...
      <bk-table-column
        ...
        <template slot-scope="{ row }">
          <cmdb-property-value
            ...
            :theme="
              column.bk_property_id === 'bk_host_id' ? 'primary' : 'default'
            "
            :value="
              row | hostValueFilter(column.bk_obj_id, column.bk_property_id)
            "
            :show-unit="false"
            :property="column"
  ...
  methods: {
    async initFilterStore() {
      ...
      if (
        ...
        currentRouteName !== MENU_BUSINESS_HOST_AND_SERVICE
      ) {
        return
      }
      this.storageRouteName = currentRouteName
    ...
    getTableCellPropertyValueRefId(property) {
      return this.$tools.isUseComplexValueType(property)
        ? `table-cell-property-value-${property.bk_property_id}`
        : null
    },
    ...
    handleValueClick(row, column) {
      if (
        column.bk_obj_id !== 'host' ||
        column.bk_property_id !== 'bk_host_id'
      ) {
        return
      }
    ...
    handleHeaderClick(column) {
      ...
      ColumnsConfig.open({
        props: {
          ...
          selected: FilterStore.defaultHeader.map(
    ...
    handleApplyColumnsConfig(properties = []) {
      return this.$store.dispatch('userCustom/saveUsercustom', {
        [this.customInstanceColumnKey]: properties.map(
          property => property.bk_property_id
        ),
      })
    },
    ...
    getNormalParams() {
      ...
      const modelConditionId = has(fieldMap, topoNodeData.bk_obj_id)
        ? topoNodeData.bk_obj_id
        : 'object'
      const modelCondition = params.condition.find(
        modelCondition => modelCondition.bk_obj_id === modelConditionId
      )
    ...
    getContainerParams() {
      ...
      const fieldMap = {
        ...
        [CONTAINER_OBJECTS.FOLDER]:
          CONTAINER_OBJECT_PROPERTY_KEYS[CONTAINER_OBJECTS.FOLDER].ID,
        [CONTAINER_OBJECTS.NAMESPACE]:
          CONTAINER_OBJECT_PROPERTY_KEYS[CONTAINER_OBJECTS.NAMESPACE].ID,
        [CONTAINER_OBJECTS.WORKLOAD]:
          CONTAINER_OBJECT_PROPERTY_KEYS[CONTAINER_OBJECTS.WORKLOAD].ID,
      ...
      if (nodeType === CONTAINER_OBJECTS.FOLDER) {
        params.folder = true
        // folder父节点为cluster节点
        params[fieldMap[CONTAINER_OBJECTS.CLUSTER]] =
          this.selectedNode.parent.data.bk_inst_id
      } else {
    ...
    validteIdleHost() {
      const invalidList = this.table.selection
        .filter(item => {
    ...
    handleDialogConfirm() {
      this.dialog.show = false
      const type =
        this.dialog.props.transferType || this.dialog.props.moduleType
    ...
    async moveHostToResource(directoryId) {
      try {
        ...
        await this.$store.dispatch(
          'hostRelation/transferHostToResourceModule',
          {
            params: {
              bk_biz_id: this.bizId,
