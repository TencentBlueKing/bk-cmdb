...
<template>
  <div v-bkloading="{ isLoading: loading }" class="module-selector-layout">
    <div class="wrapper">
      <cmdb-resize-layout
        class="topo-resize-layout"
        direction="right"
        :handler-offset="3"
        :min="480"
        :max="508">
        <div
          :class="['wrapper-column wrapper-left', { 'has-title': hasTitle }]">
          <h2 v-if="hasTitle" class="title">{{ title }}</h2>
          <bk-select
            v-model="targetBizId"
            ...
            <bk-big-tree
              ...
              <template slot-scope="{ node, data }">
                <span
                  ...
                  :class="[
                    'node-checkbox fl',
                    { 'is-checked': checked.includes(node) },
                  ]">
                </span>
                <i
                  v-if="data.default !== 0"
...
export default {
  ...
  computed: {
    ...
    authType() {
      const { HOST_TRANSFER_ACROSS_BIZ, IDLE_HOST_TRANSFER_ACROSS_BIZ } =
        this.$OPERATION
  ...
  methods: {
    ...
    async getFullAmountBusiness() {
      try {
        ...
        const availableBusiness = (data.info || []).filter(
          business => business.bk_biz_id !== this.bizId
        )
    ...
    sortBusinessByAuth(authData) {
      ...
      const list = this.businessList?.map((item, index) => ({
        ...item,
        is_pass: authData[index]?.is_pass,
      }))
    ...
    getInternalModules() {
      return this.$store
        .dispatch('objectMainLineModule/getInternalTopo', {
    ...
    handleNodeCheck(checked, currentNode) {
      ...
      checked.forEach(id => {
        ...
        if (
          node.data.default === currentNode.data.default &&
          node.data.bk_inst_id === currentNode.data.bk_inst_id
        ) {
