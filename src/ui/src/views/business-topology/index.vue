  ...
  <div
    ...
    <cmdb-resize-layout
      ...
      <topology-tree
        ref="topologyTree"
        v-test-id
        :active="activeTab"></topology-tree>
      <i
      ...
      <bk-tab
        ...
        <bk-tab-panel name="hostList" :label="$t('主机列表')">
          ...
          <host-list
            ...
            ref="hostList"
            v-test-id
            :active="activeTab === 'hostList'"></host-list>
        </bk-tab-panel>
        ...
        <bk-tab-panel
          ...
          <template v-else>
            <div v-if="!showServiceInstance" class="non-business-module">
              <div class="tips">
                <i class="bk-cc-icon icon-cc-tips"></i>
                <span>{{
                  $t('非业务模块，无服务实例，请选择业务模块查看')
                }}</span>
              </div>
            </div>
            <service-instance-view
              v-else-if="activeTab === 'serviceInstance'"
              v-test-id></service-instance-view>
          </template>
        </bk-tab-panel>
  ...
  computed: {
    ...
    isSimpleNodeInfo() {
      return (
        this.selectedNode &&
        (this.selectedNode.data.default !== 0 ||
          this.selectedNode.data.is_folder)
      )
    },
    emptySet() {
      return (
        this.selectedNode &&
        this.selectedNode.data.bk_obj_id === 'set' &&
    ...
    activeTab(tab) {
      this.$nextTick(() => {
        ...
        RouterQuery.setAll({
          ...
          topo_path: this.isContainerNode
            ? RouterQuery.get('topo_path')
            : undefined,
