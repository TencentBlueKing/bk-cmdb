  ...
  <div class="host-selector-layout">
    <div class="layout-content">
      <div class="module-host">
        ...
        <div class="host-list">
          <div class="top-bar">
            <div class="topo-path">
              已自动过滤
              <div class="path" :title="topoPath">{{ topoPath }}</div>
              可选的主机：
            </div>
          ...
          <bk-table
            ...
            <bk-table-column
              type="selection"
              width="30"
              :selectable="getSelectable"></bk-table-column>
            ...
            <bk-table-column :label="$t('管控区域')" show-overflow-tooltip>
              <template slot-scope="{ row }">{{
                row.host.bk_cloud_id | foreignkey
              }}</template>
            </bk-table-column>
          </bk-table>
        </div>
      ...
      <div class="layout-col result-preview">
        ...
        <div v-show="selected.length > 0" class="preview-content">
            ...
            <bk-collapse-item name="host" hide-arrow>
              <div class="collapse-title">
                <div class="text">
                  <i
                    :class="[
                      'bk-icon icon-angle-right',
                      { expand: collapse.expanded.host },
                    ]"></i>
                  <i18n path="已选择N台主机">
                    <template #count
                      ><span class="count">{{
              ...
              <ul slot="content" class="selected-host-list">
                <li
                  ...
                  :key="index"
                  class="host-item">
                  <div v-bk-overflow-tips="{ placement: 'left' }" class="ip">
                    {{
                      row.host.bk_host_innerip ||
                      row.host.bk_host_innerip_v6 ||
                      '--'
    ...
    <ul v-show="more.show" ref="moreMenu" class="more-menu">
...
export default {
  ...
  methods: {
    ...
    async getNoServiceInstanceHost() {
      ...
      try {
        const result = await this.$store.dispatch(
          'serviceInstance/getNoServiceInstanceHost',
          {
            params: {
    ...
    setChecked() {
      this.$nextTick(() => {
        const ids = [
          ...new Set(this.selected.map(data => data.host.bk_host_id)),
        ]
        this.list.forEach(row => {
          this.$refs.table.toggleRowSelection(
            row,
            ids.includes(row.host.bk_host_id)
          )
        })
      })
    },
    ...
    handleSelectionChange(selection) {
      ...
      const unselected = this.list.filter(
        item => !ids.includes(item.host.bk_host_id)
      )
    ...
    handleRemove(hosts) {
      ...
      this.selected = this.selected.filter(
        target => !ids.includes(target.host.bk_host_id)
      )
    },
    handleSelect(hosts) {
      const selectData = Array.isArray(hosts) ? hosts : [hosts]
      selectData.forEach(data => {
        const isExist = this.selected.find(
          target => target.host.bk_host_id === data.host.bk_host_id
        )
        if (!isExist) {
    ...
    handleCopyIp() {
      const ipList = this.selected.map(
        item => item.host.bk_host_innerip || item.host.bk_host_innerip_v6
      )
    ...
    handleCollapse(names) {
      Object.keys(this.collapse.expanded).forEach(
        key => (this.collapse.expanded[key] = false)
      )
      names.forEach(name => {
