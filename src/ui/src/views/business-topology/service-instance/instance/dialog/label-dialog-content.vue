...
<template>
  <div class="edit-label-list">
    <div class="scrollbar-box">
      <div v-for="(label, index) in list" :key="index" class="label-item">
        <div
          class="label-key"
          :class="{ 'is-error': errors.has('key-' + index) }">
          <input
            ref="tagKey"
            v-model="label.key"
            ...
            :placeholder="`Key: ${$validator.dictionary.getMessage(
              $i18n.locale,
              'instanceTagKey'
            )}`" />
        ...
        <div
          class="label-value"
          :class="{ 'is-error': errors.has('value-' + index) }">
          <input
            v-model="label.value"
            ...
            :placeholder="`Value: ${$validator.dictionary.getMessage(
              $i18n.locale,
              'instanceTagValue'
            )}`" />
        ...
        <i
          ...
          class="bk-icon icon-plus-circle-shape icon-btn"
          @click="handleAddLabel(index)">
        </i>
        <i
          :class="['bk-icon', 'icon-minus-circle-shape', 'icon-btn']"
        ...
        if (list.length === 1 && !list[0].key && !list[0].value) {
          ...
          for (let i = 0; i < defaultLabels.length; i++) {
            ...
            const index = list.findIndex(
              item =>
                item.id !== -1 &&
                defaultLabels[i].id === item.id &&
                (defaultLabels[i].key !== item.key ||
                  defaultLabels[i].value !== item.value)
            )
            if (index !== -1) {
              list[index].id = -1
              !this.removeKeysList.includes[defaultLabels[i].key] &&
                this.removeKeysList.push(defaultLabels[i].key)
            }
          }
