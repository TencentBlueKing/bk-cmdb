...
<template>
  <bk-dialog
    ...
    <div slot="header" class="reset-header">
      {{ $t('批量编辑') }}
      <span>{{
        $tc('已选择实例', serviceInstances.length, {
          num: serviceInstances.length,
        })
      }}</span>
    </div>
    <batch-content ref="batchContent" :labels="labels">
      <single-content
        ref="singleContent"
    ...
    <div slot="footer" class="edit-label-footer">
      <bk-button
        theme="primary"
