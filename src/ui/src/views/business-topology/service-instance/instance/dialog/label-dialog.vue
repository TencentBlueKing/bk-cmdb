...
<template>
  <bk-dialog
    v-model="isShow"
    class="bk-dialog-no-padding edit-label-dialog"
    :width="580"
    :mask-close="false"
    :esc-close="false"
    ...
    <label-dialog-content ref="labelComp" :default-labels="labels">
    </label-dialog-content>
    <div slot="footer" class="edit-label-dialog-footer">
      <bk-button
        theme="primary"
      ...
      <bk-button theme="default" class="ml5" @click.stop="close">{{
        $t('取消')
      }}</bk-button>
    </div>
  </bk-dialog>
...
export default {
  ...
  computed: {
    ...
    labels() {
      ...
      return Object.entries(this.serviceInstance.labels).map(
        ([key, value], index) => ({ id: index, key, value })
      )
    },
  },
  methods: {
