  ...
  <div class="options">
    <div class="left">
      ...
      <bk-dropdown-menu class="ml10" trigger="click" font-size="medium">
        ...
        <ul slot="dropdown-content" v-test-id="'moreAction'" class="menu-list">
          <cmdb-auth
            tag="li"
            class="menu-item"
            :auth="{ type: $OPERATION.D_SERVICE_INSTANCE, relation: [bizId] }">
            <span
              slot-scope="{ disabled: authDisabled }"
              class="menu-option"
          ...
          <li class="menu-item">
            <span
              :class="{ 'menu-option': true, disabled: !selection.length }"
              @click="handleCopy(!selection.length)">
              {{ $t('复制IP') }}
            </span>
          </li>
          <cmdb-auth
      ...
      <cmdb-auth
        ...
        <bk-button
          ...
          <span v-show="hasDifference && !isSyncing" class="topo-status"></span>
        </bk-button>
      </cmdb-auth>
    </div>
    <div class="right">
    ...
    <span ref="tipsContent" class="tips-content">
      <i18n v-if="!hasHost" path="当前模块主机为空，请先XXX" tag="p">
        <template #action>
          <a
            href="javascript:;"
            class="action-link"
      ...
      <i18n
        v-else-if="isEmptyServiceTemplate"
        path="当前模块所属的服务模板内进程信息为空，请先XXX"
        tag="p">
        <template #action>
          <a
            href="javascript:;"
...
export default {
    ...
    searchMenuList() {
      ...
      const list = [
        ...
        },
        {
          id: 'tagValue',
          name: this.$t('标签值'),
          conditions: Object.keys(this.historyLabels).map(key => ({
            id: key,
            name: `${key}:`,
          })),
  ...
  watch: {
    ...
    searchMenuList() {
      this.$nextTick(() => {
        const menu =
          this.$refs.searchSelect && this.$refs.searchSelect.menuInstance
  ...
  methods: {
    async updateHistoryLabels() {
      try {
        this.historyLabels = await this.$store.dispatch(
          'instanceLabel/getHistoryLabel',
          {
            params: {
    ...
    async checkDifference() {
      ...
      try {
        ...
        const { modules: syncStatus = [] } = await this.$store.dispatch(
        ...
        // eslint-disable-next-line max-len
        const difference = syncStatus.find(
          difference =>
            difference.bk_module_id === this.selectedNode.data.bk_inst_id
        )
    ...
    async checkSyncStatus() {
      const [moduleSyncData] = await this.$store.dispatch(
        'serviceTemplate/getServiceTemplateInstanceStatus',
        {
          bizId: this.bizId,
          params: {
            bk_module_ids: [this.selectedNode.data.bk_inst_id],
      ...
      this.isSyncing = ['new', 'waiting', 'executing'].includes(
        moduleSyncData?.status
      )
    ...
    handleSearch(filters) {
      ...
      filters.forEach(data => {
        if (!data.values) {
          const nameIndex = transformedFilters.findIndex(
