  ...
  <div
    ...
    <div class="sticky-row-cell selection-cell" :style="getCellStyle('id')">
      <bk-checkbox
        class="selection"
        :checked="checked"
        @change="handleCheck"></bk-checkbox>
    </div>
    <div
      class="sticky-row-cell center expand-cell"
      :style="getCellStyle('expand')">
      <i class="expand-icon bk-icon icon-play-shape" @click="handleExpand"> </i>
    </div>
    ...
    <div class="sticky-row-cell" :style="getCellStyle('tag')">
      <list-cell-tag
        :row="row"
        @update-labels="handleUpdateLabels(row, ...arguments)"></list-cell-tag>
    </div>
  </div>
</template>
...
export default {
  ...
  beforeDestroy() {
    this.table.$refs.bodyWrapper.removeEventListener(
      'scroll',
      this.updatePostion
    )
  },
