...
<template>
  <bk-table
    ...
    <bk-table-column
      :label="$t('操作')"
      :min-width="$i18n.locale === 'en' ? 200 : 150">
      <list-cell-operation
        slot-scope="{ row }"
        :row="row"
        @refresh-count="handleRefreshCount">
  ...
  methods: {
    ...
    async getList() {
      try {
        const { count, info } = await this.$store.dispatch(
          'serviceInstance/getModuleServiceInstances',
          {
            params: {
    ...
    processListRequest(reqParams, reqConfig) {
      return this.$store.dispatch(
        'processInstance/getServiceInstanceProcesses',
        {
          params: reqParams,
          config: reqConfig,
        }
      ...
      LabelBatchDialog.show({
        ...
        updateCallback: (removedKeys, newLabels) => {
          this.selection.forEach(instance => {
            instance.labels &&
              removedKeys.forEach(key => delete instance.labels[key])
