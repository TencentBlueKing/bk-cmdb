...
<template>
  <bk-table
    ref="processTable"
    v-test-id.businessHostAndService="'processList'"
    ...
    <bk-table-column
      ...
      <span slot-scope="{ row }" class="process-name">{{
        row.bk_process_name
      }}</span>
    </bk-table-column>
...
export default {
  ...
  methods: {
    ...
    async getProcessList() {
      try {
        const { count, info } = await this.$store.dispatch(
          'serviceInstance/getMoudleProcessList',
          {
            params: {
    ...
    instanceListRequest(reqParams, reqConfig) {
      return this.$store
        .dispatch('serviceInstance/getProcessListById', {
          params: reqParams,
          config: reqConfig,
        })
        .then(({ info }) => info)
