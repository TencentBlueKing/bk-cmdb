...
<template>
  <bk-table
    ...
    <bk-table-column
      :label="$t('所属实例')"
      prop="service_instance_name"
      min-width="150"
      fixed
      show-overflow-tooltip>
      <template slot-scope="{ row }">
        <span class="instance-name" @click.stop="handleView(row)">{{
    ...
    <bk-table-column
      ...
      <template slot-scope="{ row }">
        <cmdb-property-value
          v-if="property.bk_property_id !== 'bind_info'"
...
export default {
  ...
  methods: {
    ...
    checkSelection() {
      ...
      const existSelection = this.list.filter(row =>
        reserved.includes(row.process_id)
      )
