...
<template>
  <div v-if="template" class="clearfix">
    <div class="info-item fl" :title="`${$t('服务模板')} : ${template.name}`">
      ...
      <div class="value fl">
        <div
          ...
          @click="goServiceTemplate">
          <span class="text link">{{ template.name }}</span>
          <i class="icon-cc-share"></i>
        </div>
        <span v-else class="text">{{ template.name }}</span>
      </div>
    </div>
    <div
      class="info-item fl"
      :title="`${$t('服务分类')} : ${template.category || '--'}`">
...
export default {
  ...
  computed: {
    ...
    autoApplyEnable() {
      ...
      return (
        this.instance.host_apply_enabled || this.serviceTemplateHostApplyEnabled
      )
    },
  },
  watch: {
  ...
  methods: {
    ...
    async getServiceInfo() {
      ...
      const firstCategory =
        categories.find(category =>
          category.secondCategory.some(
            second => second.id === this.instance.service_category_id
          )
        ) || {}
      // eslint-disable-next-line max-len
      const secondCategory =
        (firstCategory.secondCategory || []).find(
          second => second.id === this.instance.service_category_id
        ) || {}
      this.template = {
        name: this.instance.service_template_id
          ? this.instance.bk_module_name
          : this.$t('无'),
    ...
    async getModuleApplyStatusByTemplate() {
      const [moduleApplyStatus] = await this.$store.dispatch(
      ...
      this.serviceTemplateHostApplyEnabled =
        moduleApplyStatus?.host_apply_enabled
    },
