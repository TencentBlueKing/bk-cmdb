  ...
  methods: {
    ...
    getInstanceTopology() {
      return this.$store.dispatch(
        'objectMainLineModule/getInstTopoInstanceNum',
        {
          bizId: this.bizId,
          config: {
    ...
    async getContainerTopology() {
      ...
      for (let i = 0; i < topoPathQueue.length; i++) {
        ...
        if (!asyncTopoTreeData) {
          ...
          const foundNode = parentData.find(
            parent => parent.bk_inst_id === instId
          )
          if (foundNode && data) {
        ...
        if (data) {
          const nodeIds = data
            .filter(
              item => this.isContainerLeaf(item.bk_obj_id) || item.is_folder
            )
      ...
      const currentClusterTopo = clusterTopo.find(
        topo => topo.bk_inst_id === Number(currentClusterId)
      )
    ...
    async lazyGetChildrenNode(node) {
      if (
        this.isContainerLeaf(node.data.bk_obj_id) ||
        this.isContainerFolder(node)
      ) {
        return {}
      }
      ...
      const leafIds = topoList
        ?.filter(item => item.is_workload || item.is_folder)
        ?.map(item => this.getNodeId(item))
    ...
    async setNodeCount(targetNodes, force = false) {
      const nodes = force
        ? targetNodes
        : targetNodes.filter(
            ({ data }) => !['pending', 'finished'].includes(data.status)
          )
    ...
    async setNormalNodeCount(nodes) {
      try {
        const result = await this.$store.dispatch(
          'objectMainLineModule/getTopoStatistics',
          {
            bizId: this.bizId,
            params: {
        ...
        nodes.forEach(({ data }) => {
          ...
          const count = result.find(
            count =>
              count.bk_obj_id === data.bk_obj_id &&
              count.bk_inst_id === data.bk_inst_id
          )
    ...
    async setContainerNodeCount(nodes) {
      try {
        ...
        const { hostStats, podStats } =
          await topologyInstanceService.getContainerTopoNodeStats(params)
        nodes.forEach(({ data }) => {
          const finder = item => {
            if (data.is_folder) {
    ...
    async handleShowCreateDialog(node) {
      const nodeModel = this.topologyModels.find(
        data => data.bk_obj_id === node.data.bk_obj_id
      )
      ...
      const primaryKey =
        { set: 'bk_set_id', module: 'bk_module_id' }[nextModelId] ||
        'bk_inst_id'
      this.createInfo.properties = properties.filter(
        property => property.bk_property_id !== primaryKey
      )
    },
    ...
    async handleCreateNode(value) {
      try {
        ...
        const nextModel = this.topologyModels.find(
          model => model.bk_obj_id === nextModelId
        )
        const handlerMap = {
          set: this.createSet,
        ...
        const data = await (
          handlerMap[nextModelId] || this.createCommonInstance
        )(formData)
        const nodeData = {
          default: 0,
          child: [],
        ...
        this.$refs.tree.addNode(
          nodeData,
          parentNode.id,
          parentNode.data.bk_obj_id === 'biz' ? 1 : 0
        )
    ...
    async addModulesInSetTemplate(parentNodeData, id) {
      ...
      const nextModel = this.topologyModels.find(
        model => model.bk_obj_id === 'module'
      )
      modules.info &&
        modules.info.forEach(_module => {
          const nodeData = {
            default: 0,
            child: [],
            bk_obj_name: nextModel.bk_obj_name,
    ...
    async refreshCount({ hosts, target }) {
      ...
      if (target) {
        const node = this.$refs.tree.getNodeById(
          `${target.data.bk_obj_id}-${target.data.bk_inst_id}`
        )
