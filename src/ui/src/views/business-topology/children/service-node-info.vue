...
<template>
  <div
    ...
    <cmdb-permission
      ...
      :permission="permission"></cmdb-permission>
    <cmdb-details
    ...
    <template v-else-if="type === 'update'">
      <cmdb-form
        ...
        <!-- 配置字段不允许修改，提示跳转到模板 -->
        <template
          v-for="config in propertyConfigList"
          #[config.property.bk_property_id]>
          <bk-popover
...
export default {
  ...
  computed: {
    ...
    moduleFromSetTemplate() {
      return (
        this.isModuleNode && !!this.selectedNode.parent.data.set_template_id
      )
    },
    editable() {
  ...
  methods: {
    ...
    async getNodeDetails() {
      ...
      this.disabledProperties =
        this.selectedNode.data.bk_obj_id === 'module' && this.withTemplate
          ? ['bk_module_name']
          : []
    },
    async getProperties() {
    ...
    async getSetInstance() {
      const {
        info: [instance],
      } = await this.$store.dispatch('objectSet/searchSet', {
        bizId: this.business,
        params: {
          page: { start: 0, limit: 1 },
          fields: [],
    ...
    async getPropertyConfig() {
      ...
      const templateService = this.isModuleNode
      ...
      const values = this.$tools.getInstFormValues(
        this.properties,
        this.instance,
        false
      )
      this.propertyConfigList = attributes.map(item => {
        const property =
          this.properties.find(prop => prop.id === item.bk_attribute_id) || {}
        return {
          property,
    ...
    insertNodeIdProperty(properties) {
      if (
        properties.find(
          property => property.bk_property_id === this.nodePrimaryPropertyId
        )
      ) {
        return
      }
      const nodeNameProperty = properties.find(
        property => property.bk_property_id === this.nodeNamePropertyId
      )
      properties.unshift({
        ...nodeNameProperty,
        id: Date.now(),
    ...
    async handleSubmit(value) {
      ...
      try {
        await (promiseMap[this.modelId] || this.updateCustomInstance)({
          ...value,
          bk_biz_id: this.business,
        })
        this.selectedNode.data.bk_inst_name =
          value[nameMap[this.modelId] || 'bk_inst_name']
    ...
    async getSelectedNodeHostCount() {
      ...
      const selectedModel = defaultModel.includes(selectedNode.data.bk_obj_id)
        ? selectedNode.data.bk_obj_id
        : 'object'
      const selectedModelCondition = conditionParams.condition.find(
        model => model.bk_obj_id === selectedModel
      )
