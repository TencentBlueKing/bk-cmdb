...
<template>
  <div class="create-node-layout">
    ...
    <div class="node-create-form">
      <bk-radio-group v-model="withTemplate" class="form-item mb20">
      ...
      <div v-if="withTemplate" class="form-item">
        ...
        <bk-select
          ...
          <bk-option
            v-for="option in setTemplateList"
            :id="option.id"
            :key="option.id"
            :name="option.name">
          </bk-option>
          <div
            v-if="!setTemplateList.length"
            slot="extension"
            class="add-template"
        ...
        <span v-if="errors.has('setTemplate')" class="form-error">{{
          errors.first('setTemplate')
        }}</span>
      </div>
      <div class="form-item">
        <label>
        ...
        <bk-input
          v-model="setName"
          v-validate="
            'required|longchar|businessTopoInstNames|emptySetName|setNameMap|setNameLen'
          "
          class="form-textarea"
          type="textarea"
        ...
        <span v-if="errors.has('setName')" class="form-error">{{
          errors.first('setName')
        }}</span>
      </div>
    </div>
    <div class="node-create-options">
      <bk-button
        v-test-id="'createSetSave'"
        theme="primary"
        class="mr10"
...
export default {
  ...
  watch: {
    withTemplate(value) {
      if (value) {
        this.setTemplate = this.setTemplateList[0]
          ? this.setTemplateList[0].id
          : ''
      } else {
        this.setTemplate = 0
      }
    ...
    async getSetTemplates() {
      if (has(this.setTemplateMap, this.business)) {
        ...
        try {
          ...
          const list = (data.info || []).map(template => ({
