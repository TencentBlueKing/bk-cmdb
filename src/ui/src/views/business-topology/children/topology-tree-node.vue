  ...
  setup(props, { emit }) {
    ...
    const getNodeIconTips = (node, data) => {
      ...
      if (data.is_container) {
        ...
        if (data.is_workload) {
          return `${
            getContainerObjectNames(CONTAINER_OBJECTS.WORKLOAD).FULL
          } (${getContainerObjectNames(objId).FULL})`
        }
      ...
      const tipsMap = {
        ...
        [BUILTIN_MODELS.SET]: `${t('集群')}${
          isTemplate(node) ? t('（模板创建）') : t('（自定义创建）')
        }`,
        [BUILTIN_MODELS.MODULE]: `${t('模块')}${
          isTemplate(node) ? t('（模板创建）') : t('（自定义创建）')
        }`,
      }
    ...
    const isTemplate = node =>
      node.data.service_template_id || node.data.set_template_id
...
<template>
  <div
    :class="[
      'topology-tree-node',
      { 'is-container': node.data.is_container, 'is-selected': node.selected },
    ]">
    <div
      v-bk-tooltips.top.light="{
        content: getNodeIconTips(node, data),
        disabled: data.default !== 0,
      }"
      :class="[
        'node-icon',
        {
    ...
    <div class="node-extra">
      <cmdb-auth
        v-if="isShowCreate(node, data)"
        class="node-create-trigger"
        :auth="{ type: $OPERATION.C_TOPO, relation: [bizId] }">
        <template #default="{ disabled }">
          <i
            v-if="isBlueKing && !editable"
            v-bk-tooltips.top="{
              content: $t('蓝鲸业务拓扑节点提示'),
              interactive: false,
            }"
          ...
          <i
            v-else-if="data.set_template_id"
            v-bk-tooltips.top="{
      ...
      <cmdb-loading
        :class="['node-count', { 'is-selected': node.selected }]"
