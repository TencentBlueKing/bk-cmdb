...
<template>
  <div v-bkloading="{ isLoading: $loading() }" class="create-layout clearfix">
    ...
    <div class="create-hosts">
      ...
      <div ref="createTables" class="create-tables">
        <transition-group name="service-table-list" tag="div">
          <service-instance-table
            ...
            @confirm-edit-name="
              handleConfirmEditName(data.instance, ...arguments)
            "
    ...
    async handleConfirm() {
      try {
        ...
        const params = {
          ...
          instances: serviceInstanceTables
            .filter(table => table.processList?.length)
            .map(table => {
              const { instance } = this.hosts.find(
                data => data.host.bk_host_id === table.id
              )
              return {
                bk_host_id: table.id,
        ...
        await this.$store.dispatch(
          'serviceInstance/createProcServiceInstanceWithRaw',
          { params }
        )
...
.service-table-list-enter-active,
