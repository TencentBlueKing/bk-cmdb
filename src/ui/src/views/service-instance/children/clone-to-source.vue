...
<template>
  <div class="source-layout">
    ...
    <div ref="sourceTables" class="source-table">
      <bk-table :data="cloneProcesses" @selection-change="handleSelectChange">
        <bk-table-column
          type="selection"
          align="center"
          width="60"
    ...
    <div class="page-options" :class="{ 'is-sticky': hasScrollbar }">
      <cmdb-auth
        ...
        <bk-button
          ...
          theme="primary"
          :disabled="
            !!repeatedProcesses.length || disabled || !cloneProcesses.length
          "
          @click="doClone">
          {{ $t('确定') }}
        </bk-button>
      </cmdb-auth>
      <bk-button class="options-button" @click="backToModule">{{
        $t('取消')
      }}</bk-button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    repeatedProcesses() {
      return this.cloneProcesses.filter(cloneProcess => {
        ...
        const sourceProcess = this.sourceProcesses.find(
          process => process[key] === cloneProcess[key]
        )
        return (
    ...
    bindInfoProperty() {
      return (
        this.properties.find(
          property => property.bk_property_id === 'bind_info'
        ) || {}
      )
    },
    bindInfo: {
  ...
  methods: {
    ...
    isRepeat(item) {
      return this.repeatedProcesses.some(
        process => process.bk_process_id === item.bk_process_id
      )
    },
