...
<template>
  <div class="classify">
    <h4 class="classify-name" :title="classify['bk_classification_name']">
      <span class="classify-name-text">{{
        classify['bk_classification_name']
      }}</span>
    </h4>
      ...
      <div
        ...
        <i
          :class="[
            ...
            'icon',
            model['bk_obj_icon'],
            { 'nonpre-mode': !model['ispre'] },
          ]"></i>
        <span class="model-name">{{ model['bk_obj_name'] }}</span>
        <i
          class="model-star bk-icon"
  ...
  methods: {
    ...
    toggleCustomNavigation(model) {
      if (this.isBuiltinModel(model)) {
        ...
        promise.then(() => {
          this.$success(
            isAdd ? this.$t('添加导航成功') : this.$t('取消导航成功')
          )
        })
      }
    },
    async toggleDefaultCollection(model) {
      ...
      try {
        ...
        this.$success(
          isCollected ? this.$t('取消导航成功') : this.$t('添加导航成功')
        )
      } catch (e) {
        console.error(e)
      }
...
.classify {
  ...
  box-shadow: 0 3px 6px 0 rgb(51 60 72 / 5%);
}
