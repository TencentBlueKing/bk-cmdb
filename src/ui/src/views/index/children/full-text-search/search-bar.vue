...
<template>
  <div v-click-outside="handleClickOutside" class="search-bar">
    <div class="input-bar">
      <bk-input
        ref="searchInput"
    ...
    <div v-show="showSuggestion" class="search-popover suggestion">
      <ul class="list suggestion-list">
        <li
          ...
          <i
            v-if="
              item.type === 'biz' && item.source.bk_data_status === 'disabled'
            "
            class="tag-disabled">
            {{ $t('已归档') }}
          </i>
    ...
    <div v-show="showHistory" class="search-popover history">
...
export default defineComponent({
  ...
  setup() {
    ...
    const { result, getSearchResult, onkeydownResult, selectResultIndex } =
      useResult({ route, keyword })
