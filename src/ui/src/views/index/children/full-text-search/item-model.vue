  ...
  <div class="result-item">
    ...
    <div class="result-desc" @click="data.linkTo(data.source)">
      <div
        class="desc-item hl"
        v-html="
          `${$t('唯一标识')}：${getHighlightValue(data.source.bk_obj_id, data)}`
        "></div>
      <div
        class="desc-item hl"
        v-html="
          `${$t('模型名称')}：${getHighlightValue(
            data.source.bk_obj_name,
            data
          )}`
        "></div>
      <div class="desc-item">
        {{ $t('所属模型分组') }}：{{ classificationName }}
      </div>
      <dl class="model-group-list">
        <div
          ...
          <dd class="property-list">
            <div
              v-for="(property, childIndex) in item.properties"
              :key="childIndex"
              class="property-item">
              （<span
                class="hl"
                v-html="
                  `${$t('字段名称')}：${getHighlightValue(
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const classificationName = computed(() => {
      ...
      return (
        classifications.find(item => item.bk_classification_id === id) || {}
      ).bk_classification_name
    })

    const propertyGroups = ref([])
    watchEffect(async () => {
      propertyGroups.value = await store.dispatch(
    ...
    // eslint-disable-next-line max-len
    const sortedPropertyGroups = computed(() =>
      // eslint-disable-next-line vue/no-side-effects-in-computed-properties
      propertyGroups.value.sort(
        (groupA, groupB) => groupA.bk_group_index - groupB.bk_group_index
      )
    )

    const groupedProperties = computed(() =>
      sortedPropertyGroups.value.map(group => ({
        group,
