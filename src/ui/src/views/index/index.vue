...
<template>
  <div
    class="index-layout"
    :style="{ '--defaultPaddingTop': `${paddingTop}px` }">
    <div :class="['search-layout', { sticky: showResultList }]">
      <div class="search-top">
        <div v-if="isFullTextSearch" class="search-tab">
          <span
            :class="['tab-item', { active: activeName === 'host' }]"
            @click="handleChangeTab('host')">
            {{ $t('主机搜索') }}
          </span>
          <span
            :class="['tab-item', { active: activeName === 'fullText' }]"
      ...
      <div class="search-content">
        <div
          ...
          class="loading-ghost"></div>
        <full-text-search-result-list
          v-show="!loading"
          v-if="fullTextSearchProps.showResultList"
          v-test-id
