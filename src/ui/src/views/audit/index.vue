...
<template>
  <div class="audit-layout">
    <bk-tab class="audit-tab" type="unborder-card" :active.sync="active">
      <bk-tab-panel v-for="panel in tabPanels" v-bind="panel" :key="panel.id">
      </bk-tab-panel>
    </bk-tab>
    <div class="audit-options">
      <component
        ...
        @condition-change="handleConditionChange"></component>
    </div>
    <bk-table
      ...
      <bk-table-column
        ...
        <template slot-scope="{ row }">
          <audit-business-selector
            type="info"
            :value="row.bk_biz_id"></audit-business-selector>
        </template>
      </bk-table-column>
      <bk-table-column
        prop="resource_name"
      ...
      <bk-table-column :label="$t('操作描述')">
        <template slot-scope="{ row }">{{
          `${getActionName(row)}${getResourceTypeName(row)}`
        }}</template>
      </bk-table-column>
      <bk-table-column
        width="160"
  ...
  methods: {
    setQueryWatcher() {
      this.unwatch = RouterQuery.watch(
        '*',
        ({ page, limit, sort, tab, _e: isEvent }) => {
          this.active = tab || 'host'
          this.table.pagination.current = parseInt(
            page || this.table.pagination.current,
            10
          )
          this.table.pagination.limit = parseInt(
    ...
    handleConditionChange(condition) {
      const useCondition = function (condition) {
        ...
        if (usefulCondition.resource_id) {
          usefulCondition.resource_id =
            usefulCondition.resource_type === 'dynamic_group'
              ? usefulCondition.resource_id
      ...
      if (
        usefulCondition.resource_name ||
        usefulCondition.extend_resource_name
      ) {
        const {
          fuzzy_query: fuzzy,
    ...
    getActionName(row) {
      ...
      const operation = operations.find(
        operation => operation.id === row.action
      )
