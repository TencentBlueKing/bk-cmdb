...
<template>
  <div
    ...
    <div v-if="dragIcon" class="drag-icon"></div>
    <bk-icon class="group-collapse-icon" type="down-shape" />
    <p v-bk-overflow-tips class="group-title-text">{{ title }}</p>
    <bk-dropdown-menu
      v-if="dropdownMenu"
      ...
      <template #dropdown-content>
        <ul v-if="visibleCommands.length" class="bk-dropdown-list">
    ...
    <bk-tag v-if="isNewClassify" theme="success" radius="2px">{{
      $t('新的')
    }}</bk-tag>
  </div>
</template>
