  ...
  <div class="model-relation">
    ...
    <bk-table
      ...
      <bk-table-column
        width="40px"
        type="selection"
        :selectable="relationSelectable"></bk-table-column>
      <bk-table-column :label="t('关联类型')">
        <template #default="{ row: relation }">
          <slot name="relation-desc" v-bind="{ relation }">
            {{ getRelationName(relation.bk_asst_id)
            }}<span class="model-relation-id"
              >（{{ relation.bk_asst_id }}）</span
            >
          </slot>
        </template>
      </bk-table-column>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const findModelById = modelId =>
      store.getters['objectModelClassify/getModelById'](modelId)

    const handleSelect = selectedRelations => {
      const unselectedRelations = relations.value.filter(
        relation =>
          !selectedRelations.some(
            selectedRelation =>
              relation.bk_obj_asst_id === selectedRelation.bk_obj_asst_id
          )
      )

      if (unselectedRelations.length > 0) {
        toRef(excludedRelations, modelRef.value.bk_obj_id).value =
          unselectedRelations
      } else {
    ...
    const getRelationName = relationId =>
      allAssociations.value.find(relation => relation.bk_asst_id === relationId)
        ?.bk_asst_name || relationId
    ...
    const genRelationSelection = () => {
      const modelExcludedRelations =
        excludedRelations?.[modelRef.value.bk_obj_id]
      if (relations.value.length > 0) {
        nextTick(() => {
          relations.value.forEach(relation => {
            const isSelected = !modelExcludedRelations?.some(
              excludedRelation =>
                excludedRelation.bk_obj_asst_id === relation.bk_obj_asst_id
            )
