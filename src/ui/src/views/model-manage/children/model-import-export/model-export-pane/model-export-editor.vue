      ...
      <model-group-list
        ...
        <template #model-append="{ modelGroup, model, modelIndex, groupIndex }">
          <bk-button
            ...
            icon="delete"
            @click.stop="
              removeModel(modelGroup, model, modelIndex, groupIndex)
            "></bk-button>
        </template>
      </model-group-list>
    </template>
      ...
      <model-relation
        ...
        <template #relation-target="{ relation }">
          <div
            ...
            <model-summary :data="relation.targetModel"> </model-summary>
...
export default defineComponent({
  ...
  setup({ data }, { emit }) {
    ...
    const joinToExportData = model => {
      ...
      modelGroup = modelGroups.value.find(
        group => group.bk_classification_id === model.bk_classification_id
      )

      if (
        modelGroup &&
        !modelGroup.bk_objects.some(item => item.bk_obj_id === model.bk_obj_id)
      ) {
        modelGroup.bk_objects.push(model)
      }

      if (!modelGroup) {
        modelGroup = cloneDeep(
          allModelGroups.value.find(
            group => group.bk_classification_id === model.bk_classification_id
          )
        )
     ...
     * @param {String} modelId 模型的 bk_obj_id
     */
    const isModelInExportData = modelId =>
    ...
    const removeModel = (modelGroup, model, modelIndex, groupIndex) => {
      ...
      let nextModel = null
      const modelsLen = modelGroups.value.reduce(
        (acc, group) => acc + group.bk_objects.length,
        0
      )
