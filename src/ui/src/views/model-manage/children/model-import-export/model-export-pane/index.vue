...
<template>
  <step-pane :steps="exportSteps" @cancel="cancelExport">
    <template #default="{ currentStepIndex }">
      <div v-show="currentStepIndex === 1" class="step-container">
        <user-notice>
      ...
      <div v-show="currentStepIndex === 2" class="step-container">
        <model-export-editor
          v-model="confirmedExportData"
          :data="exportData"></model-export-editor>
      </div>
      <div v-show="currentStepIndex === 3" class="step-container">
        <model-export-setting
          ref="exportSettingRef"
          v-model="exportSetting"></model-export-setting>
      </div>
      <div v-show="currentStepIndex === 4" class="step-container">
        <model-export-result
          @retry-download="handleRetryDownload"></model-export-result>
        <bk-button
          class="done-export-button"
          theme="primary"
          @click="doneExport"
          >{{ t('完成') }}</bk-button
        >
...
export default defineComponent({
  ...
  setup({ data }, ctx) {
    ...
    const downloadExportData = () => {
      ...
      params.object_id = confirmedExportData.value.modelGroups.reduce(
        (prev, next) => prev.concat(next.bk_objects.map(model => model.id)),
        []
      )
      params.excluded_asst_id = Object.values(
        confirmedExportData.value.excludedRelations
    ...
    const exportSteps = [
      ...
      { title: t('导出内容确认'), icon: 2 },
      {
        title: t('导出设置'),
        icon: 3,
        nextHandler: autoDownloadExportData,
        nextButtonText: t('确认导出'),
      },
