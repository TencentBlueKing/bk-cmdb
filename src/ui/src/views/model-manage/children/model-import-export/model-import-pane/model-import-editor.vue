...
<template>
  <content-confirm-layout>
    <template #aside>
      ...
      <model-group-list
        ...
        <template #group-header-append="{ modelGroup }">
          <template v-if="!isGroupExistedInTargetEnv(modelGroup)">
            ...
            <dropdown-select
              v-model="
                importGroupCreationTypes[modelGroup.bk_classification_id]
              "
      ...
      <div
        v-show="activeViewTabName === RELATION_TAB_NAME"
        class="relation-type-list">
        <template v-if="importRelationTypes.length > 0">
          <div
            v-for="relation in importRelationTypes"
            :key="relation.bk_asst_id"
            class="relation-type-item"
            :class="{
              'is-active':
                relation.bk_asst_id === currentRelationType.bk_asst_id,
            }"
            @click="setCurrentRelationType(relation)">
            {{ relation.bk_asst_name }}
            <span class="relation-type-id">（{{ relation.bk_asst_id }}）</span>
            <bk-tag v-show="isRelationTypeConflict(relation)" theme="warning">{{
              t('冲突')
            }}</bk-tag>
          </div>
        </template>
    ...
    <template #main>
      <div
        v-show="activeViewTabName === RELATION_TAB_NAME"
        class="relation-type-diff">
        <template v-if="importRelationTypes.length > 0">
          <div class="relation-type-diff-header">
            {{ currentRelationType.bk_asst_name }}（{{
              currentRelationType.bk_asst_name
            }}）
            <bk-tag v-if="currentRelationType.isConflict" theme="warning">{{
              t('冲突')
            }}</bk-tag>
          </div>
          <bk-form
            v-show="currentRelationType.isConflict"
            class="conflict-resolve-form">
            <bk-form-item
              :label-width="140"
              required
              :label="t('关联类型冲突处理')">
              <bk-select
                v-model="excludedRelationTypes[currentRelationType.bk_asst_id]"
                :clearable="false"
                class="resolve-method-select">
          ...
          <bk-table
            :data="currentRelationType.relationDiffs"
            class="relation-type-relation-models">
              ...
              <template #default="{ row }">
                <span
                  :class="{
                    'is-diff':
                      row.isNew &&
                      currentRelationType.conflictProperties.includes(
                        'bk_asst_name'
                      ),
                  }"
            ...
            <bk-table-column
              :label="t('唯一标志')"
              property="bk_asst_id"></bk-table-column>
          ...
          <div class="relation-model">
            ...
            <bk-table
              :data="currentRelationType.relationModels"
              :show-header="false">
              <bk-table-column>
                <template #default="{ row }">
                  <model-summary :data="row"> </model-summary>
                </template>
              </bk-table-column>
            </bk-table>
          </div>
      ...
      <model-relation
        ...
        <template #relation-target="{ relation }">
          <div
            ...
            :class="{
              'is-excluded-model':
                !isModelExistedInImportData(relation.bk_asst_obj_id) &&
                !isModelExistedInTargetEnv({
                  bk_obj_id: relation.bk_asst_obj_id,
                  bk_obj_name: relation.bk_asst_obj_name,
                }),
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const allModelGroups = computed(
      () => store.getters['objectModelClassify/classifications']
    ) // 所有的模型分组
    ...
    const genModelGroups = importModels => {
      ...
      importModels.forEach(model => {
        let newGroup = modelGroups.value.find(
          group => group.bk_classification_id === model.bk_classification_id
        )
        ...
        if (newGroup) {
          ...
          if (!isGroupExistedInTargetEnv(newGroup)) {
            toRef(
              ...
              newGroup.bk_classification_id
            ).value = JOIN_UNCATEGORIZED_GROUP
          }

          modelGroups.value.push(newGroup)
        }

        toRef(includedModels, model.bk_obj_id).value =
          !isModelExistedInTargetEnv(model)
      })
    }
    ...
    const setCurrentRelationType = relationType => {
      ...
      const oldRelation =
        allAssociations.value?.find(
          r => r.bk_asst_id === relationType.bk_asst_id
        ) || {}
      const conflictProperties = Object.keys(oldRelation).filter(
        key => oldRelation[key] !== relationType[key]
      )
      ...
      currentRelationType.relationModels =
        importData.value.import_object.filter(model =>
          model.object_asst?.some(
            asst => asst.bk_asst_id === relationType.bk_asst_id
          )
        )
    }
     ...
     * @return {Boolean} 是否存在
     */
    const isGroupExistedInTargetEnv = importGroup =>
      allModelGroups.value.some(
        group =>
          importGroup.bk_classification_id === group.bk_classification_id ||
     ...
     * @return {Boolean} 是否冲突
     */
    const isModelExistedInTargetEnv = importModel =>
      allModelGroups.value.some(group =>
        group.bk_objects.some(
          model =>
            model.bk_obj_id === importModel.bk_obj_id ||
     ...
     * @return {Boolean} 是否存在
     */
    const isModelExistedInImportData = modelId =>
      modelGroups.value.some(modelGroup =>
        modelGroup.bk_objects.some(model => model.bk_obj_id === modelId)
      )
    ...
    const isRelationTypeConflict = importRelationType => {
      const isConflict = allAssociations.value.some(relationType => {
        const isIdRepeat =
          relationType.bk_asst_id === importRelationType.bk_asst_id
        return isIdRepeat
      })
    ...
    const emitValue = () => {
      ...
      const confirmedModels = importData.import_object?.filter(model => {
        ...
        if (model.object_asst && excluded) {
          model.object_asst = model.object_asst.filter(
            asst =>
              !excluded.some(
                excludedAsst =>
                  excludedAsst.bk_obj_asst_id === asst.bk_obj_asst_id
              )
          )
        }
      ...
      confirmedModels?.forEach(model => {
        if (
          importGroupCreationTypes[model.bk_classification_id] ===
          JOIN_UNCATEGORIZED_GROUP
        ) {
          model.bk_classification_id = UNCATEGORIZED_GROUP_ID
        } else {
          model.bk_classification_id = importData.import_object.find(
            importModel => importModel.bk_obj_id === model.bk_obj_id
          )?.bk_classification_id
        }
      })

      // 过滤掉冲突中选择跳过的关联关系类型
      const confirmedRelationTypes = importData.import_asst?.filter(
        asst =>
          excludedRelationTypes?.[asst.bk_asst_id] !== SKIP &&
          importRelationTypes.value.some(
            relationType => relationType.bk_asst_id === asst.bk_asst_id
          )
      )
    ...
    const genImportRelationTypes = () => {
      ...
      importData.value.import_object.forEach(model => {
        if (includedModels[model.bk_obj_id] && model.object_asst) {
          model.object_asst.forEach(asst => {
            if (
              !excludedModelRelations.value?.[model.bk_obj_id]?.some(
                excludeItem => excludeItem.bk_asst_id === asst.bk_asst_id
              ) &&
              !effectedRelations.some(
                relation => relation.bk_asst_id === asst.bk_asst_id
              )
            ) {
      ...
      importRelationTypes.value =
        cloneDeep(importData.value.import_asst)?.filter(asst =>
          effectedRelations.some(
            relation => relation.bk_asst_id === asst.bk_asst_id
          )
        ) || []

      if (importRelationTypes.value[0])
        setCurrentRelationType(importRelationTypes.value[0])
    }
