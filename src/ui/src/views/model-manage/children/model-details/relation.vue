    ...
    <bk-table
      ...
      <bk-table-column
        ...
        <template slot-scope="{ row }">
          <span v-if="row.ispre" :class="['relation-pre', $i18n.locale]">
            {{ $t('内置') }}
          </span>
          <span v-bk-overflow-tips class="relation-id">{{
            row['bk_obj_asst_id']
          }}</span>
        </template>
      </bk-table-column>
      ...
      <bk-table-column v-if="updateAuth" prop="operation" :label="$t('操作')">
        <template slot-scope="{ row }">
          <button
...
export default {
  ...
  methods: {
    ...
    async initRelationList() {
      ...
      this.relationList = data.info.map(
        ({ bk_asst_id: asstId, bk_asst_name: asstName }) => {
          if (asstName.length) {
            return {
              id: asstId,
      ...
      this.$bkInfo({
        ...
        confirmFn: async () => {
          await this.deleteObjectAssociation({
            ...
            this.$http.cancel(
              `post_searchObjectAssociation_${this.activeModel.bk_obj_id}`
            )
          })
          this.searchRelationList()
        },
    ...
    async searchRelationList() {
      ...
      this.table.list = [
        ...source,
        ...dest.filter(des => !source.some(src => src.id === des.id)),
      ]
    },
    searchAsSource() {
    ...
    handleSliderBeforeClose() {
      const hasChanged = Object.keys(
        this.$refs.relationForm.changedValues
      ).length
      if (hasChanged) {
        const confirmFn = () => {
          this.slider.isShow = false
