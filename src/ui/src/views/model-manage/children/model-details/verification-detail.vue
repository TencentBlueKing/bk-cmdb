    ...
    <bk-form form-type="vertical">
      <bk-form-item
        ...
        <bk-radio-group v-model="ruleType">
          <bk-radio class="mr30" value="single" :disabled="readonly">{{
            $t('单独唯一')
          }}</bk-radio>
          <bk-radio value="union" :disabled="readonly">{{
            $t('联合唯一')
          }}</bk-radio>
        </bk-radio-group>
      </bk-form-item>
      <bk-form-item :label="$t('校验规则')" required>
        ...
        <p v-if="validateResult" class="rules-error">
            ...
            <i18n path="唯一校验子集提示">
              <template #name>
                <span
                  ...
                  <template v-if="index !== validateResult.rules.length - 1"
                    >、</template
                  >
                </span>
              </template>
      ...
      <bk-form-item v-if="!readonly" class="mt20">
        <bk-button
          theme="primary"
          class="mr10"
...
export default {
  ...
  computed: {
    singleRuleProperties() {
      return this.attributeList.filter(property =>
        this.singleRuleTypes.includes(property.bk_property_type)
      )
    },
    unionRuleProperties() {
      return this.attributeList.filter(property =>
        this.unionRuleTypes.includes(property.bk_property_type)
      )
    },
    displayProperties() {
    ...
    existRules() {
      return this.ruleList
        ...
        .map(item => ({
          id: item.id,
          rules: item.keys.map(key => key.key_id),
        }))
    },
    selectedName() {
  ...
  methods: {
    ...
    setupInitialRules() {
      ...
      setTimeout(() => {
        // 等待watch中调用resetRules后再赋值, resetRules中已使用nextTick，此处使用setTimeout进一步延迟执行
    ...
    validateSubset() {
      ...
      const superset = this.existRules.filter(existRule =>
        rules.every(id => existRule.rules.includes(id))
      )
    ...
    validateSuperset() {
      const rules = this.submitRules
      const subset = this.existRules.filter(existRule =>
        existRule.rules.every(id => rules.includes(id))
      )
    ...
    async createRule() {
      try {
        await this.$store.dispatch(
          'objectUnique/createObjectUniqueConstraints',
          {
            objId: this.$route.params.modelId,
            params: {
              must_check: false, // 该参数对应UI已从界面移除
              keys: this.submitRules.map(id => ({
                key_kind: 'property',
                key_id: id,
              })),
            },
    ...
    async updateRule() {
      try {
        ...
        await this.$store.dispatch(
          'objectUnique/updateObjectUniqueConstraints',
          {
            objId: this.$route.params.modelId,
            id: this.id,
            params: {
    ...
    handleErrorRuleClick(rule) {
      ...
      this.$success(
        this.$t('已切换至校验规则', { name: this.getValidateRuleName(rule) })
      )
    },
    /**
     * 外部调用
