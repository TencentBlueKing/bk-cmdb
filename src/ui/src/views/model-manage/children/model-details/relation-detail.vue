  ...
  <div class="model-slider-content">
    ...
    <div class="form-label exchange-icon-wrapper">
      ...
      <div
        ...
        :class="{ 'is-error': errors.has('asstObjId') }">
        <bk-select
      ...
      <i class="bk-icon icon-info-circle"></i>
      <span
        v-if="!(relationInfo.ispre || isReadOnly || isEdit)"
        class="exchange-icon"
        @click="exchangeObjAsst">
    ...
    <div class="form-label">
      ...
      <div
        class="cmdb-form-item"
        :class="{ 'is-error': errors.has('mapping') }">
        <cmdb-selector
    ...
    <label class="form-label">
      ...
      <div
        class="cmdb-form-item"
        :class="{ 'is-error': errors.has('asstName') }">
        <bk-input
    ...
    <div
      v-show="sourceModel.id && targetModel.id"
      class="form-label topo-preview">
      <span class="label-text">
        {{ $t('效果示意') }}
      </span>
      <div class="topo-image">
        <div class="model-item" :class="{ ispre: sourceModel.ispre }">
          ...
          <span v-bk-overflow-tips class="model-name">{{
            sourceModel['bk_obj_name']
          }}</span>
        </div>
        ...
        <div class="model-item" :class="{ ispre: targetModel.ispre }">
          ...
          <span v-bk-overflow-tips class="model-name">{{
            targetModel['bk_obj_name']
          }}</span>
        </div>
      </div>
      <div class="topo-text">
        {{ sourceModel['bk_obj_name'] }} {{ relationName }}
        {{ targetModel['bk_obj_name'] }}
      </div>
    </div>
    <div v-if="!isReadOnly" class="btn-group">
      <bk-button
        ...
        :disabled="isReadOnly"
        :loading="
          $loading(['createObjectAssociation', 'updateObjectAssociation'])
        "
        @click="saveRelation">
        {{ isEdit ? $t('保存') : $t('提交') }}
      </bk-button>
...
export default {
  ...
  computed: {
    ...
    objAsstId() {
      ...
      if (
        relationInfo.bk_obj_id.length &&
        relationInfo.bk_asst_id.length &&
        relationInfo.bk_asst_obj_id.length
      ) {
    ...
    changedValues() {
      ...
      Object.keys(this.relationInfo).forEach(propertyId => {
        if (
          JSON.stringify(this.relationInfo[propertyId]) !==
          JSON.stringify(this.originRelationInfo[propertyId])
        ) {
    ...
    relationName() {
      ...
      return (this.relationList.find(relation => relation.id === asstId) || {})
        .name
    },
  },
  watch: {
  ...
  methods: {
    ...
    exchangeObjAsst() {
      const { relationInfo } = this
      ;[relationInfo.bk_obj_id, relationInfo.bk_asst_obj_id] = [
        relationInfo.bk_asst_obj_id,
        relationInfo.bk_obj_id,
      ]
    },
  },
}
