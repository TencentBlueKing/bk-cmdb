...
<template>
  <cmdb-sticky-layout
    ...
    <template #header="{ sticky }">
      <cmdb-tips :class="['layout-header', { 'is-sticky': sticky }]">{{
        $t('同步模板功能提示')
      }}</cmdb-tips>
    </template>

    <div class="layout-top">
      <p v-if="isSingleModule" class="title">
        {{ $t('请确认单个实例更改信息') }}
      </p>
    ...
    <div class="layout-main">
      <div v-if="isSingleModule" class="module-instance-container">
        <module-instance
      ...
      <div v-else class="module-instance-group">
        <cmdb-collapse
          v-for="moduleId in moduleIds"
          :key="moduleId"
          class="module-instance-container"
...
export default {
  ...
  data() {
    const moduleIds = String(this.$route.params.modules)
      .split(',')
      .map(id => Number(id))
  ...
  async created() {
    ...
    const templateDetail = await serviceTemplateService.findById(
      this.templateId,
    ...
    this.$store.commit(
      'setTitle',
      `${this.$t(title)}【${templateDetail?.name}】`
    )
  ...
  methods: {
    ...
    loadProperties() {
      return this.$store
        .dispatch('objectModelProperty/batchSearchObjectAttribute', {
          params: {
            bk_biz_id: this.bizId,
    ...
    loadTopoPath() {
      return this.$store
        .dispatch('objectMainLineModule/getTopoPath', {
          bizId: this.bizId,
          params: {
            topo_nodes: this.moduleIds.map(moduleId => ({
              bk_obj_id: 'module',
              bk_inst_id: moduleId,
            })),
          },
        ...
        .then(({ nodes }) => {
          nodes.forEach(node => {
            ...
            this.moduleGroup[moduleId].topoPath = node.topo_path
              .reverse()
    ...
    loadDiffByModule(moduleId) {
      ...
      return this.$store
        .dispatch('businessSynchronous/getTplDiffs', {
          params: {
        ...
        .then(difference => {
          ...
          if (difference.attributes) {
            currentModule.propertyDiff = difference.attributes.map(attr => {
              const property = this.modelProperty.module.find(
     ...
     */
    genProcessDiffItem({ diffType, processId, processName }) {
      return {
        type: diffType,
    ...
    confirmAndSync() {
      ...
      this.$store
        .dispatch('businessSynchronous/syncServiceInstanceByTemplate', {
          params: {
            service_template_id: this.templateId,
    ...
    handleModuleCollapseChange(moduleId, collapse) {
      ...
      if (
        !collapse &&
        !this.moduleGroup[moduleId].loaded &&
        !this.moduleGroup[moduleId].loading
      ) {
