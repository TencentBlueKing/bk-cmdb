...
<template>
  <div class="process-difference cleafix">
    ...
    <ul class="process-list fl">
      <li
        ...
        :class="{
          ...
          'is-active':
            currentDiff.process_template_id === process.process_template_id &&
        ...
        <span class="process-name" :title="process.process_template_name">{{
          process.process_template_name
        }}</span>
      </li>
    </ul>
    ...
    <div
      ...
      <cmdb-collapse class="details-info" arrow-type="filled">
        <div slot="title" class="collapse-title">
          {{ $t('变更内容') }}
          <span v-if="currentDiff.type === 'changed'"
            >({{ currentDiff.changedProperties.length }})</span
          >
        </div>
        <div class="info-content">
          <!-- 进程新增 -->
          <div v-if="currentDiff.type === 'added'" class="process-info">
            <div class="info-item" style="width: auto">
              {{ $t('模板中新增进程') }}
              <span class="info-item-value">{{
                currentDiff.process_template_name
              }}</span>
            </div>
          </div>

          <!-- 进程删除 -->
          <div v-if="currentDiff.type === 'removed'" class="process-info">
            <div class="info-item" style="width: auto">
              <span class="info-item-value" style="font-weight: 700">{{
                currentDiff.process_template_name
              }}</span>
              {{ $t('从模板中删除') }}
            </div>
          ...
          <div
            ...
            <div
              ...
              :class="[
                'info-item fl',
                { table: changed.property.bk_property_type === 'table' },
              ]">
              {{ changed.property.bk_property_name }}：
              <span class="info-item-value">
                <span
                  v-if="
                    changed.property.bk_property_id === 'bind_info' &&
                    !changed.template_property_value.length
                  ">
                  {{ $t('移除所有进程监听信息') }}
                </span>
                <cmdb-property-value
      ...
      <cmdb-collapse
        ...
        <ul
          ...
          class="instance-list">
          <li
            v-for="(instance, instanceIndex) in currentDiff.module
              .serviceInstances"
            :key="instanceIndex"
            class="instance-item"
            @click="viewInstanceDiff(instance, moduleId)">
            <div class="instance-diff">
              <div v-bk-overflow-tips class="instance-name">
                {{ instance.name }}
              </div>
              <label
    ...
    <bk-sideslider
      ...
      <template v-if="instanceDiffSlider.show" slot="content">
...
export default {
  ...
  mounted() {
    setTimeout(() => {
      this.$refs.changeDetails.addEventListener(
        'scroll',
        this.throttleHideTooltips
      )
    }, 1000)
  },
  beforeDestroy() {
    this.$refs.changeDetails.removeEventListener(
      'scroll',
      this.throttleHideTooltips
    )
  },
  methods: {
    ...
    loadChangedProcess(process) {
      ...
      this.$store
        .dispatch('processTemplate/getProcessTemplate', {
          params: {
        ...
        .then(res => {
          ...
          this.currentDiff.process_template_id = process.process_template_id
          this.currentDiff.process_template_name = process.process_template_name
          this.currentDiff.changedProperties = this.getChangedProperties(
            res.property
          )
          process.confirmed = true
        })
    ...
    getChangedProperties(property) {
      ...
      if (property) {
        Object.keys(property).forEach(key => {
          ...
          const formatedProp = this.properties.find(
            i => i.bk_property_id === key
          )
    ...
    loadInstances(moduleId) {
      ...
      this.$store
        .dispatch('businessSynchronous/getDiffInstances', {
          params: {
    ...
    hideTooltips() {
      const instanceNameDoms =
        this.$refs?.instanceList?.getElementsByClassName('instance-name')
