...
<template>
  <div v-bkloading="{ isLoading: $loading() }" class="instance-details-wrapper">
    <bk-table :data="diffDetails" :max-height="$APP.height - 120">
      <bk-table-column
        prop="property_name"
        :label="$t('属性名称')"
      ...
      <bk-table-column prop="property_value" :label="$t('变更前')">
        <template slot-scope="{ row }">
          <process-bind-info-value
            ...
            :property="
              properties.find(
                property => property.bk_property_id === row.property_id
              )
            ">
          </process-bind-info-value>
...
export default {
  ...
  methods: {
    ...
    loadInstanceDiff() {
      this.$store
        .dispatch('businessSynchronous/getInstanceDiff', {
          params: this.diffRequestParams,
        })
        .then(
          async ({
    ...
    loadAddedData() {
      const { process_template_id: processTemplateId, service_template_id } =
        this.diffRequestParams
      return this.$store
        .dispatch('processTemplate/getBatchProcessTemplate', {
          params: {
