  ...
  <section
    ...
    <bk-big-tree
      ...
      <div
        ...
        <!-- 业务集拓扑图标 -->
        <i
          v-if="data.bk_obj_id === BUILTIN_MODELS.BUSINESS_SET"
          class="node-icon fl">
...
export default {
  ...
  methods: {
    ...
    async initTopology() {
      ...
      const currentBizSetTopo = await this.loadTopologyData(
        this.bizSetId,
      ...
      if (this.bizId) {
        currentBizNode = currentBizSetTopo.find(
          item => item.bk_inst_id === this.bizId
        )
        if (!currentBizNode) {
      ...
      if (topoPath) {
        ...
        for (let pathIndex = 0; pathIndex < topoPath.length; pathIndex++) {
          ...
          const [err, data] = await to(
            this.loadTopologyData(this.bizSetId, modelId, instanceId)
          )
          ...
          if (!topoTreeData) {
            ...
            const foundNode = parentData.find(
              parentItem => parentItem.bk_inst_id === instanceId
            )
      ...
      const expanedNodes = currentBizNode
        ? expandedIds.map(id => this.$refs.tree.getNodeById(id))
        : []
    ...
    setDefaultExpandedNode(node) {
      if (node) {
        ...
        const childrenHasLeaf = childrenNodes.some(child =>
          this.isLeaf(child.data.bk_obj_id)
        )
        const siblingHasLeaf = siblingNodes.some(child =>
          this.isLeaf(child.data.bk_obj_id)
        )
        ...
        if (childrenHasLeaf) {
          childrenNodes.forEach(childNode => {
            if (
              this.isLeaf(childNode.data.bk_obj_id) &&
        ...
        if (siblingHasLeaf) {
          siblingNodes.forEach(siblingNode => {
            if (
              this.isLeaf(siblingNode.data.bk_obj_id) &&
              !siblingNode.state.expanded
            ) {
    ...
    loadTopologyData(bizSetId, parentModelId, parentInstanceId) {
      return TopologyService.findChildren({
        ...
        if (parentModelId === BUILTIN_MODELS.BUSINESS) {
          data.sort(a =>
            a.bk_obj_id === BUILTIN_MODELS.SET && a.default === 1 ? -1 : 0
          )
        }
    ...
    getModelPrefix(modelId) {
      const modelName = this.topologyModels?.find(
        model => model.bk_obj_id === modelId
      )?.bk_obj_name
    ...
    getBizSetRootNode() {
      return this.$refs?.tree.getNodeById(
        `${BUILTIN_MODELS.BUSINESS_SET}-${this.bizSetId}`
      )
    },
    /**
     * 处理节点选择事件
    ...
    async loadNodeCount(targetNodes, force = false) {
      ...
      if (force) {
        nodes = targetNodes.filter(
          ({ data }) =>
            !doneStatuses.includes(data.status) &&
            data.bk_obj_id !== BUILTIN_MODELS.BUSINESS_SET
        )
      } else {
        nodes = targetNodes.filter(
          ({ data }) => data.bk_obj_id !== BUILTIN_MODELS.BUSINESS_SET
        )
      }
      ...
      const requests = await CombineRequest.setup(
        Symbol(),
        condition =>
          TopologyService.getInstanceCount(this.bizSetId, condition, {
            globalError: false,
          }),
