...
export default {
  ...
  methods: {
    disabledTableSettingDefaultBehavior() {
      setTimeout(() => {
        const settingReference = this.$refs.table.$el.querySelector(
    ...
    handleValueClick(row, column) {
      if (
        column.bk_obj_id !== BUILTIN_MODELS.HOST ||
    ...
    handleHeaderClick(column) {
      ...
      ColumnsConfig.open({
        props: {
          properties: FilterStore.properties.filter(
            property =>
              property.bk_obj_id === BUILTIN_MODELS.HOST ||
              (property.bk_obj_id === BUILTIN_MODELS.MODULE &&
                property.bk_property_id === 'bk_module_name') ||
              (property.bk_obj_id === BUILTIN_MODELS.SET &&
                property.bk_property_id === 'bk_set_name')
          ),
    ...
    handleApplyColumnsConfig(properties = []) {
      return this.$store.dispatch('userCustom/saveUsercustom', {
        [this.$route.meta.customInstanceColumn]: properties.map(
          property => property.bk_property_id
        ),
      })
    },
    ...
    getParams() {
      ...
      const modelConditionId = has(fieldMap, instance.bk_obj_id)
        ? instance.bk_obj_id
        : 'object'
