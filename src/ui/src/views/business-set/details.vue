...
export default defineComponent({
  ...
  setup() {
    ...
    const bizSetId = computed(() =>
      parseInt(router.app.$route.params[MODEL_ROUTEPARAMS_KEY], 10)
    )
    ...
    watchEffect(async () => {
      const [modelInst, modelProperties, modelPropertyGroups] =
        await Promise.all([
          propertySetService.findById(bizSetId.value, {
            requestId,
            cancelPrevious: true,
          }),
      ...
      store.commit(
        'setTitle',
        `${model.value.bk_obj_name}【${modelInst[MODEL_NAME_KEY]}】`
      )
    })
