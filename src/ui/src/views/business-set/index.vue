...
<template>
  <div class="business-set-layout">
      ...
      <bk-table-column :label="$t('操作')" fixed="right">
        <template slot-scope="{ row }">
          <cmdb-auth
            class="mr10"
            :auth="{
              type: $OPERATION.R_BIZ_SET_RESOURCE,
              relation: [row.bk_biz_set_id],
            }">
    ...
    <business-scope-preview
      v-bind="previewProps"
      :show.sync="previewProps.show" />
  </div>
</template>
  ...
  setup() {
    ...
    const searchParams = computed(() => {
      ...
      const { conditions, time_condition: timeCondition } =
        Utils.transformGeneralModelCondition(condition, properties.value)
    ...
    const getList = async (options = {}) => {
      try {
        const { list, count } = await businessSetService.find(
          searchParams.value,
          {
            requestId,
            cancelPrevious: true,
    ...
    const filterProperty = computed(() =>
      properties.value.find(
        property => property.bk_property_id === filter.field
      )
    )
    ...
    const filterPlaceholder = computed(() =>
      Utils.getPlaceholder(filterProperty.value)
    )
    const filterComponentProps = computed(() =>
      Utils.getBindProps(filterProperty.value)
    )

    const fastSearchProperties = computed(() =>
    ...
    const updateFilter = (field, value = '', operator = '') => {
      ...
      const options =
        filterType.value === 'singlechar'
          ...
          : {}
      const defaultData = {
        ...Utils.getDefaultData(filterProperty.value),
        ...options,
      }

      filter.operator = operator || defaultData.operator
      filter.value = formatFilterValue(
    ...
    const getColumnSortable = property =>
      isPropertySortable(property) ? 'custom' : false
