    ...
    <bk-table class="table" :data="list">
      ...
      <bk-table-column
        ...
        <template slot-scope="{ row }">{{
          getHostValue(row, 'bk_host_innerip') | singlechar
        }}</template>
      </bk-table-column>
      <bk-table-column
        :label="$t('管控区域')"
        ...
        show-overflow-tooltip>
        <template slot-scope="{ row }">{{
          getHostValue(row, 'bk_cloud_id') | foreignkey
        }}</template>
      </bk-table-column>
    </bk-table>
  </section>
  ...
  computed: {
    list() {
      return this.info.map(id => {
        const target = this.$parent.hostInfo.find(
          target => target.host.bk_host_id === id
        )
        return target || {}
      })
    },
