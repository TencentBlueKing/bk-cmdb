  ...
  methods: {
    sortInfo() {
      ...
      if (this.sort === 'module') {
        ...
        instances = [...this.info].sort(
          (A, B) =>
            order.indexOf(A.bk_module_id) - order.indexOf(B.bk_module_id)
        )
      } else {
    ...
    getName(instance) {
      ...
      const data = this.$parent.hostInfo.find(
    ...
    getBindIp(instance, info) {
      ...
      const { host } = this.$parent.hostInfo.find(
        data => data.host.bk_host_id === instance.bk_host_id
      )
      // 第一内网IP
      if (ipValue === '3') {
    ...
    getServiceInstanceOptions() {
      ...
      this.instances.forEach((instance, index) => {
        const component = this.$refs.serviceInstance.find(
    ...
    getAddedProcessList(instance, component) {
      ...
      component.processList.forEach((process, index) => {
        ...
        if (instance.service_template) {
          item.process_template_id = component.templates[index]
            ? component.templates[index].id
            : 0
        }
        processes.push(item)
      })
