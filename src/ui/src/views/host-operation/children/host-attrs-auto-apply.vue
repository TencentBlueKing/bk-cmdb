  ...
  <div class="apply-layout">
    ...
    <div class="options-row">
      <div class="option-label">
        <span
          v-bk-tooltips="$t('属性值与目标模块配置不一致的主机')"
          class="has-tips"
          >{{ $t('是否更新主机属性') }}</span
        >
      </div>
    ...
    <div
      v-if="hasMultipleConflictModule && updateOption.changed"
      class="options-row conflict">
      <div class="option-label">
        <span
          v-bk-tooltips="
            $t('目标模块配置了不同的自动应用属性，需要重新指定配置')
          "
          class="has-tips"
          >{{ $t('冲突字段配置') }}</span
        >
      ...
      <div class="option-content conflict-property-list">
        <template v-for="item in conflictPropertyList">
          <div
            v-if="item.host_apply_rules.length > 1"
            :key="item.bk_attribute_id"
            class="conflict-property-item">
            <div class="property-name" :title="item.propertyName">
              {{ item.propertyName }}
            </div>
            <bk-select
              v-model="item.selectedRuleId"
              class="property-value-selector"
              ...
              <bk-option
                v-for="rule in item.host_apply_rules"
                :id="rule.id"
                :key="rule.id"
                :name="getModulePath(rule.bk_module_id)"
                @mouseenter.native="
                  event =>
                    handleModuleRuleHover(event, rule, item.bk_attribute_id)
                "
                @mouseleave.native="
                  event =>
                    handleModuleRuleHover(event, rule, item.bk_attribute_id)
    ...
    <div
      ...
      <div
        :class="[
          'rule-list',
          { 'flex-col': moduleRulePopover.ruleList.length < 10 },
        ]">
        <div
          v-for="rule in moduleRulePopover.ruleList"
          :key="rule.id"
          :class="[
            'rule-item',
            {
              current:
                moduleRulePopover.currentPropertyId === rule.bk_attribute_id,
            },
          ]">
          <div class="property-name" :title="rule.property.bk_property_name">
            {{ rule.property.bk_property_name }}
          </div>
          <cmdb-property-value
...
export default {
  ...
  computed: {
    ...
    conflictPropertyList() {
      ...
      this.list.forEach(row => {
        row.conflicts.forEach(conflict => {
          if (
            !conflictPropertyList.some(
              item => item.bk_property_id === conflict.bk_property_id
            )
          ) {
            const property = this.hostPropertyList.find(
              item => item.bk_property_id === conflict.bk_property_id
            )
    ...
    hasMultipleConflictModule() {
      ...
      const hasMoreThanOne = this.conflictPropertyList.some(
        conflict => conflict.host_apply_rules.length > 1
      )
  ...
  methods: {
    initUpdateOption() {
      this.conflictPropertyList.forEach(item => {
        ...
        const currentRule = item.host_apply_rules.find(
          rule => rule.id === item.selectedRuleId
        )
    ...
    handleModuleRuleChange(ruleId) {
      ...
      this.list.forEach(item => {
        ...
        const targetField = item.update_fields.find(
          field => field.bk_attribute_id === rule.bk_attribute_id
        )
      ...
      const targetOption = this.updateOption.final_rules.find(
        item => item.bk_attribute_id === rule.bk_attribute_id
      )
    ...
    handleModuleRuleHover(event, rule, propertyId) {
      ...
      const moduleRuleList = this.ruleList.filter(
        item => item.bk_module_id === moduleId
      )
      ...
      this.moduleRulePopover.ruleList = moduleRuleList.map(rule => {
        const property = this.hostPropertyList.find(
          item => item.id === rule.bk_attribute_id
        )
        // 将完整的属性数据加入
      ...
      if (changed) {
        ...
        this.list.forEach(item => {
          item.update_fields.forEach(field => {
            const rule = this.updateOption.final_rules.find(
              rule => rule.bk_attribute_id === field.bk_attribute_id
            )
        ...
        this.list.forEach(item => {
          item.update_fields.forEach(field => {
            const hostCurrent = item.conflicts.find(
              conflict => conflict.bk_attribute_id === field.bk_attribute_id
            )
