    ...
    <div v-show="!$loading(Object.values(request)) && !loading">
      <div class="info clearfix mb20">
        ...
        <div class="info-content">
          <i18n path="N台主机">
            <template #count
              ><b class="info-count">{{ resources.length }}</b></template
            >
          </i18n>
          <i
            v-if="!changeHostDisabled"
            class="edit-trigger icon icon-cc-edit"
            @click="handleChangeHost"></i>
        </div>
      </div>
      <div v-if="type !== 'remove'" class="info clearfix mb10">
        <label class="info-label fl">{{ $t('转移到') }}：</label>
        <div class="info-content">
          <ul class="module-list">
            <li
              v-for="(id, index) in targetModules"
              :key="index"
              ...
              <span v-if="type === 'business'" class="module-icon">{{
                $i18n.locale === 'en' ? 'M' : '模'
              }}</span>
              {{ getModuleName(id) }}
              <span
      ...
      <div ref="changeInfo" class="info clearfix mb10">
        ...
        <div class="info-content">
          <template v-if="availableTabList.length">
            <ul class="tab clearfix">
              <template v-for="(item, index) in availableTabList">
                <li v-if="index" :key="index" class="tab-grep fl"></li>
                <li
                  :key="item.id"
                  class="tab-item fl"
                  :class="{
                    active: activeTab === item,
                    'hide-count': !item.showCount,
                  }"
                  @click="handleTabClick(item)">
                  <span class="tab-label">{{ item.label }}</span>
                  <span
                    v-if="item.showCount"
                    :class="['tab-count', { unconfirmed: !item.confirmed }]">
                    {{
                      item.props.info.length > 999
                        ? '999+'
          ...
          <div v-else-if="isSameModule" class="tab-empty">
            {{ $t('相同模块转移提示') }}
          </div>
          <div v-else-if="isEmptyChange" class="tab-empty">
            {{ $t('无转移确认信息提示') }}
          </div>
      ...
      <div class="options" :class="{ 'is-sticky': hasScrollbar }">
        <bk-button
          theme="primary"
          :disabled="isSameModule"
          @click="handleConfrim"
          >{{ confirmText }}</bk-button
        >
...
export default {
      ...
      tabList: [
        ...
        },
        {
          ...
          label: this.$t('移动到空闲机的主机', {
            idleModule: this.$store.state.globalConfig.config.idlePool.idle,
          }),
          confirmed: false,
          showCount: true,
  ...
  computed: {
    ...
    availableTabList() {
      ...
      return this.tabList.filter(
        tab => available.includes(tab.id) && tab.props.info.length > 0
      )
    },
    activeTab() {
      return (
        this.tabList.find(tab => tab.id === this.tab.active) ||
        this.availableTabList[0]
      )
    },
    isRemoveModule() {
  ...
  async created() {
    ...
    this.setBreadcrumbs()
    await Promise.all([this.getTopologyModels(), this.getHostInfo()])
    this.getPreviewData()
  },
  mounted() {
  ...
  methods: {
    resolveData(route) {
      ...
      if (!targetModules) {
        ...
        this.targetModules = String(targetModules)
          .split(',')
          .map(id => Number(id))
      }
      ...
      if (!resources) {
        ...
        this.resources = String(resources)
          .split(',')
          .map(id => Number(id))
      }
    ...
    setBreadcrumbs() {
      const titleMap = {
        idle: this.$t('转移到空闲模块', {
          idleSet: this.$store.state.globalConfig.config.set,
        }),
    ...
    async getTopologyModels() {
      try {
        const topologyModels = await this.$store.dispatch(
    ...
    setConfirmState(data) {
      ...
      this.isSameModule = data.every(
        datum =>
          !(
            datum.to_add_to_modules.length ||
            datum.to_remove_from_modules.length
          )
      )
      // 是否溢出的是空服务实例（前端流转不会创建空服务实例，但是ESB会）
      this.isEmptyChange = data.every(datum => {
        const hasAdd = !datum.to_add_to_modules.length
        const hasRemoveInstance = datum.to_remove_from_modules.some(
          module => !module.service_instances.length
        )
    ...
    async setModulePathInfo(data) {
      try {
        ...
        data.forEach(datum => {
          moduleIds.push(
            ...datum.to_add_to_modules.map(datum => datum.bk_module_id)
          )
          moduleIds.push(
            ...datum.to_remove_from_modules.map(datum => datum.bk_module_id)
          )
        })
        const uniqueModules = [...new Set(moduleIds)]
        const result = await this.$store.dispatch(
          'objectMainLineModule/getTopoPath',
          {
            bizId: this.bizId,
            params: {
              topo_nodes: uniqueModules.map(id => ({
                bk_obj_id: 'module',
                bk_inst_id: id,
              })),
            },
    ...
    setHostAttrsAutoApply(data) {
      ...
      const conflictList = conflictInfo.filter(
        item => item.conflicts?.length || item.update_fields?.length
      )
    ...
    getSearchHostParams() {
      ...
      const hostCondition = params.condition.find(
        target => target.bk_obj_id === 'host'
      )
    ...
    getModuleName(id) {
      ...
      const target =
        topoInfo.find(
          target => target.bk_obj_id === 'module' && target.bk_inst_id === id
        ) || {}
    ...
    async setMoveToIdleHost(data) {
      try {
        ...
        data.forEach(item => {
          ...
          const isIdleModule =
            finalModules.length && finalModules[0] === idleModule.bk_module_id
          if (isIdleModule) {
    ...
    handleChangeModule() {
      ...
      const firstSelectionModules = selection[0].module
        .map(module => module.bk_module_id)
        .sort()
    ...
    handleChangeHost() {
      ...
      if (this.type === 'remove') {
        props.displayNodes = [
          `${this.$route.query.sourceModel}-${this.$route.query.sourceId}`,
        ]
      }
      this.dialog.props = props
    ...
    async handleConfrim() {
      try {
        ...
        const createComponent =
          this.$refs.createServiceInstance &&
          this.$refs.createServiceInstance[0]
        const hostAttrsComponent =
          this.$refs.hostAttrsAutoApply && this.$refs.hostAttrsAutoApply[0]
        if (createComponent || hostAttrsComponent) {
          params.options = {}
          if (createComponent) {
            params.options.service_instance_options =
              createComponent.getServiceInstanceOptions()
          }
          if (hostAttrsComponent) {
            params.options.host_apply_trans_rule =
              hostAttrsComponent.getHostApplyConflictResolvers()
          }
        }
        ...
        const successText =
          { remove: '移除成功', add: '添加成功' }[this.type] || '转移成功'
