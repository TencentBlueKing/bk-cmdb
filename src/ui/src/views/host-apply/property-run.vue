...
<template>
  <div class="run-wrapper">
    <top-steps v-if="hasSteps" :current="3" />

    <div class="status-content">
      <div v-if="$loading(Object.values(requestIds))" class="status-loading">
        <div class="status-inner">
          <img class="status-icon" src="@/assets/images/icon/loading.svg" />
          <p v-show="$loading(requestIds.run)" class="status-title">
            {{ $t('正在应用中') }}
          </p>
          <p v-show="$loading(requestIds.task)" class="status-title">
            {{ $t('正在查询应用状态') }}
          </p>
        </div>
      </div>

      <div v-else-if="requestErrors.run" class="status-result">
      ...
      <div v-else-if="taskStatus" class="status-result">
        <div class="result-icon">
          <span
            v-show="taskSuccess"
            class="status-icon bk-icon icon-check-1 icon-success"></span>
          <img
            v-show="taskUndone"
            class="status-icon"
          ...
          <span
            v-show="taskFail"
            class="status-icon bk-icon icon-close icon-fail"></span>
        </div>
        <p class="result-title">
        ...
        <p class="result-subtitle">
          <span v-show="taskSuccess">{{
            $t('成功保存策略并应用到当前模块下主机')
          }}</span>
          <span v-show="taskUndone">{{
            $t('任务正在后台执行，请耐心等待')
          }}</span>
          <span v-show="taskFail">{{
            $t('本次任务运行失败，可点击下方按钮重试')
          }}</span>
        </p>
        <div class="result-options">
          <bk-button
            ...
            >{{ $t('返回列表') }}</bk-button
          >
          <bk-button v-show="taskFail" @click="handleRetry">{{
            $t('重试')
          }}</bk-button>
          <bk-button v-show="taskSuccess" @click="handleViewHost">{{
            $t('查看主机')
          }}</bk-button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async saveAndApply() {
      try {
        const requestConfig =
          this.requestConfigs[this.requestIds.run][this.mode]
        ...
        return Promise.resolve(
          res?.result ? [res.data] : [false, res.bk_error_msg]
        )
      } catch (error) {
        console.error(error)
    ...
    async getTaskStatus(taskId) {
      try {
        const requestConfig =
          this.requestConfigs[this.requestIds.task][this.mode]
        ...
        return Promise.resolve(
          result?.task_info?.find(({ task_id: id }) => id === taskId)
        )
      } catch (error) {
        console.error(error)
        return Promise.resolve(false)
