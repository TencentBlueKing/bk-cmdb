...
<template>
  <div class="apply-edit">
    <component :is="currentView" :mode="mode" :ids="targetIds" :action="action">
    </component>
  </div>
</template>
  ...
  methods: {
    ...
    async initTemplateData() {
      try {
        ...
        const list = await serviceTemplateService.findAllByIds(this.targetIds, {
    ...
    getTopopath() {
      return this.$store.dispatch('hostApply/getTopopath', {
        bizId: this.bizId,
        params: {
          topo_nodes: this.targetIds.map(id => ({
            bk_obj_id: 'module',
            bk_inst_id: id,
          })),
        },
