...
<template>
  <div class="confirm-wrapper">
    ...
    <div class="host-apply-confirm">
      <div class="update-options">
        <div class="option-label">
          <i18n path="同时更新未应用主机">
            <template #invalid>
              <span
                v-bk-tooltips="$t('属性当前值与目标值不一致的主机')"
                class="has-tips"
                >{{ $t('-未应用主机') }}</span
              >
            </template>
        ...
        <bk-radio-group v-model="updateOption.changed" class="option-content">
          <bk-radio :value="true">
            {{ $t('是将把未应用主机更新为当前的配置') }}
          </bk-radio>
      ...
      <div v-show="updateOption.changed" class="caption">
        <div class="title">{{ $t('请确认以下主机应用信息') }}</div>
        <div class="stat">
          <span class="conflict-item">
            <span
              v-bk-tooltips="{
                content: $t('当一台主机同属多个模块时，且模块配置了不同的属性'),
                width: 185,
              }">
            ...
            <i18n path="冲突主机N台">
              <template #num
                ><em class="conflict-num">{{ conflictNum }}</em></template
              >
            </i18n>
          </span>
          <i18n path="主机总数N台">
            <template #num
              ><em class="check-num">{{ totalNum }}</em></template
            >
          </i18n>
        </div>
      </div>
      ...
      <div class="bottom-actionbar">
        <div class="actionbar-inner">
          <bk-button theme="default" @click="handlePrevStep">{{
            $t('上一步')
          }}</bk-button>
          <cmdb-auth
...
export default {
  ...
  methods: {
    ...mapActions('hostApply', ['getApplyPreview', 'getTemplateApplyPreview']),
    async initData() {
      try {
        const requestConfig =
          this.requestConfigs[this.requestIds.applyPreview][this.mode]
    ...
    async handleApply() {
      ...
      const propertyConfig = {
        ...this.propertyConfig,
        ...{ changed: this.updateOption.changed },
      }
      // 更新属性配置，用于后续应用执行
