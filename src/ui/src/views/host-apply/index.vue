...
<template>
  <div class="host-apply">
    <div class="main-wrapper">
      ...
      <div
        v-bkloading="{
          isLoading: $loading([requestIds.rules, requestIds.properties]),
        }"
        class="main-content">
        <config-details
          v-show="!batchAction"
...
export default {
  ...
  methods: {
    async getData() {
      ...
      if (
        this.isModuleMode &&
        this.currentNode.service_template_host_apply_enabled
      ) {
        this.initRuleList = []
        this.hasRule = false
      ...
      try {
        ...
        this.checkedPropertyIdList = this.initRuleList.map(
    ...
    getConflictCount() {
      const requestConfig =
        this.requestConfigs[this.requestIds.conflictCount][this.mode]
    ...
    handleCloseApply() {
      ...
      this.$bkInfo({
        ...
        confirmFn: async () => {
          const requestConfig =
            this.requestConfigs[this.requestIds.setEnableStatus][this.mode]
          try {
            ...
            this.$refs.sidebar.setApplyClosed(this.targetId, {
              isClose: true,
              isClear: this.clearRules,
            })
