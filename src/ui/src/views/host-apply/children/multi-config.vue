...
<template>
  <cmdb-sticky-layout class="config-wrapper">
    <template v-if="!isDel" #header="{ sticky }">
      ...
      <div class="config-bd">
        <div class="config-item">
          ...
          <div class="item-content">
            <div
              ref="targetList"
              :class="['target-list', { 'show-more': showMore.isMoreShowed }]">
              ...
              <div
                v-show="showMore.showLink"
                :class="[
                  'target-item',
                  'more',
                  { opened: showMore.isMoreShowed },
                ]"
                :style="{ left: `${showMore.linkLeft}px` }"
                @click="handleShowMore">
                {{ showMore.isMoreShowed ? $t('收起') : $t('展开更多')
        ...
        <div class="config-item">
          ...
          <div class="item-content">
            <div v-if="!isDel" class="choose-toolbar">
              <cmdb-auth
              ...
              <span class="tips"
                ...
                >{{ $t('批量设置字段的自动应用功能提示') }}</span
              >
            </div>
            <div v-show="checkedPropertyIdList.length" class="config-table">
...
export default {
  ...
  inject: ['getModuleName', 'getModulePath', 'getTemplateName'],
  props: {
    mode: {
      type: String,
      required: true,
  ...
  methods: {
    async initData() {
      try {
        ...
        this.checkedPropertyIdList = this.hasRuleDraft
          ? [...new Set([...this.checkedPropertyIdList])]
          : checkedPropertyIdList
      } catch (e) {
        console.log(e)
      }
    ...
    getAvailablePropertyRuleList(propertyRuleList) {
      // 被忽略的不需要
      const availableList = propertyRuleList.filter(
        property => !property.__extra__.ignore
      )
    ...
    handleNextStep() {
      ...
      const availablePropertyRuleList =
        this.getAvailablePropertyRuleList(propertyRuleList)
    ...
    handleDel() {
      this.$bkInfo({
        ...
        confirmFn: async () => {
          // eslint-disable-next-line max-len
          const ruleIds = this.selectedPropertyRow.reduce(
            (acc, cur) =>
              acc.concat(cur.__extra__.ruleList.map(item => item.id)),
            []
          )
          const requestConfig =
            this.requestConfigs[this.requestIds.del][this.mode]
          try {
