...
<template>
  <bk-search-select
    ...
    <template slot="nextfix">
      <i
        v-show="showClear && searchValue.length"
...
export default {
  ...
  watch: {
    ...
    searchValue(searchValue) {
      this.searchOptions.forEach(option => {
        ...
        const selected = searchValue.some(
          value =>
            value.id === option.id &&
  ...
  methods: {
    async initOptions() {
      const availableProperties = this.configPropertyList.filter(
        property => property.host_apply_enabled
      )
      this.searchOptions = availableProperties.map(property => {
        ...
        if (type === 'enum') {
          // eslint-disable-next-line max-len
          data.children = (property.option || []).map(option => ({
            id: option.id,
            name: option.name,
            disabled: false,
          }))
          ...
          data.children = TIMEZONE.map(timezone => ({
            id: timezone,
            name: timezone,
            disabled: false,
          }))
    ...
    handleChange(values) {
      const keywords = values.filter(
        value => !has(value, 'type') && has(value, 'id')
      )
    ...
    filterChildrenMethod(list, filter) {
      if (
        ...
        this.currentMenu.children.length
      ) {
        return this.currentMenu.children.filter(
          item => item.name.toLowerCase().indexOf(filter.toLowerCase()) > -1
        )
      }
      return []
