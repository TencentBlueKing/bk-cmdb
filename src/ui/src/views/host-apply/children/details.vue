      ...
      <div class="config-panel">
        <div class="config-head">
          <h2 class="config-title">
            <div v-bk-overflow-tips class="config-name">
              {{ modeValues.title }}
            </div>
            <small
              v-if="localHasRule && ruleLastEditTime"
        ...
        <div class="config-body">
          <template v-if="serviceTemplateApplyEnabled">
            <div class="empty">
              <div class="desc">
                ...
                <span>{{
                  $t('当前模块所属的服务模板已配置了自动应用规则')
                }}</span>
              </div>
              <div class="action">
                <bk-button outline theme="primary" @click="handleGoTemplate">
                  {{ $t('跳转查看') }}
                </bk-button>
              </div>
            </div>
            ...
            <div class="view-field">
              ...
              <div class="view-ft">
                ...
                <cmdb-auth
                  ...
                  <template #default="{ disabled }">
                    <bk-button
                      ...
                      <span
                        v-if="!hasConflict"
                        v-bk-tooltips="{ content: $t('无未应用需处理') }">
                        {{ $t('未应用主机')
                        }}<em class="conflict-num">{{ conflictNum }}</em>
                      </span>
                      <span v-else>
                        {{ $t('未应用主机')
                        }}<em class="conflict-num">{{ conflictNum }}</em>
                ...
                <cmdb-auth
                  :auth="{ type: $OPERATION.U_HOST_APPLY, relation: [bizId] }">
                  <template #default="{ disabled }">
                    <bk-button :disabled="disabled" @click="$emit('close')">
                      {{ $t('关闭自动应用') }}
                    </bk-button>
                  </template>
                </cmdb-auth>
          ...
          <template v-else>
            <div v-if="!localHasRule" class="empty">
              <div class="desc">
                <i class="bk-cc-icon icon-cc-tips"></i>
                <span v-if="mode === CONFIG_MODE.MODULE">{{
                  $t('当前模块未启用自动应用策略')
                }}</span>
                <span v-else-if="mode === CONFIG_MODE.TEMPLATE">{{
                  $t('当前模板未启用自动应用策略')
                }}</span>
              </div>
              <div class="action">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const { id, hasRule, ruleList, currentNode, conflictNum } = toRefs(props)
    ...
    const serviceTemplateApplyEnabled = computed(
      () => currentNode.value?.service_template_host_apply_enabled
    )

    const localHasRule = computed(
      () => hasRule.value && ruleList.value?.length > 0
    )
