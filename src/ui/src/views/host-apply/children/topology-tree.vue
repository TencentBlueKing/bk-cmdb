...
<template>
  <div class="topology-tree-wrapper">
    <bk-big-tree
      ...
      v-bkloading="{
        isLoading: $loading([
          requestIds.getTopology,
          requestIds.mainLine,
          requestIds.moduleApplyStatus,
        ]),
      }"
      class="topology-tree"
      v-bind="treeOptions"
      ...
      <div
        ...
        <span v-show="applyEnabled(node)" class="config-icon fr"
          ><i class="bk-cc-icon icon-cc-selected"></i
        ></span>
        <div class="info-content">
          <span class="node-name" :title="data.bk_inst_name">{{
            data.bk_inst_name
          }}</span>
        </div>
      </div>
...
import topologyInstanceService, {
  requestIds as topologyrequestIds,
} from '@/service/topology/instance.js'
...
export default {
  ...
  computed: {
    ...
    mainLineModels() {
      ...
      return this.mainLine.map(data =>
        models.find(model => model.bk_obj_id === data.bk_obj_id)
      )
    },
    modelIconMap() {
      const map = {}
    ...
    Bus.$on('host-apply-closed', (mode, id) => {
      if (mode === CONFIG_MODE.TEMPLATE) {
        this.setModuleApplyStatusByTemplate(
          this.treeStat?.withTemplateModuleIdMap.get(id)
        )
      }
    })
  ...
  methods: {
    async handleSearch(params) {
      try {
        if (params.query_filter.rules.length) {
          const keywordRuleIndex = params.query_filter.rules.findIndex(
            item => item.field === 'keyword'
          )
    ...
    filterMethod({ remote: remoteData, keyword }, node) {
      ...
      const keywordFilter = (keyword, node) =>
        String(node.name).toLowerCase().indexOf(keyword) > -1
      // eslint-disable-next-line max-len
      const remoteFilter = (remoteData, node) =>
        remoteData.some(
          item =>
            item.bk_inst_id === node.data.bk_inst_id &&
            item.bk_obj_id === node.data.bk_obj_id
        )
    ...
    getTreeStat() {
      ...
      const findModule = function (data, parent) {
        ...
        for (const item of data) {
          stat.levels[item.bk_inst_id] = parent
            ? stat.levels[parent.bk_inst_id] + 1
            : 0
          if (item.bk_obj_id === 'module') {
            if (item.service_template_id) {
              if (stat.withTemplateModuleIdMap.has(item.service_template_id)) {
                stat.withTemplateModuleIdMap
                  .get(item.service_template_id)
                  .push(item.bk_inst_id)
              } else {
                stat.withTemplateModuleIdMap.set(item.service_template_id, [
                  item.bk_inst_id,
                ])
              }
    ...
    setNodeDisabled() {
      ...
      if (withTemplateHostApplyIds?.length) {
        ...
        this.$refs.tree.setDisabled(nodeIds, {
          emitEvent: true,
          disabled: Boolean(this.action),
        })
      }
      ...
      if (noRuleIds?.length) {
        // 仅需处理未配置模板的节点
        const nodeIds = noRuleIds
          .filter(id => !withTemplateHostApplyIds.includes(id))
          .map(id => `module_${id}`)
        this.$refs.tree.setDisabled(nodeIds, {
          emitEvent: true,
          disabled: this.isDel,
    ...
    applyEnabled(node) {
      return (
        this.isModule(node) &&
        (node.data.host_apply_enabled ||
          node.data.service_template_host_apply_enabled)
      )
    },
    isTemplate(node) {
    ...
    async setModuleApplyStatusByTemplate(ids) {
      ...
      try {
        const result = await this.$store.dispatch(
          'hostApply/getModuleApplyStatusByTemplate',
          {
            params: {
        ...
        withTemplateModuleIds.forEach(id => {
          ...
          this.$set(
            nodeData,
            'service_template_host_apply_enabled',
            statusItem.host_apply_enabled
          )
        })
      } catch (error) {
        console.log(error)
