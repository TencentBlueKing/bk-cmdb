    ...
    <bk-alert type="warning" class="tips-alert">
      <template slot="title">
        <i18n path="服务模板的自动应用配置将覆盖其关联模块已有的规则操作提示">
          <template #link>
            <bk-link theme="primary" class="view-link" @click="handleShow">{{
              $t('点击查看')
            }}</bk-link>
          </template>
        </i18n>
      ...
      <div class="module-list">
        <div class="searchbar">
          <i18n path="共N个关联模块" class="count">
            <template #count
              ><em class="num">{{ displayModuleList.length }}</em></template
            >
          </i18n>
          <bk-input
            class="search-input"
        ...
        <bk-table
          ...
          <bk-table-column :width="220" :label="$t('是否配置自动应用')">
          ...
          <bk-table-column :width="90" :label="$t('操作')">
            <template slot-scope="{ row }">
              <bk-button
                ...
                theme="primary"
                text
                @click="handleViewModuleHostApply(row)">
                {{ $t('跳转查看') }}
              </bk-button>
              <span v-else>--</span>
            </template>
...
import serviceTemplateService, {
  CONFIG_MODE,
} from '@/service/service-template/index.js'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watchEffect(async () => {
      const list = await serviceTemplateService.findAllByIds(
        serviceTemplateIds.value,
        { bk_biz_id: bizId }
      )
    ...
    const getModulesById = async id => {
      const { info: modules } = await store.dispatch(
        'serviceTemplate/getServiceTemplateModules',
        {
          bizId,
      ...
      // 获取模块的拓扑路径
      const topopath = await store.dispatch(
      ...
      modules.forEach(module => {
        const moduleNode = topopath.nodes.find(
          node => node.topo_node.bk_inst_id === module.bk_module_id
        )
        module.topopath = moduleNode.topo_path
    ...
    const handleFilter = keyword => {
      ...
      state.displayModuleList = state.currentModuleList.filter(item =>
        new RegExp(keyword, 'i').test(item.topopath.replaceAll(' / ', '/'))
      )
    }
