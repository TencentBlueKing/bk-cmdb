  ...
  <div class="host-apply-sidebar">
    <div class="tree-wrapper">
      <div class="config-mode">
        <bk-radio-group v-model="configMode" class="radio-group">
          <bk-radio-button
            :disabled="Boolean(actionMode) && configMode !== CONFIG_MODE.MODULE"
            :value="CONFIG_MODE.MODULE"
            >{{ $t('按业务拓扑') }}</bk-radio-button
          >
          <bk-radio-button
            :disabled="
              Boolean(actionMode) && configMode !== CONFIG_MODE.TEMPLATE
            "
            :value="CONFIG_MODE.TEMPLATE"
            >{{ $t('按服务模板') }}</bk-radio-button
          >
        </bk-radio-group>
      </div>
      <div class="searchbar">
        <div class="search-select">
          <search-select-mix ref="searchSelect" :mode="configMode" />
        </div>
        <div v-show="!actionMode" class="action-menu">
          <bk-dropdown-menu
            font-size="medium"
            ...
            <div
              ...
              :class="['dropdown-trigger', { selected: actionMode }]">
              <span>{{ $t('批量操作') }}</span>
              <i
                :class="[
                  'bk-icon icon-angle-down',
                  { 'icon-flip': showBatchDropdown },
                ]"></i>
            </div>
    ...
    <div v-show="showCheckPanel" class="checked-list">
      <batch-check-panel
        ...
        <template #title>
          <i18n v-if="isModuleMode" path="已选择N个模块">
            <template #count
              ><em class="checked-num">{{ checkedList.length }}</em></template
            >
          </i18n>
          <i18n v-else-if="isTemplateMode" path="已选择N个模板">
            <template #count
              ><em class="checked-num">{{
                checkedTemplateList.length
              }}</em></template
            >
          </i18n>
        </template>
        <template #content>
          <dl v-if="isModuleMode" class="module-list">
            <div
              v-for="item in checkedList"
              :key="item.bk_inst_id"
              class="module-item">
              <dt class="module-name">{{ item.bk_inst_name }}</dt>
              <dd class="module-path" :title="item.path.join(' / ')">
                {{ item.path.join(' / ') }}
              </dd>
              <dd class="module-icon">
                <span>{{ $i18n.locale === 'en' ? 'M' : '模' }}</span>
              </dd>
              <dd class="action-icon">
                <a
                  href="javascript:;"
                  @click="handleRemoveChecked(item.bk_inst_id)">
                  <i class="bk-icon icon-close"></i>
                </a>
              </dd>
            </div>
          </dl>
          <ul v-else-if="isTemplateMode" class="template-list">
            <li
              v-for="item in checkedTemplateList"
              :key="item.id"
              class="template-item">
              <i class="template-icon"
                ><span>{{ $i18n.locale === 'en' ? 'M' : '模' }}</span></i
              >
  ...
  methods: {
    ...
    handleTreeChecked(ids) {
      ...
      const findModuleNode = function (data, parent) {
        data.forEach(item => {
          item.path = parent
            ? [...parent.path, item.bk_inst_name]
            : [item.bk_inst_name]
          if (
            item.bk_obj_id === 'module' &&
            ids.includes(`module_${item.bk_inst_id}`)
          ) {
            modules.push(item)
          }
          if (item.child) {
    ...
    handleTemplateChecked(ids) {
      ...
      this.checkedTemplateList = this.$refs.topologyTree.displayList.filter(
    ...
    handleRemoveChecked(id) {
      if (this.configMode === CONFIG_MODE.MODULE) {
        ...
        const checkedIds = this.checkedList
          .filter(item => item.bk_inst_id !== id)
          .map(item => `module_${item.bk_inst_id}`)
        tree.removeChecked({ emitEvent: true })
        tree.setChecked(checkedIds, {
          emitEvent: true,
          beforeCheck: true,
          checked: true,
        })
...
.config-mode {
  ...
  .radio-group {
    ...
    .bk-form-radio-button {
      ...
      ::v-deep
        &:not(:first-child)
        .bk-radio-button-input:disabled
