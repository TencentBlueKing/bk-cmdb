    ...
    <bk-table-column
      ...
      <div slot-scope="{ row }" :class="{ ignore: row.__extra__.ignore }">
        ...
        <div
          ...
          {{ showMore.expanded[row.id] ? $t('收起') : $t('展开更多')
          }}<i class="bk-cc-icon icon-cc-arrow-down"></i>
        </div>
      </div>
    </bk-table-column>
    <bk-table-column
      ...
      <template slot-scope="{ row }">
        <div v-if="multiple" :class="{ ignore: row.__extra__.ignore }">
          ...
          <div v-show="idList.length > showMore.max" class="show-more">
            &nbsp;
          </div>
        </div>
        <template v-else>
          ...
          <i
            ...
            v-bk-tooltips="$t('该字段已被设为不可编辑，自动应用规则失效')"
  ...
  methods: {
    setPropertyRuleList() {
      ...
      this.checkedPropertyIdList.forEach(id => {
        // 原始主机属性对象
        const index = this.propertyRuleList.findIndex(
        ...
        if (index === -1) {
          ...
          if (findProperty) {
            ...
            // 初始化值
            if (this.multiple) {
              const rules = this.ruleList.filter(
                item => item.bk_attribute_id === property.id
              )
              ...
              if (rules?.length) {
                ...
                const isSameValue = rules.every(
                  rule => rule?.bk_property_value === firstValue
                )
                property.__extra__.value = isSameValue
                  ? firstValue
              ...
              property.__extra__.value = has(rule, 'bk_property_value')
                ? rule.bk_property_value
                : getPropertyDefaultValue(property)
            }
      ...
      this.propertyRuleList = this.propertyRuleList.filter(property =>
        this.checkedPropertyIdList.includes(property.id)
      )
    },
    setConfigData() {
      this.removeRuleIds = []
      ...
      this.ruleList.forEach(rule => {
        const findIndex = this.propertyRuleList.findIndex(
          property => property.id === rule.bk_attribute_id
        )
        if (findIndex === -1) {
          // 批量模式标记为忽略，单个标记为删除
          if (this.multiple) {
      ...
      if (this.isModuleMode) {
        return (
          (
            this.ruleList.find(
              rule =>
                rule.bk_attribute_id === attrId &&
                rule.bk_module_id === targetId
            ) || {}
          ).bk_property_value || ''
        )
      }
      return (
        (
          this.ruleList.find(
            rule =>
              rule.bk_attribute_id === attrId &&
              rule.service_template_id === targetId
          ) || {}
        ).bk_property_value || ''
      )
    },
    ...
    handlePropertyRowDel(property) {
      if (this.multiple) {
        const extra = this.propertyRuleList.find(
          item => item.id === property.id
        )?.__extra__
        this.$set(extra, 'ignore', !extra.ignore)
      } else {
        const checkedIndex = this.checkedPropertyIdList.findIndex(
          id => id === property.id
        )
