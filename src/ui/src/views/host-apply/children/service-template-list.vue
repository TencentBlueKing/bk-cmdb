...
<template>
  <div
    ref="templateListEl"
    class="service-template-list"
    :style="{ '--height': `${$APP.height - 160 - 44}px` }">
    <div
      v-for="item in displayList"
      :key="item.id"
      :class="[
        'template-item',
        { selected: item.id === current.id, disabled: isNodeDisabled(item) },
      ]"
      ...
      <span v-if="item.host_apply_enabled" class="config-icon">
  ...
  setup(props, { emit }) {
    ...
    watchEffect(async () => {
      ...
      const current = targetId.value
        ? state.fullList.find(item => item.id === targetId.value)
        : state.fullList[0]
      ...
      if (state.fullList?.length) {
        ...
        state.displayList.forEach(node => {
          node.host_apply_rule_count = counts.find(
            item => item.service_template_id === node.id
          )?.count
        })
      }
    })
    ...
    watch(
      () => state.current,
      current => emit('selected', { data: current })
    )
    ...
    const scrollSelectedIntoView = () => {
      templateListEl.value
        ?.querySelector('.template-item.selected')
    ...
    const handleSearch = async condition => {
      try {
        if (condition.query_filter.rules.length) {
          const keywordRuleIndex = condition.query_filter.rules.findIndex(
            item => item.field === 'keyword'
          )

          if (keywordRuleIndex === -1) {
            // 不存在关键字则只采用接口搜索
            const data = await store.dispatch('hostApply/searchTemplateNode', {
              params: { bk_biz_id: bizId, ...condition },
            })
            filterList({ remote: data || [] })
          } else {
            // 先取出关键字的参数值
            const keyword =
              condition.query_filter.rules[keywordRuleIndex]?.value
    ...
    const filterList = ({ remote: remoteData, keyword, clear }) => {
      ...
      if (remoteData && keyword) {
        ...
        state.displayList = state.fullList.filter(
          item => ids.includes(item.id) && reg.test(item.name)
        )
      }
