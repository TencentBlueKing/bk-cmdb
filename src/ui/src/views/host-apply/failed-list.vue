...
<template>
  <div class="failed-list">
    ...
    <bk-table
      ...
      <bk-table-column
        :label="$t('固资编号')"
        prop="host.bk_asset_id"
        show-overflow-tooltip></bk-table-column>
      <bk-table-column
        :label="$t('主机名称')"
        prop="host.bk_host_name"
        show-overflow-tooltip></bk-table-column>
      <bk-table-column
        :label="$t('所属拓扑')"
        :formatter="getTopopath"
        show-overflow-tooltip></bk-table-column>
      <bk-table-column :label="$t('失败原因')">
        <div slot-scope="{}" class="fail-reason">网络中断</div>
      </bk-table-column>
    </bk-table>
    <div class="bottom-actionbar">
      <div class="actionbar-inner">
        ...
        <bk-button theme="default" @click="handleCopyIp">{{
          $t('复制IP')
        }}</bk-button>
      ...
      modules.forEach(module => {
        ...
        if (set) {
          const business = businesses.find(
            business => business.bk_biz_id === set.bk_biz_id
          )
          if (business) {
            topology.push({
