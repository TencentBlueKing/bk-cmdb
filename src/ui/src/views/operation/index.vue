...
<template>
  <div v-bkloading="{ isLoading: fetching || $loading(request.chart) }">
    <cmdb-tips
      v-show="!(fetching || $loading(request.chart))"
      v-if="$Site.disableOperationStatistic"
      style="margin: 10px 20px">
      {{ $t('运营统计停止统计提示') }}
    </cmdb-tips>
    <div v-show="!(fetching || $loading(request.chart))" class="operate-menus">
      <div
        class="menu-items menu-items-blue"
        @click="goRouter(MENU_RESOURCE_BUSINESS)">
        <div class="item-left">
      ...
      <div
        class="menu-items menu-items-white"
        @click="goRouter(MENU_RESOURCE_HOST)">
        <div class="item-left">
      ...
      <div
        class="menu-items menu-items-blue"
        @click="goRouter(MENU_MODEL_MANAGEMENT)">
        <div class="item-left">
        ...
        <div class="item-left">
          ...
          <span
            ...
            <bk-popover
              ...
              <span slot="content" class="popover-content">
                {{ tooltip }}
              </span>
            </bk-popover>
          </span>
    ...
    <div v-show="!(fetching || $loading(request.chart))" class="main">
      <div class="operation-top">
        <span class="operation-title">{{ $t('主机统计') }}</span>
        <cmdb-auth
          :auth="{ type: $OPERATION.U_STATISTICAL_REPORT }"
      ...
      <div
        v-for="(item, key) in hostData.disList"
        ...
        <div class="chart-child">
          <div class="chart-title">
            <span>{{ item.name }}</span>
            <div v-if="updateAuth" class="charts-options">
              <i
                v-if="key === 0"
                v-bk-tooltips="$t('已置顶，无法上移')"
                class="bk-icon icon-arrows-up icon-weight icon-disable">
              </i>
              <i
                v-else
                class="bk-icon icon-arrows-up icon-weight"
                @click="moveChart('host', 'up', key, hostData.disList)">
              </i>
              <i
                v-if="key === hostData.disList.length - 1"
                v-bk-tooltips="$t('已置底，无法下移')"
                class="bk-icon icon-arrows-down icon-weight icon-disable">
              </i>
              <i
                v-else
                class="bk-icon icon-arrows-down icon-weight"
                @click="moveChart('host', 'down', key, hostData.disList)">
              </i>
              <i
                class="icon icon-cc-edit-shape"
                @click="openNew('edit', 'host', item, key)"></i>
              <i
                class="icon icon-cc-tips-close"
                @click="deleteChart('host', key, hostData.disList, item)"></i>
            </div>
          </div>
          <div class="chart-bottom">
            <div
              :id="item.report_type + item.config_id"
              class="operation-charts"></div>
          </div>
          <div v-if="item.noData" class="null-data">
            <span>{{ $t('暂无数据') }}</span>
          </div>
          <div v-if="item.showDate" class="chart-date">
            <bk-date-picker
              v-model="dateRange"
              transfer
              :options="options"
      ...
      <div
        v-for="(item, key) in instData.disList"
        :key="item.report_type + item.config_id"
        :style="{ width: item.width + '%' }"
        class="operation-layout">
        <div class="chart-child">
          <div class="chart-title">
            ...
            <div v-if="updateAuth" class="charts-options">
              ...
              <i
                class="icon icon-cc-edit-shape"
                @click="openNew('edit', item.bk_obj_id, item, key)"></i>
              <i
                class="icon icon-cc-tips-close"
...
export default {
  ...
  data() {
    const startDate = this.$tools.formatTime(
  ...
  methods: {
    ...
    dataDeal(data, res) {
      ...
      if (res && Array.isArray(res)) {
        ...
        if (data.chart_type !== 'pie')
          content.width =
            0.015 * data.x_axis_count * (data.width === '50' ? 2 : 1)
        ...
        for (const item in res) {
          ...
          if (Array.isArray(res[item])) {
            ...
            res[item].forEach(child => {
              ...
              returnData.minTime =
                returnData.minTime === '' ? time : returnData.minTime
              returnData.maxTime =
                time > returnData.maxTime ? time : returnData.maxTime
              returnData.minTime =
                time < returnData.minTime ? time : returnData.minTime
              content.x.push(time)
            ...
            for (const chartItem in barModel.chart) {
              ...
              barModel.chart[chartItem].marker.color.push(
                barModel.chart[chartItem].color
              )
            }
          }
        }
      ...
      if (data.report_type === 'host_change_biz_chart')
        this.maxRange = [returnData.minTime, returnData.maxTime]
      return returnData
    },
    drawCharts(item) {
      const layConfig = {
      ...
      if (item.report_type === 'host_change_biz_chart') {
        ...
        layConfig.fixRange =
          this.maxRange[0] >= item.data.minTime &&
          this.maxRange[1] <= item.data.maxTime
        layConfig.nticks =
          item.x_axis_count % 2 === 0
            ? item.x_axis_count
            : item.x_axis_count + 1
        layConfig.colorway = [
          '#3A84FF',
      ...
      const layout = {
        ...
        xaxis: {
          ...
          dtick:
            item.report_type === 'host_change_biz_chart'
              ? 24 * 60 * 60 * 1000
              : '',
        },
        bargap: 0.1,
      ...
      if (item.report_type !== 'model_inst_change_chart')
      ...
      if (this.editType.openType === 'edit') {
        ...
        setTimeout(() => {
          ...
          if (item.data.data[0].mode !== 'line')
            this.hoverConfig(document.getElementById(myDiv), myDiv)
        }, 100)
      } else {
        Plotly.newPlot(myDiv, data, layout, options)
        if (item.data.data[0].mode !== 'line')
        ...
        const update = {
          data: {
            maker: {
              ...
              color: [
                data.points[0].data.marker.color,
                data.points[1].data.marker.color,
                data.points[2].data.marker.color,
              ],
            },
          },
        }
    ...
    deleteChart(type, key, list, item) {
      this.$bkInfo({
        title: this.$t('是否确认删除'),
        subTitle: `${this.$t('确定要删除XXX', { target: `【${item.name}】` })}`,
    ...
    async openNew(type, host, data, key) {
      ...
      if (type === 'edit') {
        ...
        this.existedCharts =
          host === 'host' ? this.hostData.disList : this.instData.disList
      }
      this.isShow = true
    },
    async saveData(data) {
...
.title-block::before {
  ...
  border-top: 5px solid rgb(0 0 0 / 80%);
...
.operation-top {
  ...
  .icon-btn {
    ...
    .icon {
      ...
      &:hover {
        background: rgb(225 236 255 / 100%);
      }
    }
  }
}
...
.operate-menus {
  ...
  .menu-items-blue {
    background: linear-gradient(
      165deg,
      rgb(108 186 255 / 100%) 0%,
      rgb(58 132 255 / 100%) 100%
    );

    &:hover {
      box-shadow: 0 2px 6px 0 rgb(58 132 255 / 100%);
    }
  }
  ...
  .menu-items-white {
    background: linear-gradient(
      165deg,
      rgb(77 212 240 / 100%) 0%,
      rgb(27 167 207 / 100%) 100%
    );

    &:hover {
      box-shadow: 0 2px 6px 0 rgb(27 167 207 / 100%);
    }
  }
  ...
  .menu-items {
    ...
    float: left;
    cursor: pointer;
    box-shadow: 0 2px 4px 0 rgb(220 222 229 / 100%);
    border-radius: 4px;
    position: relative;
...
.operation-layout {
  ...
  .chart-child {
    ...
    &:hover {
      box-shadow: 0 2px 6px 0 rgb(220 222 229 / 100%);
    }
  }
