      ...
      <div class="dialog-content">
        ...
        <div class="content clearfix">
            ...
            <div class="content-item">
              ...
              <span class="cmdb-form-item">
                <bk-select
                  ...
                  :clearable="false">
                  <bk-option
                    v-for="option in seList.disList"
                    :id="option.name"
                    :key="option.repType"
                    :name="option.name"
                    :disabled="
                      existedCharts.findIndex(
                        item => item.report_type === option.repType
                      ) > -1
                    ">
                  </bk-option>
                </bk-select>
          ...
          <div v-if="chartType">
            ...
            <div v-if="chartData.bk_obj_id !== 'host'" class="content-item">
              <label class="label-text">
                {{ $t('统计对象') }}
              </label>
              <span class="cmdb-form-item">
            ...
            <div class="content-item">
              <label class="label-text" :style="{ marginRight: '47px' }">
                {{ $t('统计维度') }}
                <i
                  v-bk-tooltips="$t('模型中需包含枚举字段才可以作为维度统计')"
                  class="icon-cc-exclamation-tips"></i>
              </label>
              <span class="cmdb-form-item">
                <bk-select
            ...
            <div class="content-item">
              ...
              <label class="cmdb-form-radio cmdb-radio-big">
                <input
                  v-model="chartData.chart_type"
                  type="radio"
                  name="present"
                  value="pie" />
                <span class="cmdb-radio-text cmdb-radio-text-icon">{{
                  $t('饼图')
                }}</span>
              </label>
              <label class="cmdb-form-radio cmdb-radio-big">
          ...
          <div
            v-if="
              ...
              chartData.report_type !== 'host_change_biz_chart'
            "
            class="content-item">
            <label class="label-text-x">
              {{ $t('横轴坐标数量') }}
              <i
                v-bk-tooltips="$t('图标可视区横轴坐标数量，建议不超过20个')"
                class="icon-cc-exclamation-tips"></i>
            </label>
            <label class="cmdb-form-item">
              <div class="axis-picker">
                <input
                  ...
                  class="cmdb-form-input form-input"
                  name="chartNumber" />
                <i
                  class="bk-icon icon-angle-down"
                  @click="calculate('down')"></i>
                <i
                  v-if="maxNum > chartData.x_axis_count"
                  class="bk-icon icon-angle-up"
                  @click="calculate('up')"></i>
                <bk-popover
                  v-else
                  class="tool-tip"
                  placement="right"
                  :content="$t('已经超出可显示的最大数量')">
        ...
        <div slot="footer" class="footer">
          <bk-button theme="primary" @click="confirm">{{
            openType === 'add' ? $t('提交') : $t('保存')
          }}</bk-button>
          <bk-button theme="default" @click="closeChart">{{
            $t('取消')
          }}</bk-button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    filterList() {
      return this.demList.filter(item => {
        if (this.hostType === 'host') {
          return (
            item.bk_property_type === 'enum' &&
            item.bk_property_id !== 'bk_os_type'
          )
        }
    ...
    staticFilter() {
      return this.staList.filter(
        item => !item.bk_ishidden && !this.hostFilter.includes(item.bk_obj_id)
      )
    },
    typeFilter() {
      const data =
        this.chartData.bk_obj_id === 'host'
          ? this.seList.host
          : this.seList.inst
  ...
  watch: {
    chartType() {
      ...
      if (this.chartData.bk_obj_id === 'host')
    ...
    if (this.openType !== 'add')
      this.maxNum =
        this.chartData.chart_type === 'pie'
          ? this.chartData.data.data[0].labels.length
          : this.chartData.data.data[0].x.length
    else this.maxNum = 25
    this.initTitle()
  ...
  methods: {
    ...
    getDisabled(property) {
      if (this.hostType === 'host') {
        return (
          this.existedCharts.findIndex(
            item => item.field === property.bk_property_id
          ) > -1
        )
      }
      if (this.hostType === 'inst') {
        const existed = this.existedCharts.find(
          item => item.bk_obj_id === property.bk_obj_id
        )
    ...
    confirm() {
      this.$validator.validateAll().then(result => {
        if (result) {
          this.chartData.report_type = this.chartType
            ? 'custom'
            : this.typeFilter[0].repType
          this.chartData.x_axis_count = parseInt(
            this.chartData.x_axis_count,
            10
          )
    ...
    initTitle() {
      if (this.openType !== 'add')
        this.editTitle = `${this.$t('编辑')}【${this.chartData.title}】`
      else
        this.editTitle =
          this.chartData.bk_obj_id === 'host'
            ? this.$t('新增主机统计图表')
...
.footer {
  border-top: 1px solid rgb(220 222 229 / 100%);
  height: 50px;
  line-height: 50px;
  font-size: 0;
  text-align: right;
