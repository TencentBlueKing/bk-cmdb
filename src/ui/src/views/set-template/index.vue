...
<template>
  <div class="template-layout">
    ...
    <bk-table
      v-test-id.businessSetTemplate="'templateList'"
      ...
      <bk-table-column
        :label="$t('应用数量')"
        prop="set_instance_count"
        sortable></bk-table-column>
      <bk-table-column
        :label="$t('修改人')"
        prop="modifier"
      ...
      <cmdb-table-empty
        ...
        :auth="{ type: $OPERATION.C_SET_TEMPLATE, relation: [bizId] }"
        @create="handleCreate"
        @clear="handleFilterClear"></cmdb-table-empty>
    </bk-table>
  </div>
</template>
  ...
  methods: {
      ...
      try {
        if (this.originList.length) {
          const data = await this.$store.dispatch(
            'setTemplate/getSetTemplateStatus',
            {
              bizId: this.bizId,
              params: {
          ...
          this.originList.forEach(item => {
            const syncStatus = data.find(
              status => status.set_template_id === item.id
            )
            if (syncStatus) {
    ...
    handleFilterTemplate() {
      ...
      this.list = this.searchName
        ? originList.filter(
            template => template.name.indexOf(this.searchName) !== -1
          )
        : originList
