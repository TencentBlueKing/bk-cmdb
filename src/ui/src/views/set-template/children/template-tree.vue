...
<template>
  <div
    v-bkloading="{ isLoading: $loading(request.topo) }"
    :class="['template-tree', mode]">
    ...
    <ul class="node-children">
      <li
        ...
        :key="service.id"
        class="node-child clearfix"
        :class="{ selected: selected === service.id }"
        @click="handleChildClick(service)">
        <i class="node-icon fl">{{ moduleName[0] }}</i>
        <span v-if="mode !== 'view'" class="child-options fr">
          <bk-link class="action-link" @click="handleViewService(service)">{{
            $t('查看详情')
          }}</bk-link>
          <bk-popover v-if="serviceExistHost(service.id)">
            ...
            <i18n
              ...
              class="service-tips">
              <template #link
                ><span @click="handleGoTopoBusiness(service)">{{
                  $t('跳转查看')
                }}</span></template
              >
            </i18n>
        ...
        <span v-bk-overflow-tips class="child-name">{{ service.name }}</span>
      </li>
      <li
        v-if="['create', 'edit'].includes(mode)"
        class="options-child node-child clearfix"
    ...
    <bk-dialog
      ...
      <template slot="footer">
        <div v-if="dialog.name === 'add'" class="dialog-footer">
          <div v-if="serviceTemplateCount > 0" class="summary">
            <i18n path="已选个数">
              <template #count>
                <span class="stat">
          ...
          <div class="action">
            ...
            <bk-button
              theme="default"
              class="btn ml5"
              @click.stop="dialog.visible = false"
              >{{ $t('取消') }}</bk-button
            >
          </div>
        </div>
        <bk-button v-else @click="dialog.visible = false">{{
          $t('关闭')
        }}</bk-button>
      </template>
    </bk-dialog>
...
export default {
  ...
  computed: {
    ...
    setName() {
      const setModel =
        this.$store.getters['objectModelClassify/getModelById']('set') || {}
      return setModel.bk_obj_name || ''
    },
    moduleName() {
      const moduleModel =
        this.$store.getters['objectModelClassify/getModelById']('module') || {}
  ...
  methods: {
    async getSetTemplateServices() {
      try {
        const data = await this.$store.dispatch(
          'setTemplate/getSetTemplateServicesStatistics',
          {
    ...
    serviceExistHost(id) {
      const service = this.servicesHost.find(
