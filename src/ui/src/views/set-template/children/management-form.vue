...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const excludeModuleProperties = [
      BUILTIN_MODEL_PROPERTY_KEYS[BUILTIN_MODELS.SET].NAME,
    ]
    ...
    const isFormDataChanged = computed(() => {
      ...
      return (
        !isEqual(formDataCopy, formDataLatest) || isServiceTemplateChanged.value
      )
    })
  ...
  methods: {
    getData() {
      return {
        ...
        service_template_ids: this.formData.serviceTemplates.map(
      ...
      <cmdb-collapse :label="$t('基础信息')" arrow-type="filled">
        <grid-layout
          ...
          <grid-item
            ...
            <p v-if="errors.has('templateName')" class="form-error">
      ...
      <cmdb-collapse :label="$t('集群拓扑')" arrow-type="filled">
        <div class="form-content">
          <div
            :class="[
              'cmdb-form-item',
              { 'is-error': errors.has('serviceTemplates') },
            ]">
            <template-tree
              :mode="treeMode"
            ...
            <p v-if="errors.has('serviceTemplates')" class="form-error">
              {{ $t('请添加服务模板') }}
            </p>
          </div>
        </div>
