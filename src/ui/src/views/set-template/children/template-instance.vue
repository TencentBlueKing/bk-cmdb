...
<template>
  <div class="template-instance-layout">
    <div class="instance-main">
      <div class="options clearfix">
        <div class="fl">
          <cmdb-auth :auth="syncAuth">
            <bk-button
              slot-scope="{ disabled }"
              v-test-id.businessSetTemplate="'batchSync'"
              theme="primary"
      ...
      <bk-table
        ref="instanceTable"
        v-test-id.businessSetTemplate="'instanceTable'"
        v-bkloading="{
          isLoading: $loading([
            'getSetInstanceData',
        ...
        <bk-table-column
          ...
          <span
            ...
            >{{ getTopoPath(row) }}</span
          >
        </bk-table-column>
        <bk-table-column
          :label="$t('主机数量')"
          prop="host_count"></bk-table-column>
        ...
        <bk-table-column
          ...
          <template slot-scope="{ row }">
            <span>{{
              row.last_time
                ? $tools.formatTime(row.last_time, 'YYYY-MM-DD HH:mm:ss')
                : '--'
            }}</span>
          </template>
        </bk-table-column>
...
export default {
  ...
  data() {
    this.polling = new Polling(() => {
      ...
      if (needSync) {
        return Promise.all([this.updateStatusData(), this.getInstancesInfo()])
      }
    }, 5000)
  ...
  computed: {
    ...
    displayList() {
      ...
      return list.map(item => {
        ...
        const setInfo = this.listWithTopo.find(
          set => set.bk_set_id === item.bk_set_id
        )
        if (setInfo) {
        ...
        if (templateSyncInfo && templateSyncInfo.status === syncFail) {
          const failModules = (templateSyncInfo.detail || []).filter(
            module => module.status === syncFail
          )
          failTips = failModules
            .map(module => {
              const { data } = module
              const moduleName =
                data && data.module_diff && data.module_diff.bk_module_name
              const errorMsg = module.response && module.response.bk_error_msg
              const tips =
                moduleName && errorMsg ? `${moduleName} : ${errorMsg}` : ''
              return tips
            })
            .filter(tips => tips)
        }
  ...
  methods: {
    ...
    async updateStatusData() {
      const data = await this.getSetInstancesWithStatus(
        'updateStatusData',
        {
          page: {
            start: 0,
    ...
    async getSetInstancesWithTopo() {
      try {
        const data = await this.$store.dispatch(
          'setTemplate/getSetInstancesWithTopo',
          {
            bizId: this.bizId,
    ...
    async getInstancesInfo() {
      const instancesInfo = await this.$store.dispatch(
        'setSync/getInstancesSyncStatus',
        {
          bizId: this.bizId,
          setTemplateId: this.templateId,
          params: {
