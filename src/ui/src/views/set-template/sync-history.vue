    ...
    displayList() {
      ...
      return list.map(item => {
        ...
        const setInfo = this.listWithTopo.find(
          set => set.bk_set_id === item.bk_inst_id
        )
        if (setInfo) {
  ...
  methods: {
    getTopoPath(row) {
      ...
      if (topoPath.length) {
        ...
        const sortPath = topoPath.sort(
          (prev, next) => prev.bk_inst_id - next.bk_inst_id
        )
    ...
    async getSetTemplateInfo() {
      try {
        const info = await this.$store.dispatch(
          'setTemplate/getSingleSetTemplateInfo',
          {
    ...
    hanldeFilterByDate(daterange) {
      ...
      this.searchDate = daterange.map((date, index) =>
        index === 0 ? `${date} 00:00:00` : `${date} 23:59:59`
      )
      this.getData(true)
    },
  },
