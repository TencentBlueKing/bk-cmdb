  ...
  <div class="models-layout">
    ...
    <bk-table
      ...
      <bk-table-column
        ...
        :min-width="
          $tools.getHeaderPropertyMinWidth(column.property, {
            hasSort: getColumnSortable(column.property),
          })
        "
        :prop="column.id"
        :label="column.name"
    ...
    <bk-sideslider
      ...
      <template v-if="slider.contentShow" slot="content">
        <cmdb-form
          v-if="attribute.type === 'create'"
          ref="form"
        ...
        <cmdb-form-multiple
          v-else-if="attribute.type === 'multiple'"
          ref="multipleForm"
...
export default {
  ...
  computed: {
    ...
    globalCustomColumns() {
      return (
        this.globalUsercustom[`${this.objId}_global_custom_table_columns`] || []
      )
    },
    parentLayers() {
      return [
  ...
  methods: {
    ...
    setupFilter() {
      ...
      this.filterCondition = resetConditionValue(
      ...
      try {
        Object.keys(advQuery).forEach(key => {
          ...
          if (property && operator && value.length) {
            ...
            const exist =
              this.filterSelected.findIndex(
                item => item.bk_property_id === property.bk_property_id
              ) !== -1
            if (!exist) {
      ...
      // 合并快速搜索栏条件
      if (searchType === 'fast') {
        const fastSearchProperty = this.properties.find(
          property => property.bk_property_id === fastQuery.field
        )
        // eslint-disable-next-line max-len
        const exist =
          this.filterSelected.findIndex(
            item => item.bk_property_id === fastSearchProperty.bk_property_id
          ) !== -1
        ...
        const conditionValue = {
          ...
          value: this.formatFilterValue(
            { value: fastQuery.value, operator: fastQuery.operator },
            defaultData.value
          ),
        }
    ...
    setTableHeader() {
      return new Promise(resolve => {
        const customColumns = this.customColumns.length
          ? this.customColumns
    ...
    updateTableHeader(properties) {
      ...
      if (
        this.filterSelected.length >= properties.length &&
        this.isColumnApply
      ) {
        // 列表项减少
        ...
        // 列表项增加
        const newHeaders = this.filterSelected.filter(
          item => !headerIds.includes(item.bk_property_id)
        )
    ...
    getInstList(config = { cancelPrevious: true }) {
      return instanceService.find({
        ...
        params: {
          ...
          ...(Utils.transformGeneralModelCondition(
            this.filterCondition,
            this.filterSelected
          ) || {}),
        },
    ...
    async getTableData() {
      ...
      try {
        const { count, info } = await this.getInstList({
          cancelPrevious: true,
        ...
        this.table.stuff.type =
          (filter && filter.length > 0) || filterAdv ? 'search' : 'default'
    ...
    handleApplyColumnsConfig(properties) {
      ...
      this.$store.dispatch('userCustom/saveUsercustom', {
        [this.customConfigKey]: properties.map(
          property => property.bk_property_id
        ),
      })
    ...
    handleSliderBeforeClose() {
      if (this.tab.active === 'attribute') {
        const $form =
          this.attribute.type === 'multiple'
            ? this.$refs.multipleForm
            : this.$refs.form
        const confirmFn = () => {
          this.attribute.type === 'multiple'
            ? this.handleMultipleCancel
            : this.handleCancel
        }
        if ($form.hasChange) {
