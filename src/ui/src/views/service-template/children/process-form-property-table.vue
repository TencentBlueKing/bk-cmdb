    ...
    <template v-for="column in options" #[column.bk_property_id]="rowProps">
      <div
        ...
        <span
          ...
          <i v-if="isLocked(rowProps)" class="icon-cc-lock-fill"></i>
...
} from '@/dictionary/process-bind-ip.js'
import {
  PROCESS_BIND_PROTOCOL_V4_LIST,
  PROCESS_BIND_PROTOCOL_V6_LIST,
} from '@/dictionary/process-bind-protocol.js'
  ...
  methods: {
    ...
    getOptions({ row }, property) {
      ...
      if (property.bk_property_id === 'protocol') {
        if (this.IPV4Keys.includes(row.ip)) {
          return property.option.filter(item =>
            PROCESS_BIND_PROTOCOL_V4_LIST.includes(item.id)
          )
        }
        if (this.IPV6Keys.includes(row.ip)) {
          return property.option.filter(item =>
            PROCESS_BIND_PROTOCOL_V6_LIST.includes(item.id)
          )
        }
      }
    ...
    handleColumnValueChange({ row, column, index }, value) {
      ...
      if (column.property === 'ip') {
        if (
          this.IPV4Keys.includes(value) &&
          !PROCESS_BIND_PROTOCOL_V4_LIST.includes(rowValue.protocol)
        ) {
          rowValue.protocol = ''
        }
        if (
          this.IPV6Keys.includes(value) &&
          !PROCESS_BIND_PROTOCOL_V6_LIST.includes(rowValue.protocol)
        ) {
          rowValue.protocol = ''
        }
      }
    ...
    transformValueToTemplateValue(values) {
      ...
      return values.map((row, rowIndex) => {
        ...
        // 创建模式并且是新添加的行，使其默认锁定
        const defaultLocked =
          this.type === 'create' && rowIndex === values.length - 1
        ...
        Object.keys(row).forEach(key => {
          if (['process_id', 'row_id'].includes(key)) {
            ...
            templateRowValue[key] = {
              value: row[key],
              as_default_value:
                !!(this.lockStates[index] || {})[key] || defaultLocked,
            }
          }
        })
