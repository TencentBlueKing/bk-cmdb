...
export default {
  ...
  data() {
    this.polling = new Polling(() => {
      const syncingModules = this.table.data
        .filter(theModule => this.isSyncing(theModule.status))
        .map(theModule => theModule.bk_module_id)
  ...
  methods: {
    ...
    async loadIntegrationInstances() {
      ...
      if (data.count) {
        const [loadTopoPathErr, topoPath] = await to(
          this.loadTopoPath(data.info)
        )
        ...
        data.info.forEach(module => {
          ...
          const topo = topoPath.nodes.find(
            topo => topo.topo_node.bk_inst_id === moduleId
          )
          module.topoPath = topo.topo_path
            .map(path => path.bk_inst_name)
            .reverse()
            .join(' / ')
          module.hostCount = moduleHostCount.find(
            item => item.bk_inst_id === moduleId
          )?.host_count
        })
      }
      ...
      try {
        const result = await this.$store.dispatch(
          'objectMainLineModule/getTopoStatistics',
          {
            bizId: this.bizId,
            params: {
              condition: moduleIds.map(id => ({
                bk_obj_id: BUILTIN_MODELS.MODULE,
                bk_inst_id: id,
              })),
            },
          }
    ...
    renderVisibleList() {
      ...
      this.table.visibleList = this.table.data.slice(
        (current - 1) * limit,
        current * limit
      )
    },
    ...
    loadInstanceStatus(moduleIds) {
      return this.$store
        .dispatch('serviceTemplate/getServiceTemplateInstanceStatus', {
          bizId: this.bizId,
          params: {
        ...
        .then(res => {
          this.table.data.forEach(item => {
            const instanceStatus = res.find(
              r => r.bk_inst_id === item.bk_module_id
            )
            if (instanceStatus) {
    ...
    sortByPath(rowA, rowB) {
      ...
      return rowA.topoPath
        .toLowerCase()
        .localeCompare(rowB.topoPath.toLowerCase(), 'zh-Hans-CN', {
          sensitivity: 'accent',
        })
    },
