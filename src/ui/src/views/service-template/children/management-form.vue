...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const localProcessTemplate = computed(
      () => store.getters['serviceProcess/localProcessTemplate']
    )
    ...
    const excludeModuleProperties = [
      BUILTIN_MODEL_PROPERTY_KEYS[BUILTIN_MODELS.MODULE].NAME,
    ]
    ...
    watchEffect(async () => {
      ...
      if (isEdit.value) {
        ...
        store.commit(
          'serviceProcess/setLocalProcessTemplate',
          formatProcessSubmitData(processList)
        )
    ...
    const handleDeleteProcess = (template, index) => {
      $bkInfo({
        title: t('确认删除模板进程'),
        confirmFn: () => {
          store.commit('serviceProcess/deleteLocalProcessTemplate', {
            process: template,
            index,
          })
    ...
    <div class="form-group">
      <cmdb-collapse :label="$t('基础信息')" arrow-type="filled">
        <grid-layout
          ...
          <grid-item :label="$t('服务分类')" :label-width="120" required>
      ...
      <cmdb-collapse :label="$t('属性设置')" arrow-type="filled">
        <div class="form-content">
          <property-config
            ...
            <template #tips>
              <div class="property-config-tips">
                {{ $t('模板里定义的字段，在实例中将不可修改') }}
              </div>
            </template>
          </property-config>
        </div>
