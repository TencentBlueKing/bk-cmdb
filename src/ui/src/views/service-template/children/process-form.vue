            ...
            <ul class="property-list">
              <template
                ...
                <li
                  ...
                  :class="[
                    'property-item',
                    { flex: property.bk_property_type === 'table' },
                  ]">
                  <div
                    v-if="
                      !invisibleNameProperties.includes(property.bk_property_id)
                    "
                    class="property-name clearfix">
                    <span
                  ...
                  <div
                    :class="[
                      ...
                      {
                        'is-lock':
                          values[property.bk_property_id].as_default_value,
                      },
                    ]">
                    <component
                    ...
                    <span
                      ...
                      class="property-lock-state"
                      tabindex="-1"
                      @click="toggleLockState(property)">
                      <i
                        v-if="values[property.bk_property_id].as_default_value"
                        class="icon-cc-lock-fill"></i>
    ...
    <div
      ...
      <slot name="form-options">
        <cmdb-auth :auth="auth">
          <bk-button
            slot-scope="{ disabled }"
            v-test-id="type === 'create' ? 'submit' : 'save'"
            class="button-save"
            theme="primary"
            :loading="$loading()"
...
import ProcessFormPropertyTable from './process-form-property-table.vue'
export default {
  components: {
  ...
  methods: {
    ...mapMutations('serviceProcess', [
      'addLocalProcessTemplate',
      'updateLocalProcessTemplate',
    ]),
    isLocked(property) {
    ...
    getPropertyEditStatus(property) {
      const uneditable =
        ['bk_func_name'].includes(property.bk_property_id) &&
        !this.isCreatedService
        ...
        if (!['sign_id', 'process_id'].includes(propertyId)) {
          isChange = Object.keys(this.values[propertyId]).some(key => {
            ...
            const oldValue = JSON.stringify(
              this.refrenceValues[propertyId][key]
            )
    ...
    initValues() {
      const restValues = {}
      const formValues = this.$tools.getInstFormValues(
        this.properties,
        {},
        this.type === 'create'
      )
      Object.keys(formValues).forEach(key => {
        if (!has(this.inst, key)) {
          restValues[key] = {
            ...
            as_default_value:
              this.mustLocked.includes(key) || this.type === 'create',
    ...
    getFormError(property) {
      if (property.bk_property_type === 'table') {
        const hasError = this.errors.items.some(
          item => item.scope === property.bk_property_id
        )
      ...
      try {
        ...
        if (result && !this.hasChange()) {
          ...
          if (this.type === 'create') {
            ...
            this.updateLocalProcessTemplate({
              process: formatValue,
    ...
    uncollapseGroup() {
      this.errors.items.forEach(item => {
        ...
        const property = this.properties.find(
          property => property.bk_property_id === compareKey
        )
