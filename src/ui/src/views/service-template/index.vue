...
<template>
  <div ref="templateWrapper" class="template-wrapper">
    ...
    <div class="template-filter clearfix">
      ...
      <div class="filter-text fr">
        ...
        <bk-select
          ...
          <bk-option
            v-for="category in secondaryList"
            :id="category.id"
            :key="category.id"
            :name="category.name">
          </bk-option>
    ...
    <bk-table
      ...
      <bk-table-column
        ...
        sortable="custom"></bk-table-column>
      <bk-table-column
        prop="service_category"
        :label="$t('服务分类')"
        show-overflow-tooltip></bk-table-column>
      <bk-table-column prop="process_template_count" :label="$t('进程数量')">
        <template slot-scope="{ row }">
          <cmdb-loading :loading="$loading(request.count)">
            ...
            <span v-else style="color: #ff9c01"
              >{{ row.process_template_count }}（{{ $t('未配置') }}）</span
            >
          </cmdb-loading>
        </template>
      </bk-table-column>
      ...
      <cmdb-table-empty
        ...
        :auth="{ type: $OPERATION.C_SERVICE_TEMPLATE, relation: [bizId] }"
        @create="handleCreate"
        @clear="handleClearFilter"></cmdb-table-empty>
    </bk-table>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    params() {
      ...
      const id = this.categoryId ? this.categoryId : maincategoryId
      return {
        ...
        page: {
          start:
            (this.table.pagination.current - 1) * this.table.pagination.limit,
    ...
    emptyText() {
      return this.filter.mainClassification
        ? this.$t('没有二级分类')
        : this.$t('请选择一级分类')
    },
    hasFilter() {
  ...
  methods: {
    ...
    ...mapActions('serviceClassification', [
      'searchServiceCategoryWithoutAmout',
    ]),
    async getTableData() {
      try {
        ...
        this.table.list = templateData.info.map(template => {
          const second = this.allSecondaryList.find(
            clazz => clazz.id === template.service_category_id
          )
          const mainCategory = this.mainList.find(
            clazz => second && clazz.id === second.bk_parent_id
          )
    ...
    async getTemplateCount() {
      try {
        const data = await this.$store.dispatch(
          'serviceTemplate/searchServiceTemplateCount',
          {
            bizId: this.bizId,
            params: {
        ...
        this.table.list.forEach(row => {
          const counts =
            data.find(counts => counts.service_template_id === row.id) || {}
          const {
    ...
    async getServiceClassification() {
      ...
      this.mainList = this.classificationList.filter(
        classification => !classification.bk_parent_id
      )
      this.allSecondaryList = this.classificationList.filter(
        classification => classification.bk_parent_id
      )
    },
    async getTemplateSyncStatus() {
      try {
        const { service_templates: syncStatusList = [] } =
          await this.getServiceTemplateSyncStatus({
            bizId: this.bizId,
            params: {
              is_partial: true,
              service_template_ids: this.table.list.map(row => row.id),
        ...
        this.table.list.forEach(row => {
          const syncStatus =
            syncStatusList.find(
              status => status.service_template_id === row.id
            ) || {}
          if (syncStatus) {
    ...
    handleSelect(id = '') {
      this.secondaryList = this.allSecondaryList.filter(
        classification => classification.bk_parent_id === id
      )
