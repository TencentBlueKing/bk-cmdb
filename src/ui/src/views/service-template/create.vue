...
export default defineComponent({
  ...
  setup() {
    ...
    const cloneTemplateId = computed(() =>
      parseInt(router.app.$route.query.clone, 10)
    )
    ...
    const createTemplate = async formData => {
      ...
      const { id } = await serviceTemplateService.create(data, {
    ...
    <bk-dialog
      ...
      <div class="update-alert-layout">
        ...
        <i18n path="服务模板创建成功，您可以在XXX" tag="p" class="next-content">
          <template #link1>
            <bk-link theme="primary" @click="handleToSetTemplate">{{
              $t('集群模板')
            }}</bk-link>
          </template>
          <template #link2>
            <bk-link theme="primary" @click="handleToBusinessTopo">{{
              $t('业务拓扑')
            }}</bk-link>
          </template>
        </i18n>
