    ...
    <bk-table
      ...
      <bk-table-column
        ...
        <template slot-scope="{ row }">
          <cmdb-auth
            v-show="row !== rowInEdit"
            class="cell-name"
            :auth="{
              type: $OPERATION.U_CLOUD_AREA,
              relation: [row.bk_cloud_id],
            }"
            :class="{
            ...
            <template slot-scope="{ disabled }">
              <span v-bk-overflow-tips class="cell-name-text">{{
                row.bk_cloud_name
              }}</span>
              <i
                v-if="!isLimited(row)"
      ...
      <bk-table-column label="VPC" prop="bk_vpc_name" show-overflow-tooltip>
        <template slot-scope="{ row }">{{
          getVpcInfo(row) | formatter('singlechar')
        }}</template>
      </bk-table-column>
      <bk-table-column
        :label="$t('主机数量')"
...
export default {
  ...
  data() {
    return {
      ...
      debounceUpdateName: debounce(this.handleUpdateName, 100, {
        leading: true,
        trailing: false,
      }),
      rowInEdit: null,
  ...
  methods: {
    handleEditName(row) {
      ...
      this.$nextTick(() => {
        const input = this.$el
          .querySelector(`#input-${row.bk_cloud_id}`)
          .querySelector('input')
        input.focus()
      })
    },
    ...
    handleDelete(row) {
      const infoInstance = this.$bkInfo({
        ...
        confirmFn: async () => {
          try {
            await this.$store.dispatch('cloud/area/delete', {
              id: row.bk_cloud_id,
            })
    ...
    async getHostCount() {
      try {
        ...
        results.forEach(result => {
          result.forEach(data => {
            const row =
              this.list.find(row => row.bk_cloud_id === data.bk_cloud_id) || {}
