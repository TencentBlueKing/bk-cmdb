...
<template>
  <div v-show="show" class="table-layout">
    <div
      class="table-title"
      ...
      <i v-if="localExpanded" class="title-icon bk-icon icon-down-shape"></i>
      <i v-else class="title-icon bk-icon icon-right-shape"></i>
      <template v-if="!instance.editing.name">
        <span v-bk-overflow-tips class="title-label">{{ instance.name }}</span>
        <cmdb-dot-menu
          v-if="!readonly"
          ref="dotMenu"
          class="instance-menu"
        ...
        <div class="instance-label clearfix" @click.stop>
          <div class="label-list fl">
            ...
            <bk-popover
              ...
              class="label-item label-tips"
              theme="light label-tips"
              :width="290"
              placement="bottom-end">
              <span>...</span>
              <div slot="content" class="tips-label-list">
                <span
    ...
    <bk-table
      ...
      <template slot="empty">
        <span v-if="instance.service_template_id" class="process-count-tips">
        ...
        <span v-else class="process-count-tips">
          <i class="tips-icon bk-icon icon-exclamation-circle"></i>
...
export default {
  ...
  methods: {
    ...
    async createSubmitHandler(values) {
      try {
        await this.$store.dispatch(
          'processInstance/createServiceInstanceProcess',
          {
            params: {
              bk_biz_id: this.bizId,
              service_instance_id: this.instance.id,
    ...
    async editSubmitHandler(values, changedValues, instance) {
      try {
        await this.$store.dispatch(
          'processInstance/updateServiceInstanceProcess',
          {
            params: {
              bk_biz_id: this.bizId,
    ...
    handleDelete(row) {
      this.$bkInfo({
        ...
        confirmFn: async () => {
          try {
            await this.$store.dispatch(
              'processInstance/deleteServiceInstanceProcess',
              {
                config: {
                  data: {
                    bk_biz_id: this.bizId,
