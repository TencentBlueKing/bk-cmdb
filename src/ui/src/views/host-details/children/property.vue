...
<template>
  <div class="property">
    <div v-for="(group, index) in groupedProperties" :key="index" class="group">
      ...
      <ul class="property-list">
        <li
          ...
          <span v-bk-overflow-tips class="property-name">
            {{ property.bk_property_name }}
          </span>
          <template v-if="!readonly">
            <span
              v-if="hasRelatedRules(property) || !isPropertyEditable(property)"
              :id="`rule-${property.id}`">
              <i18n
                v-if="hasRelatedRules(property)"
                path="已配置属性自动应用提示">
                <template #link>
                  <bk-button text @click="handleViewRules(property)">{{
                    $t('点击跳转查看配置详情')
                  }}</bk-button>
                </template>
              </i18n>
          ...
          <template
            ...
            <template v-if="!loadingState.includes(property)">
              <template v-if="!readonly">
                <template
                  v-if="
                    hasRelatedRules(property) || !isPropertyEditable(property)
                  ">
                  <i
                    v-bk-tooltips="{
                      allowHtml: true,
                    ...
                    class="is-related property-edit icon-cc-edit">
                  </i>
                </template>
                <template v-else>
                  <cmdb-auth
    ...
    <div
      ...
      <ul class="property-list">
        <li
          ...
          <template
            v-if="!$tools.isEmptyPropertyValue(node[property.bk_property_id])">
            <div class="copy-box">
              <i
                class="property-copy icon-cc-details-copy"
                @click="handleCopy(property.id)"></i>
...
export default {
  ...
  computed: {
    ...
    host() {
      return this.$tools.getInstFormValues(
        this.properties,
        this.info.host,
        false
      )
  ...
  methods: {
    ...
    handleViewRules(property) {
      const rule =
        this.hostRelatedRules.find(
          rule => rule.bk_attribute_id === property.id
        ) || {}
    ...
    hasRelatedRules(property) {
      return this.hostRelatedRules.some(
        rule => rule.bk_attribute_id === property.id
      )
    },
    async getHostRelatedRules() {
      try {
        ...
        if (defaultType) {
          // 为0时为主机池未分配主机，不存在属性自动应用
          this.hostRelatedRules = []
        } else {
          const data = await this.$store.dispatch(
            'hostApply/getHostRelatedRules',
            {
    ...
    isPropertyEditable(property) {
      ...
      const isCloudHost =
        property.bk_property_id === 'bk_host_outerip' &&
        this.host.bk_cloud_inst_id
