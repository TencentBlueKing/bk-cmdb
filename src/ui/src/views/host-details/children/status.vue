...
<template>
  <div
    v-bkloading="{ isLoading: $loading('getHostSnapshot') }"
    :class="['status', { 'is-offline': !snapshot }]">
    <div v-if="snapshot && !$loading('getHostSnapshot')" class="status-info">
      <h2 class="info-title">
        <span>{{ $t('基本值') }}</span>
      ...
      <ul class="info-list clearfix">
        <li class="info-item fl">
          ...
          <span class="item-value"
            >{{ (snapshot.rcvRate / 100).toFixed(2) }}Mb/s</span
          >
        </li>
        <li class="info-item fl">
          <span class="item-label">{{ $t('启动时间') }}：</span>
          <span class="item-value">{{
            $tools.formatTime(snapshot.bootTime * 1000)
          }}</span>
        </li>
        <li class="info-item fl">
          <span class="item-label">{{ $t('总流出量') }}：</span>
          <span class="item-value"
            >{{ (snapshot.sendRate / 100).toFixed(2) }}Mb/s</span
          >
        </li>
        <li class="info-item fl">
        ...
        <li class="info-item fl">
          ...
          <span class="item-value"
            >{{ (snapshot.Mem / 1024).toFixed(2) }}GB</span
          >
        </li>
        <li v-if="!isWindows" class="info-item fl">
    ...
    <div v-else-if="!$loading('getHostSnapshot')" class="status-offline">
