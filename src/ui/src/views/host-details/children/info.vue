    ...
    <div class="info-topology">
      <div class="topology-label">
        ...
        <span
          ...
          <i
            v-if="isSingleColumn"
            class="topology-toggle icon-cc-single-column"
            @click="toggleDisplayType"></i>
          <i
            v-else
            class="topology-toggle icon-cc-double-column"
            @click="toggleDisplayType"></i>
        </span>
      ...
      <ul
        ...
        <li
          ...
          :key="index"
          :class="['topology-item', { 'is-internal': item.isInternal }]">
          <span
            v-bk-overflow-tips
            class="topology-path"
          ...
          <template v-if="!item.isContainer">
            <cmdb-auth
              ...
              <i
                ...
                class="topology-remove-trigger icon-cc-tips-close"
      ...
      <a
        v-if="showMore"
        class="action-btn view-all"
        href="javascript:void(0)"
        @click="viewAll">
        {{ $t('更多信息') }}
        <i
          class="bk-icon icon-angle-down"
          :class="{ 'is-all-show': showAll }"></i>
      </a>
      <a
        v-if="isBusinessHost && !readonly"
        class="action-btn change-topology"
        href="javascript:void(0)"
...
export default {
  ...
  data() {
    return {
      displayType:
        window.localStorage.getItem('host_topology_display_type') || 'double',
      showAll: false,
      topoNodesPath: [],
      dialog: {
        show: false,
  ...
  computed: {
    ...
    hostIp() {
      if (Object.keys(this.host).length) {
        const hostList = this.host.bk_host_innerip
          ? this.host.bk_host_innerip.split(',')
          : this.host.bk_host_innerip_v6.split(',')
    ...
    cloudArea() {
      return (this.host.bk_cloud_id || [])
        .map(
          cloud =>
            `${this.$t('管控区域')}：${cloud.bk_inst_name} (ID：${
              cloud.bk_inst_id
            })`
        )
        .join('\n')
    ...
    topologyList() {
      ...
      const normalTopoPaths = this.topoNodesPath
        .map(item => {
          ...
          return {
            id: instId,
            path: paths?.map(node => node.bk_inst_name).join(' / '),
            isInternal: module && module.default !== 0,
          }
        })
        .sort((itemA, itemB) =>
          itemA.path.localeCompare(itemB.path, 'zh-Hans-CN', {
            sensitivity: 'accent',
          })
        )

      const containerTopoPaths = this.containerTopoPaths.find(
        topo => topo.bk_host_id === this.host.bk_host_id
      )?.paths
  ...
  methods: {
    ...
    async getContainerTopoPaths() {
      const result = await containerHostService.getNodePath(
        { ids: [this.host.bk_host_id] },
        {
          globalError: false,
        }
      )
    ...
    getListStyle(items) {
      const itemHeight = 21
      const itemMargin = 9
      const length = this.isSingleColumn
        ? items.length
    ...
    handleDialogConfirm() {
      ...
      if (tabName !== 'acrossBusiness') {
        if (moduleType === 'idle') {
          const isAllIdleSetHost = this.modules.every(
            module => module.default !== 0
          )
          if (isAllIdleSetHost) {
