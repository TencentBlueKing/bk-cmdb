...
<template>
  <div class="new-association">
    ...
    <div class="association-filter clearfix">
      ...
      <bk-button theme="primary" class="btn-search fr" @click="search">{{
        $t('搜索')
      }}</bk-button>
    </div>
    <bk-table
      v-bkloading="{ isLoading: $loading('get_relation_inst') }"
      class="new-association-table"
    ...
    <div v-show="confirm.id" ref="confirmTips" class="confirm-tips">
      <p class="tips-content">{{ $t('更新确认') }}</p>
      <div class="tips-option">
        <bk-button class="tips-button" theme="primary" @click="confirmUpdate">{{
          $t('确认')
        }}</bk-button>
        <bk-button class="tips-button" theme="default" @click="cancelUpdate">{{
          $t('取消')
        }}</bk-button>
      </div>
    </div>
...
export default {
  ...
  async created() {
    await Promise.all([this.getAssociationType(), this.getObjAssociation()])
  ...
  methods: {
      ...
      switch (this.currentAsstObj) {
        ...
        case BUILTIN_MODELS.BUSINESS_SET: {
          auth.push({
            ...
            relation: [
              row[BUILTIN_MODEL_PROPERTY_KEYS[BUILTIN_MODELS.BUSINESS_SET].ID],
            ],
          })
          break
        }
        case BUILTIN_MODELS.HOST: {
          const originalData = this.table.originalList.find(
            data => data.host.bk_host_id === row.bk_host_id
          )
    ...
    getObjAssociation() {
      return Promise.all([
        ...
        mainLineModels = mainLineModels.filter(
          model => !['biz', 'host'].includes(model.bk_obj_id)
        )
        dataAsSource = this.getAvailableAssociation(
          dataAsSource,
          mainLineModels
        )
        dataAsTarget = this.getAvailableAssociation(
          dataAsTarget,
          mainLineModels
        )
    ...
    getAvailableAssociation(data, mainLine) {
      ...
      return data.filter(
        relation =>
          !mainLine.some(model =>
            [relation.bk_obj_id, relation.bk_asst_obj_id].includes(
              model.bk_obj_id
            )
          )
      )
    ...
    setAssociationOptions() {
      const options = this.associationObject.map(option => {
        ...
        const type = this.associationType.find(
          type => type.bk_asst_id === option.bk_asst_id
        )
        ...
        return {
          ...
          _label: `${isSource ? type.src_des : type.dest_des}-${
            model.bk_obj_name
          }`,
        }
      })
      ...
      // eslint-disable-next-line no-underscore-dangle
      this.options = uniqueLabel.map(label =>
        options.find(option => option._label === label)
      )
    },
    async handleSelectObj(asstId, option) {
      ...
      this.currentAsstObj =
        option.bk_obj_id === this.objId
          ? option.bk_asst_obj_id
          : option.bk_obj_id
    ...
    async getInstance() {
      ...
      promise
        .then(data => {
          this.table.stuff.type = !isEmptyPropertyValue(this.filter.value)
            ? 'search'
            : 'default'
    ...
    getHostInstance(config) {
      ...
      const hostParams = {
        ...
        ip: {
          flag: ipFields.includes(filter.id)
            ? filter.id
            : 'bk_host_innerip|bk_host_outerip',
          exact: 0,
          data:
            ipFields.includes(filter.id) && filter.value.length
              ? filter.value.split(',')
              : [],
        },
        page: this.page,
    ...
    getBizInstance(config) {
      ...
      if (!isEmptyPropertyValue(this.filter.value)) {
        params.condition[this.filter.id] = formatValue(
          this.filter.value,
          property
        )
      }
    ...
    getBizSetInstance(config) {
      ...
      // eslint-disable-next-line max-len
      const { conditions, time_condition: timeCondition } =
    ...
    getProperty(propertyId) {
      return this.properties.find(
        ({ bk_property_id: bkPropertyId }) => bkPropertyId === propertyId
      )
    },
