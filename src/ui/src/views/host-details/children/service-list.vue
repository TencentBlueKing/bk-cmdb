...
export default {
  ...
  watch: {
    checked() {
      this.isCheckAll =
        this.checked.length === this.instances.length &&
  ...
  methods: {
    async getHostSeriveInstances() {
      try {
        const searchKey = this.searchSelectData.find(
          item =>
            (item.id === 0 && has(item, 'values')) ||
        ...
        const data = await hostServiceInstancesProxy({
          ...
          search_key: searchKey
            ? has(searchKey, 'values')
              ? searchKey.values[0].name
              : searchKey.name
            : '',
        ...
        this.instances = data.info.map(instance => ({
          ...instance,
          editing: { name: false },
        }))
      } catch (e) {
    ...
    getSelectorParams() {
      try {
        const labels = this.searchSelectData.filter(
          item => item.id === 1 && has(item, 'values')
        )
        const labelsKey = this.searchSelectData.filter(
          item => item.id === 2 && has(item, 'values')
        )
        const submitLabel = {}
    ...
    handleSearch() {
      const instanceName = this.searchSelectData.filter(
        item =>
          (item.id === 0 && has(item, 'values')) ||
          (![0, 1].includes(item.id) && !has(item, 'values'))
      )
      if (instanceName.length) {
        ...
        this.searchSelect[0].id !== 0 &&
          this.searchSelect.unshift({
            name: this.$t('服务实例名'),
            id: 0,
          })
      }
      ...
      this.emptyStuff.type =
        this.searchSelectData.length === 0 ? 'default' : 'search'
