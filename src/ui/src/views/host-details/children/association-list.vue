...
<template>
  <div v-bkloading="{ isLoading: loading }" class="association-list">
    <div v-if="!hasAssociation" class="association-empty">
      <div class="empty-content">
        <cmdb-data-empty slot="empty" :stuff="dataEmpty"> </cmdb-data-empty>
      </div>
    </div>
    <template v-else>
...
export default {
    ...
    list() {
      try {
        ...
        associations.forEach((association, index) => {
          ...
          const modelId = isSource
            ? association.bk_asst_obj_id
            : association.bk_obj_id
          const associationType =
            this.associationTypes.find(
              item => item.bk_asst_id === association.bk_asst_id
            ) || {}
          ...
          list.push({
            ...
            instances: this.allInstances.filter(item => {
              const sameType =
                item.bk_asst_id === association.bk_asst_id && item.type === type
    ...
    list() {
      this.$nextTick(() => {
        if (this.$refs.associationListTable) {
          ...
          firstAssociationListTable &&
            (firstAssociationListTable.expanded = true)
        }
      })
    },
  },
  ...
  methods: {
    async getData() {
      try {
        const [source, target, mainLine, associationTypes, instances] =
          await Promise.all([
        ...
        const mainLineModels = mainLine.filter(
          model => !['biz', 'host'].includes(model.bk_obj_id)
        )
        const availabelSource = this.getAvailableAssociation(
          source,
          [],
          mainLineModels
        )
        const availabelTarget = this.getAvailableAssociation(
          target,
          [],
          mainLineModels
        )
    ...
    setState({ source, target, mainLine, associationTypes, instances }) {
      this.$store.commit('hostDetails/setAssociation', {
        type: 'source',
        association: source,
      })
      this.$store.commit('hostDetails/setAssociation', {
    ...
    async getAssociationType() {
      const { info } = await this.$store.dispatch(
        'objectAssociation/searchAssociationType',
        {},
        {
          config: {
    ...
    getAvailableAssociation(data, reference = [], mainLine = []) {
      return data.filter(association => {
        ...
        const isMainLine = mainLine.some(model =>
          [sourceId, targetId].includes(model.bk_obj_id)
        )
