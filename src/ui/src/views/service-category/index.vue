...
<template>
  <div
    ...
    <cmdb-tips class="mb10" tips-key="categoryTips">{{
      $t('服务分类功能提示')
    }}</cmdb-tips>
    ...
    <div class="category-list">
      <div
        v-for="(mainCategory, index) in displayList"
        :key="index"
        :class="[
          'category-item',
          'bgc-white',
          { editing: editMainStatus === mainCategory['id'] },
        ]">
        <div
          class="category-title"
          :style="{
            'background-color':
              editMainStatus === mainCategory['id'] ? '#f0f1f5' : '',
          }">
          <div
            ...
            class="main-edit"
            :style="{
              width: editMainStatus === mainCategory['id'] ? '100%' : 'auto',
            }">
            <category-input
              ref="editInput"
          ...
          <template v-else>
            <div class="category-name">
              <template v-if="mainCategory['is_built_in']">
                <div class="category-name-text is-built-in">
                  <div class="text-inner">
                    <span class="main-name" :title="mainCategory.name">{{
                      mainCategory.name
                    }}</span>
              ...
              <cmdb-auth
                ...
                :auth="{
                  type: $OPERATION.U_SERVICE_CATEGORY,
                  relation: [bizId],
                }">
                <div
                  slot-scope="{ disabled }"
                  :class="['category-name-text', { disabled }]">
                  <div
                    class="text-inner"
                    @click.stop="
                      handleEditMain(mainCategory['id'], mainCategory['name'])
                    ">
                    <span class="main-name" :title="mainCategory.name">{{
                      mainCategory.name
                    }}</span>
            ...
            <div v-if="!mainCategory['is_built_in']" class="menu-operational">
              <cmdb-auth
                :auth="{
                  type: $OPERATION.C_SERVICE_CATEGORY,
                  relation: [bizId],
                }"
              ...
              <cmdb-auth
                :auth="{
                  type: $OPERATION.D_SERVICE_CATEGORY,
                  relation: [bizId],
                }">
                <template slot-scope="{ disabled }">
                  <bk-button
                    v-if="
                      disabled || !mainCategory['child_category_list'].length
                    "
                    v-show="isMainAuthCompleted"
                    class="menu-btn"
                    :text="true"
                    :disabled="disabled"
                    @click="
                      handleDeleteCategory(mainCategory['id'], 'main', index)
                    ">
                    <i class="bk-cmdb-icon icon-cc-del"></i>
                  </bk-button>
                  <span
                    v-else
                    v-bk-tooltips="deleteBtnTips"
                    class="menu-btn no-allow-btn">
                    <i class="bk-cmdb-icon icon-cc-del"></i>
                  </span>
                </template>
              </cmdb-auth>
        ...
        <div v-test-id="'childCategory'" class="child-category">
          <div
            v-for="(childCategory, childIndex) in mainCategory[
              'child_category_list'
            ]"
            :key="childIndex"
            v-test-id="'childEdit'"
            ...
            <template v-else>
              <div class="child-title">
                <span :title="childCategory['name']">{{
                  childCategory['name']
                }}</span>
                <span class="child-id" :title="childCategory['id']">{{
                  childCategory['id']
                }}</span>
                <div
                  v-if="!childCategory['is_built_in']"
                  v-test-id="'childEdit'"
                  class="child-edit">
                  <cmdb-auth
                    class="mr10"
          ...
          <div
            v-if="!mainCategory['is_built_in'] && !addChildStatus"
            class="child-item is-add">
            <div class="child-title">
              <cmdb-auth
                :auth="{
                  type: $OPERATION.C_SERVICE_CATEGORY,
          ...
          <div
            v-if="addChildStatus === mainCategory['id']"
            v-test-id="'childEdit'"
            class="child-item child-edit">
            <category-input
              ref="editInput"
      ...
      <div
        ...
        <div
          class="category-title"
          :style="{
            'border-bottom-style': showAddMianCategory ? 'solid' : 'dashed',
          }">
          <div
...
export default {
  ...
  methods: {
    ...
    getCategoryList() {
      this.searchServiceCategory({
        ...
        const list = categoryList.filter(
          category =>
            !category.bk_parent_id &&
            !(category.name === 'Default' && category.is_built_in)
        )
        this.list = list
          .map(mainCategory => ({
            ...mainCategory,
            child_category_list: categoryList.filter(
              category => category.bk_parent_id === mainCategory.id
            ),
          }))
      ...
      if (this.keyword) {
        ...
        this.displayList = this.list.filter(mainCategory => {
          ...
          const index = childList.findIndex(
            category => reg.test(category.name) || reg.test(category.id)
          )
          return index !== -1
        })
      } else {
    ...
    handleCategoryTipsToggle(tipsInstance) {
      ...
      tipsInstance.reference.parentElement.classList[
        willShow ? 'add' : 'remove'
      ]('tips-active')
    },
    async handleAddCategory(name, bk_root_id = 0) {
      if (!(await this.$validator.validateAll())) {
        this.$bkMessage({
          message:
            this.errors.first('categoryName') || this.$t('请输入分类名称'),
          theme: 'error',
        })
      } else {
    ...
    async handleEditCategory(data, type, mainIndex) {
      if (!(await this.$validator.validateAll())) {
        ...
        data.name === this.mainCategoryName ||
        ...
        this.updateServiceCategory({
          params: {
            ...
            id: data.id,
            name:
              type === 'main' ? this.mainCategoryName : this.childCategoryName,
          },
        })
          .then(res => {
            this.$success(this.$t('保存成功'))
            ...
            if (mainIndex !== undefined && type === 'child') {
              const childList = this.list[mainIndex].child_category_list.map(
                child => {
                  if (child.id === res.id) {
                    return res
                  }
        ...
        confirmFn: async () => {
          await this.deleteServiceCategory({
            ...
            if (type === 'main') {
              ...
              let childIndex = -1
              this.list[index].child_category_list.find(
                (category, findIndex) => {
