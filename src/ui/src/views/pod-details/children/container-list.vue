...
export default defineComponent({
  ...
  setup() {
    ...
    const MODEL_NAME_KEY =
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.CONTAINER].NAME
    const MODEL_FULL_NAME_KEY =
      CONTAINER_OBJECT_INST_KEYS[CONTAINER_OBJECTS.CONTAINER].FULL_NAME
    ...
    const columnsConfig = reactive({
      disabledColumns: [MODEL_ID_KEY, MODEL_NAME_KEY, 'container_uid'],
    })
    ...
    const handleCopy = column => {
      const copyText = table.selection.map(row =>
        getPropertyCopyValue(row[column.id], column.property)
      )
