...
<template>
  <div class="relation-wrapper">
    ...
    <bk-table
      ...
      <bk-table-column fixed="right" prop="operation" :label="$t('操作')">
        <template slot-scope="{ row }">
          <cmdb-auth
            v-bk-tooltips="{
  ...
  methods: {
    ...
    async searchUsageCount() {
      ...
      this.table.list.forEach(item => {
        const asst = res.associations.find(
          ({ bk_asst_id: asstId }) => asstId === item.bk_asst_id
        )
        if (asst) {
    ...
    deleteRelation(relation) {
      this.$bkInfo({
        title: this.$tc('确定删除关联类型？', relation.bk_asst_name, {
          name: relation.bk_asst_name,
        }),
        confirmFn: async () => {
