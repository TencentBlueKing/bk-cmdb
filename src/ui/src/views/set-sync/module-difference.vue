        ...
        <div class="col before-col">
          <div class="set-tree">
            <div class="node-root">
              ...
              <div :title="setDetail.bk_set_name" class="root-name">
                {{ setDetail.bk_set_name }}
              </div>
            </div>
            <ul class="node-children">
              <li
                v-for="(node, index) in beforeList"
                :key="index"
                class="node-child">
                <i class="node-icon">{{ moduleName[0] }}</i>
                <div v-bk-overflow-tips class="module-name">
                  {{ node.bk_module_name }}
                </div>
              </li>
            </ul>
          ...
          <div class="set-tree">
            ...
            <ul class="node-children">
              <li
                ...
                <div
                  v-if="
                    node.diff_type === 'remove' && existHost(node.bk_module_id)
                  "
                  class="tips">
...
export default {
  ...
  computed: {
    beforeList() {
      return this.moduleDiff.module_diffs.filter(
        module => module.diff_type !== 'add'
      )
    },
    afterList() {
