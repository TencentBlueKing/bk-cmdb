...
<template>
  <div
    v-bkloading="{ isLoading: $loading([request.create, request.update]) }"
    class="create-layout">
    <bk-form form-type="vertical">
      <bk-form-item class="create-form-item" :label="$t('账户名称')" required>
        <bk-input
          ...
          class="create-form-meta"
          :placeholder="$t('请输入xx', { name: $t('账户名称') })"
          :data-vv-as="$t('账户名称')"
          data-vv-name="bk_account_name">
        </bk-input>
        <p v-if="errors.has('bk_account_name')" class="create-form-error">
          {{ errors.first('bk_account_name') }}
        </p>
      </bk-form-item>
      <bk-form-item class="create-form-item" :label="$t('账户类型')" required>
        <bk-select
          ...
          <cmdb-vendor
            slot="trigger"
            class="vendor-selector-trigger"
            :type="form.bk_cloud_vendor"
            empty-text="">
          </cmdb-vendor>
        </bk-select>
        <p v-if="errors.has('bk_cloud_vendor')" class="create-form-error">
      ...
      <bk-form-item class="create-form-item clearfix" label="Key" required>
        ...
        <template v-if="verifyResult.hasOwnProperty('done')">
          <p v-if="verifyResult.connected" class="create-verify-success">
            <i class="bk-icon icon-check-circle-shape"></i>
            {{ $t('账户连通成功') }}
          </p>
          <p v-else class="create-verify-fail">
            <i class="bk-icon icon-close-circle-shape"></i>
            {{ verifyResult.msg }}
          </p>
        </template>
        <p v-else-if="errors.has('bk_secret_key')" class="create-form-error">
      ...
      <bk-form-item class="create-form-item" :label="$t('备注')">
        ...
        <p v-if="errors.has('bk_description')" class="create-form-error">
          {{ errors.first('bk_description') }}
        </p>
      </bk-form-item>
      <div class="create-form-options">
        ...
        <bk-button theme="default" @click.stop.prevent="handleCancel(null)">{{
          $t('取消')
        }}</bk-button>
      </div>
    </bk-form>
...
export default {
  ...
  computed: {
    ...
    verifyRequired() {
      const changed = ['bk_cloud_vendor', 'bk_secret_id', 'bk_secret_key'].some(
        key =>
          this.verifyResult[key] && this.form[key] !== this.verifyResult[key]
      )
      return changed || !this.verifyResult.connected
    },
    vendors() {
      const vendorProperty = this.properties.find(
        property => property.bk_property_id === 'bk_cloud_vendor'
      )
      if (vendorProperty) {
    ...
    hasChange() {
      ...
      return Object.keys(this.form).some(
        key => this.form[key] !== this.account[key]
      )
    },
    auth() {
      if (this.isCreateMode) {
  ...
  methods: {
    async getCloudAreaProperties() {
      try {
        this.properties = await this.$store.dispatch(
    ...
    changeValue() {
      ...
      const originForm = {}
      Object.keys(DEFAULT_FORM).forEach(
        key => (originForm[key] = this.account[key])
      )
