  ...
  <div class="cloud-account-layout">
    ...
    <bk-table
      ...
      <bk-table-column :label="$t('操作')">
        <template slot-scope="{ row }">
          <link-button class="mr10" @click="handleView(row)">{{
            $t('查看')
          }}</link-button>
          <cmdb-auth
            :auth="{
              type: $OPERATION.D_CLOUD_ACCOUNT,
              relation: [row.bk_account_id],
            }">
            <link-button
              slot-scope="{ disabled }"
              v-bk-tooltips="{
    ...
    <account-sideslider
      ref="accountSideslider"
      @request-refresh="getData"></account-sideslider>
  </div>
</template>
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        this.list = data.info.map(account => ({
          ...account,
          pending: true,
          status: 'normal',
          error_message: '',
      ...
      try {
        ...
        this.list.forEach(account => {
          const status = results.find(
            result => result.bk_account_id === account.bk_account_id
          )
