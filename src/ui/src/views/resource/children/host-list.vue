  ...
  <div class="resource-layout">
    ...
    <bk-table
      ...
      <bk-table-column
        ...
        :class-name="
          ['bk_host_id'].includes(property.bk_property_id) ? 'is-highlight' : ''
        "
        ...
        <template slot-scope="{ row }">
          ...
          <cmdb-property-value
            ...
            :theme="
              ['bk_host_id'].includes(property.bk_property_id)
                ? 'primary'
                : 'default'
            "
            :value="
              row | hostValueFilter(property.bk_obj_id, property.bk_property_id)
            "
            :show-unit="false"
            :property="property"
...
export default {
  ...
  async created() {
    try {
      ...
      this.unwatchScopeAndDirectory = RouterQuery.watch(
        ['scope', 'directory'],
  ...
  methods: {
    ...
    setModuleNamePropertyState() {
      const property = this.moduleProperties.find(
        property => property.bk_property_id === 'bk_module_name'
      )
      if (property) {
    ...
    injectScope(params) {
      const biz = params.condition.find(
        condition => condition.bk_obj_id === 'biz'
      )
      if (this.scope === 'all') {
        biz.condition = biz.condition.filter(
          condition => condition.field !== 'default'
        )
      } else {
        const newMeta = {
          field: 'default',
        ...
        const existMeta = biz.condition.find(
          ({ field, operator }) =>
            field === newMeta.field && operator === newMeta.operator
        )
        if (existMeta) {
    ...
    injectDirectory(params) {
      ...
      const moduleCondition = params.condition.find(
        condition => condition.bk_obj_id === 'module'
      )
      const directoryMeta = {
        field: 'bk_module_id',
        operator: '$eq',
      ...
      // eslint-disable-next-line max-len
      const existMeta = moduleCondition.condition.find(
        ({ field, operator }) =>
          field === directoryMeta.field && operator === directoryMeta.operator
      )
      if (existMeta) {
    ...
    handleValueClick(item, property) {
      if (
        ...
        property.bk_property_id !== 'bk_host_id'
      ) {
        return
      }
