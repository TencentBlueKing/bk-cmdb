...
<template>
  <div class="directory-layout">
    ...
    <ul ref="dirList" class="dir-list">
      ...
      <li
        v-if="createInfo.active"
        ref="createDirItem"
        class="dir-item edit-status">
        <bk-input
          ref="createdDir"
      ...
      <li
        ...
        <template v-else>
          ...
          <span class="dir-name" :title="dir.bk_module_name">{{
            dir.bk_module_name
          }}</span>
          <template v-if="dir.default !== 1">
            <i
              ...
              :class="[
                'dir-sticky-icon',
                isSticky(dir) ? 'icon-cc-cancel-sticky' : 'icon-cc-sticky',
              ]"
            ...
            <cmdb-dot-menu
              ...
              <div class="dot-content">
                <cmdb-auth
                  :auth="{
                    type: $OPERATION.U_RESOURCE_DIRECTORY,
                    relation: [dir.bk_module_id],
                  }">
                  <bk-button
                    slot-scope="{ disabled }"
                    class="menu-btn"
                ...
                <cmdb-auth
                  :auth="{
                    type: $OPERATION.D_RESOURCE_DIRECTORY,
                    relation: [dir.bk_module_id],
                  }">
                  <div
                    slot-scope="{ disabled }"
      ...
      <cmdb-data-empty
        v-if="filterDirList.length === 0 && isShowEmpty"
        slot="empty"
        :stuff="dataEmpty"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('resourceHost', ['directoryList', 'directorySortedList']),
    stickyDirectory() {
    ...
    filterDirList() {
      ...
      if (this.dirSearch) {
        ...
        list = this.directoryList.filter(
          module =>
            module.bk_module_name.toLowerCase().indexOf(lowerCaseSearch) > -1
        )
      }
      return this.directorySortedList(this.stickyDirectory, list)
    },
    totalCount() {
      return this.directoryList.reduce(
        (accumulator, directory) => accumulator + directory.host_count,
        0
      )
    },
  },
  ...
  methods: {
    async getDirectoryList() {
      try {
        ...
        if (directoryId) {
          ...
          const directory = info.find(
            directory => directory.bk_module_id === directoryId
          )
    ...
    async handleToggleSticky(dir) {
      try {
        ...
        const current = isSticky
          ? previous.filter(id => id !== dir.bk_module_id)
          : [...previous, dir.bk_module_id]
    ...
    async updateDir() {
      try {
        ...
        const target = this.directoryList.find(
          dir => dir.bk_module_id === this.editDir.id
        )
        this.$store.commit(
          'resourceHost/updateDirectory',
    ...
    handleShowCreate() {
      ...
      this.$nextTick(() => {
        ...
        const idleNextItem =
          this.$refs.dirList.querySelector(
            '[data-default="1"]'
          ).nextElementSibling
...
.directory-layout {
  ...
  .dir-item {
    ...
    &:not(.edit-status, .disabled):hover,
