...
<template>
  <div class="options-layout clearfix">
    <div class="options-left fl">
      ...
      <span
        v-bk-tooltips="{
          content: this.$t('仅主机池主机可进行此操作'),
          disabled: isAllResourceHost,
        }"
        ...
        <bk-select
          ...
          <bk-option
            ...
            :name="
              $t('业务空闲机', {
                idleSet: $store.state.globalConfig.config.set,
              })
            "></bk-option>
          <bk-option id="toDirs" :name="$t('主机池其他目录')"></bk-option>
        </bk-select>
      </span>
      <cmdb-transfer-menu v-if="scope !== 1" class="mr10" />
      <cmdb-button-group
        class="mr10"
        :trigger-text="$t('编辑')"
      ...
      <cmdb-button-group class="mr10" :buttons="buttons" :expand="false">
      </cmdb-button-group>
    </div>
    <div class="options-right">
      <filter-fast-search
        class="option-fast-search"
        @search="searchFilter"></filter-fast-search>
      <icon-button
    ...
    <bk-sideslider
      ...
      <div
        slot="content"
        v-bkloading="{ isLoading: $loading(Object.values(slider.request)) }"
        style="height: 100%">
        <component
          :is="slider.component"
          ref="multipleForm"
    ...
    <bk-dialog
      ...
      <div v-if="assign.show" class="assign-content">
        <i18n class="assign-count" tag="div" path="已选择主机">
          <template #count
            ><span>{{ table.checked.length }}</span></template
          >
        </i18n>
      ...
      <div slot="footer" class="assign-footer">
        <bk-button
          class="mr10"
          theme="primary"
          :disabled="assign.id === ''"
...
export default {
  ...
  computed: {
    ...
    buttons() {
      ...
      // 已分配并且是容器搜索模式
      if (
        this?.$parent?.isResourceAssigned &&
        this?.$parent?.isContainerSearchMode
      ) {
      ...
      const buttonConfig = [
        {
          ...
          disabled: !this.table.checked.length,
          tooltips: {
            content: this.$t('请先勾选需要编辑的主机'),
            disabled: this.table.checked.length > 0,
          },
        },
        {
          id: 'import-edit',
  ...
  async created() {
    try {
      this.unwatchScope = RouterQuery.watch(
        'scope',
        (scope = 1) => {
  ...
  methods: {
    ...
    setAssignOptions(directoryIdList) {
      if (this.assign.curSelected === 'toBusiness') {
        ...
        const directorySortedList =
          directoryIdList?.length === 1
            ? directoryList.filter(
                item => item.bk_module_id !== directoryIdList[0]
              )
            : directoryList
    ...
    handleConfirmAssign() {
      this.assign.curSelected === 'toBusiness'
        ? this.assignHostsToBusiness()
        : this.changeHostsDir()
    },
    async assignHostsToBusiness() {
      await this.$store
        .dispatch('resourceDirectory/assignHostsToBusiness', {
          params: {
    ...
    handleMultipleDelete() {
      this.$bkInfo({
        ...
        confirmFn: () => {
          this.$store
            .dispatch('hostDelete/deleteHost', {
              params: {
                data: {
                  bk_host_id: this.table.checked.join(','),
      ...
      useExport
        .default({
          ...
          submit: (state, task) => {
            ...
            const params = {
              export_custom_fields: fields.value.map(
                property => property.bk_property_id
              ),
              bk_host_ids: this.table.selection.map(
                ({ host }) => host.bk_host_id
              ),
              export_condition: {
                page: {
                  start: 0,
    ...
    async handleNewImportInst() {
      ...
      const [, { show: showImport, setState: setImportState }] =
        useImport.default()
      setImportState({
        title: this.$t('导入主机'),
        bk_obj_id: 'host',
        ...
        submit: options => {
          ...
          return hostImportService.create({
            file: options.file,
            params,
    ...
    async handleEditImportInst() {
      ...
      setImportState({
        ...
        submit: options => {
          ...
          return hostImportService.update({
            file: options.file,
            params,
            config: options.config,
          })
