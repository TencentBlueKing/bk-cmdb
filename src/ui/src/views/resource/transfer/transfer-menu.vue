  ...
  <div class="transfer-menu">
    <bk-dropdown-menu
      ...
      <ul slot="dropdown-content" class="bk-dropdown-list">
        ...
        <li>
          <a href="javascript:;" @click="transferToBizModule">{{
            $t('业务模块')
          }}</a>
        </li>
        <li>
          <a href="javascript:;" @click="transferToResourcePool">{{
            $t('主机池')
          }}</a>
        </li>
        <li>
          <a
            v-bk-tooltips="{
              ...
              content: $t('仅允许业务下空闲机跨业务转移', {
                idleModule: $store.state.globalConfig.config.idlePool.idle,
              }),
              disabled: !transferToOtherDisabled,
            }"
            :class="{ disabled: transferToOtherDisabled }"
            href="javascript:;"
            @click="transferToOtherBizModule"
            >{{ $t('其他业务') }}</a
          >
        </li>
      </ul>
...
export default {
  ...
  computed: {
    ...
    transferToOtherDisabled() {
      return (
        !HostStore.isAllIdleSet ||
        HostStore.hosts.some(host => host.biz[0].default === 1)
      )
    },
  },
  methods: {
    ...
    transferToOtherBizModule() {
      ...
      const moduleBizs = uniq(
        HostStore.getSelected().map(host => host.biz[0].bk_biz_id)
      )
      const props = {
        moduleType: 'business',
        business: moduleBizs,
        title: this.$t('转移空闲机到其他业务', {
    ...
    async transferDirectly(modules) {
      try {
        ...
        await this.$http.post(
          ...
          {
            bk_host_ids: HostStore.getSelected().map(
              data => data.host.bk_host_id
            ),
    ...
    gotoTransferPage(modules) {
      this.$routerActions.redirect({
        ...
        query: {
          ...
          resources: HostStore.getSelected()
            .map(item => item.host.bk_host_id)
            .join(','),
        },
        history: true,
      })
        ...
        await this.$store.dispatch(
          'hostRelation/transferHostToResourceModule',
          {
            params: {
              ...
              bk_host_id: HostStore.getSelected().map(
                item => item.host.bk_host_id
              ),
    ...
    moveHostToOtherBiz(selectedNode, targetBizId) {
      ...
      HostStore.getSelected().forEach(selectedHost => {
        ...
        const existedResourceHost = resourceHosts.find(
          resourceHost => resourceHost.src_bk_biz_id === hostBizId
        )
      ...
      this.$store
        .dispatch('hostRelation/transferHostToOtherBizModule', {
          params: {
