    ...
    <div class="dynamic-group-table">
      <bk-table
        ...
        <bk-table-column
          ...
          <span slot-scope="{ row }" class="name-text">{{ row.name }}</span>
        </bk-table-column>
        <bk-table-column
          prop="id"
          label="ID"
        ...
        <bk-table-column
          ...
          <template slot-scope="{ row }">
            <cmdb-form-objuser
              :value="row.create_user"
              type="info"></cmdb-form-objuser>
          </template>
        </bk-table-column>
        <bk-table-column
          prop="create_time"
        ...
        <bk-table-column
          ...
          :label="$t('操作')"
          fixed="right"
          :min-width="$i18n.local === 'en' ? 110 : 90">
          <template slot-scope="{ row }">
            <bk-button
              class="mr10"
              :text="true"
...
export default {
  ...
  created() {
    this.unwatchQuery = RouterQuery.watch(
      '*',
  ...
  methods: {
    async getList() {
      try {
        const { info, count } = await this.$store.dispatch(
          'dynamicGroup/search',
          {
            bizId: this.bizId,
            params: {
    ...
    handleCellClick(row, column, cell, event) {
      ...
      if (
        ...
        clickTarget.classList.contains('name-text')
      ) {
        this.handleEdit(row)
      }
    },
    ...
    handleDelete(row) {
      this.$bkInfo({
        ...
        confirmFn: async () => {
          ...
          RouterQuery.set({
            page:
              this.table.list.length > 1 ? currentPage : currentPage - 1 || 1,
            _t: Date.now(),
          })
        },
      })
