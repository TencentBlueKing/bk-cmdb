  ...
  <div class="form-property-list">
    <bk-form form-type="vertical" :label-width="400">
      <bk-form-item
        ...
        <p v-if="errors.has(property.bk_property_id)" class="form-error">
  ...
  methods: {
    ...
    setDetailsCondition() {
      Object.values(this.condition).forEach(condition => {
        ...
        const detailsCondition = this.details.info.condition.find(
          detailsCondition => detailsCondition.bk_obj_id === modelId
        )
        const detailsFieldData = detailsCondition.condition.find(
          data => data.field === propertyId
        )
    ...
    handleOperatorChange(property, operator) {
      if (operator === '$range') {
        ...
        this.condition[property.id].value = Array.isArray(currentValue)
          ? currentValue[0] || ''
          : currentValue
      } else {
        ...
        this.condition[property.id].value = isTypeChanged
          ? defaultValue
          : currentValue
      }
    },
