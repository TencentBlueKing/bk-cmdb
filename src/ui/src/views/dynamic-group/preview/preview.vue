...
export default {
  ...
  methods: {
    ...
    async setPreviewProperties() {
      try {
        const previewProperties = await this.$tools.getDefaultHeaderProperties(
          this.properties
        )

        if (this.details.bk_obj_id === BUILTIN_MODELS.HOST) {
          const innerIPv6 = this.properties.find(
            item => item.bk_property_id === 'bk_host_innerip_v6'
          )
          if (innerIPv6) {
    ...
    async preview() {
      try {
        const { count, info } = await this.$store.dispatch(
          'dynamicGroup/preview',
          {
            bizId: this.bizId,
            id: this.id,
            params: {
    ...
    renderHeader(h, property) {
      if (
        !this.table.pagination.count ||
        property.bk_property_id !== 'bk_host_innerip'
      ) {
        return (
          <div v-bk-overflow-tips class="table-label">
            {this.$tools.getHeaderPropertyName(property)}
          </div>
        )
      }
      const attrs = {
