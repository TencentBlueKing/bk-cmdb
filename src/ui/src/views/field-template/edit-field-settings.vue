...
watchEffect(async () => {
  const [template, templateFieldList, templateUniqueList] = await Promise.all([
    fieldTemplateService.findById(templateId.value, {
      requestId: requestIds.template,
    }),
    fieldTemplateService.getFieldList(
      { bk_template_id: templateId.value },
      { requestId: requestIds.field }
    ),
    fieldTemplateService.getUniqueList(
      { bk_template_id: templateId.value },
      { requestId: requestIds.unique }
    ),
  ])
  ...
  // 如果存在草稿，优先使用
  fieldData.value =
    templateDraft.value.fieldList ?? (templateFieldList?.info || [])
  uniqueData.value =
    templateDraft.value.uniqueList ?? (templateUniqueList?.info || [])
