...
// 草稿数据
const templateDraft = computed(
  () => store.getters['fieldTemplate/templateDraft']
)
const fieldLocalList = computed(
  () => templateDraft.value.fieldList?.map(unwrapData) ?? []
)
...
watchEffect(async () => {
  ...
  basicData.value.description =
    templateDraft.value.basic.description ?? template.description
  bindModelData.value = modelList ?? []
  modelIdList.value = modelList
...
const { uniqueStatus, removedUniqueList } = useUnique(
  uniqueData,
  uniqueLocalList
)
...
const finalFieldList = computed(() =>
  normalizeFieldData(templateData.value.fieldList, false, fieldStatus)
)
// eslint-disable-next-line max-len
const finalUniqueList = computed(() =>
  normalizeUniqueData(
    templateData.value.uniqueList,
    templateData.value.fieldList,
    false,
    uniqueStatus
  )
)
...
const handleSubmit = async () => {
  ...
  try {
    await fieldTemplateService.update(submitData, {
  ...
  <div class="edit-binging">
    <template v-if="!isEditSuccess">
      ...
      <div class="edit-binging-footer">
        <bk-button @click="handlePrevStep">
          {{ $t('上一步') }}
        </bk-button>
        <cmdb-auth
          v-bk-tooltips="{
            disabled: hasModelEditAuth && !hasDiffConflict,
            content: !hasModelEditAuth
              ? $t('暂无对应模型的编辑权限，请先申请模型的编辑权限')
              : $t('模型存在冲突，无法提交'),
          }"
