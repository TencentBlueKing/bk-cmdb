...
const submitButtonDisabled = computed(
  () => !isDraftValid.value || !templateData.value.fieldList?.length
)
...
const handleSubmit = async () => {
  ...
  try {
    const result = await fieldTemplateService.create(submitData, {
...
<template>
  <div class="create-field-settings">
    <cmdb-sticky-layout
      v-if="!isCreateSuccess"
      class="cmdb-config-sticky-layout">
      <template #header="{ sticky }">
        <top-steps
          width="360px"
          :current="2"
          :class="{ 'is-sticky': sticky }"></top-steps>
      </template>
      <field-manage
        ...
        <div :class="['layout-footer', { 'is-sticky': sticky }]">
          <cmdb-auth :auth="{ type: $OPERATION.C_FIELD_TEMPLATE }">
            <template #default="{ disabled }">
              <bk-button :disabled="disabled" @click="handlePrevStep">
                {{ $t('上一步') }}
              </bk-button>
            </template>
          </cmdb-auth>
          ...
          <bk-button :disabled="submitButtonDisabled" @click="handlePreview">
            {{ $t('预览') }}
          </bk-button>
    ...
    <div v-else class="create-success-container">
