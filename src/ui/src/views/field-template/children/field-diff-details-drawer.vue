...
import { MENU_MODEL_FIELD_TEMPLATE } from '@/dictionary/menu-symbol'
...
watchEffect(async () => {
  modelBeforeField.value =
    props.diffType === DIFF_TYPES.NEW
...
const isTemplateBindConflict = computed(
  () =>
    props.diffType === DIFF_TYPES.CONFLICT &&
    props.fieldDiff?.data?.bk_template_id !== 0
)

const isFieldIdSame = computed(
  () => props.afterField.bk_property_id === props.beforeField.bk_property_id
)
const isFieldNameSame = computed(
  () => props.afterField.bk_property_name === props.beforeField.bk_property_name
)
const isFieldTypeSame = computed(
  () => props.afterField.bk_property_type === props.beforeField.bk_property_type
)
...
const diffTitle = computed(() => {
  const titles = {
    new: t('新增字段'),
    update: t('字段配置更新'),
    conflict: isTemplateBindConflict.value
      ? t('字段冲突，该字段已经被其他模板绑定，请删除该模型或修改模板')
      : '',
    unbound: t('不再纳管该字段'),
    unchanged: t('无变化'),
  }
  ...
  if (
    props.diffType === DIFF_TYPES.UPDATE &&
    updateDataKeys.length === 1 &&
    updateDataKeys?.[0] === 'bk_template_id'
  ) {
...
const getDefaultValue = field => {
  ...
  if ([PROPERTY_TYPES.ENUM, PROPERTY_TYPES.ENUMMULTI].includes(fieldType)) {
    const defaultOptions = (field.option || []).filter(
...
const getAfterDiffItemClass = (key, defaults = []) => {
  ...
  if (
    props.diffType === DIFF_TYPES.UPDATE &&
    Object.keys(updateData).includes(key)
  ) {
  ...
  if (
    ...
    if (
      key === 'bk_property_id' &&
      isFieldNameSame.value &&
      !isFieldIdSame.value
    ) {
      classList.push('is-conflict')
    }
    if (
      key === 'bk_property_type' &&
      isFieldIdSame.value &&
      !isFieldTypeSame.value
    ) {
      classList.push('is-conflict')
    }
  }
  ...
  <bk-sideslider
    ...
    <div
      ...
      :class="[
        'diff-details',
        diffType,
        { 'is-template-conflict': isTemplateBindConflict },
      ]">
      <div class="diff-top">
        ...
        <div :class="['top-content', diffType]">
          <template v-if="isTemplateBindConflict">
            <i18n
              path="字段冲突，该字段已经被其他模板绑定，请删除该模型或修改模板">
              <template #other>
                <bk-button
                  text
                  type="primary"
      ...
      <div class="diff-table">
        ...
        <div class="table-body">
          <div class="col before-col">
            <div class="diff-item">
              ...
              <div class="field-value">
                {{ modelBeforeField.bk_property_id || '--' }}
              </div>
            </div>
            <div class="diff-item">
              <div class="field-key">{{ $t('名称') }}</div>
              <div class="field-value">
                {{ modelBeforeField.bk_property_name || '--' }}
              </div>
            </div>
            <div class="diff-item">
              ...
              <div class="field-value">
                ...
                <span v-else>{{
                  PROPERTY_TYPE_NAMES[modelBeforeField.bk_property_type]
                }}</span>
              </div>
            </div>
            <div
              v-if="optionTypes.includes(modelBeforeField.bk_property_type)"
              class="diff-item is-option">
              <div class="field-key">
                {{ getOptionKeyName(modelBeforeField) }}
              </div>
              <div class="field-value">
                <template
                  v-if="
                    numberLikes.includes(modelBeforeField.bk_property_type)
                  ">
                  <flex-row :colon="true">
                    <div>{{ $t('最小值') }}</div>
                    <div>
                      {{
                        modelBeforeField?.option?.min ||
                        (modelBeforeField.option?.min === 0 ? 0 : '--')
                      }}
                    </div>
                  </flex-row>
                  <flex-row :colon="true">
                    ...
                    <div>
                      {{
                        modelBeforeField?.option?.max ||
                        (modelBeforeField.option?.max === 0 ? 0 : '--')
                      }}
                    </div>
                  </flex-row>
                </template>
                <template
                  v-if="
                    stringLikes.includes(modelBeforeField.bk_property_type)
                  ">
                  <div>{{ modelBeforeField.option || '--' }}</div>
                </template>
                <template
                  v-if="listLikes.includes(modelBeforeField.bk_property_type)">
                  <div
                    v-for="(option, index) in modelBeforeField?.option || []"
                    :key="index">
                    {{ option || '--' }}
                  </div>
                ...
                <template
                  v-if="enumLikes.includes(modelBeforeField.bk_property_type)">
                  <flex-row
                    v-for="(option, index) in modelBeforeField?.option || []"
                    :key="index"
                    :colon="true">
            ...
            <div
              v-if="
                ![PROPERTY_TYPES.INNER_TABLE].includes(
                  modelBeforeField.bk_property_type
                )
              "
              class="diff-item">
              ...
              <div class="field-value">
                ...
                <span v-else>{{
                  getDefaultValue(modelBeforeField) || '--'
                }}</span>
              </div>
            </div>
            <div class="diff-item">
          ...
          <div class="col after-col">
            <div
              v-if="
                [DIFF_TYPES.UNCHANGED, DIFF_TYPES.UNBOUND].includes(diffType)
              "
              class="diff-item is-full">
              {{ diffTitle }}
            </div>
            <template v-else>
              ...
              <div :class="getAfterDiffItemClass('bk_property_type')">
                <div class="field-key">{{ $t('字段类型') }}</div>
                <div class="field-value">
                  {{ PROPERTY_TYPE_NAMES[afterField.bk_property_type] }}
                </div>
              </div>
              <div
                v-if="optionTypes.includes(afterField.bk_property_type)"
                :class="getAfterDiffItemClass('option', ['is-option'])">
                <div class="field-key">{{ getOptionKeyName(afterField) }}</div>
                <div class="field-value">
                  <template
                    v-if="numberLikes.includes(afterField.bk_property_type)">
                    <flex-row :colon="true">
                      <div>{{ $t('最小值') }}</div>
                      <div>
                        {{
                          afterField.option?.min ||
                          (afterField.option?.min === 0 ? 0 : '--')
                        }}
                      </div>
                    </flex-row>
                    <flex-row :colon="true">
                      ...
                      <div>
                        {{
                          afterField.option?.max ||
                          (afterField.option?.max === 0 ? 0 : '--')
                        }}
                      </div>
                    </flex-row>
                  </template>
                  <template
                    v-if="stringLikes.includes(afterField.bk_property_type)">
                    <div>{{ afterField.option || '--' }}</div>
                  </template>
                  <template
                    v-if="listLikes.includes(afterField.bk_property_type)">
                    <div
                      v-for="(option, index) in afterField.option"
                      :key="index">
                      {{ option || '--' }}
                    </div>
                  </template>
                  <template
                    v-if="enumLikes.includes(afterField.bk_property_type)">
                    <flex-row
                      v-for="(option, index) in afterField.option"
                      :key="index"
                      :colon="true">
                  ...
                  <cmdb-property-value
                    v-if="
                      [
                        PROPERTY_TYPES.ORGANIZATION,
                        PROPERTY_TYPES.ENUMQUOTE,
                      ].includes(afterField.bk_property_type)
                    "
                    class="property-value"
              ...
              <div :class="getAfterDiffItemClass('editable')">
                ...
                <div class="field-value">
                  <span>{{
                    afterField.editable?.value ? $t('是') : $t('否')
                  }}</span>
                </div>
              </div>
              <div :class="getAfterDiffItemClass('isrequired')">
                ...
                <div class="field-value">
                  <span>{{
                    afterField.isrequired?.value ? $t('是') : $t('否')
                  }}</span>
                </div>
              </div>
