...
watchEffect(async () => {
  classifications.value = await store.dispatch(
...
const dialogHeight = computed(
  () => `${Math.floor(Math.max(store.state.appHeight * 0.8, 200) - 110)}px`
)
const dialogPos = computed(() => ({
  top: `${Math.floor(
    Math.max(
      store.state.appHeight * 0.4 - parseInt(dialogHeight.value, 10) / 2,
      20
    )
  )}`,
}))
...
const excludeModelIds = computed(() =>
  store.getters['objectMainLineModule/mainLineModels']
    ...
    list.push({
      ...
      bk_objects: classification.bk_objects.filter(
        model =>
          !model.bk_ispaused &&
          !model.bk_ishidden &&
          !excludeModelIds.value.includes(model.bk_obj_id)
      ),
    })
  })
  return list
...
const modelList = computed(() =>
  modelGroupList.value.reduce((acc, cur) => acc.concat(cur.bk_objects), [])
)
...
const displayModelGroupList = computed(() => {
  if (filterWord.value) {
    ...
    modelGroupList.value.forEach(group => {
      list.push({
        ...group,
        bk_objects: group.bk_objects.filter(model =>
          reg.test(model.bk_obj_name)
        ),
      })
    })
...
const selectedStatus = computed(() => {
  ...
  modelList.value.forEach(model => {
    status.selected[model.id] = selectedLocal.value.some(
  ...
  if (checked) {
    ...
    modelList.value.forEach(model => {
      if (
        ...
        const index = selectedLocal.value.findIndex(
          item => item.id === model.id
        )
        if (~index) {
          selectedLocal.value.splice(index, 1)
  ...
  <bk-dialog
          ...
          <template v-for="(model, modelIndex) in group.bk_objects">
            <cmdb-auth
              ...
              <template #default="{ disabled }">
                <div
                  :class="['model-item', { 'is-builtin': model.ispre }]"
                  ...
                  <bk-checkbox
                    ...
                    :value="
                      selectedStatus.selected[model.id] ||
    ...
    <template #footer>
      <div class="dialog-footer">
        <bk-checkbox
          :value="selectedLocal.length === modelList.length"
          class="checkbox-all"
          @change="handleSelectAll">
          <i18n path="全选N个模型">
            <template #count
              ><em class="count">{{ modelList.length }}</em></template
            >
          </i18n>
        </bk-checkbox>
        <div class="operation">
...
.model-item {
  ...
  &:hover {
    ...
    box-shadow: 0 2px 4px 0 rgb(25 25 41 / 5%), 0 2px 4px 0 rgb(0 0 0 / 10%);
  }
