...
const { getUniqueByField, uniqueStatus, removedUniqueList } = useUnique(
  oldUniqueList,
  uniqueListLocal
)
...
watch(
  ...
  async field => {
    if (isFieldMode.value) {
      ...
      const currentField = fieldListLocal.value.find(
      ...
      // 得到当前字段的唯一校验
      const { list: fieldUniqueList } = getUniqueByField({
        ...field,
        id: fieldId,
      })
...
const singleRuleProperties = computed(() =>
  fieldListLocal.value.filter(field =>
    singleRuleTypes.includes(field.bk_property_type)
  )
)
const unionRuleProperties = computed(() =>
  fieldListLocal.value.filter(field =>
    unionRuleTypes.includes(field.bk_property_type)
  )
)

const getDisplayProperties = unqiue =>
  unqiue.type === UNIUQE_TYPES.UNION
...
const validateSubset = (rules, id) => {
  const superset = allUniqueListLocal.value.filter(
    existRule =>
      existRule.id !== id &&
...
const validateSuperset = (rules, id) => {
  const subset = allUniqueListLocal.value.filter(
    existRule =>
      existRule.id !== id &&
      existRule.keys.length > 0 &&
      rules.length > 0 &&
...
const validateAll = () => {
  const result = allUniqueListLocal.value.map(({ keys, id }) =>
    validate(keys, id)
  )
...
const getUniqueValue = unique => {
  if (
    unique.type === UNIUQE_TYPES.UNION ||
    (isFieldMode.value && props.type === UNIUQE_TYPES.UNION)
  ) {
    return unique.keys
  }
...
const handleRecover = async removedUnique => {
  ...
  const oriUnique = oldUniqueList.value.find(
    item => item.id === removedUnique.id
  )
...
<template>
  <div class="unique-manage">
    <div class="unique-list">
      <div
        v-for="(unique, uniqueIndex) in uniqueListLocal"
        :key="unique.id"
        class="unique-item">
        <bk-compose-form-item
          :class="['compose-form', { 'is-field-mode': isFieldMode }]">
          <bk-select
            v-if="!isFieldMode"
            class="rule-type"
            :value="unique.type"
            ...
            <bk-option
              :id="UNIUQE_TYPES.SINGLE"
              :name="$t('单独唯一')"></bk-option>
            <bk-option
              :id="UNIUQE_TYPES.UNION"
              :name="$t('联合唯一')"></bk-option>
          </bk-select>
          <bk-select
            :key="unique.type"
            v-validate="
              `minSelectLength:2,${$t('需要至少两个字段组成联合唯一')}`
            "
            class="rule-selector"
            ...
            <bk-option
              v-for="property in getDisplayProperties(unique)"
              :id="property.id"
              :key="property.id"
              :disabled="
                (isFieldMode && property.id === field.id) ||
                property.id === newFieldID
              "
        ...
        <div class="operation">
          <i
            :class="['icon-cc-zoom-in', 'icon-button', 'add']"
        ...
        <div v-if="validateResult[unique.id]" class="rules-error">
            ...
            <i18n path="唯一校验子集提示">
              <template #name>
                <span
                  ...
                  :key="rule.id"
                  class="rules-error-name">
                  {{ getValidateRuleName(rule) }}
                  <template
                    v-if="index !== validateResult[unique.id].rules.length - 1"
                    >、</template
                  >
                </span>
              </template>
        ...
        <div
          v-else-if="errors.has(`ruleSelector${unique.id}`)"
          class="rules-error">
          {{ errors.first(`ruleSelector${unique.id}`) }}
        </div>
        <template v-if="!isFieldMode">
          <mini-tag
            v-if="uniqueStatus[unique.id].changed"
            :text="$t('更新')"
            theme="changed"
            class="status-tag" />
          <mini-tag
            v-if="uniqueStatus[unique.id].new"
            :text="$t('新增')"
            theme="new"
            class="status-tag" />
        </template>
    ...
    <bk-exception
      v-if="!isFieldMode && !uniqueListLocal.length"
      type="empty"
      scene="part"
      class="empty">
      <i18n path="暂无XXX，LINK">
        ...
        <template #link>
          <bk-link theme="primary" @click="handleAppendRule">{{
            $t('立即添加')
          }}</bk-link>
        </template>
      </i18n>
    ...
    <div
      v-if="!isFieldMode && removedUniqueList.length"
      class="removed-container">
      <div class="title">{{ $t('本次删除的校验') }}</div>
      <div class="unique-list">
        <div
          v-for="unique in removedUniqueList"
          :key="unique.id"
          class="unique-item">
          ...
          <div class="operation">
            <bk-link
              class="link"
              theme="primary"
              @click="handleRecover(unique)"
              >{{ $t('恢复') }}</bk-link
            >
          </div>
          <div v-if="validateResult[unique.id]" class="rules-error">
            <template
              v-if="validateResult[unique.id].type === SET_TYPE.superset">
              <i18n path="唯一校验子集提示">
                <template #name>
                  <span
                    v-for="(rule, index) in validateResult[unique.id].rules"
                    ...
                    <template
                      v-if="
                        index !== validateResult[unique.id].rules.length - 1
                      "
                      >、</template
                    >
                  </span>
          ...
          <mini-tag
            v-if="uniqueStatus[unique.id].removed"
            :text="$t('删除')"
            theme="removed"
            class="status-tag" />
        </div>
