...
const displayFilterMenus = computed(() => {
  if (filter.value?.length) {
    return filterMenus.filter(
      menu => !filter.value.some(item => item.id === menu.id)
    )
  }
...
const fetchModel = async (val, menu) => {
  const classifications = await store.dispatch(
    ...
    list.push({
      ...
      bk_objects: classification.bk_objects.filter(
        model =>
          !model.bk_ispaused &&
          !model.bk_ishidden &&
          !excludeModelIds.includes(model.bk_obj_id)
      ),
    })
  })
