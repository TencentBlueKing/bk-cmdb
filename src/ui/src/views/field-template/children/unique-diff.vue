...
const [{ uniqueChecks: modelUniqueList, pending }] =
  useUniqueCheck(currentModelId)
const [{ properties }] = useProperty(propertyParams)

const modelBeforeUniqueList = computed(() =>
  modelUniqueList.value.filter(item =>
    item.keys.every(key => properties.value.find(({ id }) => id === key.key_id))
  )
)
...
const isConflict = unique =>
  props.diffs.conflict?.some(item => item.data.id === unique.id)
const isNew = unique =>
  props.templateUniqueList?.some(item => item.id === unique.id)
const isUpdate = unique =>
  props.diffs.update?.some(item => item.data.id === unique.id)
const isUnchanged = unique =>
  props.diffs.unchanged?.some(item => item.id === unique.id)
const isUnbound = unique =>
  props.templateRemovedUniqueList?.some(
    item => item.id === unique.bk_template_id
  )

const unboundUniqueList = computed(() =>
  modelBeforeUniqueList.value.filter(unique => isUnbound(unique))
)
...
const getUniqueName = (unique, isTemplate) => {
  ...
  const fieldList = isTemplateUnqiue
    ? props.templateFieldList
    : properties.value
  return getUniqueNameBase(unique, fieldList, isTemplateUnqiue)
}

const getConflictUniqueName = unique => {
  const conflict = props.diffs.conflict?.find(
    item => item.data.id === unique.id
  )
  if (conflict) {
...
const isTemplate = unique =>
  props.templateUniqueList.some(item => item.id === unique.bk_template_id)
</script>

<template>
  <div v-bkloading="{ isLoading: pending }" class="unique-diff">
    ...
    <div class="diff-table">
      ...
      <div class="table-body">
        <div class="col before-col">
          <div
            v-for="(unique, index) in modelBeforeUniqueList"
            :key="index"
            class="diff-item">
            {{ getUniqueName(unique) }}
            <mini-tag v-if="isTemplate(unique)" :text="$t('模板')" />
          </div>
        </div>
        <div class="col after-col">
          <div
            v-for="(unique, index) in modelAfterUniqueList"
            :key="index"
            ...
            <template v-if="isConflict(unique)">
              <i
                v-bk-tooltips="{
                  content: $t('绑定后的唯一性校验冲突提示语', {
                    target: getConflictUniqueName(unique),
                  }),
                }"
                class="bk-icon icon-exclamation-circle-shape conflict-icon">
              </i>
            </template>
          </div>
        </div>
