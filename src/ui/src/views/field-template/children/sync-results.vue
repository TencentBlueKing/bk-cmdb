...
const isSyncResultStatus = computed(() =>
  ['success', 'error', 'abnormal'].includes(status.value)
)
const isEditEmptyModel = computed(
  () => props.scene === 'edit' && !props.modelIds.length
)
...
const getTaskSyncStatus = async taskIds => {
  try {
    ...
    const undone = statusList?.some(task =>
      ['new', 'waiting', 'executing'].includes(task.status)
    )
  ...
  <div class="sync-results">
    <result-status :status="status">
      <template #title>
        ...
        <i18n v-else path="字段组合模板同步结果">
          <template #success
            ><em class="count">{{ counts.success }}</em></template
          >
          <template #fail
            ><em class="count">{{ counts.fail }}</em></template
          >
        </i18n>
      </template>
      ...
      <template #actions>
        ...
        <bk-button @click="handleAction('back')">{{
          $t('返回列表')
        }}</bk-button>
      </template>
    </result-status>
