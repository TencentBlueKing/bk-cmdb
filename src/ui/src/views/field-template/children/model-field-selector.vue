...
const fieldList = computed(() =>
  properties.value.filter(
    item => !excludeFieldType.includes(item.bk_property_type) && !item.ispre
  )
)
const selectedStatus = computed(() => {
  ...
  fieldList.value.forEach(prop => {
    ...
    status.disabled[prop.id] = props.templateFieldList.some(
      item =>
        item.bk_property_id === prop.bk_property_id ||
...
const handleSelectAll = () => {
  selected.value = fieldList.value.filter(
    field => !selectedStatus.value.disabled[field.id]
  )
}
      ...
      <grid-item>
        <div class="select-toolbar">
          <div class="select-stat">
            {{ $t('请选择导入的字段：') }}
            <i18n v-show="selectedModelId" path="已选个数">
              <template #count
          ...
          <div class="select-action">
            <bk-link theme="primary" @click="handleSelectAll">{{
              $t('全选')
            }}</bk-link>
            <bk-link theme="primary" @click="handleClearSelect">{{
              $t('清空')
            }}</bk-link>
          </div>
        </div>
        <div
          ...
          :class="['field-list', { empty: !fieldList.length }]">
          <field-card
          ...
          <div v-if="!fieldList.length" class="field-list-empty">
            <div class="tips">
              <bk-icon type="info" />{{ $t('无可用字段') }}
            </div>
          </div>
        </div>
        <div v-if="!selectedModelId" class="unselected-model">
          <div class="tips">
