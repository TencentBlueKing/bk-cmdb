...
export default function useUnique(beforeUniqueList, uniqueListLocal) {
  ...
  const removedUniqueList = computed(() =>
    beforeUniqueList.value
  ...
  const getUniqueByField = field => {
    ...
    if (fieldUniqueList.length > 1) {
      ...
      type =
        fieldUniqueList[0].keys.length > 1
          ? UNIUQE_TYPES.UNION
          : UNIUQE_TYPES.SINGLE
    }
  ...
  const clearUniqueByField = field => {
    ...
    list.forEach(unique => {
      const index = uniqueListLocal.value.findIndex(
  ...
  const getFieldUniqueWithName = (field, fieldList) => {
    ...
    const fieldUniqueWithNameList = fieldUniqueList.map(item => ({
      ...item,
      names: item.keys.map(
        key => fieldList.find(field => field.id === key)?.bk_property_name
      ),
    }))
    return {
...
export const isUniqueExist = (unique, uniqueList) =>
  uniqueList.some(item => isEqual(item.keys, unique.keys))
