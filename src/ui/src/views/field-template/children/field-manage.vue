...
const { beforeFieldList: oldFieldList, beforeUniqueList: oldUniqueList } =
  toRefs(props)
...
const { getUniqueByField, clearUniqueByField } = useUnique(
  oldUniqueList,
  uniqueLocalList
)
...
// 只有字段属性的列表
const pureFieldList = computed(() =>
  fieldLocalList.value.map(item => item.field)
)
...
const displayFieldLocalList = computed(() => {
  if (filterWord.value) {
    ...
    return fieldLocalList.value.filter(item =>
      reg.test(item.field.bk_property_name)
    )
  }
...
const handleEditField = field => {
  ...
  const { list: fieldUniqueList, type: fieldUniqueType } = getUniqueByField(
    data.field
  )
...
const updateField = (id, fieldData) => {
  const fieldIndex = fieldLocalList.value.findIndex(
...
const setUnique = (uniqueList, currentField) => {
  uniqueList.forEach(unique => {
    const uniqueIndex = uniqueLocalList.value.findIndex(
      item => item.id === unique.id
    )
    if (~uniqueIndex) {
...
const getFieldUnique = field => {
  const { list: fieldUniqueList, type: fieldUniqueType } =
    getUniqueByField(field)
  const fieldUniqueWithNameList = fieldUniqueList.map(item => ({
    ...item,
    names: item.keys.map(
      key =>
        pureFieldList.value.find(field => field.id === key)?.bk_property_name
    ),
  }))
  return {
...
const handleFieldSave = async (id, fieldData) => {
  if (
    !id &&
    isFieldExist(fieldData, [
      ...fieldLocalList.value,
      ...removedFieldList.value,
    ])
  ) {
    ...
    if (slider.uniqueType === UNIUQE_TYPES.UNION) {
      ...
      list.forEach(unique => {
        ...
        if (!fieldUniqueList.some(item => item.id === unique.id)) {
          // 在原列表中的index
          const oldIndex = uniqueLocalList.value.findIndex(
            item => item.id === unique.id
          )
          if (~oldIndex) {
...
const handleChangeUnique = uniqueList => {
  // 先过滤掉keys为空的数据
  uniqueList
    .filter(unique => unique.keys.length)
    .forEach(unique => {
...
const handleRemoveField = field => {
  const index = fieldLocalList.value.findIndex(
    item => item.field.id === field.id
  )
  if (~index) {
...
const uniqueEnabledTogglePreCheck = () =>
...
const uniqueEnabledClickOutSideMiddleware = event =>
  !event.target.closest('.tippy-popper')
...
<template>
  <div class="field-manage">
    <div class="toolbar">
      ...
      <cmdb-auth :auth="{ type: $OPERATION.C_FIELD_TEMPLATE }">
        <template #default="{ disabled }">
          <bk-button :disabled="disabled" @click="handleClickImport">
            {{ $t('从模型导入') }}
          </bk-button>
        </template>
      </cmdb-auth>
    ...
    <field-grid
      ...
      <template #field-card="{ field, itemClass }">
        <field-card
          ...
          <template v-if="!isCreateMode" #flag-append>
            <div
              v-if="
                fieldStatus[field.field.id].new ||
                fieldStatus[field.field.id].changed
              "
              class="flag-append">
              <span v-if="fieldStatus[field.field.id].new" class="flag-tag new">
                <em class="tag-text">{{ $t('新增') }}</em>
              </span>
              <span
                v-else-if="fieldStatus[field.field.id].changed"
                class="flag-tag changed">
    ...
    <div v-if="removedFieldList.length" class="removed-container">
      <div class="removed-title">
        本次删除的字段（{{ removedFieldList.length }}）
      </div>
      <field-grid :field-list="removedFieldList" :disabled-sort="true">
        <template #field-card="{ field, itemClass }">
          <field-card
            ...
            <template #action-append="{ field: removedField }">
              <bk-link
                theme="primary"
                class="recover-link"
                @click="handleRecover(removedField)"
                >{{ $t('恢复') }}</bk-link
              >
            </template>
          </field-card>
    ...
    <bk-sideslider
      ...
      <template v-if="slider.isShow" #content>
        <!-- 添加/编辑字段 -->
        <field-setting-form
          v-if="slider.view === sliderViews.SETTING_FORM"
          ref="settingFormComp"
          scene="setting"
          ...
          <template #append-unique="{ disabled, fieldInfo }">
            <grid-layout
              ...
              <grid-item :label="$t('设置为唯一校验')">
                <bk-popconfirm
                  ...
                  <bk-switcher
                    ...
                    @change="
                      enabled => handleUniqueEnabledChange(enabled, fieldInfo)
                    ">
                  </bk-switcher>
                </bk-popconfirm>
                <bk-switcher
              ...
              <grid-item
                ...
                :label="$t('校验类型')">
                <bk-radio-group
                  v-model="slider.uniqueType"
                  class="full-width-radio"
                  @change="handleUniqueTypeChange">
                  <bk-radio-button
                    :disabled="
                      isUniqueTypeDisabled(fieldInfo, UNIUQE_TYPES.SINGLE)
                    "
                  ...
                  <bk-radio-button
                    :disabled="
                      isUniqueTypeDisabled(fieldInfo, UNIUQE_TYPES.UNION)
                    "
            ...
            <grid-layout
              v-show="
                slider.uniqueEnabled && slider.uniqueType === UNIUQE_TYPES.UNION
              "
          ...
          <template #append-lock="{ fieldInfo }">
            <div class="lock-option-container">
              ...
              <div
                v-if="EDITABLE_TYPES.includes(fieldInfo.bk_property_type)"
                class="option-item">
                <bk-checkbox
              ...
              <div
                v-if="REQUIRED_TYPES.includes(fieldInfo.bk_property_type)"
                class="option-item">
                <bk-checkbox
