...
const isConflict = field =>
  props.diffs.conflict?.some(
    item => item.data.bk_property_id === field.bk_property_id
  )
const isNew = field =>
  props.diffs.create?.some(item => item.bk_property_id === field.bk_property_id)
const isUpdate = field =>
  props.diffs.update?.some(item => item.bk_property_id === field.bk_property_id)
const isUnchanged = field =>
  props.diffs.unchanged?.some(
    item => item.bk_property_id === field.bk_property_id
  )
const isUnbound = field =>
  props.templateRemovedFieldList?.some(
    ({ field: item }) =>
      item.id === field.bk_template_id &&
      item.bk_property_id === field.bk_property_id
  )

const unboundFieldList = computed(() =>
  properties.value.filter(field => isUnbound(field))
)
...
const newFieldList = computed(() => {
  ...
  return news.map(item =>
    props.templateFieldList.find(
      field => field.bk_property_id === item.bk_property_id
    )
  )
})
...
const displayFieldGroups = computed(() => {
  ...
  displayFieldGroups.forEach(group => {
    group.properties = group.properties.filter(field =>
      isOnlyShowTemplateRelated.value ? isTemplateRelated(field) : true
    )
  })
  const finalFieldGroups = displayFieldGroups.filter(
    group => group.properties.length > 0
  )
...
const handleClickField = field => {
  ...
  if (DIFF_TYPES.NEW === diffType) {
    diffDetails.afterField = props.templateFieldList.find(
      item => item.bk_property_id === field.bk_property_id
    )
  }
  ...
  if (DIFF_TYPES.CONFLICT === diffType) {
    diffDetails.fieldDiff = props.diffs.conflict?.find(
      item => item.data.bk_property_id === field.bk_property_id
    )
    ...
    diffDetails.afterField =
      props.templateFieldList.find(
        item => item.bk_property_id === diffDetails.fieldDiff.bk_property_id
      ) ?? {}
  }
  if (DIFF_TYPES.UPDATE === diffType) {
    diffDetails.fieldDiff = props.diffs.update?.find(
      item => item.data.bk_property_id === field.bk_property_id
    )
    diffDetails.afterField =
      props.templateFieldList.find(
...
<template>
  <div v-bkloading="{ isLoading: pending }" class="field-diff">
    <div class="status-bar">
      <div class="diff-summary">
        ...
        <div class="summray-content">
          <diff-brand
            :count="counts.new"
            :text="$t('新增字段')"
            status="new"></diff-brand>
          <diff-brand
            :count="counts.update"
            :text="$t('更新覆盖')"
            status="update"></diff-brand>
          <diff-brand
          ...
          <diff-brand
            ...
            :text="$t('无变化')"
            status="unchanged"></diff-brand>
          <span
            id="field-template-field-diff-conflict-tooltips"
            class="tips-content">
            <div>{{ $t('字段冲突的情况：') }}</div>
            <ul class="list-item">
              <li>
                {{ $t('模板字段与模型字段 ID 类型一样，但已经被其他模板绑定') }}
              </li>
              <li>
                {{ $t('模板字段与模型字段的 ID 一样，但字段类型不一致') }}
              </li>
              <li>
                {{ $t('模板字段与模型字段的字段名称相同，但字段 ID 不一致') }}
              </li>
            </ul>
          </span>
        </div>
      ...
      <bk-checkbox
        v-model="isOnlyShowTemplateRelated"
        class="filter-checkbox"
        >{{ $t('仅显示与模板相关字段') }}</bk-checkbox
      >
    </div>
      ...
      <cmdb-collapse
        ...
        <div class="field-list">
          <field-card
            ...
            <template v-if="isConflict(field)" #flag-append>
              <i
                class="bk-icon icon-exclamation-circle-shape conflict-icon"></i>
            </template>
          </field-card>
        </div>
      </cmdb-collapse>
    ...
    <field-diff-details-drawer
      v-bind="diffDetails"
      @toggle="handelDetailsDrawerToggle" />
  </div>
</template>
