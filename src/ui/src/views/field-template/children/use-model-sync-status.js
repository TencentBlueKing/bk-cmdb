...
export default function useModelSyncStatus(templateId, modelIdList) {
  ...
  const searchModelIds = computed(() =>
    modelIds.value.filter(
      id => !statusList.value.some(item => item.object_id === id)
    )
  )

  const pollingModelIds = computed(() =>
    statusList.value
  ...
  const fetchStatus = async ids => {
    ...
    allResult = await CombineRequest.setup(
      Symbol(),
      objIds =>
        fieldTemplateService.getModelSyncStatus(
          {
    ...
    for (const result of allResult) {
      ...
      list.forEach(item => {
        const index = statusList.value.findIndex(
          sta => sta.object_id === item.object_id
        )
        if (~index) {
  ...
  const unwatchSearchModelIds = watch(
    searchModelIds,
    ids => {
      fetchStatus(ids)
    },
...
export const isSyncing = status =>
  ['new', 'waiting', 'executing'].includes(status)
