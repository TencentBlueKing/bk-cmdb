...
export default function useField(beforeFieldList, fieldListLocal) {
  const fieldStatus = computed(() => {
    ...
    fieldListLocal.value.forEach(data => {
      ...
      if (!matched) {
        ...
        const after = normalizeFieldData(
          [{ ...wrapData(data), bk_template_id: matched.bk_template_id }],
          false
        )
  ...
  const removedFieldList = computed(() =>
    beforeFieldList.value
...
export const normalizeUniqueData = (
  uniqueData,
  fieldData,
  isCreate = true,
  uniqueStatus
  ...
  const uniqueList = uniqueData.map(item => {
    const unique = {
      id: item.id,
      keys: item.keys.map(
        key => fieldData.find(field => field.id === key)?.bk_property_id
      ),
    }
...
export const isFieldSame = (field1, field2) =>
  field1.bk_property_id === field2.bk_property_id ||
  field1.bk_property_name === field2.bk_property_name

export const isFieldExist = (field, fieldList) =>
  fieldList.some(item => isFieldSame(item.field, field))
