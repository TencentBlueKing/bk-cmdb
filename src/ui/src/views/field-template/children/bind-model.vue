...
// 查找匹配第1个可用的模型作为选中模型
watch(
  [modelListLocal, modelEditAuths],
  ([modelList, modelAuths]) => {
    // 当前存在选中的模型则不变更
...
const fieldDiffCounts = computed(() => {
  ...
  Object.keys(fieldDiffs.value).forEach(modelId => {
    ...
    counts[modelId] = {
      total:
        (diffs?.create?.length ?? 0) +
        (diffs?.update?.length ?? 0) +
        (diffs?.conflict?.length ?? 0),
...
onBeforeUnmount(() => {
  ...
  const allRquestIds = Object.values(diffLoadingIds.value).reduce(
...
const isFieldConflict = model =>
  fieldDiffCounts.value[model.id] && fieldDiffCounts.value[model.id].conflict
const isUniqueConflict = model =>
...
const getTotal = id =>
  (fieldDiffCounts.value[id]?.total ?? 0) +
  (uniqueDiffCounts.value[id]?.total ?? 0)
  ...
  <div class="bind-model">
    <bk-resize-layout
      ...
      <div slot="aside" :class="['aside', { empty: !modelListLocal.length }]">
        <div class="aside-top">
          <div v-if="modelListLocal.length > 1" class="total">
            <i18n path="共N个">
              <template #count
                ><em class="count">{{ modelListLocal.length }}</em></template
              >
            </i18n>
          </div>
          <bk-button
        ...
        <div v-if="modelListLocal.length" class="model-list">
                ...
                <div
                  ...
                  <div class="model-info">
                    <div class="model-name-area" :title="model.bk_obj_name">
                      ...
                      <mini-tag v-if="model.bk_ispaused" theme="paused">{{
                        $t('已停用')
                      }}</mini-tag>
                    </div>
                    <div class="model-id">{{ model.bk_obj_id }}</div>
                  </div>
                  <div v-if="!model.bk_ispaused" class="tail">
                    <bk-button
                      v-if="!readonly"
                      class="remove-button"
                      :text="true"
        ...
        <bk-exception
          v-if="readonly && !modelListLocal.length"
          type="empty"
          scene="part"
          class="empty">
              ...
              <div
                :class="[
                  'tab-item',
                  {
                    ...
                    'is-conflict':
                      fieldDiffCounts[selectedModel.id] &&
                      fieldDiffCounts[selectedModel.id].conflict > 0,
                  },
                ]"
                @click="handleToggleTab('field')">
                {{ $t('字段对比') }}
                <span
                  v-if="fieldDiffCounts[selectedModel.id]"
                  class="count-tag">
              ...
              <div
                :class="[
                  'tab-item',
                  {
                    ...
                    'is-conflict':
                      uniqueDiffCounts[selectedModel.id] &&
                ...
                {{ $t('唯一校验对比') }}
                <span
                  v-if="uniqueDiffCounts[selectedModel.id]"
                  class="count-tag">
