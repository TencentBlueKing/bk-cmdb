...
import useModelSyncStatus, {
  isSyncing,
  isSynced,
} from './children/use-model-sync-status'
...
const onePageMaxCount = computed(() =>
  Math.ceil(tableMaxHeight.value / TABLE_ROW_HEIGHT)
)

const activeModelIdList = computed(() =>
  bindModelList.value.filter(model => !model.bk_ispaused).map(model => model.id)
)

const { statusMap, clear: clearModelSyncReq } = useModelSyncStatus(
  props.templateId,
  activeModelIdList
)
...
<template>
  <div v-bkloading="{ isLoading: firstLoading }" class="details-model">
    ...
    <bk-table
      ...
      :scroll-loading="{
        ...scrollLadingOption,
        isLoading: $loading(requestIds.rollList),
      }"
      ...
      <bk-table-column
        ...
        <template #default="{ row }">
          <div
            :class="['model-cell', { paused: row.bk_ispaused }]"
            @click="handleLinkToModel(row)">
            <div class="model-icon">
              <i class="icon" :class="row.bk_obj_icon"></i>
            </div>
            <span class="model-name">{{ row.bk_obj_name }}</span>
            <mini-tag v-if="row.bk_ispaused" theme="paused">{{
              $t('已停用')
            }}</mini-tag>
          </div>
        </template>
      </bk-table-column>
      <bk-table-column :label="$t('同步状态')">
        <template #default="{ row }">
          <model-sync-status :model="row" />
        </template>
      </bk-table-column>
      <bk-table-column :label="$t('最近同步时间')" show-overflow-tooltip>
        <template #default="{ row }">
        ...
        <template #default="{ row }">
          <cmdb-auth
            v-bk-tooltips="{
              ...
              disabled:
                row.bk_ispaused ||
                (statusMap[row.id] && !isSyncing(statusMap[row.id].status)),
            }"
            :auth="{
              type: $OPERATION.U_FIELD_TEMPLATE,
              relation: [templateId],
            }">
            <template #default="{ disabled }">
              <bk-button
                ...
                :disabled="
                  ...
                  (!row.bk_ispaused &&
                    statusMap[row.id] &&
                    isSyncing(statusMap[row.id].status))
                "
                :text="true"
          ...
          <cmdb-auth
            v-if="
              !row.bk_ispaused &&
              statusMap[row.id] &&
              !isSynced(statusMap[row.id].status)
            "
            class="ml20"
            :auth="{
              type: $OPERATION.U_FIELD_TEMPLATE,
