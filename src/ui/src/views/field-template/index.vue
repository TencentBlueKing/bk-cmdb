...
const searchParams = computed(() => {
  ...
  filter.value.forEach(item => {
    ...
    const operator = queryBuilderOperator(
      itemValue?.length > 1 ? QUERY_OPERATOR.IN : QUERY_OPERATOR.LIKE
    )
...
const getList = async (options = {}) => {
  try {
    const { list, count } = await fieldTemplateService.find(
      searchParams.value,
      {
...
<template>
  <div class="field-template">
    ...
    <div class="toolbar">
      <cmdb-auth :auth="{ type: $OPERATION.C_FIELD_TEMPLATE }">
        <template #default="{ disabled }">
          <bk-button theme="primary" :disabled="disabled" @click="handleCreate">
            {{ $t('新建') }}
          </bk-button>
        </template>
      </cmdb-auth>
      <search-select :default-filter="filter" @search="handleSearch">
      </search-select>
    </div>
    ...
    <bk-table
      ...
      <bk-table-column
        ...
        <template slot-scope="{ row }">
          <cmdb-auth
            tag="div"
            class="template-name-auth"
            :auth="{ type: $OPERATION.R_FIELD_TEMPLATE, relation: [row.id] }">
            <template #default="{ disabled }">
              <div
                :class="['cell-link-content', { disabled }]"
                @click.stop="handleRowIDClick(row)">
                {{ row.name }}
              </div>
            </template>
          </cmdb-auth>
      ...
      <bk-table-column min-width="150" :label="$t('绑定的模型')">
        <template slot-scope="{ row }">
          <cmdb-loading :loading="$loading(requestIds.modelCount)">
            <span v-if="row.model_count === 0" class="cell-unbind"
              >0（{{ $t('未绑定') }}）</span
            >
            <span
              v-else
              class="cell-link-content"
              @mouseenter="
                event => handleBoundModelCountMouseenter(event, row)
              ">
              {{ row.model_count ?? '--' }}
            </span>
          </cmdb-loading>
      ...
      <bk-table-column :label="$t('操作')" min-width="250" fixed="right">
        <template slot-scope="{ row }">
          <cmdb-auth
            class="mr10"
            :auth="{ type: $OPERATION.U_FIELD_TEMPLATE, relation: [row.id] }">
          ...
          <cmdb-auth
            ...
            <template #default="{ disabled }">
              <div
                v-bk-tooltips.top="{
                  content: $t('已被模型绑定，不能删除'),
                  disabled: !row.model_count,
                }">
                <bk-button
                  theme="primary"
      ...
      <cmdb-table-empty
        ...
        <bk-exception type="403" scene="part">
          <i18n path="字段组合模板列表提示语" class="table-empty-tips">
            <template #auth>
              <bk-link theme="primary" @click="handleApplyPermission">{{
                $t('申请查看权限')
              }}</bk-link>
            </template>
            <template #create>
    ...
    <div v-show="boundModelPopover.show" ref="boundModelPopoverContentRef">
      <div
        v-bkloading="{
          isLoading: $loading(requestIds.modelList),
          theme: 'primary',
          mode: 'spin',
          size: 'mini',
        }"
        class="bound-model-popover-content">
        <div
          ...
          <div :class="['model-item', { paused: item.bk_ispaused }]">
            ...
            <span class="model-name" :title="item.bk_obj_name">{{
              item.bk_obj_name
            }}</span>
            <mini-tag v-if="item.bk_ispaused" theme="paused">{{
              $t('已停用')
            }}</mini-tag>
          </div>
          <i class="link-icon icon-cc-share" @click="handleLinkToModel(item)">
          </i>
        </div>
      </div>
    </div>
