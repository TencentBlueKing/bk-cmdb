...
<template>
  <cmdb-sticky-layout>
    <bk-form class="info-form clearfix">
      <bk-form-item
        class="form-item clearfix fl"
        :label="$t('任务名称')"
        :label-width="$i18n.locale === 'en' ? 85 : 85">
        <span class="form-value">{{ task.bk_task_name }}</span>
      </bk-form-item>
      <bk-form-item
        class="form-item clearfix fl"
        :label="$t('账户名称')"
        :label-width="$i18n.locale === 'en' ? 115 : 85">
        <task-account-selector
          class="form-value"
          display="info"
          :value="task.bk_account_id">
        </task-account-selector>
      </bk-form-item>
      <bk-form-item
        class="form-item clearfix fl"
        ...
        <task-resource-selector
          class="form-value"
          display="info"
          :value="task.bk_resource_type">
        </task-resource-selector>
      </bk-form-item>
      <bk-form-item
        class="form-item clearfix"
        :label="$t('管控区域设定')"
        :label-width="$i18n.locale === 'en' ? 180 : 85">
      </bk-form-item>
    </bk-form>
    <div class="info-table">
      <bk-table :data="task.bk_sync_vpcs" :row-class-name="getRowClass">
          ...
          <template slot-scope="{ row }">
            <div class="info-cloud-area">
              <task-cloud-area-input
                :id="row.bk_cloud_id"
                display="info"></task-cloud-area-input>
              <span
                v-if="row.destroyed"
        ...
        <bk-table-column
          :label="$t('主机数量')"
          prop="bk_host_count"></bk-table-column>
        <bk-table-column
          :label="$t('主机录入到')"
          prop="directory"
          width="150"
