  ...
  <bk-table
    ...
    <bk-table-column
      ...
      <template slot-scope="{ row }">
        ...
        <div v-else class="info-cloud-area">
          <task-cloud-area-input
            :id="row.bk_cloud_id"
    ...
    <bk-table-column
      ...
      prop="directory"
      width="200"
      :render-header="directoryHeaderRender"
      :resizable="false">
      <template slot-scope="{ row }">
        <task-directory-selector
          v-if="row.destroyed"
          display="info"
          :value="row.bk_sync_dir"></task-directory-selector>
        <task-directory-selector
          v-else
          :key="row.bk_vpc_id"
          v-model="row.bk_sync_dir"
          v-validate="'required'"
          class="form-table-selector"
          :data-vv-name="`directory-selector-${row.bk_vpc_id}`"
          :class="{
            'is-error': errors.has(`directory-selector-${row.bk_vpc_id}`),
          }">
        </task-directory-selector>
      </template>
    </bk-table-column>
