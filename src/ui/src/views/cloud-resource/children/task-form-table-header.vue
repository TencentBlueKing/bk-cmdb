  ...
  <div class="custom-header">
    ...
    <bk-popover
      ...
      <i
        ...
        class="icon-cc-batch-update"
        :class="{ 'is-disabled': disabled }"
        @click="handleClick">
      </i>
      <bk-form slot="content" form-type="vertical">
        <bk-form-item :label="$t('批量编辑')">
          ...
          <div class="selector-options">
            <link-button class="selector-link-button" @click="handleConfirm">{{
              $t('确定')
            }}</link-button>
            <link-button
              class="selector-link-button ml10"
              @click="handleCancel"
              >{{ $t('取消') }}</link-button
            >
          </div>
