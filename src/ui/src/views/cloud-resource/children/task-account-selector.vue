...
<template>
  ...
  <span v-else>{{
    selectedAccount ? selectedAccount.bk_account_name : '--'
  }}</span>
</template>
  ...
  computed: {
    ...
    selectedAccount() {
      return this.accounts.find(
        account => account.bk_account_id === this.selected
      )
    },
    accountVendor() {
      ...
      return this.vendors.find(
        vendor => vendor.id === this.selectedAccount.bk_cloud_vendor
      )
    },
  },
  async created() {
    try {
      ...
      const venderProperty = properties.find(
