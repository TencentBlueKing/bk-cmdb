  ...
  methods: {
    getCursorData(directory) {
      return {
        ...
        auth: {
          type: this.$OPERATION.C_RESOURCE_HOST,
          relation: [directory.bk_module_id],
        },
    ...
    injectAuth(directories) {
      return new Promise(resolve => {
        const fakeComponent = {
          ...
          auth: directories.map(directory => ({
            type: this.$OPERATION.C_RESOURCE_HOST,
            relation: [directory.bk_module_id],
          })),
          updateAuth: results => {
            directories.forEach(directory => {
              const result = results.find(
                result =>
                  result.parent_layers[0].resource_id === directory.bk_module_id
              )
              this.$set(
                directory,
                'authorized',
                result ? result.is_pass : false
              )
            })
            resolve()
          },
    ...
    getInfo() {
      const directory = this.directories.find(
        directory => directory.bk_module_id === this.value
      )
      if (directory) {
