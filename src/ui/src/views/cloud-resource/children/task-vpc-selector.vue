...
<template>
  <div class="vpc-selector clearfix">
    <div class="left-column fl">
      ...
      <div
        v-bkloading="{ isLoading: $loading([request.vpc, request.region]) }"
        class="vpc-wrapper">
        <template v-if="hasVpc">
          ...
          <ul class="vpc-list">
            <li
              ...
              :key="vpc.bk_vpc_id"
              class="vpc-item">
              <span v-bk-overflow-tips class="vpc-item-name">{{
                getVpcName(vpc)
              }}</span>
              <div class="vpc-item-option">
                <span :class="['vpc-item-count', $i18n.locale]">{{
                  vpc.bk_host_count
                }}</span>
                <bk-checkbox
                  class="vpc-item-checkbox"
        ...
        <p v-else class="vpc-empty">{{ $t('该地域暂无VPC') }}</p>
      </div>
    </div>
    <div
      v-bkloading="{ isLoading: $loading(request.region) }"
      class="right-column fl">
      <div class="right-options">
        ...
        <bk-link class="selection-clear" theme="primary" @click="handleClear">{{
          $t('清空')
        }}</bk-link>
      </div>
      <bk-table
        class="selected-vpc-table"
        :height="368"
        :data="selection"
        :outer-border="false"
        :header-border="false"
        ...
        <bk-table-column label="VPC" prop="bk_vpc_id" width="200">
          <template slot-scope="{ row }">
            <div class="vpc-info">
              <span v-bk-overflow-tips class="vpc-name">{{
                getVpcName(row)
              }}</span>
              <span
                v-if="row.destroyed"
                v-bk-tooltips="$t('VPC已销毁')"
                class="info-destroyed">
                {{ $t('已失效') }}
              </span>
            </div>
          </template>
        ...
        <bk-table-column :label="$t('操作')" width="80">
          <template slot-scope="{ row }">
            <bk-link theme="primary" @click="handleRemoveSelection(row)">{{
              $t('移除')
            }}</bk-link>
          </template>
        </bk-table-column>
...
export default {
  ...
  computed: {
    selectState() {
      ...
      const selected = vpcList.filter(vpc =>
        has(this.selectionMap, vpc.bk_vpc_id)
      )
      state.indeterminate =
        selected.length > 0 && selected.length < vpcList.length
      state.all = selected.length === vpcList.length
      return state
    },
    hasVpc() {
      return (
        this.currentRegion && (this.regionVPC[this.currentRegion] || []).length
      )
    },
    selectionMap() {
      const map = {}
  ...
  methods: {
    ...
    handleToggleSelectAll(checked) {
      const vpcList = this.regionVPC[this.currentRegion]
      if (checked) {
        const appendVpc = vpcList.filter(
          vpc => !has(this.selectionMap, vpc.bk_vpc_id)
        )
        this.selection.unshift(...appendVpc)
      } else {
        this.selection = this.selection.filter(
          exist => !vpcList.some(vpc => exist.bk_vpc_id === vpc.bk_vpc_id)
        )
      }
    },
    handleVpcChange(vpc, checked) {
      if (checked) {
        ...
        const index = this.selection.findIndex(
          exist => exist.bk_vpc_id === vpc.bk_vpc_id
        )
    ...
    handleRemoveSelection(row) {
      const index = this.selection.findIndex(
        vpc => vpc.bk_vpc_id === row.bk_vpc_id
      )
      index > -1 && this.selection.splice(index, 1)
    },
    handleClear() {
