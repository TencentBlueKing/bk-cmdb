...
<template>
  <div v-if="display === 'input'" class="cloud-area-input-wrapper">
    <cmdb-auth :auth="{ type: $OPERATION.C_CLOUD_AREA }">
      <input
        v-model.trim="localValue"
        slot-scope="{ disabled }"
        class="cloud-area-input"
        :readonly="readonly"
        :disabled="disabled"
        :class="{
          'has-tips': hasTips,
    ...
    <i
      ...
      class="tips-icon icon icon-cc-tips">
    </i>
    <i
      v-else-if="error"
      v-bk-tooltips="{
      ...
      class="tips-icon bk-icon icon-exclamation-circle-shape">
    </i>
  </div>
  <span v-else v-bk-overflow-tips class="cloud-area-info">{{
    localValue
  }}</span>
</template>
