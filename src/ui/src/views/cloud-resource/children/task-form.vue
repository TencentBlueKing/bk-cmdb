  ...
  <cmdb-sticky-layout>
    <bk-form class="form-layout" form-type="inline" :label-width="300">
      <bk-form-item class="form-item" :label="$t('任务名称')" required>
        ...
        <p v-if="errors.has('bk_task_name')" class="form-error">
          {{ errors.first('bk_task_name') }}
        </p>
      </bk-form-item>
      <bk-form-item class="form-item" :label="$t('账户名称')" required>
        ...
        <bk-link
          theme="primary"
          class="form-account-link"
          @click="handleLinkToCloudAccount">
          <i class="form-account-link-icon icon-cc-share"></i>
          <span>{{ $t('跳转账户管理') }}</span>
        </bk-link>
        <p v-if="form.bk_account_id" class="form-tips">
          <i class="icon-cc-exclamation-tips"></i>
          {{ $t('同步频率提示语') }}
        </p>
        <p v-if="errors.has('bk_account_id')" class="form-error">
          {{ errors.first('bk_account_id') }}
        </p>
      </bk-form-item>
      <bk-form-item class="form-item" :label="$t('资源类型')" required>
        ...
        <p v-if="errors.has('bk_resource_type')" class="form-error">
          {{ errors.first('bk_resource_type') }}
        </p>
      </bk-form-item>
    </bk-form>
    <bk-form
      v-if="form.bk_account_id"
      class="form-layout"
      form-type="inline"
      :label-width="300">
      <bk-form-item class="form-item" :label="$t('管控区域设定')" required>
        ...
        <input
          ...
          type="hidden"
          :value="selectedVPC.length"
          name="vpc-count" />
        <p v-if="errors.has('vpc-count')" class="form-error">
          {{ $t('请至少选择一个VPC') }}
        </p>
      </bk-form-item>
    </bk-form>
    <div v-if="form.bk_account_id" class="form-setting-component">
      <task-form-table
        ref="vpcTable"
...
export default {
  ...
  methods: {
    ...
    handleRemove(target) {
      const index = this.selectedVPC.findIndex(
        vpc => vpc.bk_vpc_id === target.bk_vpc_id
      )
    ...
    async handleSumbit() {
      ...
      const isDirectoryValid =
        this.$refs.vpcTable &&
        (await this.$refs.vpcTable.$validator.validateAll())
