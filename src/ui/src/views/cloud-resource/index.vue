...
<template>
  <div class="cloud-account-layout">
    ...
    <bk-table
      ...
      <bk-table-column
        :label="$t('资源')"
        prop="bk_resource_type"
        :formatter="resourceTypeFormatter"></bk-table-column>
      <bk-table-column
        :label="$t('账户名称')"
      ...
      <bk-table-column
        :label="$t('账户类型')"
        prop="bk_cloud_vendor"
        sortable="custom">
        <cmdb-vendor
          slot-scope="{ row }"
          :type="row.bk_cloud_vendor"></cmdb-vendor>
      </bk-table-column>
      <bk-table-column
        :label="$t('最近同步状态')"
        prop="bk_sync_status"
        ...
        <div
          ...
          v-bk-tooltips.right="{
            disabled: !(
              row.bk_status_description && row.bk_status_description.error_info
            ),
            content:
              row.bk_status_description && row.bk_status_description.error_info,
          }"
          ...
          <template v-if="row.bk_sync_status">
            <i
              :class="[
                'status',
                { 'is-error': row.bk_sync_status !== 'cloud_sync_success' },
              ]"></i>
            {{
              row.bk_sync_status !== 'cloud_sync_success'
                ? $t('失败')
                : $t('成功')
            }}
          </template>
      ...
      <bk-table-column
        :label="$t('最近同步时间')"
        prop="bk_last_sync_time"
        show-overflow-tooltip
        sortable="custom">
        <template slot-scope="{ row }">{{
          row.bk_last_sync_time | formatter('time')
        }}</template>
      </bk-table-column>
      <bk-table-column :label="$t('编辑人')" prop="bk_last_editor">
        <template slot-scope="{ row }">{{
          row.bk_last_editor | formatter('singlechar')
        }}</template>
      </bk-table-column>
      <bk-table-column :label="$t('操作')" fixed="right">
        <template slot-scope="{ row }">
          <cmdb-auth
            class="mr10"
            :auth="{
              type: $OPERATION.U_CLOUD_RESOURCE_TASK,
              relation: [row.bk_task_id],
            }">
            <link-button
              slot-scope="{ disabled }"
              :disabled="disabled"
          ...
          <cmdb-auth
            class="mr10"
            :auth="{
              type: $OPERATION.D_CLOUD_RESOURCE_TASK,
              relation: [row.bk_task_id],
            }">
            <link-button
              slot-scope="{ disabled }"
      ...
      <cmdb-table-empty
        slot="empty"
        :stuff="table.stuff"
        :auth="{ type: $OPERATION.C_CLOUD_RESOURCE_TASK, relation: [bizId] }"
        @create="handleCreate"
        @clear="handleClearFilter">
      </cmdb-table-empty>
    </bk-table>
    <task-sideslider ref="taskSideslider" @request-refresh="getData">
    </task-sideslider>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      scheduleSearch: throttle(this.handlePageChange, 800, {
        leading: false,
        trailing: true,
      }),
      table: {
