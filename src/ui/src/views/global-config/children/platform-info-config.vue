...
<template>
  <div
    ...
    class="platform-info-config"
    :class="`is-${language}`">
    <div>
      <bk-popover placement="bottom">
        <p class="setting-title setting-title-ref">
          {{ $t('网页 Title 设置：') }}
        </p>
        <template #content>
          <img
            style="width: 270px"
            src="../../../assets/images/doc-title-tooltip.webp" />
        </template>
      </bk-popover>
    </div>
    <bk-form
      ...
      <bk-form-item
        ext-cls="site-name-form-item"
        :label="$t('平台名称')"
        required
        property="name">
        <bk-input v-model="siteForm.name" style="width: 240px"></bk-input>
      </bk-form-item>
      <bk-form-item :label="$t('分隔符')" required property="separator">
        <bk-input v-model="siteForm.separator" style="width: 100px"></bk-input>
      </bk-form-item>
    </bk-form>
    <p class="mt40 setting-title">{{ $t('页脚信息设置：') }}</p>
    <bk-form ref="footerFormRef" :label-width="labelWidth">
      ...
      <bk-form-item class="form-action-item">
        ...
        <bk-button class="action-button" @click="preview">{{
          $t('预览')
        }}</bk-button>
        <bk-popconfirm
          trigger="click"
          ...
          :content="
            $t(
              '该操作将会把平台信息选项内容重置为最后一次保存的状态，请谨慎操作！'
            )
          "
          @confirm="reset">
    ...
    <bk-dialog
      ...
      :mask-close="true">
      <TheFooter
        :preview-contact="footerForm.contact"
        :preview-copyright="footerForm.copyright"></TheFooter>
    </bk-dialog>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const copyrightTips = t(
      '支持日期变量，{{current_year}} 为当前年份，{{current_month}} 为当前月份，{{current_day}} 为当前日'
    )
    ...
    const save = () => {
      siteFormRef.value.validate().then(() => {
        ...
        store.dispatch('globalConfig/updateConfig', newSetting).then(() => {
          initForm()
          bkMessage({
            theme: 'success',
            message: t('保存成功'),
