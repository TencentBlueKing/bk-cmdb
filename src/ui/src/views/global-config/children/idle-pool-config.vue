...
<template>
  <div
    ...
    class="idle-machine-pool-config">
    <bk-form ref="idleFormRef" :model="idleForm" :rules="idleFormRules">
      ...
      <bk-form-item
        ...
        <ModuleBuilder
          ...
          :module-name-placeholder="
            globalConfig.config.validationRules.businessTopoInstNames.message
          "
          :module-name.sync="idleForm.set"
          @before-confirm="beforeClusterConfirm"
          @cancel="clearError"></ModuleBuilder>
      </bk-form-item>

      <!-- 内置空闲机模块 -->
      <bk-form-item
        v-for="(
          idleModuleName, idleModuleKey, idleModuleIndex
        ) in idleForm.buildInModules"
        :key="idleModuleKey"
        required
      ...
      <bk-form-item
        ...
        <ModuleBuilder
          ...
          @remove="
            removeUserModule(userModule.moduleKey, userModule.moduleName)
          ">
        </ModuleBuilder>
      </bk-form-item>

      <bk-form-item ext-cls="action-form-item">
        <bk-button text :disabled="addBtnDisabled" @click="addUserModule">{{
          $t('添加模块')
        }}</bk-button>
      </bk-form-item>
    </bk-form>
...
export default defineComponent({
  ...
  setup() {
    ...
    const businessTopoInstNameRule = value => ({
      required: true,
      message:
        globalConfig.value.config.validationRules.businessTopoInstNames.message,
      validator: async () => {
        const { valid } = await veeVlidate.verify(
          value(),
    ...
    const addBtnDisabled = computed(() =>
      idleForm.userModules.some(userModule => userModule.state === 'editting')
    )
    ...
    const generateUserModuleRules = ruleKey => [
      {
        required: true,
        validator: async () => {
          const newModule = idleForm.userModules.find(
            userModule => userModule.ruleKey === ruleKey
          )
      ...
      businessTopoInstNameRule(
        () =>
          idleForm.userModules.find(
            userModule => userModule.ruleKey === ruleKey
          ).moduleName
      ),
    ]
    ...
    const initForm = () => {
      ...
      // 用户自定义模块处理
      const userModules = idlePool?.userModules?.map(
      ...
      Object.keys(idleForm.buildInModules).forEach(buildInModuleKey => {
        buildInRules[buildInModuleKey] =
          generateBuildInModuleRules(buildInModuleKey)
      })
      ...
      idleFormRef.value
        ...
        .then(() => {
          bkInfoBox({
            ...
            confirmFn: async () => {
              const [err] = await to(
                store.dispatch('globalConfig/updateIdleSet', {
                  setKey: 'set',
      ...
      idleFormRef.value
        ...
        .then(() => {
          bkInfoBox({
            ...
            confirmFn: async () => {
              const [err] = await to(
                store.dispatch('globalConfig/updateIdleModule', {
                  moduleKey,
                  moduleName,
                })
              )
    ...
    const createModule = ({ moduleKey, moduleName }, done) => {
      idleFormRef.value.validate().then(() => {
        bkInfoBox({
          ...
          confirmFn: async () => {
            const [err] = await to(
              store.dispatch('globalConfig/createIdleModule', {
                moduleKey,
                moduleName,
              })
            )
    ...
    const removeUserModule = (moduelId, moduleName) => {
      bkInfoBox({
        ...
        confirmFn: async () => {
          const [err] = await to(
            store.dispatch('globalConfig/deleteIdleModule', {
              moduleKey: moduelId,
              moduleName,
            })
          )
