...
<template>
  <div v-bkloading="{ isLoading: globalConfig.loading }">
    <bk-form
      ref="bizGeneralFormRef"
      :rules="bizGeneralFormRules"
      :label-width="labelWidth"
      :model="bizGeneralForm">
      <bk-form-item
        :desc="{
          width: 400,
          content: $t(
            '快照存储的业务名需要和 GSE 的数据写入配置保持一致，修改后需要重启 datacollection 服务，否则 CMDB 将会无法正常消费到主机快照数据。'
          ),
        }"
        :label="$t('业务快照名称')"
        :icon-offset="bizNameIconOffsetLeft"
        property="snapshotBizName"
        required>
        <bk-input
      ...
      <bk-form-item
        :label="$t('拓扑最大可建层级')"
        :icon-offset="topoLevelIconOffsetLeft"
        property="maxBizTopoLevel"
        required>
        <bk-input
      ...
      <bk-form-item class="form-action-item">
        ...
        <bk-popconfirm
          ...
          :content="
            $t(
              '该操作将会把业务通用选项内容重置为最后一次保存的状态，请谨慎操作！'
            )
          "
          @confirm="reset">
...
export default defineComponent({
  ...
  setup() {
    ...
    const bizNameIconOffsetLeft = computed(() =>
      language === 'zh_CN' ? 30 : 10
    )
    const topoLevelIconOffsetLeft = computed(() =>
      language === 'zh_CN' ? 0 : -20
    )
      ...
      snapshotBizName: [
        {
          ...
          validator: async bizName => {
            const [err, { info: businesses }] = await to(
              store.dispatch('objectBiz/getFullAmountBusiness')
            )
            if (err) {
              return false
            }
            const isExistedBiz = businesses?.some(
              biz => bizName === biz.bk_biz_name
            )
            return isExistedBiz
          },
        },
