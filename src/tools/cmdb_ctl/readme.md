...
- `full-text-search`子命令的命令行参数
     ```
          --tenant-id="": 全文检索初始化或需要同步全文检索数据的租户ID，该参数必填
...
- 示例
     ```
         全文检索初始化:
             ./tool_ctl sync full-text-search --zk-addr=127.0.0.1:2181 --tenant-id="system" --is-migrate
     ```

     ```
          将全量的全文检索数据同步到Elasticsearch:
             ./cmdb_ctl sync full-text-search --zk-addr=127.0.0.1:2181 --tenant-id="system" --is-sync-data --is-all
     ```

     ```
          将指定的索引的全文检索数据同步到Elasticsearch:
             ./cmdb_ctl sync full-text-search --zk-addr=127.0.0.1:2181 --tenant-id="system" --is-sync-data --index="bk_cmdb.host"
     ```

     ```
          将指定的全文检索数据同步到Elasticsearch:
             ./cmdb_ctl sync full-text-search --zk-addr=127.0.0.1:2181 --tenant-id="system" --is-sync-data --collection="HostBase" --oids=65291386812c38ce22236665,6535e53a225ab588783ae11c
     ```
