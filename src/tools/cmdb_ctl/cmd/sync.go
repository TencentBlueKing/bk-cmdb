...
func (c *fullTextSearchConf) addFlags(cmd *cobra.Command) {
	cmd.PersistentFlags().StringVar(&c.tenantID, "tenant-id", "", "tenant id")
...
func runFullTextSearchSync(c *fullTextSearchConf) error {
	if c.tenantID == "" {
		return fmt.Errorf("tenant-id option must be set")
	}

	if c.isMigrate {
		return runFullTextSearchMigrate(c.tenantID)
	}

	if c.isSyncData {
		return runFullTextSearchDataSync(c.tenantID, &c.dataOpt)
	}

	return fmt.Errorf("one of is-migrate and is-sync-data option must be set")
}

func runFullTextSearchMigrate(tenantID string) error {
	resp, err := doCmdbHttpRequest(types.CC_MODULE_SYNC, "/sync/v3/migrate/full/text/search", tenantID, nil)
	if err != nil {
		return err
	}
	...
	fmt.Printf("do full text search migration success, result: %s\n", string(resJs))
	return nil
}
...
func runFullTextSearchDataSync(tenantID string, opt *ftypes.SyncDataOption) error {
	resp, err := doCmdbHttpRequest(types.CC_MODULE_SYNC, "/sync/v3/sync/full/text/search/data", tenantID, opt)
	if err != nil {
		return err
	}
