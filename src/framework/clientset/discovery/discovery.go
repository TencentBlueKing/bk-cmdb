...
func DiscoveryAPIServer(zkAddr string) (Interface, error) {
	...
	apiserver := "apiserver"
	path := fmt.Sprintf("%s/%s", types.CC_SERV_BASEPATH, apiserver)
	svr, err := newServerDiscover(disc, path)
	if err != nil {
		return nil, fmt.Errorf("discover %s failed, err: %v", apiserver, err)
	}
