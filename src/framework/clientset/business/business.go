...
func (b *biz) CreateBusiness(info *types.CreateBusinessInfo) (types2.MapStr, error) {
	resp := new(types.BusinessResponse)
	subPath := fmt.Sprintf("/biz/%s", info.Tenancy)
	err := b.client.Post().
		WithContext(info.Ctx).
		Body(info.BussinessInfo).
		SubResource(subPath).
...
func (b *biz) UpdateBusiness(info *types.UpdateBusinessInfo) error {
	resp := new(types.BusinessResponse)
...
func (b *biz) DeleteBusiness(info *types.DeleteBusinessInfo) error {
	resp := new(types.BusinessResponse)
	subPath := fmt.Sprintf("/biz/%s/%d", info.Tenancy, info.BusinessID)
	err := b.client.Delete().
		WithContext(info.Ctx).
...
func (b *biz) GetBusiness(info *types.DeleteBusinessInfo) error {
	resp := new(types.BusinessResponse)
	subPath := fmt.Sprintf("/biz/%s/%d", info.Tenancy, info.BusinessID)
	err := b.client.Delete().
		WithContext(info.Ctx).
