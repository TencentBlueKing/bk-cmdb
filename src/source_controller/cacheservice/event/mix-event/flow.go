...
func (f *MixEventFlow) doBatch(dbInfo *types.DBInfo, es []*types.Event) (unhandledEvents []*types.Event, retry bool) {
	if len(es) == 0 {
		return es, false
	}
	...
	txnErr := mongo.WithSession(context.Background(), session, func(sc mongo.SessionContext) error {
		...
		for tenantID, chainNodes := range chainNodeMap {
			...
			lastNodeInfo := map[string]interface{}{
				f.Key.Collection() + "." + common.BKFieldID:     lastNode.ID,
				f.Key.Collection() + "." + common.BKCursorField: lastNode.Cursor,
			}

			filter := map[string]interface{}{
				"_id": watch.GenDBWatchTokenID(dbInfo.UUID, f.MixKey.Collection()),
			}
