...
func (b *bizSetRelation) rearrangeBizSetEvents(es []*types.Event, rid string) ([]*types.Event, error) {
	...
	deletedOids := make(map[string][]string, 0)
	...
	for i := len(es) - 1; i >= 0; i-- {
		...
		if _, exists := lastEventMap[e.Oid]; !exists {
			...
			if err != nil {
				blog.Errorf("biz set event collection %s is invalid, doc: %s, rid: %s", e.Collection, e.DocBytes, rid)
				continue
			}
			...
			if e.OperationType == types.Delete {
				deletedOids[tenantID] = append(deletedOids[tenantID], e.Oid)
			}
		}
	}
	...
	for _, e := range hitEvents {
		...
		if e.OperationType != types.Delete && gjson.Get(string(e.DocBytes), "bk_scope.match_all").Bool() {
			matchAllTenantIDs = append(matchAllTenantIDs, tenantID)
			break
		}
...
func (b *bizSetRelation) rearrangeBizEvents(es []*types.Event, rid string) ([]*types.Event, error) {
	...
	deletedOidsMap := make(map[string][]string, 0)
	for _, one := range es {
		tenantID, _, err := common.SplitTenantTableName(one.Collection)
		if err != nil {
			blog.Errorf("biz event collection %s is invalid, doc: %s, rid: %s", one.Collection, one.DocBytes, rid)
			continue
		}
		if one.OperationType == types.Delete {
			deletedOidsMap[tenantID] = append(deletedOidsMap[tenantID], one.Oid)
		}
	}

	deletedDetailMap := make(map[string]map[string]interface{})
	for tenantID, deletedOids := range deletedOidsMap {
		filter := map[string]interface{}{
...
func (b *bizSetRelation) parseBizEvents(es []*types.Event, deletedDetailMap map[string]map[string]interface{},
	...
	params := convertBizEventParams{
		es:                            es,
		insertedAndDeletedBiz:         make(map[string][]map[string]interface{}, 0),
		insertedAndDeletedBizIndexMap: make(map[string]map[int]int, 0),
		updatedFieldsIndexMap:         make(map[string]map[string]int, 0),
	...
	for index, one := range es {
		...
		case types.Insert:
			...
			params.insertedAndDeletedBiz[tenantID] = append(params.insertedAndDeletedBiz[tenantID],
				*one.Document.(*map[string]interface{}))
...
type convertBizEventParams struct {
	...
	// rid request id
	rid string
	// tenantIDs are biz events related tenant ids
	tenantIDs []string
}
...
func (b *bizSetRelation) convertBizEvent(params convertBizEventParams) ([]*types.Event, error) {
	...
	bizSetsMap, err := b.getAllBizSets(context.Background(), params.tenantIDs, params.rid)
	if err != nil {
		return nil, err
	}

	// get biz events' related biz sets
	relatedBizSets, containsMatchAllBizSetTenants := b.getRelatedBizSets(params, bizSetsMap)
	...
	for _, tenantID := range containsMatchAllBizSetTenants {
		if err := b.refreshAllBizIDStr(tenantID, params.rid); err != nil {
			return nil, err
		}
	}
...
func (b *bizSetRelation) getAllBizSets(ctx context.Context, tenantIDs []string, rid string) (map[string][]bizSetWithOid,
	error) {
	...
	for _, tenantID := range tenantIDs {
		...
		for {
			...
			err := mongodb.Shard(sharding.NewShardOpts().WithTenant(tenantID)).Table(common.BKTableNameBaseBizSet).
				Find(cond, findOpts).Fields(common.BKBizSetIDField, common.BKBizSetScopeField).Limit(step).
				Sort(common.BKBizSetIDField).All(ctx, &oneStep)
			if err != nil {
...
func (b *bizSetRelation) getDeleteEventDetails(oidMap map[string][]string, rid string) (map[string][]byte,
	error) {
	...
	for tenantID, oids := range oidMap {
		filter := map[string]interface{}{
