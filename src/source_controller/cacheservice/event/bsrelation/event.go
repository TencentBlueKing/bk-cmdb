...
func (b *bizSetRelation) parseEvent(e *types.Event, id uint64, rid string) (string, *watch.ChainNode, []byte, bool,
	...
	if err != nil {
		blog.Errorf("biz set relation event, received invalid event collection %s, doc: %s, rid: %s", e.Collection,
			e.DocBytes, rid)
		return "", nil, nil, false, nil
	}
	...
	return tenantID, chainNode, detailBytes, false, nil
}
...
func (b *bizSetRelation) getBizSetRelationDetail(e *types.Event, rid string) (string, error) {
	tenantID, _, err := common.SplitTenantTableName(e.Collection)
	if err != nil {
		blog.Errorf("biz set relation event collection %s is invalid, doc: %s, rid: %s", e.Collection, e.DocBytes, rid)
		return "", err
	}
	...
	allBizID := b.allBizIDStr.Get(tenantID)
	...
	bizIDStr, err := b.getBizIDArrStrByCond(tenantID, bizSetBizCond, rid)
	if err != nil {
		return "", err
	}
