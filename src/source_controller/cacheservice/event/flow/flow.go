...
func (f *Flow) doBatch(dbInfo *types.DBInfo, es []*types.Event) (unhandledEvents []*types.Event, retry bool) {
	eventLen := len(es)
	if eventLen == 0 {
		return es, false
	}
	...
	for index, e := range es {
		...
		tenant, chainNode, detail, retry, err := f.parseEvent(dbInfo.CcDB, f.key, e, oidDetailMap, ids[index], rid)
		if err != nil {
			return es, retry
		}
		if chainNode == nil {
