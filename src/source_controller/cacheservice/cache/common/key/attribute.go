...
func init() {
	addKeyGenerator(AttributeType, attributeKey)
}

var attributeKey = KeyGenerator{
	namespace: fmt.Sprintf("%s%s:", common.BKCacheKeyV3Prefix, AttributeType),
	watchOpt: types.Options{
		EventStruct: new(mapstr.MapStr),
		Collection:  common.BKTableNameObjAttDes,
	},
	...
	dataGetterMap: map[KeyKind]dataGetter{
		IDKind: func(db dal.DB, keys ...string) ([]interface{}, error) {
			return commonIDDataGetter(db, common.BKTableNameObjAttDes, common.BKFieldID, keys...)
		},
		ObjIDKind: func(db dal.DB, keys ...string) ([]interface{}, error) {
			return commonKeyDataGetter(db, common.BKTableNameObjAttDes, common.BKObjIDField, keys...)
		},
	},
}
