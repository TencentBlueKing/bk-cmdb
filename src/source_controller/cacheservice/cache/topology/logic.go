...
// sortNodes sort nodes by their initials in the order of 0-9a-z ignoring english case or chinese pinyin
func sortNodes(nodes []*Node) {
	sort.Slice(nodes, func(i, j int) bool {
		iInitials := strings.ToLower(util.GetInitials(nodes[i].Name))
		jInitials := strings.ToLower(util.GetInitials(nodes[j].Name))
