...
func (m *customLevel) runMainlineTopology() error {
	...
	_, err := redis.Client().Get(m.key.mainlineListDoneKey()).Result()
	if err != nil {
		if err != redis.Nil {
...
func (m *customLevel) runCustomWatch(objID string) error {
	...
	_, err := redis.Client().Get(m.key.listDoneKey(objID)).Result()
	if err != nil {
		if err != redis.Nil {
...
func (m *customLevel) onChange(_ *types.Event) {
	...
	redis.Client().Set(m.key.topologyKey(), m.key.topologyValue(rank), 0)
}

func (m *customLevel) onMainlineTopologyListDone() {
	if err := redis.Client().Set(m.key.mainlineListDoneKey(), "done", 0).Err(); err != nil {
...
func (m *customLevel) onCustomInstanceListDone(objID string) {
	if err := redis.Client().Set(m.key.listDoneKey(objID), "done", 0).Err(); err != nil {
...
func (m *customLevel) upsertOid(objID string, bizID int64, instID int64, oid string) {
	...
	if err := redis.Client().HSet(customKey.objectIDKey(), oid, value).Err(); err != nil {
...
func (m *customLevel) delOid(oid string) {
	if err := redis.Client().HDel(customKey.objectIDKey(), oid).Err(); err != nil {
...
func (m *customLevel) getOidValue(oid string) (*oidValue, error) {
	value, err := redis.Client().HGet(customKey.objectIDKey(), oid).Result()
	if err != nil {
