...
func (s *coreService) GetHostModuleRelation(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Errorf("geth host module relation error, err: %v, rid: %s", err, ctx.Kit.Rid)
		ctx.RespAutoError(err)
		return
	}
...
func (s *coreService) HostIdentifier(ctx *rest.Contexts) {
	...
	if err != nil {
		blog.Infof("Identifier host identifier handle error, err: %v, input: %+v, rid:%s", err.Error(), inputData,
			ctx.Kit.Rid)
		ctx.RespAutoError(err)
		return
	}
