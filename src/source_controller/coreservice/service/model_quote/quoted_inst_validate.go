...
func validateCreateQuotedInst(kit *rest.Kit, objID string, instance mapstr.MapStr,
	...
	for key, val := range instance {
		...
		// skip the reserved "id" field which conflicts with the table's built-in record
		// identification field and will be auto-generated by the system during creation.
		if key == common.BKFieldID {
			delete(instance, key)
			continue
		}
