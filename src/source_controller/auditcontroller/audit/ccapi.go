...
func (ccAPI *CCAPIServer) Start() error {
	...
	go func() {
		err := ccAPI.rd.Start()
		blog.Error("auditcontroller rdiscover start failed! err:%s", err.Error())
		chErr <- err
	}()
...
func (ccAPI *CCAPIServer) initHttpServ() error {
	...
	// MetricServer
	conf := metric.Config{
		ModuleName:    types.CC_MODULE_AUDITCONTROLLER,
...
func (ccAPI *CCAPIServer) HealthMetric() metric.HealthMeta {
	...
	for _, item := range meta.Items {
		if item.IsHealthy == false {
			...
			meta.Message = "auditcontroller is not healthy"
			break
		}
	}
