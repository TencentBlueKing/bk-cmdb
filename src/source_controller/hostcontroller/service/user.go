...
func (s *Service) AddUserConfig(req *restful.Request, resp *restful.Response) {
	...
	if 0 != rowCount {
		...
		resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: defErr.Errorf(common.CCErrCommDuplicateItem, addQuery.Name)})
		return
	}
...
func (s *Service) UpdateUserConfig(req *restful.Request, resp *restful.Response) {
	...
	if len(data.Name) != 0 {
		...
		if 0 < rowCount {
			...
			resp.WriteError(http.StatusBadRequest, &meta.RespError{Msg: defErr.Errorf(common.CCErrCommDuplicateItem, data.Name)})
			return
		}
	}
