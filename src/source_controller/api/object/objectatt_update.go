...
func (cli *Client) UpdateMetaObjectAtt(forward *ForwardParam, objAttID int, data []byte) error {
	...
	rst, err := cli.base.HttpCli.PUT(fmt.Sprintf("%s/object/v1/meta/objectatt/%d", cli.address, objAttID), forward.Header, data)

	if nil != err {
		blog.Error("update objectatt failed, error:%v", err)
