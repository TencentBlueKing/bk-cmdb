...
func (cli *Client) UpdateMetaObjectAttGroup(forward *ForwardParam, data []byte) error {
	...
	rst, err := cli.base.HttpCli.PUT(fmt.Sprintf("%s/object/v1/meta/objectatt/group/update", cli.address), forward.Header, data)
	if nil != err {
...
func (cli *Client) UpdateMetaObjectAttGroupProperty(forward *ForwardParam, data []byte) error {
	...
	rst, err := cli.base.HttpCli.PUT(fmt.Sprintf("%s/object/v1/meta/objectatt/group/property", cli.address), forward.Header, data)
	if nil != err {
		blog.Error("request failed, error:%v", err)
		return Err_Request_Object
	}
