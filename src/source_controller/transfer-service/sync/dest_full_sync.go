...
func (s *resSyncer) pullFullSyncData(subRes string, ack bool) {
	kit := util.NewKit()
	blog.Infof("start pull %s-%s full sync data, rid: %s", s.lgc.ResType(), subRes, kit.Rid)
	...
	blog.Infof("pull %s-%s full sync data successfully, rid: %s", s.lgc.ResType(), subRes, kit.Rid)
}
