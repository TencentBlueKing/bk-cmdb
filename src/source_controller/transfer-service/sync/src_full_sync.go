...
func (s *Syncer) loopPushFullSyncData(interval time.Duration) {
	...
	for {
		...
		util.RetryWrapper(3, func() (bool, error) {
			objIDs, quotedObjIDs, err = s.metadata.GetObjIDs()
			if err != nil {
...
func (s *resSyncer) pushFullSyncData(subRes string) {
	kit := util.NewKit()
	blog.Infof("start push %s-%s full sync data, rid: %s", s.lgc.ResType(), subRes, kit.Rid)
	...
	blog.Infof("push %s-%s full sync data successfully, rid: %s", s.lgc.ResType(), subRes, kit.Rid)
}
