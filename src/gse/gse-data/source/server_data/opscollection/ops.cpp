...
#include "opscollection/report_client.h"
#include "utils.h"
...
const std::string name_ops_queune_msg_count = "data_ops_queue_msg_count";
...
void OpsStatWorker::makePhysicalTagJson(rapidjson::Document &physical_tag_json)
{
    ...
    str_cluster_name_value.SetString(m_gseCfg.getDataSvrClusterName().c_str(), m_gseCfg.getDataSvrClusterName().length(), physical_tag_json.GetAllocator());
    ...
    str_cluster_instanceid_value.SetString(m_gseCfg.getDataSvrInstanceId().c_str(), m_gseCfg.getDataSvrInstanceId().length(), physical_tag_json.GetAllocator());
...
void OpsStatWorker::MakeCustomTag(DataCellOPS *ptr_ops, OpsDataStat * dataloss, std::string &key)
{
    ...
    if (dataloss->m_customTags.find(key) == dataloss->m_customTags.end())
    {
        ...
        dataloss->m_customTags.insert(std::make_pair("instanceid", m_gseCfg.getDataSvrInstanceId()));
        dataloss->m_customTags.insert(std::make_pair("cluster_name", m_gseCfg.getDataSvrClusterName()));
        ...
        if (ptr_ops->m_extensionsInfo.size() > 0)
        {
            dataloss->m_customTags.insert(std::make_pair("tdm_sourceid", ptr_ops->m_extensionsInfo[0]));
            if (ptr_ops->m_extensionsInfo.size() > 1)
            {
                dataloss->m_customTags.insert(std::make_pair("tdm_eventname", ptr_ops->m_extensionsInfo[1]));
            }

            if (ptr_ops->m_extensionsInfo.size() > 2)
            {
                dataloss->m_customTags.insert(std::make_pair("tdm_businessid", ptr_ops->m_extensionsInfo[2]));
            }
        }
