...
GseMsgDispatcher::GseMsgDispatcher(GseMsgHandlerIf * handler)
:m_threadPool(handler->getWorkerNum()),m_handler(handler),m_nextThread(0)
{}

GseMsgDispatcher::~GseMsgDispatcher()
{
    if(m_handler != NULL)
        delete m_handler;
}

void GseMsgDispatcher::localNotifyHandler(int fd, short witch)
{
...
void GseMsgDispatcher::notifyHandler(int fd, short witch, void * arg)
{
    GseMsgDispatcher * pDispatcher = reinterpret_cast<GseMsgDispatcher *>(arg);
    pDispatcher->localNotifyHandler(fd, witch);
}

int GseMsgDispatcher::notify(char * pMsg, uint32_t len, void * arg)
{
...
void GseMsgDispatcher::stop()
{
    m_threadPool.stop();
    unInit();
}

int GseMsgDispatcher::init()
{
    ...
    for(int i = 0; i < workerNum; i++)
    {
        ...
        m_threadPool.regReadEvent(i, reinterpret_cast<EVENT_HANDLER>(GseMsgDispatcher::notifyHandler), fd[0], this, false);
...
GseMsgDispatcher::MsgDispItem::MsgDispItem()
