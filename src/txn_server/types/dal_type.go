...
type DALClient interface {
	Find(ctx context.Context, result interface{}, filter Filter)          // 查询多个并反序列化到 Result
	FindOne(ctx context.Context, result interface{}, filter Filter)       // 查询单个并反序列化到 Result
	Insert(ctx context.Context, doc Document) error                       // 插入单个，如果tag有id, 则回设
	InsertMulti(ctx context.Context, docs []Document) error               // 插入多个, 如果tag有id, 则回设
	Update(ctx context.Context, doc Document, filter Filter) error        // 更新数据
	Delete(ctx context.Context, filter Filter) error                      // 删除数据
	Count(ctx context.Context, filter Filter) (uint64, error)             // 统计数量(非事务)
...
type TxDALClient interface {
	Commit() error       // 提交事务
	Abort() error        // 取消事务
	TxnInfo() Tansaction // 当前事务信息，用于事务发起者往下传递
	DALClient
}
