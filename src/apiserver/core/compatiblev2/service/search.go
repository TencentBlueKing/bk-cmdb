...
func (s *service) getModuleInfoByApp(ctx context.Context, appID int64, srvData *srvComm) (map[int64]mapstr.MapStr, ccErr.CCError) {
	...
	result, err := s.Engine.CoreAPI.TopoServer().OpenAPI().SearchModuleByApp(ctx, strconv.FormatInt(appID, 10), srvData.header, param)
	if err != nil {
...
func (s *service) getIPAndProxyByCompany(req *restful.Request, resp *restful.Response) {
	...
	result, err := s.Engine.CoreAPI.HostServer().GetIPAndProxyByCompany(srvData.ctx, srvData.header, input)
	if err != nil {
...
func (s *service) getSetHostList(req *restful.Request, resp *restful.Response) {
	...
	result, err := s.Engine.CoreAPI.HostServer().HostSearchBySetID(srvData.ctx, srvData.header, param)
	if err != nil {
...
func (s *service) getModuleHostList(req *restful.Request, resp *restful.Response) {
	...
	result, err := s.Engine.CoreAPI.HostServer().HostSearchByModuleID(srvData.ctx, srvData.header, param)
	if err != nil {
...
func (s *service) getHostsByProperty(req *restful.Request, resp *restful.Response) {
	...
	result, err := s.Engine.CoreAPI.HostServer().HostSearchByProperty(srvData.ctx, srvData.header, param)
	if err != nil {
