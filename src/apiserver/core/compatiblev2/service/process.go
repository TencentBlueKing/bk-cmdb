...
func (s *service) getProcessPortByApplicationID(req *restful.Request, resp *restful.Response) {
	...
	result, err := s.Engine.CoreAPI.ProcServer().OpenAPI().GetProcessPortByApplicationID(srvData.ctx, appID, srvData.header, modulesMap)
	if err != nil {
...
func (s *service) getModulesByAppId(ctx context.Context, appID string, srvData *srvComm) ([]mapstr.MapStr, ccError.CCError) {
	...
	result, err := s.Engine.CoreAPI.TopoServer().OpenAPI().SearchModuleByApp(ctx, appID, srvData.header, searchParams)
	if nil != err {
