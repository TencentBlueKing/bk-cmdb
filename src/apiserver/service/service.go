...
func (s *service) routeNeedAuthAPI(ws *restful.WebService, errFunc func() errors.CCErrorIf) {
	...
	if auth.EnableAuthorize() {
		...
		filter = s.authFilterWrapper(s.URLFilterChan, errFunc)
	}

	ws.Route(ws.GET("{.*}").Filter(filter).To(s.Get))
	ws.Route(ws.POST("{.*}").Filter(filter).To(s.Post))
	ws.Route(ws.PUT("{.*}").Filter(filter).To(s.Put))
	ws.Route(ws.DELETE("{.*}").Filter(filter).To(s.Delete))
}
