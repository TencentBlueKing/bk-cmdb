...
func (cli *BaseAction) IsSuccess(rst []byte) (*api.BKAPIRsp, bool) {
...
func (cli *BaseAction) CallResponseEx(callfunc func() (httpcode int, reply interface{}, err error), resp *restful.Response) {
	...
	if nil == err {
		...
		case errors.CCErrorCoder:
			cli.ResponseFailedEx(httpcode, e.GetCode(), e.Error(), reply, resp)
		}
	}
}
...
// ResponseRspString  parse the api.BKAPIRsp struct
...
func (cli *BaseAction) Response(rst *api.BKAPIRsp, resp *restful.Response) {
...
func (cli *BaseAction) ResponseFailedEx(httpcode int, errno int, errmsg, info interface{}, resp *restful.Response) {

	rsp, rsperr := cli.CC.CreateAPIRspErrStrWithData(errno, errmsg, info)
	if nil != rsperr {
...
func (cli *BaseAction) ResponseNative(datamsg string, resp *restful.Response) {
	var rst api.APIRsp
	var bkrst api.BKAPIRsp
	err := json.Unmarshal([]byte(datamsg), &rst)
	bkrst.HTTPCode = rst.HTTPCode
	bkrst.Code = rst.Code
	bkrst.Message = rst.Message
	bkrst.Result = rst.Result
	bkrst.Data = rst.Data
	data, _ := json.Marshal(bkrst)
	if nil != err {
