...
func init() {
	// Register the arrayLongchar attribute type
...
// Name returns the name of the arrayLongchar attribute.
...
// RealType returns the db type of the arrayLongchar attribute.
...
// Validate validates the arrayLongchar attribute value
func (a arrayLongchar) Validate(ctx context.Context, objID string, propertyType string, required bool,
	...
	if value == nil {
		if required {
			blog.Errorf("array_longchar attribute %s.%s value is required but got nil, rid: %s",
				objID, propertyType, rid)
			return fmt.Errorf("array_longchar attribute %s.%s value is required but got nil",
				objID, propertyType)
		}
		return nil
	}
	...
	if !ok {
		blog.Errorf("array_longchar attribute %s.%s value must be []interface{}, got %T, rid: %s",
			objID, propertyType, value, rid)
		return fmt.Errorf("array_longchar attribute %s.%s value must be []interface{}, got %T",
	...
	for i, item := range strArray {
		...
		if !ok {
			blog.Errorf("array_longchar attribute %s.%s array item [%d] type %T is not string, rid: %s", objID,
				propertyType, i, item, rid)
			return fmt.Errorf("array_longchar attribute %s.%s array item [%d] type %T is not string", objID,
				propertyType, i, item)
		}

		// Validate length
		if len(strVal) > common.FieldTypeLongLenChar {
			blog.Errorf("array_longchar attribute %s.%s array item [%d] length %d exceeds max %d, rid: %s",
				objID, propertyType, i, len(strVal), common.FieldTypeLongLenChar, rid)
			return fmt.Errorf("array_longchar attribute %s.%s array item [%d] length exceeds max %d",
				objID, propertyType, i, common.FieldTypeLongLenChar)
		}

		// Validate regex
		if !pattern.MatchString(strVal) {
			blog.Errorf("array_longchar attribute %s.%s array item [%d] value '%s' does not match regex, rid: %s",
				objID, propertyType, i, strVal, rid)
			return fmt.Errorf("array_longchar attribute %s.%s array item [%d] does not match regex pattern",
...
func (a arrayLongchar) FillLostValue(ctx context.Context, valData mapstr.MapStr, propertyId string,
	...
	for i, item := range strArray {
		...
		if len(strVal) > common.FieldTypeLongLenChar {
			blog.Errorf("array_longchar default value array item [%d] length %d exceeds max %d, rid: %s", i, len(strVal),
				common.FieldTypeLongLenChar, rid)
			return fmt.Errorf("array_longchar default value array item [%d] length exceeds max %d", i,
				common.FieldTypeLongLenChar)
		}

		if !pattern.MatchString(strVal) {
			blog.Errorf("array_longchar default value array item [%d] value '%s' does not match regex, rid: %s",
				i, strVal, rid)
...
func (a arrayLongchar) ValidateOption(ctx context.Context, option interface{}, defaultVal interface{}) error {
	...
	for i, item := range strArray {
		...
		if len(strVal) > common.FieldTypeLongLenChar {
			blog.Errorf("array_longchar default value array item [%d] length exceeds max %d, rid: %s",
				i, common.FieldTypeLongLenChar, rid)
			return fmt.Errorf("array_longchar default value array item [%d] length exceeds max %d",
				i, common.FieldTypeLongLenChar)
		}
