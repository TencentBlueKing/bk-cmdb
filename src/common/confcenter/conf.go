...
func (cc *ConfCenter) Start() error {
	...
	confPath := types.CC_SERVCONF_BASEPATH + "/" + cc.currentProcName
	...
	var errorResEvent <-chan *confregdiscover.DiscoverEvent
	...
	var langResEvent <-chan *confregdiscover.DiscoverEvent
	langResEvent, err = cc.confRegDiscv.DiscoverConfig(types.CC_SERVLANG_BASEPATH)
	if err != nil {
		blog.Errorf("fail to discover configure for migrate service. err:%s", err.Error())
		return err
	}
