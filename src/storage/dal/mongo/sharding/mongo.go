...
func (c *shardingMongoClient) newTenantDB(tenant string, tm *local.TxnManager, conf *local.MongoCliConf) local.DB {
	if tenant == "" {
	...
	db, err := local.NewMongo(client, tm, conf, &local.MongoOptions{Tenant: tenant})
	if err != nil {
...
func (c *shardingMongoClient) newIgnoreTenantDB(tm *local.TxnManager, conf *local.MongoCliConf) local.DB {
	db, err := local.NewMongo(c.masterCli, tm, conf, &local.MongoOptions{IgnoreTenant: true})
	if err != nil {
		return local.NewErrDB(err)
	}
	return db
...
func (c *shardingMongoClient) execForAllDB(handler func(db local.DB) error, tm *local.TxnManager,
	conf *local.MongoCliConf) error {

	for uuid, client := range c.dbClientMap {
		db, err := local.NewMongo(client, tm, conf, &local.MongoOptions{IgnoreTenant: true})
		if err != nil {
...
func (m *ShardingMongoManager) Tenant(tenant string) local.DB {
	...
	return m.shardingMongoClient.newTenantDB(tenant, txnManager, m.conf)
}
...
func (m *ShardingMongoManager) IgnoreTenant() local.DB {
	...
	return m.shardingMongoClient.newIgnoreTenantDB(txnManager, m.conf)
}
...
func (m *ShardingMongoManager) ExecForAllDB(handler func(db local.DB) error) error {
	...
	return m.shardingMongoClient.execForAllDB(handler, txnManager, m.conf)
}
...
func (m *WatchMongo) Tenant(tenant string) local.DB {
	return m.shardingMongoClient.newTenantDB(tenant, new(local.TxnManager), &local.MongoCliConf{IDGenStep: 1})
}

// IgnoreTenant returns the master db client that do not use tenant
func (m *WatchMongo) IgnoreTenant() local.DB {
	return m.shardingMongoClient.newIgnoreTenantDB(new(local.TxnManager), &local.MongoCliConf{IDGenStep: 1})
}
...
func (m *WatchMongo) ExecForAllDB(handler func(db local.DB) error) error {
	return m.shardingMongoClient.execForAllDB(handler, new(local.TxnManager), &local.MongoCliConf{IDGenStep: 1})
}
