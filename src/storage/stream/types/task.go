...
type TaskTokenHandler interface {
	SetLastWatchToken(ctx context.Context, uuid string, token *TokenInfo) error
	GetStartWatchToken(ctx context.Context, uuid string) (*TokenInfo, error)
}
...
type TaskOneHandler struct {
	DoAdd    func(watchDB *local.Mongo, ccDB local.DB, event *Event) (retry bool)
	DoUpdate func(watchDB *local.Mongo, ccDB local.DB, event *Event) (retry bool)
	DoDelete func(watchDB *local.Mongo, ccDB local.DB, event *Event) (retry bool)
}
...
type TaskBatchHandler struct {
	DoBatch func(watchDB *local.Mongo, ccDB local.DB, es []*Event) (unhandledEvents []*Event, retry bool)
}
