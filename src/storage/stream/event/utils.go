...
func genWatchFilter(collCondMap map[string]*filter.Expression, collOpTypeMap map[string][]types.OperType) [][]bson.E {
	allFilters := make([][]bson.E, 0)
	...
	for regex, cond := range collCondMap {
		...
		filters := []bson.E{{Key: "ns.coll", Value: bson.M{common.BKDBLIKE: regex}}}
