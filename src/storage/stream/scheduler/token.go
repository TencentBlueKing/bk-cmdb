...
func (w *dbWatcher) setTaskLastTokenInfo(taskID, token string) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.lastTokenInfo[taskID] = token
}

func (w *dbWatcher) setLastWatchToken() {
	...
	for taskID, task := range w.taskMap {
		...
		isFinished := true
		w.mu.RLock()
		if token.Token < w.lastTokenInfo[taskID] {
			isFinished = false
		}
		w.mu.RUnlock()
	...
	if lastToken == nil || lastToken.Token == "" || lastToken.Token <= w.lastToken.Token {
		return
	}
	...
	if err := w.watchDB.Table(common.BKTableNameWatchToken).Update(context.Background(), filter, data); err != nil {
		blog.Errorf("set db %s last watch token failed, err: %v, data: %+v", w.uuid, err, data)
		// notify retry signal, retry loop watch logics
		w.notifyRetry()
		return
	}
	w.lastToken = lastToken
}

// getStartWatchToken get start watch token of db watch task
func (w *dbWatcher) getStartWatchToken(ctx context.Context) (*types.TokenInfo, error) {
	filter := map[string]interface{}{
		"_id": w.uuid,
	}

	data := new(types.TokenInfo)
	err := w.watchDB.Table(common.BKTableNameWatchToken).Find(filter).One(ctx, data)
	if err != nil {
		if !mongodb.IsNotFoundError(err) {
			blog.Errorf("get db %s last watch token failed, err: %v", w.uuid, err)
			return nil, err
		}
...
func (w *dbWatcher) resetWatchToken(startAtTime types.TimeStamp) error {
	filter := map[string]interface{}{
		"_id": w.uuid,
	}
	...
	if err := w.watchDB.Table(common.BKTableNameWatchToken).Update(context.Background(), filter, data); err != nil {
		blog.Errorf("reset db %s watch token failed, err: %v, data: %+v", w.uuid, err, data)
		return err
	}
	return nil
}
