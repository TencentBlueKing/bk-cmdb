...
func newDBWatchTask(task *watchTask, dbUUID string, watchDB *local.Mongo, ccDB local.DB) (*dbWatchTask, error) {
	lastToken, err := task.tokenHandler.GetStartWatchToken(context.Background(), dbUUID)
	if err != nil {
		blog.Errorf("get task %s db %s last watch token failed, err: %v", task.name, dbUUID, err)
		return nil, err
	}
		...
		if t.needList {
			...
			for event := range t.listChan {
				...
				if len(events) == t.batchSize {
					t.eventHandler.DoBatch(t.watchDB, t.ccDB, events)
				}
			...
			for {
				...
				case one := <-t.eventChan:
					...
					if !compareToken(one, t.lastToken) {
						blog.V(4).Infof("%s-%s job, skip previous event(%s)", t.name, t.dbUUID, one.String())
						continue
					}
...
func (t *dbWatchTask) handleEvents(events []*types.Event) {
	...
	rid := first.ID()
	blog.Infof("%s-%s job, received %d events, first op-time: %s, fist token: %s, rid: %s", t.name, t.dbUUID,
	...
	for {
		...
		if needRetry {
			...
			lastToken, err := t.tokenHandler.GetStartWatchToken(ctx, t.dbUUID)
			if err != nil {
				blog.Errorf("get task %s db %s last watch token failed, err: %v, rid: %s", t.name, t.dbUUID, err, rid)
				time.Sleep(t.retryOptions.RetryDuration)
				continue
			}
		...
		if len(events) > 0 {
			events, needRetry = t.eventHandler.DoBatch(t.watchDB, t.ccDB, events)
			if needRetry {
		...
		if err := t.tokenHandler.SetLastWatchToken(ctx, t.dbUUID, lastToken); err != nil {
			blog.Errorf("set task %s db %s last watch token(%+v) failed, err: %v, rid: %s", t.name, t.dbUUID,
				*lastToken, err, rid)
			needRetry = true
			continue
		}
