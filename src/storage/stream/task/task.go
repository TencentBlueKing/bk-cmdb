...
func New(db, watchDB dal.Dal, isMaster discovery.ServiceManageInterface, opts *types.NewTaskOptions) (*Task, error) {
	...
	watchDBRelation, err := genWatchDBRelationMap(db)
	if err != nil {
		return nil, err
	}
...
func (t *Task) AddLoopOneTask(opts *types.LoopOneTaskOptions) error {
	...
	batchOpts := &types.LoopBatchTaskOptions{
		...
		EventHandler: &types.TaskBatchHandler{
			DoBatch: func(watchDB *local.Mongo, ccDB local.DB, es []*types.Event) (unhandledEvents []*types.Event,
				retry bool) {
				for _, e := range es {
					switch e.OperationType {
					case types.Insert:
						retry = opts.EventHandler.DoAdd(watchDB, ccDB, e)
					case types.Update, types.Replace:
						retry = opts.EventHandler.DoUpdate(watchDB, ccDB, e)
					case types.Delete:
						retry = opts.EventHandler.DoDelete(watchDB, ccDB, e)
					default:
...
func (t *Task) Start() error {
	...
	for taskName, task := range t.watchTasks {
		...
		for uuid, dbClient := range t.dbClients {
			dbTask, err := newDBWatchTask(task, uuid, t.watchClients[uuid], dbClient)
			if err != nil {
				return err
			}
	...
	for uuid, loopWatch := range t.loopWatch {
		opts := &types.LoopBatchOptions{
			LoopOptions: types.LoopOptions{
				...
				TokenHandler: newDBTokenHandler(uuid, t.watchClients[uuid], dbWatchTasks[uuid]),
