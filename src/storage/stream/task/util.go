...
func compareToken(event *types.Event, token *types.TokenInfo) bool {
	return event.Token.Data > token.Token || event.ClusterTime.Sec > token.StartAtTime.Sec ||
		event.ClusterTime.Sec == token.StartAtTime.Sec && event.ClusterTime.Nano > token.StartAtTime.Nano
}
