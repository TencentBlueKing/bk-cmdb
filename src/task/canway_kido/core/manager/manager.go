...
func (m *Manager) Actions() []httpserver.Action {
	...
	httpactions = append(httpactions, httpserver.Action{Method: "GET", Path: "/get_host_count", Handler: GetHostCountByBiz})
	return httpactions
}

func GetHostCountByBiz(req *restful.Request, resp *restful.Response) {
	res, err := client.GetClient().CCV3(client.Params{SupplierAccount: "0"}).Host().SearchHost(common.CreateCondition())
	if nil != err {
		log.Errorf("failed to serch the host, %s", err.Error())
		resp.ResponseWriter.Write([]byte("false"))
		return
	}
	...
	if nil != err {
		log.Errorf("failed to json Marshal, %s", err.Error())
		resp.ResponseWriter.Write([]byte("false"))
		return
	}
	resp.ResponseWriter.Write([]byte(data))
	return
}
