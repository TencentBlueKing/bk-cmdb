...
import (
	...
	"configcenter/pkg/backbone/service_mange/zk"
	"configcenter/pkg/blog"
	crd "configcenter/pkg/confregdiscover"
	"configcenter/pkg/errors"
	"configcenter/pkg/language"
	"configcenter/pkg/metrics"
...
func newConfig(ctx context.Context, srvInfo *types.ServerInfo, discovery discovery.DiscoveryInterface, apiMachineryConfig *util.APIMachineryConfig) (*Config, error) {
	...
	machinery, err := api.NewApiMachinery(apiMachineryConfig, discovery)
	if err != nil {
...
func newApiMachinery(disc discovery.DiscoveryInterface,
	config *util.APIMachineryConfig) (api.ClientSetInterface, error) {

	machinery, err := api.NewApiMachinery(config, disc)
	if err != nil {
		return nil, fmt.Errorf("new api machinery failed, err: %v", err)
	}
...
// Engine TODO
type Engine struct {
	CoreAPI            api.ClientSetInterface
