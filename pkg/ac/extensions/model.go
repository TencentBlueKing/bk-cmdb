...
func (am *AuthManager) MakeResourcesByObjects(ctx context.Context, header http.Header, action meta2.Action, objects ...metadata.Object) ([]meta2.ResourceAttribute, error) {
...
func (am *AuthManager) AuthorizeByObjectIDs(ctx context.Context, header http.Header, action meta2.Action, bizID int64,
	objIDs ...string) error {
...
func (am *AuthManager) GenObjectBatchNoPermissionResp(ctx context.Context, header http.Header, action meta2.Action,
	...
	for _, object := range objects {
		iamObjects = append(iamObjects, []metadata.IamResourceInstance{{
			Type: string(iamtype.SysModel),
			ID:   strconv.FormatInt(object.ID, 10),
		}})
	}

	iamAction, err := iamtype.ConvertResourceAction(meta2.Model, action, bizID)
	if err != nil {
		return nil, err
	}
...
func (am *AuthManager) AuthorizeResourceCreate(ctx context.Context, header http.Header, businessID int64, resourceType meta2.ResourceType) error {
	if !am.Enabled() {
		return nil
	}
