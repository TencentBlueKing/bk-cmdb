...
func (am *AuthManager) MakeResourcesByServiceTemplate(header http.Header, action meta2.Action, businessID int64, templates ...metadata.ServiceTemplate) []meta2.ResourceAttribute {
	resources := make([]meta2.ResourceAttribute, 0)
	for _, template := range templates {
		resource := meta2.ResourceAttribute{
			Basic: meta2.Basic{
				Action:     action,
				Type:       meta2.ProcessServiceTemplate,
				Name:       template.Name,
...
func (am *AuthManager) AuthorizeByServiceTemplateID(ctx context.Context, header http.Header, action meta2.Action, ids ...int64) error {
	if !am.Enabled() {
		return nil
	}
...
func (am *AuthManager) AuthorizeByServiceTemplates(ctx context.Context, header http.Header, action meta2.Action, templates ...metadata.ServiceTemplate) error {
	if !am.Enabled() {
		return nil
	}
