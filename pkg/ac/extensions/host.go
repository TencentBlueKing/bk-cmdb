...
func (am *AuthManager) MakeResourcesByHosts(ctx context.Context, header http.Header, action meta2.Action, hosts ...HostSimplify) ([]meta2.ResourceAttribute, error) {
	rid := util.ExtractRequestIDFromContext(ctx)
...
func (am *AuthManager) AuthorizeByHosts(ctx context.Context, header http.Header, action meta2.Action, hosts ...HostSimplify) error {
	if !am.Enabled() {
		return nil
	}
...
func (am *AuthManager) GenHostBatchNoPermissionResp(ctx context.Context, header http.Header, action meta2.Action, hostIDs []int64) (*metadata.BaseResp, error) {
	...
	permission := &metadata.IamPermission{SystemID: iamtype.SystemIDCMDB}
	if len(bizHosts) > 0 {
		action, err := iamtype.ConvertResourceAction(meta2.HostInstance, action, bizID)
		if err != nil {
			return nil, err
		}
	...
	if len(resourceHosts) > 0 {
		action, err := iamtype.ConvertResourceAction(meta2.HostInstance, action, 0)
		if err != nil {
			return nil, err
		}
		permission.Actions = append(permission.Actions, metadata.IamAction{
...
func (am *AuthManager) GenEditBizHostNoPermissionResp(ctx context.Context, header http.Header, hostIDs []int64) (*metadata.BaseResp, error) {
	...
	permission := &metadata.IamPermission{
		...
		Actions: []metadata.IamAction{{
			ID: string(iamtype.EditBusinessHost),
			RelatedResourceTypes: []metadata.IamResourceType{{
				SystemID:  iamtype.SystemIDCMDB,
				Type:      string(iamtype.Host),
				Instances: instances,
...
func (am *AuthManager) AuthorizeByHostsIDs(ctx context.Context, header http.Header, action meta2.Action, hostIDs ...int64) error {
	rid := util.ExtractRequestIDFromContext(ctx)

	if !am.Enabled() {
		return nil
	}
	if am.SkipReadAuthorization && (action == meta2.Find || action == meta2.FindMany) {
...
func (am *AuthManager) AuthorizeCreateHost(ctx context.Context, header http.Header, bizID int64) error {
	...
	return am.AuthorizeResourceCreate(ctx, header, bizID, meta2.HostInstance)
}
