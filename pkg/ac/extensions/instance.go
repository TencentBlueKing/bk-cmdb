...
func (am *AuthManager) MakeResourcesByInstances(ctx context.Context, header http.Header, action meta2.Action,
	instances ...InstanceSimplify) ([]meta2.ResourceAttribute, error) {
	...
	resultResources := make([]meta2.ResourceAttribute, 0)
	for objID, instances := range objectIDInstancesMap {
		...
		for _, mainline := range mainlineAsst.Info {
			if object.ObjectID == mainline.ObjectID {
				resourceType = meta2.MainlineInstance
			}
		}

		if resourceType == meta2.MainlineInstance {
			parentResources, err := am.MakeResourcesByObjects(ctx, header, meta2.EmptyAction, object)
			if err != nil {
			...
			layers = append(layers, meta2.Item{
...
func (am *AuthManager) AuthorizeByInstanceID(ctx context.Context, header http.Header, action meta2.Action, objID string, ids ...int64) error {
	if !am.Enabled() {
		return nil
	}
...
func (am *AuthManager) AuthorizeByInstances(ctx context.Context, header http.Header, action meta2.Action, instances ...InstanceSimplify) error {
	rid := util.ExtractRequestIDFromContext(ctx)
