...
func (ps *parseStream) objectInstanceAssociationLatest() *parseStream {
	...
	if ps.hitPattern(createObjectInstanceAssociationLatestPattern, http.MethodPost) {
		...
		if len(models) == 1 {
			...
			ps.Attribute.Resources = []meta2.ResourceAttribute{*instRes, *asstInstRes}
			return ps
		}
...
func (ps *parseStream) objectInstanceLatest() *parseStream {
	...
	if ps.hitRegexp(deleteObjectInstanceBatchLatestRegexp, http.MethodDelete) {
		...
		if isMainline {
			...
			instanceType = meta2.MainlineInstance
		}
	...
	if ps.hitRegexp(findObjectInstanceSubTopologyLatestRegexp, http.MethodPost) {
		...
		ps.Attribute.Resources = []meta2.ResourceAttribute{
			{
				Basic: meta2.Basic{
					Type:       meta2.ModelInstanceTopology,
					Action:     meta2.Find,
					InstanceID: instID,
				},
				Layers: []meta2.Item{
		...
		ps.Attribute.Resources = []meta2.ResourceAttribute{
			{
				...
				Layers: []meta2.Item{{Type: meta2.ModelClassification, InstanceID: classification.ID}},
			},
		}
		return ps
	}
		...
		ps.Attribute.Resources = []meta2.ResourceAttribute{
			{
				BusinessID: bizID,
				Basic: meta2.Basic{
					Type:   meta2.ModelClassification,
					Action: meta2.Create,
				},
			},
		}
		...
		ps.Attribute.Resources = []meta2.ResourceAttribute{
			{
				...
				BusinessID: bizID,
				Basic: meta2.Basic{
					Type:   meta2.ModelAttributeGroup,
					Action: meta2.Create,
				},
				Layers: []meta2.Item{{Type: meta2.Model, InstanceID: model.ID}},
			},
		...
		ps.Attribute.Resources = []meta2.ResourceAttribute{
			{
				BusinessID: bizID,
				Basic: meta2.Basic{
					Type: meta2.MainlineModelTopology,
					// Action: meta.Find,
					Action: meta2.SkipAction,
				},
			},
