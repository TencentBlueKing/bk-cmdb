...
var (
	...
	// redis is default
	cacheMap      = make(map[string]redis2.Client, 0)
	defaultClient redis2.Client
	lock          = &sync.RWMutex{}
...
func ClientInstance(prefix string) redis2.Client {
	lock.RLock()
	defer lock.RUnlock()
...
// ParseConfig TODO
func ParseConfig(prefix string, configMap map[string]string) (*redis2.Config, errors.CCErrorCoder) {
	lastConfigErr = nil
...
// InitClient TODO
func InitClient(prefix string, config *redis2.Config) errors.CCErrorCoder {
	lock.Lock()
	defer lock.Unlock()
	...
	lastInitErr = nil
	db, dbErr := redis2.NewFromConfig(*config)
	if dbErr != nil {
...
// UpdateConfig TODO
func UpdateConfig(prefix string, config redis2.Config) {
	// 不支持热更行
	return
}
